...
<Steps>
  ...
  <Step title="Add runtime hooks (as needed)">
    ...
    | `native-anthropic-by-model` | Same Claude-by-model policy as `anthropic-by-model`, plus tool-call-id sanitation and native Anthropic tool-use id preservation for transports that must keep vendor-native ids | `anthropic-vertex`, `luckynemo-router` |
