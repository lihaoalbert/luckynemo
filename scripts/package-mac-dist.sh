...
# Output:
# - dist/LuckyNemo.app
# - dist/LuckyNemo-<version>.zip
# - dist/LuckyNemo-<version>.dmg
...
export BUNDLE_ID="${BUNDLE_ID:-ai.luckynemo.mac}"
...
canonical_sparkle_build() {
  ...
  stderr_file="$(mktemp "${TMPDIR:-/tmp}/luckynemo-sparkle-build.XXXXXX")" || {
...
APP="$ROOT_DIR/dist/LuckyNemo.app"
if [[ ! -d "$APP" ]]; then
...
ZIP="$ROOT_DIR/dist/LuckyNemo-$VERSION.zip"
DMG="$ROOT_DIR/dist/LuckyNemo-$VERSION.dmg"
NOTARY_ZIP="$ROOT_DIR/dist/LuckyNemo-$VERSION.notary.zip"
DSYM_ZIP="$ROOT_DIR/dist/LuckyNemo-$VERSION.dSYM.zip"
