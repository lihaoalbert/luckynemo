...
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://raw.githubusercontent.com/lihaoalbert/luckynemo/main/scripts/install-cli.sh | bash -s -- [--json] [--prefix <path>] [--version <ver>] [--node-version <ver>] [--onboard]
...
PREFIX="${OPENCLAW_PREFIX:-${HOME}/.luckynemo}"
...
GIT_DIR="${OPENCLAW_GIT_DIR:-${OPENCLAW_EFFECTIVE_HOME}/luckynemo}"
...
Usage: install-cli.sh [options]
  ...
  --prefix <path>                     Install prefix (default: ~/.luckynemo; use \$OPENCLAW_PREFIX to override)
...
cleanup_legacy_submodules() {
  local repo_dir="${1:-${OPENCLAW_GIT_DIR:-${OPENCLAW_EFFECTIVE_HOME}/luckynemo}}"
