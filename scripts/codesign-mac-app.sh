...
ENT_TMP_DIR=$(mktemp -d -t luckynemo-entitlements.XXXXXX)
trap cleanup EXIT
...
MLX_TTS_HELPER="$APP_BUNDLE/Contents/MacOS/luckynemo-mlx-tts"
if [ -f "$MLX_TTS_HELPER" ]; then
  echo "Signing MLX TTS helper"; sign_item "$MLX_TTS_HELPER" "$APP_ENTITLEMENTS"
fi

# Sign main binary
if [ -f "$APP_BUNDLE/Contents/MacOS/LuckyNemo" ]; then
  echo "Signing main binary"; sign_item "$APP_BUNDLE/Contents/MacOS/LuckyNemo" "$APP_ENTITLEMENTS"
fi
