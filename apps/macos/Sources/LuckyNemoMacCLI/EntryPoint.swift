...
struct LuckyNemoMacCLI {
    static func main() async {
        ...
        case let .unknown(exitCode):
            fputs("luckynemo-mac: unknown command\n", stderr)
            printUsage()
            exit(exitCode)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      luckynemo-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
      ...
      luckynemo-mac discover [--timeout <ms>] [--json] [--include-local]
      luckynemo-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      luckynemo-mac connect
      luckynemo-mac configure-remote --ssh-target user@gateway.local --remote-port 18789
      luckynemo-mac connect --url ws://127.0.0.1:18789 --json
      luckynemo-mac discover --timeout 3000 --json
      luckynemo-mac wizard --mode local
    """)
}
