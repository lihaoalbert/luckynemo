...
private let appDefaultsSuites = ["ai.luckynemo.mac", "ai.luckynemo.mac.debug"]
...
func runConfigureRemote(_ args: [String]) {
    do {
        ...
        if opts.help {
            print("""
            luckynemo-mac configure-remote

            Usage:
              luckynemo-mac configure-remote --ssh-target <user@host[:port]> [--local-port <port>]
              ...
              luckynemo-mac configure-remote --direct-url <ws://host:port|wss://host> [--token <token>]
            ...
            Options:
              ...
              --project-root <p>  Remote LuckyNemo checkout for CLI commands.
              --cli-path <path>   Remote luckynemo executable or entrypoint.
              --json              Emit JSON.
              -h, --help          Show help.
            """)
            return
    ...
    defaultsSuites: [String]) throws -> ConfigureRemoteOutput
{
    ...
    let configURL = luckyNemoConfigURL()
...
private func luckyNemoConfigURL() -> URL {
    if let raw = ProcessInfo.processInfo.environment["LUCKYNEMO_CONFIG_PATH"],
       !raw.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
    {
        return URL(fileURLWithPath: NSString(string: raw).expandingTildeInPath)
    }
    return FileManager().homeDirectoryForCurrentUser.appendingPathComponent(".luckynemo/luckynemo.json")
}
...
private func writeAppDefaults(opts: ConfigureRemoteOptions, target: String, suites: [String]) {
    for suite in suites {
        ...
        defaults.set("remote", forKey: "luckynemo.connectionMode")
        setDefaultString(defaults, key: "luckynemo.remoteTarget", value: target)
        defaults.set(true, forKey: "luckynemo.onboardingSeen")
        defaults.set(appOnboardingVersion, forKey: "luckynemo.onboardingVersion")
        setDefaultStringIfProvided(defaults, key: "luckynemo.remoteIdentity", value: opts.identity)
        setDefaultStringIfProvided(defaults, key: "luckynemo.remoteProjectRoot", value: opts.projectRoot)
        setDefaultStringIfProvided(defaults, key: "luckynemo.remoteCliPath", value: opts.cliPath)
...
private func printConfigureRemoteOutput(_ output: ConfigureRemoteOutput, json: Bool) {
    ...
    print("LuckyNemo macOS Remote Config")
