...
extension ChannelsStore {
    ...
    func applyConfigSnapshot(_ snap: ConfigSnapshot, sourceKey: String, force: Bool) {
        ...
        self.configStatus = snap.valid == false
            ? "Config invalid; fix it in ~/.luckynemo/luckynemo.json."
            : nil
    ...
    private func currentConfigCacheSourceKey() -> String {
        ...
        return [
            ...
            "token:\(Self.configFingerprint(env["LUCKYNEMO_GATEWAY_TOKEN"]))",
            "password:\(Self.configFingerprint(env["LUCKYNEMO_GATEWAY_PASSWORD"]))",
        ].joined(separator: "|")
    }
