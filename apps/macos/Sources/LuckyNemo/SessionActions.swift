...
enum SessionActions {
    ...
    {
        let request = LuckyNemoChatGatewayRequests.patchSessionPreferences(
            sessionKey: key,
            agentID: nil,
    ...
    static func resetSession(key: String) async throws {
        let request = LuckyNemoChatGatewayRequests.resetSession(sessionKey: key, agentID: nil)
        _ = try await ControlChannel.shared.request(request)
    }

    static func deleteSession(key: String) async throws {
        let request = LuckyNemoChatGatewayRequests.deleteSession(sessionKey: key, agentID: nil)
        _ = try await ControlChannel.shared.request(request)
    }
    ...
    static func openSessionLogInCode(sessionId: String, storePath: String?) {
        let candidates: [URL] = {
            ...
            urls.append(LuckyNemoPaths.stateDirURL.appendingPathComponent("sessions/\(sessionId).jsonl"))
            return urls
        }()
