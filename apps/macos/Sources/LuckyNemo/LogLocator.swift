...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["LUCKYNEMO_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/luckynemo")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("luckynemo-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("luckynemo-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/luckynemo/ (rolling or stdout), or nil if none exist.
