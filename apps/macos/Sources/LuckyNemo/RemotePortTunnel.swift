...
final class RemotePortTunnel: @unchecked Sendable {
    private static let logger = Logger(subsystem: "ai.luckynemo", category: "remote.tunnel")
        ...
        root: [String: Any]) -> Int?
    {
        ...
        guard let sshKey = LuckyNemoConfigFile.canonicalHostForComparison(sshHost),
              let urlKey = LuckyNemoConfigFile.canonicalHostForComparison(host)
        else {
            return nil
        }
    ...
    private static func findPort(preferred: UInt16?, allowRandom: Bool) async throws -> UInt16 {
        ...
        return try await withCheckedThrowingContinuation { cont in
            let queue = DispatchQueue(label: "ai.luckynemo.remote.tunnel.port", qos: .utility)
            do {
