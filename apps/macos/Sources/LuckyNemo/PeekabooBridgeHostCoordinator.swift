...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.luckynemo", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: LuckyNemoPeekabooBridgeServices?

    private static let legacySocketDirectoryNames = ["clawdbot", "clawdis", "moltbot"]

    private static var luckynemoSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        return Self.makeSocketPath(for: "LuckyNemo", in: base)
    }
    ...
    private func startIfNeeded() async {
        ...
        let services = LuckyNemoPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.luckynemoSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.luckynemoSocketPath, privacy: .public)")
    }
        ...
        do {
            ...
            if linkValues?.isSymbolicLink == true {
                ...
                let destinationURL = URL(fileURLWithPath: destination, relativeTo: linkURL.deletingLastPathComponent())
                    .standardizedFileURL
                if destinationURL.path == URL(fileURLWithPath: Self.luckynemoSocketPath).standardizedFileURL.path {
                    return
                }
            ...
            try fileManager.createSymbolicLink(atPath: legacyPath, withDestinationPath: Self.luckynemoSocketPath)
        } catch {
...
@MainActor
private final class LuckyNemoPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.luckynemo.peekaboo")
