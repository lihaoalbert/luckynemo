...
final class AppState {
    private static let logger = Logger(subsystem: "ai.luckynemo", category: "app-state")
        ...
        execApprovalsReadRetryDelay: Duration = .milliseconds(250))
    {
        ...
        let configRoot = LuckyNemoConfigFile.loadDict()
    ...
    private static func sshTunnelGatewayUrl(existingUrl: String?, expectedRemoteHost: String?) -> String {
        ...
        let preservePort: Bool = if LoopbackHost.isLoopbackHost(host) {
            ...
            LuckyNemoConfigFile.canonicalHostForComparison(host) ==
                LuckyNemoConfigFile.canonicalHostForComparison(expectedRemoteHost)
        } else {
            false
        }
    ...
    private func startConfigWatcher() {
        let configUrl = LuckyNemoConfigFile.url()
    ...
    private func applyConfigFromDisk() {
        ...
        NotificationCenter.default.post(name: .luckynemoConfigDidChange, object: nil)
    }
...
extension AppState {
    ...
    func syncGatewayConfigNow() -> Bool {
        ...
        let synced = Self.syncedGatewayRoot(
            currentRoot: LuckyNemoConfigFile.loadDict(),
            draft: draft)
        guard synced.changed else {
            self.setGatewayConfigSyncState(.current)
            return true
        }
        guard LuckyNemoConfigFile.saveDict(synced.root) else {
            self.setGatewayConfigSyncState(.failed)
...
extension AppState {
    static var preview: AppState {
        ...
        state.remoteProjectRoot = "~/Projects/luckynemo"
        state.remoteCliPath = ""
        return state
    }
}
