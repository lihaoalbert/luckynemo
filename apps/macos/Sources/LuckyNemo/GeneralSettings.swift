...
struct GeneralSettings: View {
    ...
    private var generalPage: some View {
        VStack(alignment: .leading, spacing: 20) {
            SettingsPageHeader(
                title: "General",
                subtitle: "Everyday LuckyNemo app behavior.")

            self.luckyNemoStatusPanel

            SettingsCardGroup("App") {
                SettingsCardToggleRow(
                    title: "Launch at login",
                    subtitle: self.state.bundleLocationAllowsPersistentIntegration
                        ? "Automatically start LuckyNemo after you sign in."
                        : "Move LuckyNemo to Applications before enabling launch at login.",
                ...
                SettingsCardToggleRow(
                    ...
                    subtitle: """
                    Keep LuckyNemo visible in the Dock. When off, windows still show the Dock icon while open.
                    """,
    ...
    private var luckyNemoStatusPanel: some View {
        HStack(alignment: .center, spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text(self.state.isPaused ? "LuckyNemo paused" : "LuckyNemo active")
                    .font(.headline)
            ...
            Toggle("LuckyNemo active", isOn: self.activeBinding)
                .labelsHidden()
                .toggleStyle(.switch)
        }
    ...
    private var connectionStatusSubtitle: String {
        switch self.state.connectionMode {
        case .local:
            return "LuckyNemo starts and monitors the Gateway on this Mac."
        case .remote:
        ...
        SettingsCardGroup("Advanced") {
            DisclosureGroup(isExpanded: self.$showRemoteAdvanced) {
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    self.advancedTextField(
                        "Project root",
                        placeholder: "/home/you/Projects/luckynemo",
                        text: self.$state.remoteProjectRoot)
                    self.advancedTextField(
                        "CLI path",
                        placeholder: "/Applications/LuckyNemo.app/.../luckynemo",
...
extension GeneralSettings {
    ...
    private func revealLogs() {
        ...
        Looked for luckynemo logs in /tmp/luckynemo/.
...
extension GeneralSettings {
    static func exerciseForTesting() {
        ...
        state.remoteProjectRoot = "/tmp/luckynemo"
        state.remoteCliPath = "/tmp/luckynemo"
