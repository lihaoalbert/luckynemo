...
private let deepLinkLogger = Logger(subsystem: "ai.luckynemo", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        case let .agent(link):
            guard !AppStateStore.shared.isPaused else {
                self.presentAlert(title: "LuckyNemo is paused", message: "Unpause LuckyNemo to run agent actions.")
                return
            }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run LuckyNemo agent?", message: body) else { return }
        }
