...
final class CLIInstallPrompter {
    ...
    private let logger = Logger(subsystem: "ai.luckynemo", category: "cli.prompt")
    ...
    func installTargetForCurrentBuild(confirmStable: Bool = false) -> CLIInstaller.InstallTarget? {
            ...
            isDebug: CLIInstallBuild.isDebug)
        {
            ...
            let alert = NSAlert()
            alert.messageText = "Install LuckyNemo CLI?"
    ...
    private func chooseChannel(suggested: CLIInstaller.Channel) -> CLIInstaller.Channel? {
        ...
        let alert = NSAlert()
        alert.messageText = "Choose LuckyNemo CLI channel"
        alert.informativeText =
            "This is an unreleased LuckyNemo build. " +
        ...
        restartManagedGateway: Bool = false) async -> Bool
    {
        ...
        if installed {
            ...
            if usesLocalGateway {
                await status.set("Starting LuckyNemo Gateway…")
                if !showCompletionAlert {
                    self.logger.info("managed CLI repair: Starting LuckyNemo Gateway…")
                }
            ...
            case .ready:
                "LuckyNemo Gateway is ready."
            case .deferred:
                "LuckyNemo is installed. The Gateway will start when This Mac is active and resumed."
            case .failed:
                "LuckyNemo was installed, but the Gateway did not start. Open Settings to retry."
            case nil:
                "LuckyNemo CLI is ready for the Mac node."
            }
            await status.set(message)
            if !showCompletionAlert {
                self.logger.info("managed CLI repair: \(message, privacy: .public)")
    ...
    private func openSettings(tab: SettingsTab) {
        ...
        DispatchQueue.main.async {
            NotificationCenter.default.post(name: .luckynemoSelectSettingsTab, object: tab)
        }
    }
