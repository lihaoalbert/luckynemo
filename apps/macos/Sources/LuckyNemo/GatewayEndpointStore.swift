...
actor GatewayEndpointStore {
    ...
    private static let staticLogger = Logger(subsystem: "ai.luckynemo", category: "gateway-endpoint")
        ...
        launchdSnapshot: LaunchAgentPlistSnapshot?) -> String?
    {
        ...
        let raw = env["LUCKYNEMO_GATEWAY_PASSWORD"] ?? ""
        ...
        if !trimmed.isEmpty {
                ...
                !configPassword.isEmpty
            {
                self.warnEnvOverrideOnce(
                    kind: .password,
                    envVar: "LUCKYNEMO_GATEWAY_PASSWORD",
    ...
    {
        let serviceEnv = launchdSnapshot?.environment ?? [:]
        let raw = env["LUCKYNEMO_GATEWAY_TOKEN"] ?? ""
        let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
        if !trimmed.isEmpty {
                ...
                configToken != trimmed
            {
                self.warnEnvOverrideOnce(
                    kind: .token,
                    envVar: "LUCKYNEMO_GATEWAY_TOKEN",
    ...
    private let logger = Logger(subsystem: "ai.luckynemo", category: "gateway-endpoint")
    ...
    init(deps: Deps = .live) {
        ...
        if let modeRaw {
            ...
            let seen = UserDefaults.standard.bool(forKey: "luckynemo.onboardingSeen")
        ...
        let bind = GatewayEndpointStore.resolveGatewayBindMode(
            ...
            env: ProcessInfo.processInfo.environment)
        let customBindHost = GatewayEndpointStore.resolveGatewayCustomBindHost(root: LuckyNemoConfigFile.loadDict())
...
extension GatewayEndpointStore {
    ...
    {
        if let raw = env["LUCKYNEMO_GATEWAY_PORT"],
    ...
    {
        if let envBind = env["LUCKYNEMO_GATEWAY_BIND"] {
    ...
    private static func resolveGatewayScheme(
        ...
        env: [String: String]) -> String
    {
        if let envValue = env["LUCKYNEMO_GATEWAY_TLS"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !envValue.isEmpty
        {
