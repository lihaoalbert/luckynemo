...
extension Notification.Name {
    static let luckynemoPermissionsChanged = Notification.Name("luckynemo.permissions.changed")
}
...
enum PermissionManager {
    ...
    static func ensure(_ caps: [Capability], interactive: Bool) async -> [Capability: Bool] {
        ...
        if interactive {
            await MainActor.run {
                NotificationCenter.default.post(name: .luckynemoPermissionsChanged, object: nil)
            }
        }
        return results
    }
...
enum AppleScriptPermission {
    private static let logger = Logger(subsystem: "ai.luckynemo", category: "AppleScriptPermission")
