...
struct OnboardingAISetupView: View {
        ...
        HStack(spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                Text(self.model.waitingForPendingActivationDeadline
                    ...
                    .font(.callout.weight(.semibold))
                Text(self.model.waitingForPendingActivationDeadline
                    ? "LuckyNemo will check again before changing any inference settings."
        ...
        if SystemAgentAvailability.shouldShow(configuredModel: self.model.connectedModelRef) {
            HStack {
                ...
                Button {
                    ...
                    Label("Need help? Chat with LuckyNemo", systemImage: "questionmark.bubble")
                        .font(.caption)
                }
                .buttonStyle(.link)
            }
    ...
    private func candidateRow(_ candidate: OnboardingAISetupModel.Candidate) -> some View {
        ...
        .luckyNemoSelectableRowChrome(selected: selected && !Self.isFailed(status))
    }
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                Text(
                    "Use an existing subscription or provider account. " +
                        "LuckyNemo opens the provider’s own sign-in flow, then verifies it with a real reply.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
    ...
    private var apiKeysRow: some View {
        ...
        .luckyNemoSelectableRowChrome(selected: self.model.showManualEntry)
    }
    ...
    private func providerAuthRow(_ option: OnboardingAISetupModel.AuthOption) -> some View {
        ...
        .buttonStyle(.plain)
        .disabled(self.model.isBusy)
        .luckyNemoSelectableRowChrome(selected: false)
    }
    ...
    private var manualProviderHelp: String {
        ...
        guard let hint, !hint.isEmpty else {
            return "Paste the key or token here, and LuckyNemo checks it with a real test question."
        }
        return "\(hint). Paste it here, and LuckyNemo checks it with a real test question."
    }

    private var systemAgentSheet: some View {
        VStack(spacing: 8) {
            HStack {
                Label("LuckyNemo — setup helper", systemImage: "lifepreserver")
                    .font(.headline)
                Spacer(minLength: 0)
                Button("Done") {
...
/// Every onboarding failure points at a docs.luckynemo.ai page so people are
        ...
        HStack(alignment: .top, spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                HStack(spacing: 14) {
                    ...
                    Button("Open help…") {
                        if let url = URL(string: "https://docs.luckynemo.ai/\(docsSlug)") {
                            NSWorkspace.shared.open(url)
                        }
                    }
                    .buttonStyle(.link)
