...
final class MacNodeModeCoordinator: NSObject {
    ...
    private let logger = Logger(subsystem: "ai.luckynemo", category: "mac-node")
    ...
    override private convenience init() {
        ...
        let nodeHostWorker = MacNodeHostWorker(session: session) {
            NotificationCenter.default.post(name: .luckynemoNodeHostWorkerFailed, object: nil)
        }
        self.init(
            session: session,
        ...
        routeInvalidationHook: (@Sendable () async -> Void)? = nil)
    {
        ...
        self.notificationCenter.addObserver(
            self,
            selector: #selector(self.refreshNodeConfiguration),
            name: .luckynemoPermissionsChanged,
            object: nil)
        self.notificationCenter.addObserver(
            self,
            selector: #selector(self.nodeHostWorkerFailed),
            name: .luckynemoNodeHostWorkerFailed,
            object: nil)
        self.notificationCenter.addObserver(
            self,
            selector: #selector(self.nodeHostConfigurationChanged),
            name: .luckynemoConfigDidChange,
            object: nil)
        self.notificationCenter.addObserver(
            self,
            selector: #selector(self.nodeHostConfigurationChanged),
            name: .luckynemoCLIInstalled,
            object: nil)
    }
    ...
    private func run() async {
        ...
        while !Task.isCancelled {
            ...
            let browserControlEnabled = LuckyNemoConfigFile.browserControlEnabled()
        ...
        claudeSessionCatalogEnabled: Bool) async throws -> ConnectionAttempt?
    {
        ...
        if !nativeCaps.contains(LuckyNemoCapability.computer.rawValue) {
            await self.runtime.releaseHeldComputerInput()
        }
            ...
            onInvoke: { [weak self] req in
                guard let self else {
                    return BridgeInvokeResponse(
                        id: req.id,
                        ok: false,
                        error: LuckyNemoNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                }
        ...
        claudeSessionCatalogEnabled: Bool) -> [String]
    {
        ...
        return Self.resolvedCaps(
            ...
            cameraEnabled: cameraEnabled,
            computerControlEnabled: computerControlEnabled,
            locationMode: LuckyNemoLocationMode(rawValue: rawLocationMode) ?? .off,
            connectionMode: AppStateStore.shared.connectionMode,
            codexThreadCatalogEnabled: codexThreadCatalogEnabled,
            claudeSessionCatalogEnabled: claudeSessionCatalogEnabled)
    }
    ...
    private func startNodeHostWorkerIfConfigured() async throws -> MacNodeHostManifest? {
        ...
        if let projectExecutable = CommandResolver.projectLuckyNemoExecutable() {
    ...
    private func buildSessionBox(url: URL, connectionMode: AppState.ConnectionMode) -> WebSocketSessionBox? {
        ...
        guard let params = Self.tlsParams(
            ...
            root: LuckyNemoConfigFile.loadDict(),
...
extension MacNodeModeCoordinator {
    nonisolated static func resolvedCaps(
        ...
        locationMode: LuckyNemoLocationMode,
        connectionMode: AppState.ConnectionMode,
        codexThreadCatalogEnabled: Bool = false,
        claudeSessionCatalogEnabled: Bool = false) -> [String]
    {
        var caps: [String] = [
            LuckyNemoCapability.canvas.rawValue,
            LuckyNemoCapability.screen.rawValue,
        ]
        _ = browserControlEnabled
        if cameraEnabled { caps.append(LuckyNemoCapability.camera.rawValue) }
        ...
        if computerControlEnabled {
            caps.append(LuckyNemoCapability.computer.rawValue)
        }
        if locationMode != .off { caps.append(LuckyNemoCapability.location.rawValue) }
    ...
    nonisolated static func resolvedCommands(caps: [String]) -> [String] {
        var commands: [String] = [
            LuckyNemoCanvasCommand.present.rawValue,
            ...
            LuckyNemoCanvasA2UICommand.reset.rawValue,
            MacNodeScreenCommand.snapshot.rawValue,
            MacNodeScreenCommand.record.rawValue,
            LuckyNemoSystemCommand.notify.rawValue,
        ]

        let capsSet = Set(caps)
        if capsSet.contains(LuckyNemoCapability.camera.rawValue) {
            commands.append(LuckyNemoCameraCommand.list.rawValue)
            commands.append(LuckyNemoCameraCommand.snap.rawValue)
            commands.append(LuckyNemoCameraCommand.clip.rawValue)
        }
        if capsSet.contains(LuckyNemoCapability.location.rawValue) {
            commands.append(LuckyNemoLocationCommand.get.rawValue)
        }
        ...
        if capsSet.contains(LuckyNemoCapability.computer.rawValue) {
            commands.append(LuckyNemoComputerCommand.act.rawValue)
        }
