    ...
    func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        if self.isCanvasCommand(command), !Self.canvasEnabled() {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: LuckyNemoNodeError(
                    code: .unavailable,
                    message: "CANVAS_DISABLED: enable Canvas in Settings"))
        }
        do {
            switch command {
            case LuckyNemoCanvasCommand.present.rawValue,
                 LuckyNemoCanvasCommand.hide.rawValue,
                 LuckyNemoCanvasCommand.navigate.rawValue,
                 LuckyNemoCanvasCommand.evalJS.rawValue,
                 LuckyNemoCanvasCommand.snapshot.rawValue:
                return try await self.handleCanvasInvoke(req)
            case LuckyNemoCanvasA2UICommand.reset.rawValue,
                 LuckyNemoCanvasA2UICommand.push.rawValue,
                 LuckyNemoCanvasA2UICommand.pushJSONL.rawValue:
                return try await self.handleA2UIInvoke(req)
            case LuckyNemoCameraCommand.snap.rawValue,
                 LuckyNemoCameraCommand.clip.rawValue,
                 LuckyNemoCameraCommand.list.rawValue:
                return try await self.handleCameraInvoke(req)
            case LuckyNemoLocationCommand.get.rawValue:
            ...
            case LuckyNemoComputerCommand.act.rawValue:
                return try await self.handleComputerActInvoke(req)
            case LuckyNemoSystemCommand.notify.rawValue:
...
extension MacNodeRuntime {
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case LuckyNemoCanvasCommand.present.rawValue:
            let params = (try? Self.decodeParams(LuckyNemoCanvasPresentParams.self, from: req.paramsJSON)) ??
                LuckyNemoCanvasPresentParams()
        ...
        case LuckyNemoCanvasCommand.hide.rawValue:
            let sessionKey = self.mainSessionKey
            await MainActor.run {
                CanvasManager.shared.hide(sessionKey: sessionKey)
            }
            return BridgeInvokeResponse(id: req.id, ok: true)
        case LuckyNemoCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(LuckyNemoCanvasNavigateParams.self, from: req.paramsJSON)
            ...
            return BridgeInvokeResponse(id: req.id, ok: true)
        case LuckyNemoCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(LuckyNemoCanvasEvalParams.self, from: req.paramsJSON)
            let sessionKey = self.mainSessionKey
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case LuckyNemoCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(LuckyNemoCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case LuckyNemoCanvasA2UICommand.push.rawValue,
             LuckyNemoCanvasA2UICommand.pushJSONL.rawValue:
...
extension MacNodeRuntime {
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        switch req.command {
        case LuckyNemoCameraCommand.snap.rawValue:
            let params = (try? Self.decodeParams(LuckyNemoCameraSnapParams.self, from: req.paramsJSON)) ??
                LuckyNemoCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case LuckyNemoCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(LuckyNemoCameraClipParams.self, from: req.paramsJSON)) ??
                LuckyNemoCameraClipParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case LuckyNemoCameraCommand.list.rawValue:
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(LuckyNemoLocationGetParams.self, from: req.paramsJSON)) ??
            LuckyNemoLocationGetParams()
        ...
        do {
            ...
            let payload = LuckyNemoLocationPayload(
    ...
    private func handleComputerActInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params: LuckyNemoComputerActParams
        do {
            params = try Self.decodeParams(LuckyNemoComputerActParams.self, from: req.paramsJSON)
        } catch {
            return Self.errorResponse(
                req,
                code: .invalidRequest,
        ...
        do {
            ...
            case .accessibilityNotTrusted:
                return Self.errorResponse(
                    req,
                    code: .unavailable,
                    message: "ACCESSIBILITY_REQUIRED: grant Accessibility permission to LuckyNemo")
...
extension MacNodeRuntime {
    private func handleA2UIReset(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        (() => {
          const host = globalThis.luckynemoA2UI;
          if (!host) return JSON.stringify({ ok: false, error: "missing luckynemoA2UI" });
    ...
    private func handleA2UIPush(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let command = req.command
        let messages: [LuckyNemoKit.AnyCodable]
        if command == LuckyNemoCanvasA2UICommand.pushJSONL.rawValue {
            let params = try Self.decodeParams(LuckyNemoCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
            messages = try LuckyNemoCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
        } else {
            do {
                let params = try Self.decodeParams(LuckyNemoCanvasA2UIPushParams.self, from: req.paramsJSON)
        ...
        let messagesJSON = try LuckyNemoCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
        let js = """
        (() => {
          try {
            const host = globalThis.luckynemoA2UI;
...
extension MacNodeRuntime {
    private func handleSystemNotify(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(LuckyNemoSystemNotifyParams.self, from: req.paramsJSON)
...
extension MacNodeRuntime {
    ...
    private nonisolated static func locationMode() -> LuckyNemoLocationMode {
        let raw = UserDefaults.standard.string(forKey: locationModeKey) ?? "off"
        return LuckyNemoLocationMode(rawValue: raw) ?? .off
    }
    ...
    private static func errorResponse(
        ...
        code: LuckyNemoNodeErrorCode,
        message: String) -> BridgeInvokeResponse
    {
        BridgeInvokeResponse(
            id: req.id,
            ok: false,
            error: LuckyNemoNodeError(code: code, message: message))
    }

    private static func encodeCanvasSnapshot(
        image: NSImage,
        format: LuckyNemoCanvasSnapshotFormat,
        maxWidth: Int?,
