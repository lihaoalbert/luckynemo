...
enum MacNodeCodexThreadCatalog {
    ...
    private static let commandEnvironmentKey = "LUCKYNEMO_CODEX_APP_SERVER_BIN"
    private static let argumentsEnvironmentKey = "LUCKYNEMO_CODEX_APP_SERVER_ARGS"
    ...
    static func shouldAdvertise(root: [String: Any]? = nil) -> Bool {
        let root = root ?? LuckyNemoConfigFile.loadDict()
        guard LuckyNemoConfigFile.configuredBundledPluginAllowed(
...
extension MacNodeCodexThreadCatalog {
    ...
    private static func configuredPlugin(root: [String: Any]) throws -> ConfiguredPlugin? {
        guard let entry = LuckyNemoConfigFile.pluginEntry(
            MacNodeCodexThreadCatalogContract.pluginId,
            root: root)
        else { return nil }
...
private final class CodexAppServerThreadRequestSession: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "ai.luckynemo.codex-thread-catalog")
    ...
    private static func initializeRequestData() throws -> Data {
        try self.jsonData([
            ...
            "params": [
                "clientInfo": [
                    "name": "luckynemo_macos",
                    "title": "LuckyNemo macOS Node",
