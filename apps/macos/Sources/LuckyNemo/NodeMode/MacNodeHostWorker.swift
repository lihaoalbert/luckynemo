...
extension Notification.Name {
    static let luckynemoNodeHostWorkerFailed = Notification.Name("luckynemo.node-host-worker.failed")
}
...
final class MacNodeHostWorker: MacNodeHostWorking, @unchecked Sendable {
    ...
    private let logger = Logger(subsystem: "ai.luckynemo", category: "node-host-worker")
    private let queue = DispatchQueue(label: "ai.luckynemo.node-host-worker")
    private let writerQueue = DispatchQueue(label: "ai.luckynemo.node-host-worker.writer")
    ...
    private func startLocked(command: [String]) {
        ...
        environment["LUCKYNEMO_NODE_EXEC_HOST"] = "app"
        environment["LUCKYNEMO_NODE_EXEC_FALLBACK"] = "0"
    ...
    private static func decodeInvokeResponse(_ result: [String: Any], id: String) -> BridgeInvokeResponse {
        ...
        let code = LuckyNemoNodeErrorCode(rawValue: rawError?["code"] as? String ?? "UNAVAILABLE") ?? .unavailable
        let error = ok ? nil : LuckyNemoNodeError(
            code: code,
    ...
    private static func unavailableResponse(_ id: String, _ message: String) -> BridgeInvokeResponse {
        BridgeInvokeResponse(
            id: id,
            ok: false,
            error: LuckyNemoNodeError(code: .unavailable, message: message))
    }
