...
extension OnboardingView {
    ...
    func welcomePage() -> some View {
        onboardingPage {
            VStack(spacing: 18) {
                ...
                Label {
                    Text(
                        "LuckyNemo can take actions using the permissions and services you enable. " +
    ...
    func connectionPage() -> some View {
        onboardingPage {
            Text("Where should your assistant live?")
                .font(.largeTitle.weight(.semibold))
            Text(
                "Most people pick this Mac — LuckyNemo installs everything and keeps it " +
            ...
            if self.selectedConnectionMode == .unconfigured {
                Text("OK — LuckyNemo won’t start anything yet. Pick Local or Remote later in Settings → General.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
        ...
        .onChange(of: self.state.connectionMode) { _, newValue in
            ...
            // retires route-owned AI/LuckyNemo state. This nested observer owns probe copy only.
                    ...
                    if self.state.remoteTransport == .ssh {
                        ...
                        GridRow {
                            Text("Project root")
                                .font(.callout.weight(.semibold))
                                .frame(width: labelWidth, alignment: .leading)
                            TextField("/home/you/Projects/luckynemo", text: self.$state.remoteProjectRoot)
                                .textFieldStyle(.roundedBorder)
                                .frame(width: fieldWidth)
                        }
                        GridRow {
                            Text("CLI path")
                                ...
                                .frame(width: labelWidth, alignment: .leading)
                            TextField(
                                "/Applications/LuckyNemo.app/.../luckynemo",
            ...
            if self.state.remoteTokenUnsupported {
                Text(
                    "The current gateway.remote.token value is not plain text. "
                        + "LuckyNemo for macOS cannot use it directly; "
        ...
        action: @escaping () -> Void) -> some View
    {
        Button {
            ...
            .luckyNemoSelectableRowChrome(selected: selected)
        }
        .buttonStyle(.plain)
    }
    ...
    func permissionsPage(contentHeight: CGFloat) -> some View {
        ...
        VStack(spacing: 12) {
            ...
            Text(
                "These macOS permissions let LuckyNemo automate apps and capture context on this Mac. " +
    ...
    func cliPage() -> some View {
        ...
        let detail = if remoteMode {
            "LuckyNemo is installing the matching runtime for this Mac node. " +
                "It will connect to your selected Gateway without starting another one here."
        } else {
            "LuckyNemo is setting up its background service on this Mac. " +
            ...
            self.onboardingCard {
                ...
                if self.state.connectionMode == .remote {
                    self.featureRow(
                        ...
                        subtitle: """
                        On your gateway host: install/update the `luckynemo` package and make sure credentials exist
                        (typically `~/.luckynemo/credentials/oauth.json`). Then connect again if needed.
                        """,
                        systemImage: "network")
                    Divider()
                        .padding(.vertical, 6)
                ...
                self.featureRow(
                    ...
                    subtitle: "Click the LuckyNemo menu bar icon for the compact chat panel and status.",
