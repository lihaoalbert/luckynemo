...
enum DebugActions {
    private static let verboseDefaultsKey = "luckynemo.debug.verboseMain"
    private static let onboardingSeenKey = "luckynemo.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = LuckyNemoPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "LuckyNemo", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = LuckyNemoPaths.configURL
        guard
