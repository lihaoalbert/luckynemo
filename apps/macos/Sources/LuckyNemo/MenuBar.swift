...
@main
struct LuckyNemoApp: App {
    @NSApplicationDelegateAdaptor(AppDelegate.self) private var delegate
    @Environment(\.openWindow) private var openWindow
    @State private var state: AppState
    private static let logger = Logger(subsystem: "ai.luckynemo", category: "app")
    ...
    init() {
        LuckyNemoLogging.bootstrapIfNeeded()
    ...
    var body: some Scene {
        ...
        Window("LuckyNemo Settings", id: SettingsWindowOpener.windowID) {
    ...
    private func applyStatusItemAppearance(paused _: Bool, sleeping _: Bool) {
        ...
        self.statusItem?.button?.toolTip = self.state.voiceWakeMeterActive
            ? "LuckyNemo - Voice Wake live meter active"
            : "LuckyNemo"
    }
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    private static let dashboardURL = URL(string: "luckynemo://dashboard")!
    private var state: AppState?
    private var terminationCleanupTask: Task<Void, Never>?
    private var terminationDeadlineTask: Task<Void, Never>?
    private var terminationCleanupFinished = false
    private let webChatAutoLogger = Logger(subsystem: "ai.luckynemo", category: "Chat")
    ...
    func applicationDidFinishLaunching(_: Notification) {
        ...
        if ProcessInfo.processInfo.environment["LUCKYNEMO_DEBUG_PAIRING_DEMO"] == "1" {
            DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
    ...
    private func scheduleFirstRunOnboardingIfNeeded(gatewayConnected: Bool) async {
           ...
           gatewayConnected
        {
            ...
            // default-agent model cannot run LuckyNemo and must stay in onboarding.
            do {
...
extension SparkleUpdaterController: SPUUpdaterDelegate {
    func allowedChannels(for _: SPUUpdater) -> Set<String> {
        allowedSparkleChannels(forGatewayUpdateChannel: LuckyNemoConfigFile.gatewayUpdateChannel())
    }
