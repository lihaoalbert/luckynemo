...
extension Notification.Name {
    static let luckynemoCLIInstalled = Notification.Name("luckynemo.cli.installed")
}
...
enum CLIInstaller {
    ...
    enum Status: Equatable {
        ...
        var message: String {
            ...
            case let .ready(_, version):
                "LuckyNemo Gateway \(version) is ready."
            case .missing:
                "LuckyNemo Gateway is not installed yet."
            case .unusable:
                "The LuckyNemo Gateway could not be verified. Setup will repair it."
        ...
        fileManager: FileManager) -> [String]
    {
        ...
        for basePath in searchPaths {
            let candidate = URL(fileURLWithPath: basePath).appendingPathComponent("luckynemo").path
    ...
    static func managedExecutableLocation() -> String {
        URL(fileURLWithPath: self.installPrefix())
            .appendingPathComponent("bin/luckynemo")
            .path
    }
        ...
        statusHandler: @escaping @MainActor @Sendable (String) async -> Void) async -> Bool
    {
        ...
        await statusHandler("Installing LuckyNemo CLI (\(target.selector))…")
        guard let installerURL = Bundle.main.url(forResource: "install-cli", withExtension: "sh") else {
            await statusHandler("Install failed: installer resource is missing. Reinstall LuckyNemo.")
            return false
        }
        ...
        if response.success {
            ...
            let summary = installedVersion.map { "Installed luckynemo \($0)." } ?? "Installed luckynemo."
            self.rememberInstallPolicy(target)
            await statusHandler(summary)
            NotificationCenter.default.post(name: .luckynemoCLIInstalled, object: nil)
            return true
        }
    ...
    private static func installPrefix() -> String {
        FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent(".luckynemo")
            .path
    }
        ...
        -> ManagedCLIUpdateOutcome
    {
        ...
        await statusHandler(repair
            ? String(localized: "Repairing the LuckyNemo Gateway update…")
            : String(localized: "Updating the LuckyNemo Gateway to \(targetVersion)…"))
        ...
        NotificationCenter.default.post(name: .luckynemoCLIInstalled, object: nil)
        await statusHandler(String(localized: "LuckyNemo Gateway \(installedVersion) is installed."))
        return .success(
    ...
    private static func devCheckoutLocation(prefix: String) -> String {
        URL(fileURLWithPath: prefix)
            .appendingPathComponent("dev/luckynemo")
            .path
    }
