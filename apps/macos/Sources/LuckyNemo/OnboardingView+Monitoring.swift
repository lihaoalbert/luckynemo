...
extension OnboardingView {
    ...
    func startExistingCLIActivationIfNeeded() {
        ...
        OnboardingController.shared.busyReason = "LuckyNemo is starting the Gateway service."
        cliStatus = "Starting LuckyNemo Gateway…"
    ...
    func finishExistingCLIActivation() async {
        ...
        case .ready:
            cliInstalled = true
            cliStatus = "LuckyNemo Gateway is ready."
        case .deferred:
            cliInstalled = false
            cliStatus = "LuckyNemo is paused. Resume it, then retry setup to start the Gateway."
        case .failed:
            cliInstalled = false
            cliStatus = "LuckyNemo is installed, but the Gateway did not start. Retry setup."
        }
    }
    ...
    func startCLIInstall() {
        ...
        OnboardingController.shared.busyReason = "LuckyNemo is installing the Gateway service."
    ...
    func runCLIInstall() async {
        ...
        if !Self.shouldActivateLocalGateway(afterCLIInstallFor: self.state.connectionMode) {
            cliStatus = "LuckyNemo CLI is ready for the Mac node."
            cliInstalled = true
            return
        }
        cliStatus = "Starting LuckyNemo Gateway…"
        ...
        case .ready:
            cliStatus = "LuckyNemo Gateway is ready."
        case .deferred:
            cliStatus = "LuckyNemo is installed. The Gateway will start when This Mac is active and resumed."
        case .failed:
            cliStatus = "LuckyNemo was installed, but the Gateway did not start. Retry setup."
            return
        }
        cliInstalled = true
    }
    ...
    func refreshLocalGatewayProbe() async {
        ...
        await MainActor.run {
            ...
            let expectedTokens = ["node", "luckynemo", "tsx", "pnpm", "bun"]
