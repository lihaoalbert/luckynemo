...
# LuckyNemo CLI installer (non-interactive, no onboarding)
# Usage: install-cli.sh [--json] [--prefix <path>] [--version <ver>] [--node-version <ver>] [--onboard]
...
resolve_luckynemo_effective_home() {
  local luckynemo_home="${LUCKYNEMO_HOME:-}"
  if [[ -z "$luckynemo_home" ]]; then
    echo "$HOME"
    return 0
  fi

  case "$luckynemo_home" in
    \~)
      echo "$HOME"
      ;;
    \~/*)
      echo "${HOME}/${luckynemo_home#~/}"
      ;;
    *)
      echo "$luckynemo_home"
      ;;
...
LUCKYNEMO_EFFECTIVE_HOME="$(resolve_luckynemo_effective_home)"
PREFIX="${LUCKYNEMO_PREFIX:-${HOME}/.luckynemo}"
LUCKYNEMO_VERSION="${LUCKYNEMO_VERSION:-latest}"
DEFAULT_NODE_VERSION="24.15.0"
ARMV7_DEFAULT_NODE_VERSION="22.22.3"
NODE_VERSION="${LUCKYNEMO_NODE_VERSION:-${DEFAULT_NODE_VERSION}}"
NODE_VERSION_REQUESTED=0
if [[ -n "${LUCKYNEMO_NODE_VERSION:-}" ]]; then
...
NPM_LOGLEVEL="${LUCKYNEMO_NPM_LOGLEVEL:-error}"
INSTALL_METHOD="${LUCKYNEMO_INSTALL_METHOD:-npm}"
GIT_DIR="${LUCKYNEMO_GIT_DIR:-${LUCKYNEMO_EFFECTIVE_HOME}/luckynemo}"
GIT_UPDATE="${LUCKYNEMO_GIT_UPDATE:-1}"
JSON=0
RUN_ONBOARD=0
SET_NPM_PREFIX=0
PNPM_CMD=()
...
Usage: install-cli.sh [options]
  ...
  --prefix <path>                     Install prefix (default: ~/.luckynemo; use \$LUCKYNEMO_PREFIX to override)
  --install-method, --method npm|git  Install via npm (default) or from a git checkout
  --npm                               Shortcut for --install-method npm
  --git, --github                     Shortcut for --install-method git
  --git-dir, --dir <path>             Checkout directory (default: ~/luckynemo, or $LUCKYNEMO_HOME/luckynemo)
  --version <ver>                     LuckyNemo version (default: latest)
  --node-version <ver>                Node version (default: 24.15.0; 22.22.3 on Linux ARMv7)
  --onboard                           Run "luckynemo onboard" after install
  --no-onboard                        Skip onboarding (default)
  --set-npm-prefix                    Force npm prefix to ~/.npm-global if current prefix is not writable (Linux)

Environment variables:
  LUCKYNEMO_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  LUCKYNEMO_INSTALL_METHOD=git|npm
  LUCKYNEMO_HOME=...
  LUCKYNEMO_PREFIX=...
  LUCKYNEMO_VERSION=latest|next|<semver>
  LUCKYNEMO_GIT_DIR=...
  LUCKYNEMO_GIT_UPDATE=0|1
EOF
}
...
cleanup_legacy_submodules() {
  local repo_dir="${1:-${LUCKYNEMO_GIT_DIR:-${LUCKYNEMO_EFFECTIVE_HOME}/luckynemo}}"
...
is_luckynemo_source_package_install_spec() {
  local value="${1:-}"
  local normalized_value=""
  normalized_value="$(to_lowercase_ascii "$value")"
  normalized_value="${normalized_value#luckynemo@}"

  [[ "$normalized_value" == "main" ]] && return 0
  [[ "$normalized_value" =~ ^github:lihaoalbert/luckynemo($|[#/]) ]] && return 0

  normalized_value="${normalized_value#git+}"
  [[ "$normalized_value" =~ ^https?://github\.com/lihaoalbert/luckynemo(\.git)?($|[?#]) ]] && return 0
  [[ "$normalized_value" =~ ^ssh://git@github\.com[:/]lihaoalbert/luckynemo(\.git)?($|[?#]) ]] && return 0
  [[ "$normalized_value" =~ ^git://github\.com/lihaoalbert/luckynemo(\.git)?($|[?#]) ]] && return 0
  [[ "$normalized_value" =~ ^git@github\.com:lihaoalbert/luckynemo(\.git)?($|[?#]) ]] && return 0
  return 1
}

resolve_git_luckynemo_ref() {
  local requested="${LUCKYNEMO_VERSION:-latest}"
  local resolved_version=""

  case "$requested" in
    ""|latest)
      resolved_version="$("$(npm_bin)" view "luckynemo" "dist-tags.${requested:-latest}" 2>/dev/null || true)"
...
install_luckynemo() {
  local requested="${LUCKYNEMO_VERSION:-latest}"
  if is_luckynemo_source_package_install_spec "$requested"; then
    fail "npm installs do not support LuckyNemo GitHub source targets like '${requested}'. Use --install-method git --version main, latest, beta, an exact version, or a built .tgz package."
  fi
  ...
  emit_json "{\"event\":\"step\",\"name\":\"luckynemo\",\"status\":\"start\",\"version\":\"${requested}\"}"
  log "Installing LuckyNemo (${requested})..."
  if [[ "$SET_NPM_PREFIX" -eq 1 ]]; then
    fix_npm_prefix_if_needed
  fi

  if [[ "${requested}" == "latest" ]]; then
    if ! env -u NPM_CONFIG_BEFORE -u npm_config_before -u NPM_CONFIG_MIN_RELEASE_AGE -u npm_config_min_release_age -u npm_config_min-release-age "$(npm_bin)" install -g --prefix "$(node_dir)" "${npm_args[@]}" "luckynemo@latest"; then
      log "npm install luckynemo@latest failed; retrying luckynemo@next"
      emit_json "{\"event\":\"step\",\"name\":\"luckynemo\",\"status\":\"retry\",\"version\":\"next\"}"
      env -u NPM_CONFIG_BEFORE -u npm_config_before -u NPM_CONFIG_MIN_RELEASE_AGE -u npm_config_min_release_age -u npm_config_min-release-age "$(npm_bin)" install -g --prefix "$(node_dir)" "${npm_args[@]}" "luckynemo@next"
      requested="next"
    fi
  else
    env -u NPM_CONFIG_BEFORE -u npm_config_before -u NPM_CONFIG_MIN_RELEASE_AGE -u npm_config_min_release_age -u npm_config_min-release-age "$(npm_bin)" install -g --prefix "$(node_dir)" "${npm_args[@]}" "luckynemo@${requested}"
  fi

  mkdir -p "${PREFIX}/bin"
  rm -f "${PREFIX}/bin/luckynemo"
  cat > "${PREFIX}/bin/luckynemo" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec "${PREFIX}/tools/node/bin/node" "$(node_dir)/lib/node_modules/luckynemo/dist/entry.js" "\$@"
EOF
  chmod +x "${PREFIX}/bin/luckynemo"
  emit_json "{\"event\":\"step\",\"name\":\"luckynemo\",\"status\":\"ok\",\"version\":\"${requested}\"}"
}
...
install_luckynemo_from_git() {
  local repo_dir="$1"
  local repo_url="https://github.com/lihaoalbert/luckynemo.git"
  ...
  emit_json "{\"event\":\"step\",\"name\":\"luckynemo\",\"status\":\"start\",\"method\":\"git\",\"repo\":\"${repo_url//\"/\\\"}\"}"
  if [[ -d "$repo_dir/.git" ]]; then
    log "Installing LuckyNemo from git checkout: ${repo_dir}"
  else
    log "Installing LuckyNemo from GitHub (${repo_url})..."
  fi
  ...
  local git_ref
  git_ref="$(resolve_git_luckynemo_ref)"
  if [[ -z "$(git -C "$repo_dir" status --porcelain 2>/dev/null || true)" ]]; then
    log "Using git ref: ${git_ref}"
    checkout_git_luckynemo_ref "$repo_dir" "$git_ref"
  else
...
EOF
  chmod +x "${PREFIX}/bin/luckynemo"
  emit_json "{\"event\":\"step\",\"name\":\"luckynemo\",\"status\":\"ok\",\"method\":\"git\"}"
}

resolve_luckynemo_version() {
  local version=""
  if [[ -x "${PREFIX}/bin/luckynemo" ]]; then
    version="$("${PREFIX}/bin/luckynemo" --version 2>/dev/null | head -n 1 | tr -d '\r')"
  fi
  echo "$version"
}
...
refresh_gateway_service_if_loaded() {
  local claw="${PREFIX}/bin/luckynemo"
  if [[ ! -x "$claw" ]]; then
    return 0
  fi
...
main() {
  ...
  if [[ "${LUCKYNEMO_NO_ONBOARD:-0}" == "1" ]]; then
    RUN_ONBOARD=0
  fi
  ...
  if [[ "$INSTALL_METHOD" == "git" ]]; then
    install_luckynemo_from_git "$GIT_DIR"
  ...
  local installed_version
  installed_version="$(resolve_luckynemo_version)"
  if [[ -n "$installed_version" ]]; then
    emit_json "{\"event\":\"done\",\"ok\":true,\"version\":\"${installed_version//\"/\\\"}\"}"
    log "LuckyNemo installed (${installed_version})."
  else
    emit_json "{\"event\":\"done\",\"ok\":true}"
    log "LuckyNemo installed."
  fi

  if [[ "$RUN_ONBOARD" -eq 1 ]]; then
    "${PREFIX}/bin/luckynemo" onboard
  fi
}

if [[ "${LUCKYNEMO_INSTALL_CLI_SH_NO_RUN:-0}" != "1" ]]; then
  main "$@"
fi
