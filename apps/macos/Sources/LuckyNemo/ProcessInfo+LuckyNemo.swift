...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.luckynemo.mac`) even if the shipped
    ...
    {
        if environment["LUCKYNEMO_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "luckynemo.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "luckynemo.nixMode") { return true }
