...
enum LaunchAgentPlist {
        ...
        generatedEnvironmentWrapperURL: URL? = nil) -> LaunchAgentPlistSnapshot?
    {
        ...
        let token = env["LUCKYNEMO_GATEWAY_TOKEN"]?.trimmingCharacters(in: .whitespacesAndNewlines).nonEmpty
        let password = env["LUCKYNEMO_GATEWAY_PASSWORD"]?.trimmingCharacters(in: .whitespacesAndNewlines).nonEmpty
        ...
        wrapperURL: URL?) -> [String: String]
    {
        ...
        // Read only the canonical file when the LaunchAgent uses LuckyNemo's generated wrapper.
