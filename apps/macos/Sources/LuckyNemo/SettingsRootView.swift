...
struct SettingsRootView: View {
    ...
    var body: some View {
        ...
        .onReceive(NotificationCenter.default.publisher(for: .luckynemoSelectSettingsTab)) { note in
        ...
        .onReceive(NotificationCenter.default.publisher(for: .luckynemoConfigDidChange)) { _ in
    ...
    private var nixManagedBanner: some View {
        ...
        let configPath = self.snapshotPaths.configPath ?? LuckyNemoPaths.configURL.path
        let stateDir = self.snapshotPaths.stateDir ?? LuckyNemoPaths.stateDirURL.path
    ...
    private func scheduleInferenceRefresh(clearPrevious: Bool, resetSystemAgent: Bool = false) {
        if resetSystemAgent {
            // LuckyNemo sessions are gateway-owned. Re-key the cached detail so a route
            // change cannot send old conversation state to a new endpoint.
            self.systemAgentChatIdentity = UUID()
        }
        if clearPrevious {
            // Preserve an active or pending LuckyNemo request while config truth is revalidated.
...
struct SettingsTabGroup: Identifiable {
    ...
    static func defaultGroups(showDebug: Bool, showSystemAgent: Bool) -> [SettingsTabGroup] {
        ...
        var groups = [
            ...
            SettingsTabGroup(title: "LuckyNemo", tabs: [.about]),
        ]
...
extension Notification.Name {
    static let luckynemoSelectSettingsTab = Notification.Name("luckynemoSelectSettingsTab")
}
