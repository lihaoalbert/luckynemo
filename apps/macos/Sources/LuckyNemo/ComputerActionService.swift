...
final class ComputerActionExecutionQueue {
    typealias Operation = @MainActor (LuckyNemoComputerActParams, UInt64) async throws
        -> LuckyNemoComputerActResult
    ...
    private struct QueuedAction {
        ...
        let continuation: CheckedContinuation<LuckyNemoComputerActResult, Error>
    ...
    private var currentActionTask: Task<LuckyNemoComputerActResult, Error>?
    ...
    func perform(
        ...
        operation: @escaping Operation) async throws -> LuckyNemoComputerActResult
    {
        let actionID = UUID()
        let cancellationState = ComputerActionCancellationState()
    ...
    private func drain() async {
        while !self.pendingActions.isEmpty {
            ...
            let outcome: Result<LuckyNemoComputerActResult, Error>
            do {
...
final class ComputerActionService {
        ...
        lifecycleGeneration: UInt64) async throws -> LuckyNemoComputerActResult
    {
        ...
        return LuckyNemoComputerActResult(ok: true, cursorX: cursor.x, cursorY: cursor.y)
    }
    ...
    private func peekabooClick(at point: CGPoint, action: LuckyNemoComputerAction) async throws {
    ...
    private struct ResolvedDisplay {
        var geometry: LuckyNemoComputerDisplayGeometry
    ...
    private func point(
        _ x: Double?,
        _ y: Double?,
        params: LuckyNemoComputerActParams,
        display: ResolvedDisplay) throws -> CGPoint?
    {
        if x == nil, y == nil {
            return nil
        ...
        let capturedWidth = LuckyNemoComputerInputGeometry.capturedWidth(
            refWidth: refWidth,
            sourceWidth: display.sourceWidth,
            sourceHeight: display.sourceHeight)
        let mapped = LuckyNemoComputerInputGeometry.mapReferencePointToGlobal(
            x: x,
            y: y,
        ...
        let clamped = LuckyNemoComputerInputGeometry.clampToDisplay(
            x: mapped.x,
            y: mapped.y,
            display: geometry)
    ...
    static func validatedCurrentCursorPoint(
        _ point: CGPoint?,
        display: LuckyNemoComputerDisplayGeometry) throws -> CGPoint
    {
        guard let point,
    ...
    static func validateHeldButtonTransition(
        action: LuckyNemoComputerAction,
    ...
    func typeTextForTesting(
        _ text: String,
        lifecycleGeneration: UInt64 = 0) async throws -> LuckyNemoComputerActResult
    {
        let params = LuckyNemoComputerActParams(action: .type, text: text)
    ...
    private func resolveDisplay(params: LuckyNemoComputerActParams) async throws -> ResolvedDisplay {
        ...
        let geometry = LuckyNemoComputerDisplayGeometry(
        ...
        guard LuckyNemoComputerInputGeometry.isValidMappingGeometry(
        ...
        if Self.usesScreenshotCoordinates(params) {
            let refWidth = try Self.requiredReferenceWidth(params)
            let currentFrameId = LuckyNemoComputerInputGeometry.displayFrameId(
    ...
    private static func usesScreenshotCoordinates(_ params: LuckyNemoComputerActParams) -> Bool {
        params.x != nil || params.y != nil || params.fromX != nil || params.fromY != nil
    }

    private static func requiredReferenceWidth(_ params: LuckyNemoComputerActParams) throws -> Int {
    ...
    private static func scrollDirection(
        _ direction: LuckyNemoComputerScrollDirection) -> PeekabooFoundation.ScrollDirection
    {
        switch direction {
        case .up: .up
        case .down: .down
    ...
    private func rawScroll(direction: LuckyNemoComputerScrollDirection, amount: Int, flags: CGEventFlags) throws {
