...
enum ApplicationRelocator {
    ...
    private static let logger = Logger(subsystem: "ai.luckynemo", category: "app-relocation")
    ...
    private nonisolated static let replacementSourceBundleEnvironmentKey = "LUCKYNEMO_REPLACEMENT_SOURCE_BUNDLE"
    private nonisolated static let replacementParentPIDEnvironmentKey = "LUCKYNEMO_REPLACEMENT_PARENT_PID"
    private nonisolated static let replacementCodeHashEnvironmentKey = "LUCKYNEMO_REPLACEMENT_CODE_HASH"
    private nonisolated static let replacementReadyFDEnvironmentKey = "LUCKYNEMO_REPLACEMENT_READY_FD"
    private nonisolated static let replacementBootoutTargetEnvironmentKey = "LUCKYNEMO_REPLACEMENT_BOOTOUT_TARGET"
    private nonisolated static let replacementSupervisorLabelEnvironmentKey =
        "LUCKYNEMO_REPLACEMENT_SUPERVISOR_LABEL"
    private nonisolated static let replacementSupervisorPlistEnvironmentKey =
        "LUCKYNEMO_REPLACEMENT_SUPERVISOR_PLIST"
        ...
        processInfo: ProcessInfo = .processInfo) -> LaunchDisposition
    {
        ...
        case .continueLaunch:
            #if DEBUG
            let monitorDebugReplacement = processInfo.environment["LUCKYNEMO_MONITOR_APP_REPLACEMENT"] == "1"
            #else
        ...
        case let .offerInstall(destination, replacing):
            ...
            do {
                ...
                showFailure(
                    "LuckyNemo couldn’t be installed in Applications. Move it there manually, then open that copy.")
                return .continueLaunch(startUpdater: false)
            }
        case .cannotInstall:
            let message =
                "LuckyNemo is running from a temporary location. " +
...
extension ApplicationRelocator {
    ...
    private static func confirmInstall(replacing: Bool) -> Bool {
        ...
        alert.messageText = replacing
            ? "Replace the older LuckyNemo in Applications?"
            : "Install LuckyNemo in Applications?"
        alert.informativeText = replacing
            ? "This copy is newer than the installed app. LuckyNemo will replace it and reopen from Applications."
            : "LuckyNemo will copy itself to Applications and reopen there so updates and launch at login stay reliable."
    ...
    private static func relaunchAndTerminate(at destination: URL) -> LaunchDisposition {
        ...
        do {
            ...
            self.showFailure(
                "LuckyNemo is installed in Applications, but couldn’t reopen automatically. Open it there manually.")
            return .continueLaunch(startUpdater: false)
        }
    }
    ...
    private static func startSupervisorRestorationWatcher(_ supervisor: KeepAliveSupervisor) -> Bool {
        ...
        watcher.arguments = [
            ...
            "luckynemo-supervisor-restoration",
    ...
    private static func showFailure(_ message: String) {
        let alert = NSAlert()
        alert.messageText = "Move LuckyNemo to Applications"
