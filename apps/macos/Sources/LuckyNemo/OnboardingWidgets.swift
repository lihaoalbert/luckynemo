...
/// Onboarding hero logo: the LuckyNemo clownfish brand mark (bundled
/// `luckynemo-hero.png`) with a soft glow (drop-shadow at ~10% of size).
/// The `mood`/`accessory` parameters are kept for call-site compatibility;
/// the static brand mark does not animate between moods.
struct GlowingLuckyNemoIcon: View {
    let size: CGFloat

    init(
        size: CGFloat = 148,
        mood _: LuckyNemoMascotMood = .idle,
        accessory _: LuckyNemoMascotAccessory = .none)
    {
        self.size = size
    }
    ...
    var body: some View {
        Group {
            if let image = Bundle.main.image(forResource: "luckynemo-hero") {
                Image(nsImage: image)
                    .resizable()
                    .interpolation(.high)
                    .aspectRatio(contentMode: .fit)
            } else {
                LuckyNemoMascotView(mood: .idle, accessory: .none, interactive: false)
                    .aspectRatio(contentMode: .fit)
            }
        }
        .frame(width: self.size, height: self.size)
        .shadow(color: Color(red: 0.36, green: 0.56, blue: 0.95).opacity(0.35), radius: self.size * 0.1)
    }
}
...
extension OnboardingView {
    ...
    var mascotMood: LuckyNemoMascotMood {
    ...
    var mascotAccessory: LuckyNemoMascotAccessory {
    ...
    static func mascotMood(for snapshot: MascotMoodSnapshot) -> LuckyNemoMascotMood {
    ...
    static func mascotAccessory(for page: MascotPage) -> LuckyNemoMascotAccessory {
        switch page {
        case .ready: .gradCap
    ...
    static func memoryImportMood(for phase: OnboardingMemoryImportModel.Phase) -> LuckyNemoMascotMood {
        switch phase {
