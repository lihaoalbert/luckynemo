import Dispatch
import Foundation
import LuckyNemoMLXTTSProtocol
import OSLog
...
actor TalkMLXSpeechSynthesizer {
    ...
    private let logger = Logger(subsystem: "ai.luckynemo", category: "talk.mlx")
    ...
    fileprivate static func helperInvocation() -> HelperInvocation {
        ...
        if let override = ProcessInfo.processInfo.environment["LUCKYNEMO_MLX_TTS_BIN"], !override.isEmpty {
        ...
        if let executableDir = Bundle.main.executableURL?.deletingLastPathComponent() {
            let bundled = executableDir.appendingPathComponent("luckynemo-mlx-tts")
        ...
        return HelperInvocation(
            ...
            argumentPrefix: ["luckynemo-mlx-tts"],
            displayName: "luckynemo-mlx-tts")
    }
