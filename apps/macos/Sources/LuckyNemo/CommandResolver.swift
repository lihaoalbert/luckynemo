...
enum CommandResolver {
    private static let projectRootDefaultsKey = "luckynemo.gatewayProjectRootPath"
    private static let helperName = "luckynemo"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let luckynemoEntry = root.appendingPathComponent("luckynemo.mjs").path
        if FileManager().isReadableFile(atPath: luckynemoEntry) { return luckynemoEntry }
        let binEntry = root.appendingPathComponent("bin/luckynemo.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/luckynemo")
    ...
    static func preferredPaths() -> [String] {
        ...
        let validatedExecutable = self.validatedLuckyNemoExecutable(
            defaults: .standard,
            fileManager: .default,
        ...
        validatedExecutable: String? = nil) -> [String]
    {
        ...
        let managedPaths = self.luckynemoManagedPaths(home: home)
    ...
    static func validatedLuckyNemoExecutable(
    ...
    private static func luckynemoManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".luckynemo"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func luckynemoExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectLuckyNemoExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
        ...
        let sourceEntrypoint = root.appendingPathComponent("luckynemo.mjs").path
    ...
    static func luckynemoNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        projectRoot: URL? = nil) -> [String]
    {
        ...
        let root = projectRoot ?? self.projectRoot()
        if let luckynemoPath = projectLuckyNemoExecutable(projectRoot: root) {
            return [luckynemoPath, subcommand] + extraArgs
        }
        if let luckynemoPath = luckynemoExecutable(searchPaths: searchPaths) {
            return [luckynemoPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "luckynemo", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            luckynemo CLI not found. Install the CLI, or run pnpm build in an LuckyNemo source checkout.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real luckynemo CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/luckynemo"
        ...
        \(cliSection)
        \(projectSection)
        if command -v luckynemo >/dev/null 2>&1; then
          CLI="$(command -v luckynemo)"
          luckynemo \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/luckynemo.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/luckynemo.mjs"
            node "$PRJ/luckynemo.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/luckynemo.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/luckynemo.js"
            node "$PRJ/bin/luckynemo.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent luckynemo"
          pnpm --silent luckynemo \(quotedArgs);
        else
          echo "luckynemo CLI missing on remote host"; exit 127;
        fi
        """
    ...
    {
        let root = configRoot ?? LuckyNemoConfigFile.loadDict()
