...
/// Menu contents for the LuckyNemo menu bar extra.
struct MenuContent: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            self.debugMenu
            Button("About LuckyNemo") { self.open(tab: .about) }
    ...
    private var connectionLabel: String {
        ...
        case .unconfigured:
            "LuckyNemo Not Configured"
        case .remote:
            "Remote LuckyNemo Active"
        case .local:
            "LuckyNemo Active"
        }
    }
    ...
    private var macNodeStatus: (label: String, color: Color)? {
        ...
        if let entry = self.nodesStore.nodes.first(where: { $0.nodeId == deviceId }) {
            ...
            let missingRequiredCommands = [
                ...
                LuckyNemoSystemCommand.run.rawValue,
                LuckyNemoSystemCommand.which.rawValue,
