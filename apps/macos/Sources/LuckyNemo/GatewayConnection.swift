...
private let gatewayConnectionLogger = Logger(subsystem: "ai.luckynemo", category: "gateway.connection")
...
private enum GatewayActivationBindingKeyStore {
    private static let service = "ai.luckynemo.onboarding-route-binding"
...
actor GatewayConnection {
    ...
    nonisolated static let operatorClientCaps = [LuckyNemoGatewayClientCapability.inlineWidgets]
    ...
    #if DEBUG
    nonisolated(unsafe) private static var testingActivationBindingKey: SymmetricKey = .init(size: .bits256)
...
extension GatewayConnection {
    ...
    func sessionRoutingIdentity(
        ifCurrentRoute route: Route) async throws -> LuckyNemoChatSessionRoutingIdentity
    {
        let data = try await request(
            LuckyNemoChatGatewayRequests.agentsList(),
            ifCurrentRoute: route)
        return try LuckyNemoChatGatewayPayloadCodec.decodeSessionRoutingIdentity(data)
    }
        ...
        shutdownGeneration: UInt64) async throws -> GatewayChannelActor
    {
        ...
        nonisolated(unsafe) let activationBindingKeyForActor = activationBindingKey
        let client = GatewayChannelActor(
            url: url,
            token: token,
            password: password,
            authBindingKey: activationBindingKeyForActor,
            session: sessionBox,
...
extension GatewayConnection {
    ...
    private func sessionDefaultString(_ defaults: [String: LuckyNemoProtocol.AnyCodable]?, key: String) -> String {
...
extension GatewayConnection {
    ...
    func healthOK(timeoutMs: Int = 8000) async throws -> Bool {
        ...
        return (try? self.decoder.decode(LuckyNemoGatewayHealthOK.self, from: data))?.ok ?? true
    }
    ...
    func sessionsPreview(
        ...
        limit: Int? = nil,
        maxChars: Int? = nil,
        timeoutMs: Int? = nil) async throws -> LuckyNemoSessionsPreviewPayload
    {
        let resolvedKeys = keys
            .map { self.canonicalizeSessionKey($0) }
            .filter { !$0.isEmpty }
        if resolvedKeys.isEmpty {
            return LuckyNemoSessionsPreviewPayload(ts: 0, previews: [])
        }
    ...
    func chatHistory(
        ...
        maxChars: Int? = nil,
        timeoutMs: Int? = nil,
        ifCurrentRoute route: Route? = nil) async throws -> LuckyNemoChatHistoryPayload
    {
        let resolvedKey = self.canonicalizeSessionKey(sessionKey)
        let request = LuckyNemoChatGatewayRequests.history(
        ...
        if let route {
            let data = try await self.request(
                request,
                ifCurrentRoute: route)
            return try self.decoder.decode(LuckyNemoChatHistoryPayload.self, from: data)
        }
    ...
    func chatSend(
        ...
        distinguishPreDispatchRouteChange: Bool = false) async throws -> LuckyNemoChatSendResponse
    {
        let resolvedKey = self.canonicalizeSessionKey(sessionKey)
        ...
        if let route {
            let data = try await self.request(
                ...
                ifCurrentRoute: route,
                distinguishPreDispatchRouteChange: distinguishPreDispatchRouteChange)
            return try self.decoder.decode(LuckyNemoChatSendResponse.self, from: data)
        }
        let data = try await self.request(request)
        return try self.decoder.decode(LuckyNemoChatSendResponse.self, from: data)
    }
