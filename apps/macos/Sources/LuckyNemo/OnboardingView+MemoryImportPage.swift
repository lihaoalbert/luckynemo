...
extension OnboardingView {
    func memoryImportPage(contentHeight: CGFloat) -> some View {
        VStack(spacing: 12) {
            ...
            Text("LuckyNemo can bring useful context from AI tools you already use into your new assistant.")
                .font(.body)
        ...
        return VStack(spacing: 14) {
            self.onboardingCard(spacing: 12) {
                Label("Your memories are ready", systemImage: "checkmark.circle.fill")
                    .font(.headline)
                    .foregroundStyle(.green)
                Text("Imported \(imported) \(self.memoryLabel(imported)) into LuckyNemo.")
                    .font(.body)
