        ...
        return HStack(alignment: .center, spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text(complete ? "All access granted" : "\(granted) of \(total) permissions granted")
                    .font(.headline)
                Text("LuckyNemo only asks for macOS capabilities when a feature needs them.")
                    .font(.footnote)
...
private struct LocationAccessSettings: View {
    ...
    @AppStorage(locationModeKey) private var locationModeRaw: String = LuckyNemoLocationMode.off.rawValue
    @AppStorage(locationPreciseKey) private var locationPreciseEnabled: Bool = true
    @State private var lastLocationModeRaw: String = LuckyNemoLocationMode.off.rawValue
    ...
    var body: some View {
        VStack(spacing: 0) {
                ...
                subtitle: "Allow agents to use device location when a tool asks for it.")
            {
                Picker("Location Access", selection: self.$locationModeRaw) {
                    Text("Off").tag(LuckyNemoLocationMode.off.rawValue)
                    Text("While Using").tag(LuckyNemoLocationMode.whileUsing.rawValue)
                    Text("Always").tag(LuckyNemoLocationMode.always.rawValue)
                }
                .labelsHidden()
                .pickerStyle(.menu)
        ...
        .onChange(of: self.locationModeRaw) { _, newValue in
            ...
            guard let mode = LuckyNemoLocationMode(rawValue: newValue) else { return }
            Task {
    ...
    private var locationMode: LuckyNemoLocationMode {
        LuckyNemoLocationMode(rawValue: self.locationModeRaw) ?? .off
    }

    private func requestLocationAuthorization(mode: LuckyNemoLocationMode) async -> Bool {
