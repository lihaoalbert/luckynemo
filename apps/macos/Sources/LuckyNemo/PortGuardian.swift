...
actor PortGuardian {
    ...
    private let logger = Logger(subsystem: "ai.luckynemo", category: "portguard")
    #if DEBUG
    private var testingDescriptors: [Int: Descriptor] = [:]
    #endif
    private nonisolated static let appSupportDir: URL = {
        let base = FileManager().urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
        return base.appendingPathComponent("LuckyNemo", isDirectory: true)
    }()
    ...
    static func classifyTunnelRecord(_ record: Record, process: TunnelProcessInfo?) -> TunnelRecordAction {
        ...
        // live parent may be a concurrent LuckyNemo instance (prod + dev), so hands off.
        ...
        tunnelHealthy: Bool?) -> PortReport
    {
        ...
        let expectedCommands = ["node", "luckynemo", "tsx", "pnpm", "bun"]
        ...
        managedGatewayPID: Int32? = nil) -> Bool
    {
        ...
        case .local:
            ...
            if full.contains("gateway-daemon") || full.contains("luckynemo-gateway")
                || cmd.contains("luckynemo-gateway")
            {
                return true
            }
            if self.isNodeLuckyNemoGatewayCommand(full) { return true }
            // If args are unavailable, treat a CLI listener as expected.
            if cmd.contains("luckynemo"), full == cmd { return true }
            return false
        case .unconfigured:
            return false
        }
    ...
    private static func isNodeLuckyNemoGatewayCommand(_ fullCommand: String) -> Bool {
        ...
        return self.isLuckyNemoDistEntrypointToken(tokens[1])
            && tokens[2].lowercased() == "gateway"
    }

    private static func isLuckyNemoDistEntrypointToken(_ token: String) -> Bool {
