...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: LuckyNemoProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.luckynemo", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: LuckyNemoKit.AnyCodable]()) {
                $0[$1.key] = LuckyNemoKit.AnyCodable($1.value.base)
            }
        ...
        if let urlError = error as? URLError {
            ...
            case .notConnectedToInternet:
                if Self.isLikelyLocalNetworkPermissionBlock() {
                    return """
                    macOS is blocking LuckyNemo Local Network access.
                    Allow LuckyNemo in System Settings → Privacy & Security → Local Network, then relaunch the app.
                    """
                }
    ...
    private static func bridgeToProtocolArgs(
        _ value: LuckyNemoProtocol.AnyCodable?) -> [String: LuckyNemoProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: LuckyNemoProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: LuckyNemoKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: LuckyNemoProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("luckynemo.control.heartbeat")
}
