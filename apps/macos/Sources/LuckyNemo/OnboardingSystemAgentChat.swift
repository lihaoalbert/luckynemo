...
/// Onboarding talks to LuckyNemo over the gateway `luckynemo.chat` RPC.
...
final class SystemAgentOnboardingChatModel {
    ...
    var input = ""
    /// Set when LuckyNemo hands off to the normal agent ("talk to agent").
    ...
    /// status/repair greeting (used by Settings → LuckyNemo).
    ...
    func startIfNeeded() async {
        ...
        if Task.isCancelled, self.requestGeneration == generation {
            self.started = false
            self.errorMessage = "LuckyNemo was interrupted. Restart to try again."
        }
    }
        ...
        do {
            ...
            if error is CancellationError || Task.isCancelled {
                ...
                self.errorMessage = Task.isCancelled
                    ? "LuckyNemo was interrupted. Restart to try again."
                    : "The Gateway connection changed. Restart LuckyNemo to reconnect."
                return
            }
            ...
            HStack(spacing: 8) {
                Group {
                    if self.model.expectsSensitiveReply {
                        ...
                        TextField(
                            "Reply to LuckyNemo… (yes sets everything up)",
...
private struct SystemAgentChatBubble: View {
    ...
    private var attributedText: AttributedString {
        // LuckyNemo replies use light markdown (headings, bold, backticks).
