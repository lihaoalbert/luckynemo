...
actor CameraCaptureService {
    ...
    private let logger = Logger(subsystem: "ai.luckynemo", category: "camera")
        ...
        outPath: String?) async throws -> (path: String, durationMs: Int, hasAudio: Bool)
    {
        ...
        let tmpMovURL = FileManager().temporaryDirectory
            .appendingPathComponent("luckynemo-camera-\(UUID().uuidString).mov")
        ...
        let outputURL: URL = {
            ...
            return FileManager().temporaryDirectory
                .appendingPathComponent("luckynemo-camera-\(UUID().uuidString).mp4")
        }()
