...
enum ExecApprovalsStore {
    private static let logger = Logger(subsystem: "ai.luckynemo", category: "exec-approvals")
    ...
    private static func trustedRootURL() -> URL {
        guard let configured = LuckyNemoEnv.path("LUCKYNEMO_HOME") else {
    ...
    private static func stateDirURL() -> URL {
        guard let configured = LuckyNemoEnv.path("LUCKYNEMO_STATE_DIR") else {
            return self.trustedRootURL().appendingPathComponent(".luckynemo", isDirectory: true)
        }
...
extension ExecApprovalsStore {
    ...
    static func expandPath(_ raw: String) -> String {
        ...
        let configuredHome = LuckyNemoEnv.path("LUCKYNEMO_HOME")
