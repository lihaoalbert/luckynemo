...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "ai.luckynemo", category: "gateway.env")
    private static let supportedBindModes: Set<String> = ["loopback", "tailnet", "lan", "auto"]

    static func gatewayPort() -> Int {
        if let raw = ProcessInfo.processInfo.environment["LUCKYNEMO_GATEWAY_PORT"] {
            let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
            if let parsed = Int(trimmed), parsed > 0 { return parsed }
        }
        if let configPort = LuckyNemoConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.luckynemoExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "luckynemo CLI not found in PATH; install the CLI.")
            }
    ...
    private static func preferredGatewayBind() -> String? {
        ...
        if let env = ProcessInfo.processInfo.environment["LUCKYNEMO_GATEWAY_BIND"] {
    ...
    static func normalizeGatewayVersionOutput(_ raw: String?) -> String? {
        ...
        if normalized.lowercased().hasPrefix("luckynemo ") {
            normalized = String(normalized.dropFirst("luckynemo ".count))
        }
        ...
        // Tolerate brand/agent-name prefixes the CLI may print ("徐大恩 2026.7.2"
        // → "2026.7.2"); only the semver itself is comparable.
        if let semverRange = normalized.range(
            of: #"\d+\.\d+\.\d+(?:[-+][0-9A-Za-z.-]+)?"#,
            options: .regularExpression)
        {
            normalized = String(normalized[semverRange])
        }
        return normalized
    }
