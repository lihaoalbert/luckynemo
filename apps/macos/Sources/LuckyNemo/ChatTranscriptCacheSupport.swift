...
/// (`~/Library/Application Support/LuckyNemo/chat-cache.sqlite`), matching the
/// existing LuckyNemo app-support layout. macOS has no per-file Data Protection
...
enum MacChatTranscriptCache {
    struct Context {
        let store: LuckyNemoChatSQLiteTranscriptCache
        let routingIdentity: LuckyNemoChatSessionRoutingIdentity?
    }
    ...
    @MainActor private static var storesByGatewayID: [String: LuckyNemoChatSQLiteTranscriptCache] = [:]
    ...
    ///   session data: distinct profiles (distinct `LUCKYNEMO_STATE_DIR`) never
    ...
    static func currentGatewayID() -> String? {
        ...
        return self.gatewayID(
            mode: mode,
            localStateDir: LuckyNemoConfigFile.stateDirURL(),
    ...
    static func makeContext() -> Context? {
        ...
        let databaseURL = base.appendingPathComponent("LuckyNemo/chat-cache.sqlite", isDirectory: false)
        return Context(
            store: self.store(databaseURL: databaseURL, gatewayID: gatewayID),
            routingIdentity: LuckyNemoChatSQLiteTranscriptCache.loadSessionRoutingIdentity(
                databaseURL: databaseURL,
                gatewayID: gatewayID))
    }

    @MainActor
    static func store(databaseURL: URL, gatewayID: String) -> LuckyNemoChatSQLiteTranscriptCache {
        if let store = storesByGatewayID[gatewayID] {
            return store
        }
        let store = LuckyNemoChatSQLiteTranscriptCache(databaseURL: databaseURL, gatewayID: gatewayID)
