...
actor TalkModeRuntime {
    ...
    private let logger = Logger(subsystem: "ai.luckynemo", category: "talk.runtime")
    private let ttsLogger = Logger(subsystem: "ai.luckynemo", category: "talk.tts")
...
extension TalkModeRuntime {
        ...
        return await withTaskGroup(of: String?.self) { group in
            group.addTask { [runId, sessionKey] in
                ...
                for await push in stream {
                    ...
                    if let text = LuckyNemoChatEventText.assistantText(from: chatEvent) {
                        latestText = text
                    }
    ...
    private func latestAssistantText(sessionKey: String, since: Double? = nil) async -> String? {
        do {
            ...
            let decoded: [LuckyNemoChatMessage] = messages.compactMap { item in
