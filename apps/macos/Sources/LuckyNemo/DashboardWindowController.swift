...
/// pinned at `--luckynemo-native-titlebar-height`, resurrecting the traffic-light
...
final class DashboardWindowController: NSWindowController, WKNavigationDelegate, WKUIDelegate, NSWindowDelegate {
    private static let linkMessageHandlerName = "luckynemoLink"
    private static let windowDragMessageHandlerName = "luckynemoWindowDrag"
    private static let updateMessageHandlerName = "luckynemoUpdate"
    ...
    func setUpdateBridgeEnabled(_ enabled: Bool) {
        ...
        self.webView.evaluateJavaScript(
            "window.dispatchEvent(new CustomEvent('luckynemo:native-update-availability-changed'))")
    }
    ...
    fileprivate func receiveUpdateMessage(_ message: WKScriptMessage) {
        ...
        guard DashboardManager.updateBridgeEnabled(mode: AppStateStore.shared.connectionMode) else {
            ...
            self.webView.evaluateJavaScript(
                "window.dispatchEvent(new CustomEvent('luckynemo:native-update-declined'))")
            return
        }
    ...
    private func publishNativeHistoryState() {
        ...
        self.webView.evaluateJavaScript(
            """
            window.__LUCKYNEMO_NATIVE_HISTORY__ = {canGoBack:\(canGoBack),canGoForward:\(canGoForward)};
            window.dispatchEvent(new CustomEvent('luckynemo:native-history-state', \
            {detail:window.__LUCKYNEMO_NATIVE_HISTORY__}));
            """)
    }
    ...
    private static func makeWindow(contentView: NSView) -> NSWindow {
        ...
        // (--luckynemo-native-titlebar-height); without it they hug the top edge.
    ...
    private static func installNativeChromeScript(into userContentController: WKUserContentController) {
        ...
        let capabilityScript = "window.__LUCKYNEMO_NATIVE_WEB_CHROME__ = true;"
        ...
        // `html.luckynemo-native-macos` styles fold the titlebar clearance into
        ...
        @media (min-width: 700px) {
          ...
          html.luckynemo-native-macos .sidebar-shell,
          html.luckynemo-native-macos .settings-sidebar__header {
            padding-top: max(14px, var(--luckynemo-native-titlebar-height)) !important;
          }
        }
        """
        let script = """
        (() => {
          try {
            if (document.getElementById("luckynemo-native-macos-chrome")) return;
            const style = document.createElement("style");
            style.id = "luckynemo-native-macos-chrome";
            style.textContent = \(Self.jsStringLiteral(css));
            document.documentElement.classList.add("luckynemo-native-macos", "luckynemo-native-web-chrome");
        ...
        auth: DashboardWindowAuth)
    {
        ...
        (() => {
          try {
            ...
            Object.defineProperty(window, "__LUCKYNEMO_NATIVE_CONTROL_AUTH__", {
              value: \(json),
              configurable: true,
            });
          } catch {}
