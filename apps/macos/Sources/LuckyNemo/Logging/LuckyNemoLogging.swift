...
enum LuckyNemoLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = LuckyNemoOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = LuckyNemoFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        ...
        let label = LuckyNemoLogging.makeLabel(subsystem: subsystem, category: category)
...
struct LuckyNemoOSLogHandler: AppLogLevelBackedHandler {
...
struct LuckyNemoFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]

    func log(event: LogEvent) {
        guard AppLogSettings.fileLoggingEnabled() else { return }
        let (subsystem, category) = LuckyNemoLogging.parseLabel(self.label)
