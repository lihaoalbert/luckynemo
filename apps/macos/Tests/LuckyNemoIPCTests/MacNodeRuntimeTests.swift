...
struct MacNodeRuntimeTests {
    ...
    actor CanvasRefreshProbe {
        ...
        func refresh() -> String? {
            self.calls += 1
            return "http://127.0.0.1:18789/__luckynemo__/cap/refreshed-token"
        }
    }

    actor CanvasReconnectProbe {
        private var surfaceURL = "http://127.0.0.1:18789/__luckynemo__/cap/old-token"
        ...
        func reconnectDuringRefresh() -> String? {
            self.surfaceURL = "http://127.0.0.1:18789/__luckynemo__/cap/new-token"
            return nil
        }
    }
    ...
    final class ScreenSnapshotProbeServices: MacNodeRuntimeMainActorServices, @unchecked Sendable {
        ...
        func snapshotScreen(
            ...
            maxWidth: Int?,
            quality: Double?,
            format: LuckyNemoScreenSnapshotFormat?) async throws -> ScreenSnapshotResult
        {
            ...
            outPath _: String?) async throws -> (path: String, hasAudio: Bool)
        {
            let url = FileManager().temporaryDirectory
                .appendingPathComponent("luckynemo-test-screen-record-\(UUID().uuidString).mp4")
            ...
            lifecycleGeneration: UInt64) async throws -> LuckyNemoComputerActResult
        {
            ...
            return LuckyNemoComputerActResult(ok: true, cursorX: 0, cursorY: 0)
        }
    ...
    @Test func `A2UI host capability refresh uses injected node session refresher`() async {
        ...
        let resolver = MacNodeCanvasHostedSurfaceResolver(
            currentSurfaceURL: { "http://127.0.0.1:18789/__luckynemo__/cap/current-token" },
            refreshSurfaceURL: { _ in await probe.refresh() })

        let current = await resolver.resolveA2UIURL()
        #expect(current ==
            "http://127.0.0.1:18789/__luckynemo__/cap/current-token/__luckynemo__/a2ui/?platform=macos")
        #expect(await probe.calls == 0)

        let refreshed = await resolver.resolveA2UIURL(forceRefresh: true)
        #expect(refreshed ==
            "http://127.0.0.1:18789/__luckynemo__/cap/refreshed-token/__luckynemo__/a2ui/?platform=macos")
    ...
    @Test func `hosted Canvas commands refresh capability and preserve target components`() async throws {
        ...
        let resolved = try await resolver.resolveTarget(
            "/__luckynemo__/canvas/demo%20page.html?mode=proof#result")
        #expect(resolved?.url.absoluteString ==
            "http://127.0.0.1:18789/__luckynemo__/cap/refreshed-token/__luckynemo__/canvas/demo%20page.html?mode=proof#result")
    ...
    @Test func `hosted Canvas commands use replacement route after refresh fails`() async throws {
        ...
        let resolved = try await resolver.resolveTarget("/__luckynemo__/canvas/demo.html")

        #expect(resolved?.url.absoluteString ==
            "http://127.0.0.1:18789/__luckynemo__/cap/new-token/__luckynemo__/canvas/demo.html")
    }

    @Test func `handle invoke rejects empty notification`() async throws {
        let runtime = MacNodeRuntime()
        let params = LuckyNemoSystemNotifyParams(title: "", body: "")
        let json = try String(data: JSONEncoder().encode(params), encoding: .utf8)
        let response = await runtime.handleInvoke(
            BridgeInvokeRequest(id: "req-3", command: LuckyNemoSystemCommand.notify.rawValue, paramsJSON: json))
        #expect(response.ok == false)
    }
    ...
    @Test func `handle invoke camera list requires enabled camera`() async {
        await TestIsolation.withUserDefaultsValues([cameraEnabledKey: false]) {
            let runtime = MacNodeRuntime()
            let response = await runtime.handleInvoke(
                BridgeInvokeRequest(id: "req-4", command: LuckyNemoCameraCommand.list.rawValue))
            #expect(response.ok == false)
    ...
    @Test func `handle invoke screen record uses injected services`() async throws {
        ...
        final class FakeMainActorServices: MacNodeRuntimeMainActorServices, @unchecked Sendable {
            ...
            func currentLocation(
                desiredAccuracy _: LuckyNemoLocationAccuracy,
                maxAgeMs _: Int?,
    ...
    final class ComputerActProbeServices: MacNodeRuntimeMainActorServices, @unchecked Sendable {
        var receivedParams: LuckyNemoComputerActParams?
        var actError: Error?
            ...
            lifecycleGeneration: UInt64) async throws -> LuckyNemoComputerActResult
        {
            ...
            return LuckyNemoComputerActResult(ok: true, cursorX: params.x ?? 0, cursorY: params.y ?? 0)
        }
    ...
    @Test func `handle invoke rejects computer act when control disabled`() async throws {
        ...
        let params = LuckyNemoComputerActParams(action: .leftClick, x: 5, y: 6, refWidth: 1280)
        let json = try String(data: JSONEncoder().encode(params), encoding: .utf8)
        let response = await runtime.handleInvoke(
            BridgeInvokeRequest(
                id: "req-computer-disabled",
                command: LuckyNemoComputerCommand.act.rawValue,
                paramsJSON: json))
    ...
    @Test func `handle invoke routes computer act to the injected services when enabled`() async throws {
        ...
        let params = LuckyNemoComputerActParams(action: .leftClick, x: 12, y: 34, refWidth: 1280)
        let json = try String(data: JSONEncoder().encode(params), encoding: .utf8)
        let response = await runtime.handleInvoke(
            BridgeInvokeRequest(
                id: "req-computer-ok",
        ...
        let payloadJSON = try #require(response.payloadJSON)
        let result = try JSONDecoder().decode(LuckyNemoComputerActResult.self, from: Data(payloadJSON.utf8))
    ...
    @Test func `lifecycle release invalidates first invoke awaiting service initialization`() async throws {
        ...
        let runtime = MacNodeRuntime(
            ...
            computerControlEnabled: { true })
        let params = LuckyNemoComputerActParams(action: .leftMouseDown, x: 12, y: 34, refWidth: 1280)
        let json = try String(data: JSONEncoder().encode(params), encoding: .utf8)
        let invoke = Task {
            await runtime.handleInvoke(BridgeInvokeRequest(
    ...
    @Test func `handle invoke maps accessibility denial to unavailable`() async throws {
        ...
        let params = LuckyNemoComputerActParams(action: .leftClick, x: 1, y: 1, refWidth: 1280)
        let json = try String(data: JSONEncoder().encode(params), encoding: .utf8)
        let response = await runtime.handleInvoke(
            BridgeInvokeRequest(
                id: "req-computer-ax",
        ...
        #expect(response.ok == false)
        #expect(response.error?.code == .unavailable)
        #expect(response.error?.message == "ACCESSIBILITY_REQUIRED: grant Accessibility permission to LuckyNemo")
    }
