import Foundation
import Testing
@testable import LuckyNemoDiscovery
...
struct WideAreaGatewayDiscoveryTests {
    @Test func `discovers beacon from tailnet dns sd fallback`() async {
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.100.100.100" {
                        return "steipetacstudio-gateway._luckynemo-gw._tcp.luckynemo.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
                    return "0 0 18789 steipetacstudio.luckynemo.internal."
                }
                if recordType == "TXT" {
                    return "\"displayName=Peter\\226\\128\\153s Mac Studio (LuckyNemo)\" \"gatewayPort=18789\" \"tailnetDns=peters-mac-studio-1.sheep-coho.ts.net\" \"cliPath=/Users/steipete/luckynemo/src/entry.ts\""
                }
                return ""
            })

        // Env mutation must hold TestIsolationLock; raw setenv races parallel
        // tests scanning environ (e.g. LUCKYNEMO_CONFIG_PATH readers).
        let beacons = await TestIsolation.withEnvValues(
            ["LUCKYNEMO_WIDE_AREA_DOMAIN": "luckynemo.internal"])
        {
        ...
        let expectedDisplay = "Peter\u{2019}s Mac Studio (LuckyNemo)"
        ...
        #expect(beacon.cliPath == "/Users/steipete/luckynemo/src/entry.ts")
    }
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.64.0.2" {
                        return "evil._luckynemo-gw._tcp.luckynemo.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
