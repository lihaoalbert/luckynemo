...
struct PairingCardPresentationTests {
    ...
    @Test func `card copy summarizes the requesting hardware`() {
        ...
        #expect(PairingCardPresentation.headerSummary(for: [card]) == "A node wants to connect to LuckyNemo.")
    }
    ...
    @Test func `device scopes render friendly names`() {
        ...
        #expect(PairingCardPresentation.title(for: card) == "LuckyNemo Mac app")
