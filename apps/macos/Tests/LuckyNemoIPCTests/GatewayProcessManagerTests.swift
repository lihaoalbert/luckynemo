...
struct GatewayProcessManagerTests {
        ...
        _ body: () async throws -> T) async throws -> T
    {
        ...
        return try await TestIsolation.withEnvValues(["LUCKYNEMO_CONFIG_PATH": configPath], body)
    }
    ...
    @Test func `coalesces concurrent launch agent enable requests`() async throws {
        ...
        let marker = FileManager.default.temporaryDirectory
            .appendingPathComponent("luckynemo-launchagent-marker-\(UUID().uuidString)")
        ...
        try await self.withLocalGatewayConfig {
            ...
            GatewayLaunchAgentManager.setTestingDaemonStatusPayloads([
                ...
                {"ok":true,"service":{
                  ...
                  "command":{"programArguments":["luckynemo","gateway","--port","\(secondPort)"]},
    ...
    @Test func `restart waits for disable before attaching`() async throws {
        ...
        let descriptor = PortGuardian.Descriptor(
            pid: 4242,
            command: "luckynemo-gateway",
            executablePath: "/tmp/luckynemo-gateway")
        ...
        try await self.withLocalGatewayConfig {
            ...
            GatewayLaunchAgentManager.setTestingDaemonStatusPayload(
                ...
                {"ok":true,"service":{
                  "loaded":true,
                  "runtime":{"status":"running","pid":4242},
                  "command":{"programArguments":["luckynemo","gateway","--port","\(port)"]},
                  "configAudit":{"ok":true,"issues":[]}
                }}
                """)
            GatewayLaunchAgentManager.clearTestingDaemonCommandCalls()
        ...
        try await self.withLocalGatewayConfig {
            ...
            let staleStatuses = [
                ...
                {"ok":true,"service":{
                  "loaded":true,
                  "runtime":{"status":"running","pid":4242},
                  "command":{"programArguments":["luckynemo","gateway","--port","19084"]},
                  "configAudit":{"ok":true,"issues":[]}
                }}
                """,
                """
    ...
    @Test func `attaches to existing gateway without spawning launchd`() async throws {
        ...
        do {
            ...
            do {
                ...
                #expect(details.contains("pid 4343 luckynemo-gateway @ /tmp/luckynemo-gateway"))
                #expect(manager._testControlChannelRefreshForces().last == true)
                await cleanup()
            } catch {
                await cleanup()
