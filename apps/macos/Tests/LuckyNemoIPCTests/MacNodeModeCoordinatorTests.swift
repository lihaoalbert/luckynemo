...
struct MacNodeModeCoordinatorTests {
    ...
    @Test @MainActor func `config and CLI changes restart startup scoped node host worker`() async throws {
        ...
        notificationCenter.post(name: .luckynemoConfigDidChange, object: nil)

        try await self.waitUntil("node-host worker restart", timeout: restartTimeout) {
            await worker.stops() == 1
        }

        notificationCenter.post(name: .luckynemoCLIInstalled, object: nil)
    ...
    @Test func `native manifest excludes CLI-owned node commands`() {
        ...
        #expect(!caps.contains(LuckyNemoCapability.browser.rawValue))
        #expect(!commands.contains(LuckyNemoBrowserCommand.proxy.rawValue))
        #expect(commands.contains(LuckyNemoCanvasCommand.present.rawValue))
        #expect(commands.contains(LuckyNemoSystemCommand.notify.rawValue))
        #expect(!commands.contains(LuckyNemoFileSystemCommand.listDir.rawValue))
        #expect(!commands.contains(LuckyNemoSystemCommand.run.rawValue))
    }
    ...
    @Test func `remote mode advertises native session catalogs`() {
        ...
        #expect(!MacNodeModeCoordinator.routeSnapshotAllowsCodexCatalogInvoke(
            ...
            catalogAdvertised: false))
        #expect(MacNodeModeCoordinator.routeSnapshotAllowsCodexCatalogInvoke(
            command: LuckyNemoSystemCommand.notify.rawValue,
            catalogAdvertised: false))
        #expect(caps.contains(MacNodeClaudeSessionCatalogContract.capability))
        #expect(commands.contains(MacNodeClaudeSessionCatalogContract.listCommand))
        #expect(commands.contains(MacNodeClaudeSessionCatalogContract.readCommand))
    ...
    @Test func `Codex supervision activation respects the plugin flag and global policy`() {
        ...
        #expect(LuckyNemoConfigFile.explicitlyEnabledPluginConfigFlag(
            "codex",
        ...
        #expect(LuckyNemoConfigFile.configuredBundledPluginAllowed(
            "codex",
        ...
        #expect(LuckyNemoConfigFile.pluginEntry("codex", root: ambiguousEntryAliases) == nil)
        #expect(!LuckyNemoConfigFile.explicitlyEnabledPluginConfigFlag(
            "codex",
            path: ["supervision", "enabled"],
    ...
    @Test func `computer control cap gates the computer.act command`() {
        ...
        #expect(enabledCaps.contains(LuckyNemoCapability.computer.rawValue))
        #expect(enabledCommands.contains(LuckyNemoComputerCommand.act.rawValue))
        ...
        #expect(!disabledCaps.contains(LuckyNemoCapability.computer.rawValue))
        #expect(!disabledCommands.contains(LuckyNemoComputerCommand.act.rawValue))
    }
