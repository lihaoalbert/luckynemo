...
@Suite(.serialized) struct VoiceWakeGlobalSettingsSyncTests {
    private func voiceWakeChangedEvent(payload: LuckyNemoProtocol.AnyCodable) -> EventFrame {
        EventFrame(
            type: "event",
    ...
    @Test func `applies voice wake changed event to app state`() async {
        ...
        let evt = self.voiceWakeChangedEvent(payload: LuckyNemoProtocol.AnyCodable(["triggers": [
            "luckynemo",
            "computer",
        ]]))
        ...
        #expect(updated == ["luckynemo", "computer"])
    ...
    @Test func `ignores voice wake changed event with invalid payload`() async {
        let previous = await applyTriggersAndCapturePrevious(["before"])
        let evt = self.voiceWakeChangedEvent(payload: LuckyNemoProtocol.AnyCodable(["unexpected": 123]))
