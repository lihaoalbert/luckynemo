...
@Suite(.serialized) struct CommandResolverTests {
    ...
    @Test func `prefers open claw binary`() throws {
        ...
        let cmd = CommandResolver.luckynemoCommand(
            subcommand: "gateway",
            defaults: defaults,
            configRoot: [:],
            searchPaths: searchPaths,
            projectRoot: tmp)
        #expect(cmd.prefix(2).elementsEqual([luckynemoPath.path, "gateway"]))
    }

    @Test func `source checkout entrypoint wins when package bin link is absent`() throws {
        let defaults = self.makeLocalDefaults()
        let tmp = try makeTempDirForTests()
        let sourceEntrypoint = tmp.appendingPathComponent("luckynemo.mjs")
        let staleGlobalBin = tmp.appendingPathComponent("global/bin")
        try makeExecutableForTests(at: sourceEntrypoint)
        try makeExecutableForTests(at: staleGlobalBin.appendingPathComponent("luckynemo"))
    ...
    @Test func `falls back to node and script`() throws {
        ...
        let scriptPath = tmp.appendingPathComponent("bin/luckynemo.js")
    ...
    @Test func `prefers open claw binary over pnpm`() throws {
        ...
        let luckynemoPath = binDir.appendingPathComponent("luckynemo")
        ...
        #expect(cmd.prefix(2).elementsEqual([luckynemoPath.path, "rpc"]))
    }
    ...
    @Test func `falls back to pnpm`() throws {
        ...
        #expect(cmd.prefix(4).elementsEqual([pnpmPath.path, "--silent", "luckynemo", "rpc"]))
    }
    ...
    @Test func `pnpm keeps extra args after subcommand`() throws {
        ...
        #expect(cmd.prefix(5).elementsEqual([pnpmPath.path, "--silent", "luckynemo", "health", "--json"]))
    ...
    @Test func `missing CLI explains install and source checkout paths`() throws {
        ...
        #expect(cmd.last?.contains("luckynemo CLI not found") == true)
        #expect(cmd.last?.contains("Install the CLI") == true)
        #expect(cmd.last?.contains("run pnpm build in an LuckyNemo source checkout") == true)
    }
    ...
    @Test func `managed install only precedes external installs after validation`() throws {
        ...
        let managedBin = home.appendingPathComponent(".luckynemo/bin")
        try FileManager().createDirectory(at: managedBin, withIntermediateDirectories: true)
        let managedExecutable = managedBin.appendingPathComponent("luckynemo")
    ...
    @Test func `validated CLI preference expires when the app requires a newer version`() throws {
        ...
        let executable = root.appendingPathComponent("luckynemo")
        ...
        #expect(CommandResolver.validatedLuckyNemoExecutable(
            defaults: defaults,
            fileManager: .default,
    ...
    @Test func `builds SSH command for remote mode`() {
        ...
        defaults.set("luckynemo@example.com:2222", forKey: remoteTargetKey)
        defaults.set("/tmp/id_ed25519", forKey: remoteIdentityKey)
        defaults.set("/srv/luckynemo", forKey: remoteProjectRootKey)
        ...
        if let marker = cmd.firstIndex(of: "--") {
            #expect(cmd[marker + 1] == "luckynemo@example.com")
        } else {
            #expect(Bool(false))
        }
        ...
        if let script = cmd.last {
            #expect(script.contains("PRJ='/srv/luckynemo'"))
            #expect(script.contains("cd \"$PRJ\""))
            #expect(script.contains("luckynemo"))
    ...
    @Test func `remote SSH with explicit blank target fails closed`() throws {
        ...
        defaults.set("stale-gateway-alias", forKey: remoteTargetKey)
        let tmp = try makeTempDirForTests()
        let localLuckyNemo = tmp.appendingPathComponent("node_modules/.bin/luckynemo")
        try makeExecutableForTests(at: localLuckyNemo)

        let command = CommandResolver.luckynemoCommand(
            subcommand: "status",
            defaults: defaults,
            configRoot: [
                "gateway": [
    ...
    @Test func `direct remote route uses local CLI despite stale SSH defaults`() throws {
        ...
        #expect(command == [localLuckyNemo.path, "status"])
    }
    ...
    @Test func `config root local overrides remote defaults`() throws {
        ...
        #expect(cmd.first == luckynemoPath.path)
        #expect(cmd.count >= 2)
        if cmd.count >= 2 {
            #expect(cmd[1] == "daemon")
        }
