...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: LuckyNemoProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: LuckyNemoProtocol.AnyCodable] = [
            "presence": .init([LuckyNemoProtocol.AnyCodable(entry)]),
        ]
        let payload = LuckyNemoProtocol.AnyCodable(payloadMap)
