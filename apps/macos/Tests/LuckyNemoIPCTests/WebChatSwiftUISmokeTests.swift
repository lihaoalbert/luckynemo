...
struct WebChatSwiftUISmokeTests {
    private struct TestTransport: LuckyNemoChatTransport {
        func requestHistory(sessionKey: String) async throws -> LuckyNemoChatHistoryPayload {
            let json = """
            {"sessionKey":"\(sessionKey)","sessionId":null,"messages":[],"thinkingLevel":"off"}
            """
            return try JSONDecoder().decode(LuckyNemoChatHistoryPayload.self, from: Data(json.utf8))
        }
            ...
            attachments _: [LuckyNemoChatAttachmentPayload]) async throws -> LuckyNemoChatSendResponse
        {
            ...
            """
            return try JSONDecoder().decode(LuckyNemoChatSendResponse.self, from: Data(json.utf8))
        }
    ...
    @Test func `window controller merges titlebar and keeps toolbar controls`() throws {
        let traceKeys = [
            LuckyNemoChatWindowShell.assistantTraceDefaultsKey,
    ...
    @Test func `controller explicit agent wins and nil falls back to cached default`() throws {
        let cachedIdentity = try #require(LuckyNemoChatSessionRoutingIdentity(
            scope: "global",
            mainSessionKey: "main",
            defaultAgentID: "main"))
    ...
    @Test func `max and Ultra thinking preferences survive reopen`() throws {
        ...
        for level in ["max", "ultra"] {
            defaults.set(level, forKey: "luckynemo.webchat.thinkingLevel")
    ...
    @Test func `verbosity preference survives reopen`() throws {
        ...
        defaults.set("full", forKey: "luckynemo.webchat.verboseLevel")
        #expect(WebChatSwiftUIWindowController.persistedVerboseLevel(defaults: defaults) == "full")
        defaults.set("invalid", forKey: "luckynemo.webchat.verboseLevel")
    ...
    @Test func `inherited verbosity preference clears persisted override`() throws {
        ...
        #expect(WebChatSwiftUIWindowController.persistedVerboseLevel(defaults: defaults) == nil)
        #expect(defaults.object(forKey: "luckynemo.webchat.verboseLevel") == nil)
    }
}
