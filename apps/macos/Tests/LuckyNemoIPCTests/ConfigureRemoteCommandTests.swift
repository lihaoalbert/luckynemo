import Foundation
import Testing
@testable import LuckyNemoMacCLI
...
struct ConfigureRemoteCommandTests {
    @Test @MainActor func `configure remote writes ssh config and app defaults`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("luckynemo-configure-remote-\(UUID().uuidString).json")
        ...
        try await TestIsolation.withIsolatedState(env: ["LUCKYNEMO_CONFIG_PATH": configURL.path]) {
            let output = try configureRemote(
                .init(
                    ...
                    identity: nil,
                    projectRoot: nil,
                    cliPath: "/opt/homebrew/bin/luckynemo"),
            ...
            for (_, defaults) in defaultsBySuite {
                #expect(defaults.string(forKey: "luckynemo.connectionMode") == "remote")
                #expect(defaults.string(forKey: "luckynemo.remoteTarget") == "alice@gateway.example")
                #expect(defaults.bool(forKey: "luckynemo.onboardingSeen") == true)
                #expect(defaults.string(forKey: "luckynemo.remoteCliPath") == "/opt/homebrew/bin/luckynemo")
            }
        }
    }
    ...
    @Test @MainActor func `configure remote preserves existing optional credentials when flags omitted`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("luckynemo-configure-remote-preserve-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }
    ...
    @Test @MainActor func `configure remote defaults SSH host key policy to strict`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("luckynemo-configure-remote-strict-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }
    ...
    @Test @MainActor func `configure remote can write direct private url`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("luckynemo-configure-direct-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }
    ...
    @Test @MainActor func `configure remote rejects plaintext public prefix bypass`() async {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("luckynemo-configure-direct-reject-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }

        _ = await TestIsolation.withIsolatedState(env: ["LUCKYNEMO_CONFIG_PATH": configURL.path]) {
            #expect(throws: Error.self) {
