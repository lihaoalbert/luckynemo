...
struct GatewayLaunchAgentManagerTests {
    @Test func `reads Gateway service ownership command directly from launchd`() throws {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("luckynemo-gateway-\(UUID().uuidString).plist")
    ...
    @Test func `attach only runtime override blocks gateway launch agent writes`() async throws {
        let dir = FileManager().temporaryDirectory
            .appendingPathComponent("luckynemo-attach-only-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func `launch agent plist snapshot parses args and env`() throws {
        let url = FileManager().temporaryDirectory
            .appendingPathComponent("luckynemo-launchd-\(UUID().uuidString).plist")
        let plist: [String: Any] = [
            "ProgramArguments": ["luckynemo", "gateway", "--port", "18789", "--bind", "loopback"],
            "EnvironmentVariables": [
                "LUCKYNEMO_GATEWAY_TOKEN": " secret ",
                "LUCKYNEMO_GATEWAY_PASSWORD": "pw",
            ],
        ]
    ...
    @Test func `launch agent plist snapshot merges canonical generated environment`() throws {
        let directory = FileManager().temporaryDirectory
            .appendingPathComponent("luckynemo-launchd-env-\(UUID().uuidString)", isDirectory: true)
        let plistURL = directory.appendingPathComponent("ai.luckynemo.gateway.plist")
        let environmentFileURL = directory.appendingPathComponent("ai.luckynemo.gateway.env")
        let wrapperURL = directory.appendingPathComponent("ai.luckynemo.gateway-env-wrapper.sh")
        try FileManager().createDirectory(at: directory, withIntermediateDirectories: true)
        try "#!/bin/sh\n".write(to: wrapperURL, atomically: true, encoding: .utf8)
        try """
        # Generated by LuckyNemo. Do not edit while the gateway service is installed.
        export CUSTOM_GATEWAY_TOKEN='custom-token'
        export LUCKYNEMO_GATEWAY_PASSWORD='service'\\''pass'
        export LUCKYNEMO_GATEWAY_TOKEN=' service-token '
        ...
        let plist: [String: Any] = [
            ...
            "EnvironmentVariables": ["LUCKYNEMO_GATEWAY_TOKEN": "stale-inline-token"],
        ]
        let data = try PropertyListSerialization.data(fromPropertyList: plist, format: .xml, options: 0)
    ...
    @Test func `launch agent plist snapshot ignores unreferenced generated environment`() throws {
        ...
        try FileManager().createDirectory(at: directory, withIntermediateDirectories: true)
        try "#!/bin/sh\n".write(to: wrapperURL, atomically: true, encoding: .utf8)
        try "export LUCKYNEMO_GATEWAY_TOKEN='unreferenced-token'\n"
            .write(to: environmentFileURL, atomically: true, encoding: .utf8)
        let plist: [String: Any] = [
            "ProgramArguments": ["luckynemo", "gateway"],
        ]
        let data = try PropertyListSerialization.data(fromPropertyList: plist, format: .xml, options: 0)
        try data.write(to: plistURL, options: [.atomic])
        defer { try? FileManager().removeItem(at: directory) }
    ...
    @Test func `launch agent plist snapshot allows missing bind`() throws {
        let url = FileManager().temporaryDirectory
            .appendingPathComponent("luckynemo-launchd-\(UUID().uuidString).plist")
        let plist: [String: Any] = [
            "ProgramArguments": ["luckynemo", "gateway", "--port", "18789"],
        ]
        let data = try PropertyListSerialization.data(fromPropertyList: plist, format: .xml, options: 0)
        try data.write(to: url, options: [.atomic])
        defer { try? FileManager().removeItem(at: url) }
