...
struct VoiceWakeRuntimeTests {
    ...
    @Test func `has content after trigger false when only trigger`() {
        let triggers = ["luckynemo"]
        let text = "hey luckynemo"
    ...
    @Test func `trigger only rejects trailing wake word mentions in ordinary speech`() {
        let triggers = ["luckynemo"]
        let text = "tell me about luckynemo"
        #expect(!VoiceWakeRuntime._testIsTriggerOnly(text, triggers: triggers))
    }
    ...
    @Test func `matched trigger finds trigger not at transcript start`() {
        let triggers = ["luckynemo"]
        let text = "uh luckynemo"
        #expect(VoiceWakeRuntime._testMatchedTriggerWord(text, triggers: triggers) == "luckynemo")
    }
    ...
    @Test func `matched trigger prefers most specific overlapping phrase`() {
        let triggers = ["luckynemo", "hey luckynemo"]
        let text = "hey luckynemo"
        #expect(VoiceWakeRuntime._testMatchedTriggerWord(text, triggers: triggers) == "hey luckynemo")
    }
    ...
    @Test func `text only fallback populates matched trigger`() {
        let transcript = "hey luckynemo do thing"
        let config = WakeWordGateConfig(triggers: ["luckynemo"], minCommandLength: 1)
        let match = VoiceWakeRecognitionDebugSupport.textOnlyFallbackMatch(
            transcript: transcript,
            triggers: ["luckynemo"],
            config: config,
            trimWake: VoiceWakeRuntime._testTrimmedAfterTrigger)
        #expect(match?.trigger == "luckynemo")
    }

    @Test func `text only fallback keeps the first trigger phrase when later words match another trigger`() {
        let transcript = "luckynemo tell me about computer vision"
        let config = WakeWordGateConfig(triggers: ["luckynemo", "computer"], minCommandLength: 1)
        let match = VoiceWakeRecognitionDebugSupport.textOnlyFallbackMatch(
            transcript: transcript,
            triggers: ["luckynemo", "computer"],
            config: config,
            trimWake: VoiceWakeRuntime._testTrimmedAfterTrigger)
        #expect(match?.trigger == "luckynemo")
    }
    ...
    @Test func `gate requires gap between trigger and command`() {
        ...
        let config = WakeWordGateConfig(triggers: ["luckynemo"], minPostTriggerGap: 0.3)
    ...
    @Test func `gate command text handles foreign string ranges`() {
        ...
        let segments = [
            ...
            WakeWordSegment(text: "luckynemo", start: 0.2, duration: 0.1, range: transcript.range(of: "luckynemo")),
