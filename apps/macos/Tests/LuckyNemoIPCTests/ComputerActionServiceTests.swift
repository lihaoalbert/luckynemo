...
struct ComputerActionServiceTests {
    ...
    private final class ActionProbe {
            ...
            lifecycleGeneration: UInt64) async throws -> LuckyNemoComputerActResult
        {
            ...
            return LuckyNemoComputerActResult(ok: true, cursorX: Double(actionID), cursorY: 0)
        }
    }
    ...
    private final class ActionTaskBox {
        var task: Task<LuckyNemoComputerActResult, Error>?
    }
    ...
    @Test func `coordinate input requires the current display frame identity`() throws {
        ...
        let missing = LuckyNemoComputerActParams(
            action: .leftClick,
            x: 1,
            y: 2,
            refWidth: 1280)
        ...
        let stale = LuckyNemoComputerActParams(
            action: .leftClick,
        ...
        let current = LuckyNemoComputerActParams(
            action: .leftClick,
        ...
        let missingScale = LuckyNemoComputerActParams(
            action: .leftClick,
            displayFrameId: currentFrameId,
            x: 1,
            y: 2)
        ...
        let display = LuckyNemoComputerDisplayGeometry(
            originX: 0,
            originY: 0,
            widthPoints: 1280,
            heightPoints: 800)
        let screenshotFrameId = LuckyNemoComputerInputGeometry.displayFrameId(
            displayID: 1,
            sourceWidth: 2560,
            sourceHeight: 1600,
            referenceWidth: 1280,
            display: display)
        let mismatchedScaleFrameId = LuckyNemoComputerInputGeometry.displayFrameId(
            displayID: 1,
            sourceWidth: 2560,
            sourceHeight: 1600,
            referenceWidth: 640,
            display: display)
        let mismatchedScale = LuckyNemoComputerActParams(
            action: .leftClick,
        ...
        let keyboard = LuckyNemoComputerActParams(action: .type, text: "hello")
    ...
    @Test func `computer actions execute in FIFO order without overlap`() async throws {
        ...
        let firstParams = LuckyNemoComputerActParams(action: .leftClick, x: 1, y: 0, refWidth: 1280)
        let secondParams = LuckyNemoComputerActParams(action: .leftClick, x: 2, y: 0, refWidth: 1280)
    ...
    @Test func `cancelled queued action never executes`() async throws {
        ...
        let queue = ComputerActionExecutionQueue(onLifecycleRelease: { true })
        let firstParams = LuckyNemoComputerActParams(action: .leftClick, x: 1, y: 0, refWidth: 1280)
        let cancelledParams = LuckyNemoComputerActParams(action: .leftClick, x: 2, y: 0, refWidth: 1280)
    ...
    @Test func `cancelled active action releases held input before it settles`() async {
        ...
        let params = LuckyNemoComputerActParams(action: .leftMouseDown, x: 1, y: 0, refWidth: 1280)
    ...
    @Test func `cancellation retries failed release before its main actor hop`() async {
        ...
        let action = Task { @MainActor in
            try await queue.perform(params, lifecycleGeneration: 0) { _, _ in
                taskBox.task?.cancel()
                return LuckyNemoComputerActResult(ok: true, cursorX: 1, cursorY: 0)
            }
        }
        taskBox.task = action
    ...
    @Test func `new lifecycle generation cancels old work before fresh action`() async throws {
        ...
        let releaseProbe = LifecycleReleaseProbe(allowed: true)
        let queue = ComputerActionExecutionQueue(onLifecycleRelease: releaseProbe.attempt)
        let oldParams = LuckyNemoComputerActParams(action: .leftClick, x: 1, y: 0, refWidth: 1280)
        let freshParams = LuckyNemoComputerActParams(action: .leftClick, x: 2, y: 0, refWidth: 1280)
    ...
    @Test func `failed lifecycle mouse up blocks newer generation until retry succeeds`() async throws {
        ...
        let queue = ComputerActionExecutionQueue(onLifecycleRelease: releaseProbe.attempt)
        let params = LuckyNemoComputerActParams(action: .type, x: 2, y: 0, refWidth: 1280)
