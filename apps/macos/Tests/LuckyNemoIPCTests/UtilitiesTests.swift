...
@Suite(.serialized) struct UtilitiesTests {
    ...
    @Test func `gateway entrypoint prefers dist over bin`() throws {
        ...
        let bin = tmp.appendingPathComponent("bin/luckynemo.js")
    ...
    @Test func `log locator picks newest log file`() throws {
        let fm = FileManager()
        let dir = URL(fileURLWithPath: "/tmp/luckynemo", isDirectory: true)
        try? fm.createDirectory(at: dir, withIntermediateDirectories: true)

        let older = dir.appendingPathComponent("luckynemo-old-\(UUID().uuidString).log")
        let newer = dir.appendingPathComponent("luckynemo-new-\(UUID().uuidString).log")
