...
struct DeviceIdentityStoreTests {
    ...
    func `device auth owners preserve exact unicode bytes`() throws {
        ...
        let stateDirPath = try #require(getenv("LUCKYNEMO_STATE_DIR").map { String(cString: $0) })
    ...
    func `secondary profiles use separate identity and auth files`() throws {
        ...
        // getenv, not ProcessInfo: the trait pins LUCKYNEMO_STATE_DIR via setenv and
    ...
    func `does not overwrite an existing unrecognized identity file`() throws {
        ...
        let stored = """
        {
          "schema": "future-luckynemo-device-identity",
    ...
    // Regression: an explicit LUCKYNEMO_STATE_DIR override must never import the
