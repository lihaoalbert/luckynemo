...
private func cachedMessage(role: String, text: String, timestamp: Double) -> LuckyNemoChatMessage {
...
private func visibleTexts(_ vm: LuckyNemoChatViewModel) async -> [String] {
    await MainActor.run { vm.messages.map { $0.content.compactMap(\.text).joined() } }
}

private func cachedSessionEntry(key: String, updatedAt: Double) -> LuckyNemoChatSessionEntry {
...
private actor TestTranscriptCache: LuckyNemoChatTranscriptCache {
    private var transcripts: [String: [LuckyNemoChatMessage]]
    private var sessions: [LuckyNemoChatSessionEntry]
    private let loadSessionsHook: (@Sendable () async -> Void)?
    private(set) var storedTranscriptSessionKeys: [String] = []
    private(set) var storedTranscripts: [[LuckyNemoChatMessage]] = []
    private(set) var storedSessionsCallCount = 0

    init(
        transcripts: [String: [LuckyNemoChatMessage]] = [:],
...
private final class GatedHistoryChatTransport: @unchecked Sendable, LuckyNemoChatTransport {
    private let historyGate: AsyncStream<Void>.Continuation?
    private let historyGateStream: AsyncStream<Void>?
    private let historyResult: @Sendable (String, Int) throws -> LuckyNemoChatHistoryPayload
    ...
    init(
        gated: Bool,
        historyResult: @escaping @Sendable (String, Int) throws -> LuckyNemoChatHistoryPayload)
    {
        ...
        var eventCont: AsyncStream<LuckyNemoChatTransportEvent>.Continuation!
    ...
    {
        LuckyNemoChatSendResponse(runId: idempotencyKey, status: "accepted")
    }
...
struct ChatViewModelTranscriptCacheTests {
    @Test func `cold open paints cached transcript then live history replaces it`() async throws {
        ...
        let vm = await MainActor.run {
            LuckyNemoChatViewModel(sessionKey: "main", transport: transport, transcriptCache: cache)
        }
