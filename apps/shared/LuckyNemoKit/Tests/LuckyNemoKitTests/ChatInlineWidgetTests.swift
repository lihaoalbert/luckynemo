...
struct ChatInlineWidgetTests {
    @Test func `decodes projected canvas widget block`() throws {
        let data = Data(
            #"{"role":"assistant","content":[{"type":"text","text":"Done"},{"type":"canvas","preview":{"kind":"canvas","surface":"assistant_message","render":"url","title":"Status","preferredHeight":240,"url":"/__luckynemo__/canvas/documents/widget-1/index.html","sandbox":"scripts"}}]}"#
                .utf8)

        let message = try JSONDecoder().decode(LuckyNemoChatMessage.self, from: data)
        let preview = try #require(message.content.last?.preview)
        #expect(preview.inlineWidgetPath == "/__luckynemo__/canvas/documents/widget-1/index.html")
        #expect(preview.inlineWidgetHeight == 240)

        let unsafe = LuckyNemoChatCanvasPreview(
            kind: "canvas",
    ...
    @Test func `resolves only capability scoped widget documents`() {
        let surface = "https://gateway.example/__luckynemo__/cap/token"
        let target = "/__luckynemo__/canvas/documents/widget-1/index.html"

        #expect(LuckyNemoChatWidgetURLResolver.resolve(
            surfaceURL: surface,
            target: target)?.absoluteString ==
            "https://gateway.example/__luckynemo__/cap/token/__luckynemo__/canvas/documents/widget-1/index.html")
        #expect(LuckyNemoChatWidgetURLResolver.resolve(surfaceURL: "https://gateway.example", target: target) == nil)
        #expect(LuckyNemoChatWidgetURLResolver.resolve(
            surfaceURL: surface,
            target: "https://attacker.example/widget.html") == nil)
        #expect(LuckyNemoChatWidgetURLResolver.resolve(
            surfaceURL: surface,
            target: "/__luckynemo__/a2ui/index.html") == nil)
        #expect(LuckyNemoChatWidgetURLResolver.resolve(
            surfaceURL: surface,
            target: "/__luckynemo__/canvas/documents/%252e%252e/index.html") == nil)
        #expect(LuckyNemoChatWidgetURLResolver.resolve(
            surfaceURL: surface,
            target: "/__luckynemo__/canvas/documents/%2525252525252525252525252525252525/index.html") == nil)
    }
    ...
    @Test func `resource equality ignores internal recovery metadata`() throws {
        ...
        let publicResource = LuckyNemoChatWidgetResource(url: url)
        let trackedResource = LuckyNemoChatWidgetResource(
            url: url,
    ...
    @Test func `uses replacement route after capability refresh loses its lease`() async throws {
        let target = "/__luckynemo__/canvas/documents/widget-1/index.html"
        let oldSurface = "https://gateway.example/__luckynemo__/cap/old"
        let newSurface = "https://gateway.example/__luckynemo__/cap/new"
        let failedURL = try #require(LuckyNemoChatWidgetURLResolver.resolve(
            surfaceURL: oldSurface,
            target: target))
        let failedResource = LuckyNemoChatWidgetResource(url: failedURL)
        ...
        let resolved = await LuckyNemoChatWidgetURLResolver.resolveResource(
            target: target,
        ...
        #expect(resolved?.url == LuckyNemoChatWidgetURLResolver.resolve(surfaceURL: newSurface, target: target))
    ...
    @Test func `accepts a same URL widget route when it acquires a TLS pin`() async throws {
        ...
        let initialResource = await LuckyNemoChatWidgetURLResolver.resolveResource(
            target: target,
            replacing: nil,
            currentSurfaceRoutes: { await probe.current() },
    ...
    @Test func `refreshes node once before trying the operator fallback`() async {
        ...
        let oldSurface = "https://gateway.example/__luckynemo__/cap/old"
        let newSurface = "https://gateway.example/__luckynemo__/cap/new"
        let fallbackSurface = "https://operator.example/__luckynemo__/cap/fallback"
        let probe = ChatWidgetRouteReconnectProbe(
            route: GatewayCanvasHostRoute(url: oldSurface, tlsFingerprintSHA256: nil),
            replacement: GatewayCanvasHostRoute(url: newSurface, tlsFingerprintSHA256: nil),
            operatorRoute: GatewayCanvasHostRoute(url: fallbackSurface, tlsFingerprintSHA256: nil))
        let initialNode = await LuckyNemoChatWidgetURLResolver.resolveResource(
            target: target,
            replacing: nil,
            currentSurfaceRoutes: { await probe.current() },
            refreshNodeSurfaceRoute: { _ in nil },
        ...
        let refreshedNode = await LuckyNemoChatWidgetURLResolver.resolveResource(
            target: target,
        ...
        #expect(refreshedNode?.url == LuckyNemoChatWidgetURLResolver.resolve(surfaceURL: newSurface, target: target))

        let fallback = await LuckyNemoChatWidgetURLResolver.resolveResource(
            target: target,
        ...
        #expect(fallback?.url == LuckyNemoChatWidgetURLResolver.resolve(
    ...
    @Test func `refreshes the operator capability when the node route is unavailable`() async throws {
        let target = "/__luckynemo__/canvas/documents/widget-1/index.html"
        let oldSurface = "https://operator.example/__luckynemo__/cap/old"
        let newSurface = "https://operator.example/__luckynemo__/cap/new"
        let failedURL = try #require(LuckyNemoChatWidgetURLResolver.resolve(
            surfaceURL: oldSurface,
            target: target))
        let failedResource = LuckyNemoChatWidgetResource(url: failedURL)
    ...
    @Test func `rejects a TLS pin on a cleartext widget route`() async {
        ...
        let route = GatewayCanvasHostRoute(
            url: "http://gateway.example/__luckynemo__/cap/token",
