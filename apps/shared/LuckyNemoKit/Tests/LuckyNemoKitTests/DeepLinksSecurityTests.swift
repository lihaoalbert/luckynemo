...
@Suite struct DeepLinksSecurityTests {
    ...
    @Test func gatewayDeepLinkRejectsInsecureNonLoopbackWs() {
        let url = URL(
            string: "luckynemo://gateway?host=attacker.example&port=18789&tls=0&token=abc")!
        #expect(DeepLinkParser.parse(url) == nil)
    }

    @Test func gatewayDeepLinkRejectsInsecurePrefixBypassHost() {
        let url = URL(
            string: "luckynemo://gateway?host=127.attacker.example&port=18789&tls=0&token=abc")!
        #expect(DeepLinkParser.parse(url) == nil)
    }

    @Test func gatewayDeepLinkAllowsLoopbackWs() {
        let url = URL(
            string: "luckynemo://gateway?host=127.0.0.1&port=18789&tls=0&token=abc")!
        #expect(
    ...
    @Test func setupCodeAllowsMDNSWs() {
        let payload = #"{"url":"ws://luckynemo.local:18789","bootstrapToken":"tok"}"#
        #expect(
            GatewayConnectDeepLink.fromSetupCode(setupCode(from: payload)) == .init(
                host: "luckynemo.local",
                port: 18789,
                tls: false,
                bootstrapToken: "tok",
                token: nil,
    ...
    @Test func setupCodeAllowsPrivateLanHostPayload() {
        let payload = #"{"host":"luckynemo.local","port":18789,"tls":false,"bootstrapToken":"tok"}"#
        #expect(
            GatewayConnectDeepLink.fromSetupCode(setupCode(from: payload)) == .init(
                host: "luckynemo.local",
                port: 18789,
