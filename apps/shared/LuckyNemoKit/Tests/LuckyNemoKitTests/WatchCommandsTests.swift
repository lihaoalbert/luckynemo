...
struct WatchCommandsTests {
    @Test func `app snapshot dual writes semantic and legacy status fields`() throws {
        let message = LuckyNemoWatchAppSnapshotMessage(
            gatewayStatus: LuckyNemoWatchAppStatus(code: .gatewayConnected),
            gatewayStatusText: "Connected",
            gatewayConnected: true,
            agentName: "Main",
            sessionKey: "main",
            talkStatus: LuckyNemoWatchAppStatus(code: .talkOff),
            talkStatusText: "Off",
            talkEnabled: false,
            talkListening: false,
            talkSpeaking: false,
            pendingApprovalCount: 0,
            chatStatus: LuckyNemoWatchAppStatus(code: .chatNoMessages),
    ...
    @Test func `semantic chat status always writes the shipped text field`() throws {
        let message = LuckyNemoWatchAppSnapshotMessage(
            ...
            talkSpeaking: false,
            pendingApprovalCount: 0,
            chatStatus: LuckyNemoWatchAppStatus(code: .chatUnavailable))
    ...
    @Test func `unknown semantic statuses fall back to legacy text`() throws {
        ...
        let message = try JSONDecoder().decode(
            LuckyNemoWatchAppSnapshotMessage.self,
            from: Data(json.utf8))

        #expect(message.gatewayStatus == LuckyNemoWatchAppStatus(
            code: .legacy,
            verbatim: "Future gateway state"))
        #expect(message.talkStatus == LuckyNemoWatchAppStatus(
            code: .legacy,
            verbatim: "Future Talk state"))
        #expect(message.chatStatus == LuckyNemoWatchAppStatus(
            code: .legacy,
            verbatim: "Future chat state"))
    }

    @Test func `approval resolution dual writes semantic and legacy outcomes`() throws {
        let message = LuckyNemoWatchExecApprovalResolvedMessage(
