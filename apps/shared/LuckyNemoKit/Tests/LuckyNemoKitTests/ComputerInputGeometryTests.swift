...
struct ComputerInputGeometryTests {
    private func display(
        ...
        originY: Double = 0,
        width: Double,
        height: Double) -> LuckyNemoComputerDisplayGeometry
    {
        LuckyNemoComputerDisplayGeometry(
            originX: originX,
            originY: originY,
            widthPoints: width,
            heightPoints: height)
    ...
    private func capturedWidth(refWidth: Int?, sourceWidth: Double, sourceHeight: Double = 0) -> Double {
        LuckyNemoComputerInputGeometry.capturedWidth(
            refWidth: refWidth,
    ...
    @Test func `display frame identity changes with physical display geometry`() {
        ...
        let original = LuckyNemoComputerInputGeometry.displayFrameId(
            displayID: 42,
            sourceWidth: 2560,
            sourceHeight: 1600,
            referenceWidth: 1280,
            display: display)
        let repeated = LuckyNemoComputerInputGeometry.displayFrameId(
            displayID: 42,
            sourceWidth: 2560,
            sourceHeight: 1600,
            referenceWidth: 1280,
            display: display)
        let reindexed = LuckyNemoComputerInputGeometry.displayFrameId(
            displayID: 43,
            sourceWidth: 2560,
            sourceHeight: 1600,
            referenceWidth: 1280,
            display: display)
        let resized = LuckyNemoComputerInputGeometry.displayFrameId(
            displayID: 42,
            sourceWidth: 1920,
            sourceHeight: 1200,
            referenceWidth: 1280,
            display: display)
        let moved = LuckyNemoComputerInputGeometry.displayFrameId(
            displayID: 42,
            sourceWidth: 2560,
            sourceHeight: 1600,
            referenceWidth: 1280,
        ...
        let rescaled = LuckyNemoComputerInputGeometry.displayFrameId(
            displayID: 42,
            sourceWidth: 2560,
            sourceHeight: 1600,
            referenceWidth: 640,
    ...
    @Test func `accepts finite positive mapping geometry with negative display origins`() {
        ...
        #expect(LuckyNemoComputerInputGeometry.isValidMappingGeometry(
            sourceWidth: 2560,
            sourceHeight: 1440,
            display: display))
    }
    ...
    @Test func `clamps far-edge and out-of-bounds points strictly inside the display`() {
        ...
        let farEdge = LuckyNemoComputerInputGeometry.clampToDisplay(
            x: 100 + 1280,
            y: 200 + 800,
            display: display)
        ...
        let negative = LuckyNemoComputerInputGeometry.clampToDisplay(x: 99, y: 199, display: display)
        #expect(negative.x == 100)
        #expect(negative.y == 200)
        // A point already inside is unchanged.
        let inside = LuckyNemoComputerInputGeometry.clampToDisplay(x: 640, y: 400, display: display)
        #expect(inside.x == 640)
        #expect(inside.y == 400)
    }
    ...
    @Test func `decodes partial act params`() throws {
        ...
        let params = try JSONDecoder().decode(LuckyNemoComputerActParams.self, from: data)
    ...
    @Test func `decodes scroll and hold actions`() throws {
        ...
        let scrollParams = try JSONDecoder().decode(LuckyNemoComputerActParams.self, from: scroll)
        ...
        let holdParams = try JSONDecoder().decode(LuckyNemoComputerActParams.self, from: hold)
