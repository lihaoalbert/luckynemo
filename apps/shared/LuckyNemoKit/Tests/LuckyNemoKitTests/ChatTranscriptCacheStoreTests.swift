...
private func cacheSessionEntry(key: String, updatedAt: Double) -> LuckyNemoChatSessionEntry {
...
private func messageTexts(_ messages: [LuckyNemoChatMessage]) -> [String] {
...
private func outboxCommand(
    ...
    text: String,
    attachments: [LuckyNemoChatOutboxAttachment] = [],
    thinking: String = "off",
    createdAt: Double = Date().timeIntervalSince1970) -> LuckyNemoChatOutboxCommand
{
...
struct ChatTranscriptCacheStoreTests {
    @Test func `verified routing identity survives a cold store reopen`() async throws {
        ...
        let identity = try #require(LuckyNemoChatSessionRoutingIdentity(
            scope: " Per-Sender ",
            mainSessionKey: " Work ",
            defaultAgentID: " Main "))
        let store = LuckyNemoChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")

        await store.storeSessionRoutingIdentity(identity)
        #expect(LuckyNemoChatSQLiteTranscriptCache.loadSessionRoutingIdentity(
            databaseURL: url,
            gatewayID: "gw-a") == identity)

        let reopened = LuckyNemoChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
    ...
    @Test func `transcript keeps only most recent messages within bound`() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = LuckyNemoChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
        let bound = LuckyNemoChatSQLiteTranscriptCache.maxCachedMessagesPerSession
    ...
    @Test func `sessions list is bounded to most recently updated`() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = LuckyNemoChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
        let bound = LuckyNemoChatSQLiteTranscriptCache.maxCachedSessions
    ...
    @Test func `transcript eviction keeps most recent sessions`() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = LuckyNemoChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
        let bound = LuckyNemoChatSQLiteTranscriptCache.maxCachedTranscripts
    ...
    @Test func `transcripts are scoped per gateway identity`() async throws {
        let url = try makeDatabaseURL()
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let storeA = LuckyNemoChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
        let storeB = LuckyNemoChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-b")
    ...
    @Test func `reset retirement permits physical removal of all gateway rows`() async throws {
        ...
        await storeA.retire()
        await storeB.retire()
        LuckyNemoChatSQLiteTranscriptCache.removeDatabaseFiles(at: url)
        await storeA.storeTranscript(
            sessionKey: "main",
            messages: [cacheMessage(role: "user", text: "late write", timestamp: 3)])

        let readerA = LuckyNemoChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
        let readerB = LuckyNemoChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-b")
    ...
    @Test func `removing one gateway database preserves another`() async throws {
        ...
        let storeA = LuckyNemoChatSQLiteTranscriptCache(databaseURL: urlA, gatewayID: "gw-a")
        let storeB = LuckyNemoChatSQLiteTranscriptCache(databaseURL: urlB, gatewayID: "gw-b")
        ...
        await storeA.retire()
        LuckyNemoChatSQLiteTranscriptCache.removeDatabaseFiles(at: urlA)

        let readerA = LuckyNemoChatSQLiteTranscriptCache(databaseURL: urlA, gatewayID: "gw-a")
    ...
    @Test func `v1 transcript cache migrates to durable outbox schema`() async throws {
        ...
        let migrated = LuckyNemoChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
    ...
    @Test func `v2 migration parks rows without a routing contract`() async throws {
        ...
        #expect(commands.map(\.lastError) == [
            LuckyNemoChatSQLiteTranscriptCache.outboxUnknownTargetError,
            LuckyNemoChatSQLiteTranscriptCache.outboxUnknownTargetError,
            LuckyNemoChatSQLiteTranscriptCache.outboxUnconfirmedError,
            LuckyNemoChatSQLiteTranscriptCache.outboxUnconfirmedError,
            LuckyNemoChatSQLiteTranscriptCache.outboxUnknownTargetError,
            LuckyNemoChatSQLiteTranscriptCache.outboxUnknownTargetError,
        ])
    ...
    @Test func `unknown schema preserves durable outbox bytes and fails closed`() async throws {
        ...
        let blocked = LuckyNemoChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
        ...
        #expect(sqlite3_exec(
            raw,
            "PRAGMA user_version = \(LuckyNemoChatSQLiteTranscriptCache.schemaVersion)",
            nil,
            nil,
            nil) == SQLITE_OK)
        sqlite3_close_v2(raw)
        let recovered = LuckyNemoChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
    ...
    @Test func `undecodable row is dropped and treated as miss`() async throws {
        ...
        let reader = LuckyNemoChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
...
struct ChatCommandOutboxStoreTests {
    ...
    @Test func `interrupted sending rows fail closed on recovery`() async throws {
        ...
        #expect(recovered.map(\.lastError) == [LuckyNemoChatSQLiteTranscriptCache.outboxUnconfirmedError])
    ...
    @Test func `queued commands expire to failed at the staleness boundary`() async throws {
        ...
        let store = LuckyNemoChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
        let now = Date().timeIntervalSince1970
        let maxAge = LuckyNemoChatSQLiteTranscriptCache.outboxCommandMaxAge
        #expect(await store.enqueueCommand(
        ...
        #expect(stale.lastError == LuckyNemoChatSQLiteTranscriptCache.outboxExpiredError)
        #expect(fresh.status == .queued)
        #expect(fresh.lastError == nil)
        #expect(unconfirmed.status == .failed)
        #expect(unconfirmed.lastError == LuckyNemoChatSQLiteTranscriptCache.outboxUnconfirmedError)
    }
    ...
    @Test func `enqueue refuses beyond the queue bound`() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = LuckyNemoChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
        let bound = LuckyNemoChatSQLiteTranscriptCache.maxQueuedCommands
    ...
    @Test func `attachment byte admission enforces command and gateway bounds`() {
        let commandBound = LuckyNemoChatSQLiteTranscriptCache.maxAttachmentBytesPerCommand
        let gatewayBound = LuckyNemoChatSQLiteTranscriptCache.maxQueuedAttachmentBytes

        #expect(LuckyNemoChatSQLiteTranscriptCache.canEnqueueAttachmentBytes(
    ...
    @Test func `enqueue persists attachment bytes and refuses a full byte budget`() async throws {
        let url = try makeDatabaseURL()
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let attachment = LuckyNemoChatOutboxAttachment(
            type: "file",
            mimeType: "audio/mp4",
        ...
        #expect(sqlite3_exec(
            ...
            SET attachment_bytes = \(LuckyNemoChatSQLiteTranscriptCache.maxQueuedAttachmentBytes)
        ...
        let fullStore = LuckyNemoChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
