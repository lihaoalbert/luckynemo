...
final class VoiceNoteRecorderTests: XCTestCase {
    ...
    func testStartAndFinishProduceRecordingWithDuration() async throws {
        ...
        let recorder = LuckyNemoVoiceNoteRecorder(capture: capture)
    ...
    func testDurationLabelBoundsMalformedHistoryValues() {
        XCTAssertEqual(luckyNemoVoiceNoteDurationLabel(.infinity), "0:00")
        XCTAssertEqual(luckyNemoVoiceNoteDurationLabel(.nan), "0:00")
        XCTAssertEqual(luckyNemoVoiceNoteDurationLabel(1e100), "3:00")
        XCTAssertEqual(luckyNemoVoiceNoteDurationLabel(-1), "0:00")
    }
    ...
    func testRecordButtonRequiresAttachmentInput() {
        let recorder = LuckyNemoVoiceNoteRecorder(capture: FakeVoiceNoteAudioCapture())
        let control = LuckyNemoChatVoiceNoteControl(recorder: recorder, isTalkActive: false)
        let button = LuckyNemoVoiceNoteButton(
            control: control,
            compact: false,
