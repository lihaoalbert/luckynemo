...
private func outboxTestCommand(id: String, text: String, createdAt: Double) -> LuckyNemoChatOutboxCommand {
    LuckyNemoChatOutboxCommand(
        id: id,
        sessionKey: "main",
...
private func userTexts(_ vm: LuckyNemoChatViewModel) async -> [String] {
    await MainActor.run {
        vm.messages
            .filter { $0.role == "user" }
...
private final class OutboxTestTransport: @unchecked Sendable, LuckyNemoChatTransport {
    let state: OutboxTransportState
    private let sessions: [LuckyNemoChatSessionEntry]
    ...
    func listCommands(sessionKey _: String) async throws -> [LuckyNemoChatCommandChoice] {
    ...
    private func requestHistory(
        sessionKey: String,
        agentID: String?,
        expectedRoute: Int?) async throws -> LuckyNemoChatHistoryPayload
    {
        ...
        let durableUserRows = keys.indices.compactMap { index -> AnyCodable? in
            ...
            return AnyCodable([
                ...
                "__luckynemo": ["idempotencyKey": "\(key):user"],
            ] as [String: Any])
        }
        return LuckyNemoChatHistoryPayload(
            sessionKey: sessionKey,
    ...
    private func sendMessage(
        ...
        thinking: String,
        idempotencyKey: String,
        expectedRoute: Int?) async throws -> LuckyNemoChatSendResponse
    {
        if let expectedRoute, await state.routeGeneration != expectedRoute {
        ...
        if await self.state.sendRoutingChanged {
            throw GatewayResponseError(
                ...
                details: [
                    "reason": AnyCodable(LuckyNemoChatSessionRoutingContract.changedErrorReason),
                ])
        }
        if await self.state.sendRejects {
            // Gateway responded but refused to start the run.
            return LuckyNemoChatSendResponse(runId: idempotencyKey, status: "error")
        }
        ...
        return LuckyNemoChatSendResponse(runId: idempotencyKey, status: "accepted")
    }
...
private func outboxSessionEntry(
    key: String,
    thinkingLevels: [String]) -> LuckyNemoChatSessionEntry
{
    LuckyNemoChatSessionEntry(
        ...
        model: nil,
        contextTokens: nil,
        thinkingLevels: thinkingLevels.map { LuckyNemoChatThinkingLevelOption(id: $0, label: $0) })
}

private func makeOutboxViewModel(
    transport: OutboxTestTransport,
    outbox: any LuckyNemoChatCommandOutbox,
    transcriptCache: (any LuckyNemoChatTranscriptCache)? = nil,
    retryDelaysMs: [UInt64] = [1, 1],
    sessionKey: String = "main",
    activeAgentID: String? = "main",
    sessionRoutingContract: String? = "per-sender|main|main") async -> LuckyNemoChatViewModel
{
    await MainActor.run {
...
private func sendWhileOffline(_ vm: LuckyNemoChatViewModel, text: String) async throws {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
@MainActor
private func queuedStateCount(_ vm: LuckyNemoChatViewModel) -> Int {
...
private actor DelayingOutbox: LuckyNemoChatCommandOutbox {
    private nonisolated let base: LuckyNemoChatSQLiteTranscriptCache
    ...
    init(base: LuckyNemoChatSQLiteTranscriptCache) {
        self.base = base
    }

    nonisolated func changes() -> AsyncStream<LuckyNemoChatOutboxChange> {
        self.base.changes()
    }
    ...
    func enqueueCommand(_ command: LuckyNemoChatOutboxCommand) async -> Bool {
    ...
    func loadCommands() async -> [LuckyNemoChatOutboxCommand] {
    ...
    func loadCommandsIfAvailable() async -> [LuckyNemoChatOutboxCommand]? {
        if self.loadDelayNanoseconds > 0 {
            try? await Task.sleep(nanoseconds: self.loadDelayNanoseconds)
        }
    ...
    func claimNextCommand() async -> LuckyNemoChatOutboxCommand? {
    ...
    func markCommandAwaitingConfirmation(id: String) async -> LuckyNemoChatOutboxUpdateResult {
    ...
    func markCommandFailedIfPresent(
        id: String,
        retryCount: Int,
        lastError: String?) async -> LuckyNemoChatOutboxUpdateResult
    {
    ...
    func markCommandRetriedIfPresent(
        ...
        routingContract: String) async -> LuckyNemoChatOutboxUpdateResult
    {
    ...
    func cancelCommand(id: String) async -> LuckyNemoChatOutboxUpdateResult {
        await self.base.cancelCommand(id: id)
    }

    func confirmCommand(id: String) async -> LuckyNemoChatOutboxUpdateResult {
...
private actor SnapshotHoldingOutbox: LuckyNemoChatCommandOutbox {
    private nonisolated let base: LuckyNemoChatSQLiteTranscriptCache
...
private actor CancellationHoldingOutbox: LuckyNemoChatCommandOutbox {
    private nonisolated let base: LuckyNemoChatSQLiteTranscriptCache
...
struct ChatViewModelOutboxTests {
    @Test func `offline send queues durably and renders queued row`() async throws {
        ...
        let store = LuckyNemoChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-test")
    ...
    @Test func `unsupported gateway keeps queued work and surfaces upgrade action`() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = LuckyNemoChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-test")
        let message = LuckyNemoChatTransportUpgradeMessage.routingContract
    ...
    @Test func `inert outbox does not capability gate healthy live chat`() async throws {
        ...
        let transport = OutboxTestTransport(
            healthy: true,
            routeUnavailableReason: LuckyNemoChatTransportUpgradeMessage.routingContract)
        let vm = await makeOutboxViewModel(transport: transport, outbox: store)
        ...
        parked.lastError = LuckyNemoChatSQLiteTranscriptCache.outboxChangedTargetError
    ...
    @Test func `reconnect waits for canonical session metadata before flushing Ultra`() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = LuckyNemoChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-test")
        #expect(await store.enqueueCommand(LuckyNemoChatOutboxCommand(
            id: "alpha-ultra",
            sessionKey: "main",
    ...
    @Test func `changed default agent parks command visibly for retry`() async throws {
        ...
        #expect(await store.loadCommands().first?.lastError ==
            LuckyNemoChatSQLiteTranscriptCache.outboxChangedTargetError)
    ...
    @Test func `atomic gateway routing rejection parks without retrying`() async throws {
        ...
        let command = try #require(await store.loadCommands().first)
        #expect(command.lastError == LuckyNemoChatSQLiteTranscriptCache.outboxChangedTargetError)
    ...
    @Test func `failed alias row remains reachable after owner change`() async throws {
        ...
        #expect(await store.enqueueCommand(LuckyNemoChatOutboxCommand(
            ...
            status: .failed,
            retryCount: 1,
            lastError: LuckyNemoChatSQLiteTranscriptCache.outboxExpiredError)))
        let transport = OutboxTestTransport(healthy: false)
        let vm = await makeOutboxViewModel(
            transport: transport,
            outbox: store,
    ...
    @Test func `ownerless global retry stays failed without a selected agent`() async throws {
        ...
        #expect(await store.enqueueCommand(LuckyNemoChatOutboxCommand(
            ...
            status: .failed,
            retryCount: 0,
            lastError: LuckyNemoChatSQLiteTranscriptCache.outboxUnknownTargetError)))
        let transport = OutboxTestTransport(healthy: false)
        let vm = await makeOutboxViewModel(
            transport: transport,
            outbox: store,
    ...
    @Test func `delete race preserves a turn already proven by canonical history`() async throws {
        ...
        let canonicalMessage = LuckyNemoChatMessage(
            role: "user",
            content: [LuckyNemoChatMessageContent(
                type: "text",
                text: "delivered already",
                mimeType: nil,
                fileName: nil,
    ...
    @Test func `gateway rejections burn attempts then fail terminally and support tap retry`() async throws {
        ...
        #expect(failed.retryCount == LuckyNemoChatViewModel.maxOutboxSendAttempts)
    ...
    @Test func `unavailable terminal write drops health instead of advancing FIFO`() async throws {
        ...
        #expect(await store.enqueueCommand(LuckyNemoChatOutboxCommand(
            ...
            createdAt: Date().timeIntervalSince1970,
            status: .queued,
            retryCount: LuckyNemoChatViewModel.maxOutboxSendAttempts - 1,
            lastError: "rejected")))
        let outbox = DelayingOutbox(base: store)
    ...
    @Test func `ambiguous live transport failure requires explicit retry`() async throws {
        ...
        #expect(preserved.lastError == LuckyNemoChatSQLiteTranscriptCache.outboxUnconfirmedError)
    ...
    @Test func `tap retry refreshes createdAt so an expired command can resend`() async throws {
        ...
        let staleCreatedAt = Date().timeIntervalSince1970 -
            LuckyNemoChatSQLiteTranscriptCache.outboxCommandMaxAge - 60
        ...
        #expect(await store.loadCommands().map(\.lastError) == [
            LuckyNemoChatSQLiteTranscriptCache.outboxExpiredError,
        ])
    ...
    @Test func `background canonical alias event confirms by idempotency key`() async throws {
        ...
        transport.emit(.sessionMessage(LuckyNemoSessionMessageEventPayload(
    ...
    @Test func `full queue refuses enqueue and keeps the draft`() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = LuckyNemoChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-test")
        for index in 0..<LuckyNemoChatSQLiteTranscriptCache.maxQueuedCommands {
        ...
        #expect(await userTexts(vm).isEmpty)
        #expect(await store.loadCommands().count == LuckyNemoChatSQLiteTranscriptCache.maxQueuedCommands)
    }
    ...
    @Test func `queued send transport failure fails closed until explicit retry`() async throws {
        ...
        let command = try #require(await store.loadCommands().first)
        #expect(command.lastError == LuckyNemoChatSQLiteTranscriptCache.outboxUnconfirmedError)
        #expect(command.retryCount == 0)
        #expect(await transport.state.sentIdempotencyKeys.isEmpty)
