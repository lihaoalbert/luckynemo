...
private final class ContextUsageTestTransport: @unchecked Sendable, LuckyNemoChatTransport {
...
struct ChatContextUsageTests {
    private func message(
        ...
        usage: LuckyNemoChatUsage? = nil) -> LuckyNemoChatMessage
    {
    ...
    private func usage(
        ...
        cacheWrite: Int? = nil,
        total: Int? = nil,
        costTotal: Double? = nil) throws -> LuckyNemoChatUsage
    {
        ...
        return try JSONDecoder().decode(LuckyNemoChatUsage.self, from: data)
    }
    ...
    @Test func `falls back to session totals without message usage`() {
        let entry = LuckyNemoChatSessionEntry(
            key: "main",
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    @Test @MainActor func `view model resolves context totals through a selected global alias`() {
        let vm = LuckyNemoChatViewModel(
            ...
            activeAgentId: "ops")
        vm.sessions = [LuckyNemoChatSessionEntry(
