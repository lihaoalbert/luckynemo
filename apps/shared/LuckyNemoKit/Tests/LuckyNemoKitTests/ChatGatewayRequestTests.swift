...
struct ChatGatewayRequestTests {
    @Test func `session targets share normalization while preserving platform routing policy`() {
        #expect(LuckyNemoChatSessionTarget.resolve(
            " Matrix:Channel:Room ",
    ...
    @Test func `settings patch request encodes default model as null`() {
        let request = LuckyNemoChatGatewayRequests.patchSessionSettings(
    ...
    @Test func `settings patch request encodes fast values and explicit resets`() {
        let reset = LuckyNemoChatGatewayRequests.patchSessionSettings(
            sessionKey: "main",
            agentID: nil,
            thinkingLevel: .some(nil),
            fastMode: .some(nil),
            verboseLevel: .some(nil))
        let automatic = LuckyNemoChatGatewayRequests.patchSessionSettings(
            sessionKey: "main",
            agentID: nil,
    ...
    @Test func `fork and create requests preserve routing identity`() {
        let fork = LuckyNemoChatGatewayRequests.forkSession(
        ...
        let create = LuckyNemoChatGatewayRequests.createSession(
    ...
    @Test func `rename clear archive and fork use session mutation contracts`() {
        let rename = LuckyNemoChatGatewayRequests.patchSession(
            ...
            archived: nil,
            unread: nil)
        let archive = LuckyNemoChatGatewayRequests.patchSession(
            sessionKey: "agent:main:child",
            agentID: nil,
            label: nil,
            category: nil,
    ...
    @Test func `commands request selects session agent before fallback`() {
        let scoped = LuckyNemoChatGatewayRequests.commandsList(
        ...
        let global = LuckyNemoChatGatewayRequests.commandsList(
            sessionKey: "global",
    ...
    @Test func `send request shares attachment encoding and timeout policy`() throws {
        let request = LuckyNemoChatGatewayRequests.sendMessage(
            sessionKey: "global",
            agentID: " reviewer ",
    ...
    @Test func `question resolve request preserves nested answer contract`() throws {
        let request = LuckyNemoChatGatewayRequests.resolveQuestion(
            id: "ask_123",
    ...
    @Test func `long running requests share exact gateway timeout margins`() {
        #expect(LuckyNemoChatGatewayRequests.agentWait(runID: "run-1", timeoutMs: 1).timeoutMs == 5001)
        #expect(LuckyNemoChatGatewayRequests.agentWait(runID: "run-1", timeoutMs: 30000).timeoutMs == 35000)
        #expect(LuckyNemoChatGatewayRequests.compactSession(
            sessionKey: "main",
...
struct ChatGatewayPayloadCodecTests {
    @Test func `session key extracts canonical agent identity`() {
        #expect(LuckyNemoChatSessionKey.agentID(from: " agent:Reviewer:main ") == "Reviewer")
        #expect(LuckyNemoChatSessionKey.agentID(from: "agent::main") == nil)
        #expect(LuckyNemoChatSessionKey.agentID(from: "global") == nil)
    }

    @Test func `agent wait distinguishes terminal and retryable timeouts`() throws {
        #expect(try LuckyNemoChatGatewayPayloadCodec.decodeAgentWaitObservation(
    ...
    @Test func `routing identity decodes agent and canonical contract`() throws {
        let identity = try LuckyNemoChatGatewayPayloadCodec.decodeSessionRoutingIdentity(
    ...
    @Test func `model choices preserve metadata and replace blank names`() throws {
        let choices = try LuckyNemoChatGatewayPayloadCodec.decodeModelChoices(Data(
            #"{"models":[{"id":"gpt-5","name":"  ","provider":"openai","contextWindow":200000,"reasoning":true}]}"#
                .utf8))

        #expect(choices == [LuckyNemoChatModelChoice(
            modelID: "gpt-5",
            name: "gpt-5",
            provider: "openai",
    ...
    @Test func `command choice normalizes source aliases and identity`() {
        let choice = LuckyNemoChatGatewayPayloadCodec.commandChoice(CommandEntry(
            name: "review",
            textaliases: [" /review ", ""],
    ...
    @Test func `event frames map to shared chat transport events`() {
        ...
        guard case let .sessionsChanged(change) = LuckyNemoChatGatewayPayloadCodec.event(from: sessionsChanged)
        else {
        ...
        guard case let .chat(payload) = LuckyNemoChatGatewayPayloadCodec.event(from: chat) else {
        ...
        #expect(LuckyNemoChatGatewayPayloadCodec.event(from: EventFrame(
            type: "event",
            event: "unknown")) == nil)
    }
}
