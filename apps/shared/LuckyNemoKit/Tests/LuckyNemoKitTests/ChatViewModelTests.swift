    ...
    idempotencyKey: String? = nil) -> AnyCodable
{
    ...
    if let idempotencyKey {
        message["__luckynemo"] = ["idempotencyKey": idempotencyKey]
    }
...
private func chatTextModelMessage(
    ...
    text: String,
    timestamp: Double,
    idempotencyKey: String? = nil) -> LuckyNemoChatMessage
{
    LuckyNemoChatMessage(
        role: role,
        content: [
...
extension [LuckyNemoChatMessage] {
...
private func historyPayload(
    ...
    inFlightRun: LuckyNemoChatInFlightRun? = nil) -> LuckyNemoChatHistoryPayload
{
...
private func sessionEntry(
    ...
    contextTokens: Int? = nil) -> LuckyNemoChatSessionEntry
{
...
private func sessionsListResponse(_ sessions: [LuckyNemoChatSessionEntry]) -> LuckyNemoChatSessionsListResponse {
...
private func thinkingOption(_ id: String, label: String? = nil) -> LuckyNemoChatThinkingLevelOption {
    LuckyNemoChatThinkingLevelOption(id: id, label: label ?? id)
}
...
private func modelChoice(
    ...
    reasoning: Bool? = nil) -> LuckyNemoChatModelChoice
{
...
private func sessionsResponse(
    _ session: LuckyNemoChatSessionEntry,
    defaults: LuckyNemoChatSessionsDefaults? = nil) -> LuckyNemoChatSessionsListResponse
{
    LuckyNemoChatSessionsListResponse(
        ts: 1,
        path: nil,
...
private func commandChoice(
    ...
    aliases: [String],
    description: String = "",
    source: LuckyNemoChatCommandChoice.Source = .command,
    acceptsArgs: Bool = false) -> LuckyNemoChatCommandChoice
{
    LuckyNemoChatCommandChoice(
...
private func makeViewModel(
    ...
    historyResponses: [LuckyNemoChatHistoryPayload],
    sessionRoutingContract: String? = nil,
    sessionsResponses: [LuckyNemoChatSessionsListResponse] = [],
    modelResponses: [[LuckyNemoChatModelChoice]] = [],
    modelPatchResults: [LuckyNemoChatModelPatchResult?] = [],
    thinkingPatchResults: [LuckyNemoChatModelPatchResult?] = [],
    commandResponses: [[LuckyNemoChatCommandChoice]] = [],
    requestHistoryHook: (@Sendable (String) async throws -> Void)? = nil,
    historyResponseHook: (@Sendable (String, Int, [String]) async throws -> LuckyNemoChatHistoryPayload?)? = nil,
    ...
    sessionSettingsPatchHook: (
        @Sendable (LuckyNemoChatSessionSettingsPatch) async throws -> LuckyNemoChatModelPatchResult?)? = nil,
    ...
    listSessionsHook: (
        @Sendable (TestSessionListQuery) async throws -> LuckyNemoChatSessionsListResponse?)? = nil,
    sendMessageHook: (@Sendable (String) async throws -> LuckyNemoChatSendResponse)? = nil,
    sendMessageStatus: String = "ok",
    waitForRunCompletionHook: (@Sendable (String, Int) async -> LuckyNemoChatRunObservation)? = nil,
    ...
    -> (TestChatTransport, LuckyNemoChatViewModel)
{
...
private func sendUserMessage(_ vm: LuckyNemoChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, LuckyNemoChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [LuckyNemoChatHistoryPayload]
    private let sessionsResponses: [LuckyNemoChatSessionsListResponse]
    private let modelResponses: [[LuckyNemoChatModelChoice]]
    private let modelPatchResults: [LuckyNemoChatModelPatchResult?]
    private let thinkingPatchResults: [LuckyNemoChatModelPatchResult?]
    private let commandResponses: [[LuckyNemoChatCommandChoice]]
    private let requestHistoryHook: (@Sendable (String) async throws -> Void)?
    private let historyResponseHook:
        (@Sendable (String, Int, [String]) async throws -> LuckyNemoChatHistoryPayload?)?
    ...
    private let sessionSettingsPatchHook:
        (@Sendable (LuckyNemoChatSessionSettingsPatch) async throws -> LuckyNemoChatModelPatchResult?)?
    ...
    private let listSessionsHook:
        (@Sendable (TestSessionListQuery) async throws -> LuckyNemoChatSessionsListResponse?)?
    private let sendMessageHook: (@Sendable (String) async throws -> LuckyNemoChatSendResponse)?
    private let sendMessageStatus: String
    private let waitForRunCompletionHook:
        (@Sendable (String, Int) async -> LuckyNemoChatRunObservation)?
        ...
        healthResponses: [Bool] = [true])
    {
        ...
        var cont: AsyncStream<LuckyNemoChatTransportEvent>.Continuation!
        ...
        worktree _: Bool?) async throws -> LuckyNemoChatCreateSessionResponse
    {
        ...
        return LuckyNemoChatCreateSessionResponse(ok: true, key: key, sessionId: "created-\(key)")
    }
    ...
    func requestHistory(sessionKey: String) async throws -> LuckyNemoChatHistoryPayload {
        ...
        return self.historyResponses.last ?? LuckyNemoChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
        ...
        attachments _: [LuckyNemoChatAttachmentPayload]) async throws -> LuckyNemoChatSendResponse
    {
        ...
        return LuckyNemoChatSendResponse(runId: idempotencyKey, status: self.sendMessageStatus)
    }
        ...
        archived: Bool) async throws -> LuckyNemoChatSessionsListResponse
    {
        ...
        return self.sessionsResponses.last ?? LuckyNemoChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    private func patchSessionThinking(
        sessionKey: String,
        thinkingLevel: String) async throws -> LuckyNemoChatModelPatchResult?
    {
        ...
        return LuckyNemoChatModelPatchResult(
            key: sessionKey,
            modelProvider: nil,
            model: nil,
            thinkingLevel: thinkingLevel)
    ...
    func emit(_ evt: LuckyNemoChatTransportEvent) {
...
struct ChatViewModelTests {
    @Test @MainActor func `locally expired question is evicted after terminal grace`() {
        let viewModel = LuckyNemoChatViewModel(
            sessionKey: "main",
    ...
    @Test @MainActor func `stale question list cannot overwrite a newer event`() async throws {
        ...
        let viewModel = LuckyNemoChatViewModel(sessionKey: "main", transport: transport)
    ...
    @Test func `context usage fraction validates freshness and token bounds`() {
        func fraction(total: Int?, fresh: Bool? = true, context: Int?) -> Double? {
            LuckyNemoChatViewModel.chatContextUsageFraction(
                for: sessionEntry(
                    key: "main",
                    updatedAt: 1,
                    totalTokens: total,
    ...
    @Test @MainActor func `event listener does not retain discarded view model`() async throws {
        ...
        var viewModel: LuckyNemoChatViewModel? = LuckyNemoChatViewModel(
            sessionKey: "main",
            transport: transport)
    ...
    @Test func `decodes in-flight run from chat history`() throws {
        ...
        let payload = try JSONDecoder().decode(LuckyNemoChatHistoryPayload.self, from: data)

        #expect(payload.inFlightRun?.runId == "run-active")
        #expect(payload.inFlightRun?.text == "partial")
        #expect(payload.inFlightRun?.plan?.steps == [
            LuckyNemoChatPlanStep(step: "Reconnect", status: .inProgress),
        ])
    ...
    @Test func `decodes agent scope from chat event`() throws {
        ...
        let payload = try JSONDecoder().decode(LuckyNemoChatEventPayload.self, from: data)
    ...
    @Test func `bootstrap adopts active history run and consumes live events`() async throws {
        let activeHistory = historyPayload(
            ...
            inFlightRun: LuckyNemoChatInFlightRun(runId: "run-active", text: "partial reply"))
    ...
    @Test func `adopts Codex history run without buffered text`() async throws {
        let history = historyPayload(
            inFlightRun: LuckyNemoChatInFlightRun(runId: "run-codex", text: ""))
    ...
    @Test func `bootstrap adopts in-flight plan snapshot`() async throws {
        let history = historyPayload(
            inFlightRun: LuckyNemoChatInFlightRun(
                runId: "run-plan",
                text: "working",
                plan: LuckyNemoChatPlanSnapshot(
                    steps: [
                        LuckyNemoChatPlanStep(step: "Inspect", status: .completed),
                        LuckyNemoChatPlanStep(step: "Reconnect", status: .inProgress),
                    ],
    ...
    @Test func `history plan reconciliation contract`() async {
        let retainedSteps = [LuckyNemoChatPlanStep(step: "Retained", status: .inProgress)]
        let liveSteps = [LuckyNemoChatPlanStep(step: "New live plan", status: .inProgress)]
        let cases: [(
            name: String,
            payload: LuckyNemoChatHistoryPayload,
            expectedRunId: String?,
            expectedSteps: [LuckyNemoChatPlanStep],
            ...
            (
                ...
                historyPayload(
                    inFlightRun: LuckyNemoChatInFlightRun(
                        ...
                        text: "working",
                        plan: LuckyNemoChatPlanSnapshot(
                            steps: [LuckyNemoChatPlanStep(step: "Replacement", status: .completed)]))),
                "run-retained",
                [LuckyNemoChatPlanStep(step: "Replacement", status: .completed)],
                false),
            (
                "legacy-preserve",
                historyPayload(
                    inFlightRun: LuckyNemoChatInFlightRun(runId: "run-retained", text: "working")),
                "run-retained",
                retainedSteps,
                false),
            (
                ...
                historyPayload(
                    inFlightRun: LuckyNemoChatInFlightRun(
                        ...
                        text: "next",
                        plan: LuckyNemoChatPlanSnapshot(
                            steps: [LuckyNemoChatPlanStep(step: "Next run", status: .inProgress)]))),
                "run-next",
                [LuckyNemoChatPlanStep(step: "Next run", status: .inProgress)],
                false),
            (
                "active-preserve",
            ...
            (
                ...
                historyPayload(
                    inFlightRun: LuckyNemoChatInFlightRun(
                        runId: "run-retained",
                        text: "working",
                        plan: LuckyNemoChatPlanSnapshot(steps: []))),
                nil,
                [],
                false),
        ]
    ...
    @Test func `foreground history refreshes adopted run snapshot`() async throws {
        let firstHistory = historyPayload(
            inFlightRun: LuckyNemoChatInFlightRun(runId: "run-active", text: "first partial"))
        let resumedHistory = historyPayload(
            inFlightRun: LuckyNemoChatInFlightRun(runId: "run-active", text: "resumed partial"))
        let historyCalls = AsyncCounter()
        ...
        let (_, vm) = await makeViewModel(
            ...
            historyResponseHook: { _, index, sentRunIds in
                ...
                return historyPayload(
                    ...
                    inFlightRun: LuckyNemoChatInFlightRun(runId: runId, text: "working"))
            },
    ...
    @Test func `foreground discovers run started without local ownership`() async throws {
        ...
        let activeHistory = historyPayload(
            inFlightRun: LuckyNemoChatInFlightRun(runId: "run-external", text: "external partial"))
    ...
    @Test func `foreground keeps active run with intermediate assistant history`() async throws {
        ...
        let activeHistory = historyPayload(
            ...
            inFlightRun: LuckyNemoChatInFlightRun(runId: "run-tool", text: "still working"))
        let (_, vm) = await makeViewModel(historyResponses: [idleHistory, activeHistory])
    ...
    @Test func `foreground synthesizes activity when no run snapshot or local run exists`() async throws {
        ...
        transport.emit(
            .sessionMessage(
                LuckyNemoSessionMessageEventPayload(
                    sessionKey: "main",
    ...
    @Test func `foreground clears completed run without assistant output`() async throws {
        let activeHistory = historyPayload(
            ...
            inFlightRun: LuckyNemoChatInFlightRun(
                ...
                text: "",
                plan: LuckyNemoChatPlanSnapshot(
                    steps: [LuckyNemoChatPlanStep(step: "Finish", status: .inProgress)])))
        let completedHistory = historyPayload(
            messages: [chatTextMessage(role: "user", text: "quiet task", timestamp: 1)],
            hasActiveRun: false)
    ...
    @Test func `foreground missing snapshot does not clear an in-flight send`() async throws {
        ...
        let historyCalls = AsyncCounter()
        let activeHistory = historyPayload(
            inFlightRun: LuckyNemoChatInFlightRun(runId: "run-active", text: "accepted"))
        let (_, vm) = await makeViewModel(
            historyResponses: [historyPayload(), historyPayload(), activeHistory],
            requestHistoryHook: { _ in _ = await historyCalls.increment() },
            sendMessageHook: { _ in
                await sendGate.wait()
                return LuckyNemoChatSendResponse(runId: "run-active", status: "pending")
            })
    ...
    @Test func `post-send history keeps active run with intermediate assistant output`() async throws {
        let activeHistory = historyPayload(
            ...
            inFlightRun: LuckyNemoChatInFlightRun(runId: "run-active", text: "working"))
        let (_, vm) = await makeViewModel(
            historyResponses: [historyPayload(), activeHistory],
            sendMessageHook: { _ in
                LuckyNemoChatSendResponse(runId: "run-active", status: "pending")
            })
    ...
    @Test func `post-send stale inactive history preserves newer live plan`() async throws {
        ...
        #expect(await MainActor.run { vm.planSteps == [
            LuckyNemoChatPlanStep(step: "Finish", status: .inProgress),
        ] })
    }
    ...
    @Test func `live chat delta owns run while bootstrap history is pending`() async throws {
        ...
        let staleHistory = historyPayload(
            ...
            inFlightRun: LuckyNemoChatInFlightRun(runId: "run-stale", text: "stale partial"))
        let (transport, vm) = await makeViewModel(
    ...
    @Test func `global chat delta adopts only selected agent run`() async throws {
        let bareGlobalMatches = await MainActor.run {
            (
                LuckyNemoChatViewModel.matchesCurrentSessionKey(
                    incoming: "global",
                    agentId: "work",
                    current: "global",
                    mainSessionKey: "main",
    ...
    @Test func `intermediate session message preserves pending recovery snapshot`() async throws {
        ...
        let historyCalls = AsyncCounter()
        let activeHistory = historyPayload(
            inFlightRun: LuckyNemoChatInFlightRun(runId: "run-active", text: "active partial"))
        let (transport, vm) = await makeViewModel(
    ...
    @Test func `manual refresh re-adopts active run after clearing local ownership`() async throws {
        ...
        let refreshedHistory = historyPayload(
            inFlightRun: LuckyNemoChatInFlightRun(runId: "run-active", text: "refreshed partial"))
    ...
    @Test func `older history cannot replace newer run snapshot`() async throws {
        ...
        let initialHistory = historyPayload(
            inFlightRun: LuckyNemoChatInFlightRun(runId: "run-initial", text: "initial"))
        let (_, vm) = await makeViewModel(
            ...
            historyResponseHook: { _, index, _ in
                if index == 1 {
                    ...
                    return historyPayload(
                        inFlightRun: LuckyNemoChatInFlightRun(runId: "run-older", text: "older"))
                }
                if index == 2 {
                    return historyPayload(
                        inFlightRun: LuckyNemoChatInFlightRun(runId: "run-newer", text: "newer"))
                }
                return nil
            })
    ...
    @Test func `delayed history cannot overwrite newer live run text`() async throws {
        ...
        let activeHistory = historyPayload(
            inFlightRun: LuckyNemoChatInFlightRun(runId: "run-active", text: "initial"))
        let (transport, vm) = await makeViewModel(
            historyResponses: [activeHistory],
            requestHistoryHook: { _ in _ = await historyCalls.increment() },
            historyResponseHook: { _, index, _ in
                ...
                return historyPayload(
                    inFlightRun: LuckyNemoChatInFlightRun(runId: "run-active", text: "stale"))
            })
    ...
    @Test func `external terminal event protects current run from delayed snapshot`() async throws {
        ...
        let currentHistory = historyPayload(
            inFlightRun: LuckyNemoChatInFlightRun(runId: "run-current", text: "current"))
        let (transport, vm) = await makeViewModel(
            ...
            historyResponseHook: { _, index, _ in
                if index == 1 {
                    ...
                    _ = await staleCompletions.increment()
                    return historyPayload(
                        inFlightRun: LuckyNemoChatInFlightRun(runId: "run-finished", text: "stale"))
                }
                if index == 2 {
                    throw NSError(domain: "test", code: 1)
                }
    ...
    @Test func `sequence gap re-adopts active run from history`() async throws {
        ...
        let recoveredHistory = historyPayload(
            inFlightRun: LuckyNemoChatInFlightRun(runId: "run-recovered", text: "recovered partial"))
    ...
    @Test func `displays error message fallback only for assistant error turns`() throws {
        func decodeMessage(role: String, stopReason: String, contentText: String? = nil) throws -> LuckyNemoChatMessage {
        ...
        #expect(
            LuckyNemoChatMessage.displayText(
                contentText: "",
                role: assistantError.role,
                stopReason: assistantError.stopReason,
                errorMessage: assistantError.errorMessage) == "stale provider failure")
    ...
    @Test func `duplicate final events append one provisional reply`() async throws {
        ...
        await sendUserMessage(vm, text: "hello")
        let runId = try await waitForLastSentRunId(transport)
        let final = LuckyNemoChatEventPayload(
            runId: runId,
            sessionKey: "main",
            state: "final",
            message: chatTextMessage(
    ...
    @Test func `later identical session reply does not adopt prior turn provisional final`() async throws {
        ...
        let (transport, vm) = await makeViewModel(
            ...
            sendMessageHook: { runId in
                LuckyNemoChatSendResponse(runId: runId, status: "pending")
            })
        try await loadAndWaitBootstrap(vm: vm, sessionId: sessionId)
    ...
    @Test func `plan event parses typed and legacy steps`() async throws {
        ...
        #expect(await MainActor.run { vm.planSteps } == [
            LuckyNemoChatPlanStep(step: "Inspect state", status: .inProgress),
            LuckyNemoChatPlanStep(step: "Write fix", status: .pending),
            LuckyNemoChatPlanStep(step: "Verify", status: .completed),
        ])
    ...
    @Test func `plan snapshots replace and empty snapshot clears`() async throws {
        ...
        try await waitUntil("replacement plan snapshot applies") {
            await MainActor.run {
                vm.planSteps == [LuckyNemoChatPlanStep(step: "Replacement", status: .pending)] &&
    ...
    @Test func `terminal event for another run preserves active plan`() async throws {
        ...
        let activeHistory = historyPayload(
            inFlightRun: LuckyNemoChatInFlightRun(
                ...
                text: "",
                plan: LuckyNemoChatPlanSnapshot(
                    steps: [LuckyNemoChatPlanStep(step: "Keep working", status: .inProgress)])))
        let (transport, vm) = await makeViewModel(
            historyResponses: [initialHistory, activeHistory, activeHistory],
            sendMessageHook: { _ in
                LuckyNemoChatSendResponse(runId: activeRunId, status: "pending")
            })
        try await loadAndWaitBootstrap(vm: vm, sessionId: "sess-main")
        ...
        #expect(await MainActor.run { vm.pendingRunCount } == 1)
        #expect(await MainActor.run { vm.planSteps } == [
            LuckyNemoChatPlanStep(step: "Keep working", status: .inProgress),
        ])
    }
    ...
    @Test func `rekeys optimistic user message when gateway reuses active run`() async throws {
        ...
        let (_, vm) = await makeViewModel(
            ...
            sendMessageHook: { _ in
                ...
                return LuckyNemoChatSendResponse(runId: remoteRunId, status: "in_flight")
            })
        try await loadAndWaitBootstrap(vm: vm, sessionId: sessionId)
    ...
    @Test func `canonical projected reply after steering adopts reused provisional final`() async throws {
        ...
        let (transport, vm) = await makeViewModel(
            ...
            sendMessageHook: { _ in
                LuckyNemoChatSendResponse(runId: remoteRunId, status: "in_flight")
            })
        try await loadAndWaitBootstrap(vm: vm, sessionId: sessionId)
    ...
    @Test func `terminal timeout send ack surfaces error and allows next send`() async throws {
        ...
        let (transport, vm) = await makeViewModel(
            ...
            sendMessageHook: { runId in
                ...
                return LuckyNemoChatSendResponse(
                    runId: runId,
    ...
    @Test @MainActor func `bounded repeated same text reply invalidates older stale refresh`() async throws {
        ...
        transport.emit(LuckyNemoChatTransportEvent.seqGap)
    ...
    @Test func `session choices prefer main and recent`() async throws {
        ...
        let history = historyPayload()
        let sessions = LuckyNemoChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func `bootstraps model selection from session and defaults`() async throws {
        ...
        let sessions = LuckyNemoChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: LuckyNemoChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func `selecting default model patches nil and updates selection`() async throws {
        ...
        await MainActor.run { vm.selectModel(LuckyNemoChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == LuckyNemoChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func `selecting provider qualified model disambiguates duplicate model I ds`() async throws {
        ...
        let sessions = LuckyNemoChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: LuckyNemoChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
            ...
            historyResponseHook: { _, index, sentRunIds in
                ...
                if (1...3).contains(index) {
                    ...
                    return historyPayload(
                        ...
                        sessionId: sessionId,
                        messages: [chatTextMessage(role: "user", text: "hello", timestamp: now)],
                        inFlightRun: LuckyNemoChatInFlightRun(runId: runId, text: ""))
                }
    ...
    @Test func `late model completion does not replay current session selection into previous session`() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = LuckyNemoChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = LuckyNemoChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func `explicit thinking level wins over history and persists changes`() async throws {
        let history = LuckyNemoChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
    ...
    @Test @MainActor func `Ultra is canonical while ultrathink remains a high alias`() {
        #expect(LuckyNemoChatViewModel.normalizedThinkingLevel("ultra") == "ultra")
        #expect(LuckyNemoChatViewModel.normalizedThinkingLevel("ULTRA") == "ultra")
        #expect(LuckyNemoChatViewModel.normalizedThinkingLevel("ultrathink") == "high")
        #expect(
            LuckyNemoChatViewModel.normalizedThinkingLevel(
                "ultra",
    ...
    @Test func `decodes authoritative model patch thinking state`() throws {
        ...
        let result = try JSONDecoder().decode(LuckyNemoChatModelPatchResult.self, from: data)
    ...
    @Test func `decodes gateway thinking metadata from session list`() throws {
        ...
        let decoded = try JSONDecoder().decode(
            LuckyNemoChatSessionsListResponse.self,
            from: Data(json.utf8))
        ...
        try await waitUntil("models loaded with default selection") {
            await MainActor.run {
                vm.modelChoices.count == 2 &&
                    vm.modelSelectionID == LuckyNemoChatViewModel.defaultModelSelectionID
            }
        }
    ...
    @Test func `default settings patch returns accepted thinking state`() async throws {
        ...
        let result = try await transport.patchSessionSettings(
            sessionKey: "main",
            agentID: nil,
            patch: LuckyNemoChatSessionSettingsPatch(thinkingLevel: .some("high")))
    ...
    @Test func `default thinking selection clears override and adopts resolved level`() async throws {
        ...
        try await loadAndWaitBootstrap(vm: vm, sessionId: "sess-main")
        await MainActor.run {
            vm.selectThinkingLevel(LuckyNemoChatViewModel.inheritedThinkingSelectionID)
        }
        await vm.waitForPendingSessionSettings(in: "main")

        #expect(await MainActor.run { vm.thinkingSelectionID } ==
            LuckyNemoChatViewModel.inheritedThinkingSelectionID)
        #expect(await MainActor.run { vm.thinkingLevel } == "medium")
        #expect(await MainActor.run { vm.sessions.first?.thinkingLevel } == nil)
    ...
    @Test func `inherited verbosity does not masquerade as persisted override`() async throws {
        ...
        #expect(await MainActor.run { vm.verboseLevel } == LuckyNemoChatViewModel.inheritedThinkingSelectionID)
    ...
    @Test func `fast and verbosity rejection restores inherited overrides`() async throws {
        ...
        await vm.waitForPendingSessionSettings(in: "main")
        #expect(await MainActor.run { vm.fastModeSelectionID } == LuckyNemoChatViewModel.inheritedThinkingSelectionID)
    ...
    @Test func `fast and verbosity default selections clear overrides`() async throws {
        ...
        try await loadAndWaitBootstrap(vm: vm, sessionId: "sess-main")
        await MainActor.run {
            vm.selectFastMode(LuckyNemoChatViewModel.inheritedThinkingSelectionID)
        }
        await vm.waitForPendingSessionSettings(in: "main")
        #expect(await MainActor.run { vm.sessions.first?.fastMode } == nil)
        ...
        await MainActor.run {
            vm.selectVerboseLevel(LuckyNemoChatViewModel.inheritedThinkingSelectionID)
        }
        await vm.waitForPendingSessionSettings(in: "main")
        #expect(await MainActor.run { vm.sessions.first?.verboseLevel } == nil)
        #expect(await MainActor.run { vm.verboseLevel } == LuckyNemoChatViewModel.inheritedThinkingSelectionID)
    ...
    @Test func `strips inbound metadata from history messages`() async throws {
        ...
        let vm = await MainActor.run { LuckyNemoChatViewModel(sessionKey: "main", transport: transport) }
...
struct ChatViewModelSessionManagementTests {
    @Test @MainActor func `session list organizer orders pinned first with key tiebreak`() {
        let organized = LuckyNemoChatSessionListOrganizer.organize([
    ...
    @Test @MainActor func `session list organizer filters across display fields`() {
        ...
        let matched = LuckyNemoChatSessionListOrganizer.filter(sessions, search: "TRIP")
        #expect(matched.map(\.key) == ["agent:main:topic-a", "agent:main:trip-notes"])
        #expect(LuckyNemoChatSessionListOrganizer.filter(sessions, search: "  ") == sessions)
    }
