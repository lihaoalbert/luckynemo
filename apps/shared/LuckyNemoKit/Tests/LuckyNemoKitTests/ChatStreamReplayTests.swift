...
/// Replays scripted gateway traffic against `LuckyNemoChatViewModel` with deterministic
...
private final class ScriptedChatTransport: @unchecked Sendable, LuckyNemoChatTransport {
    private actor State {
        var history: LuckyNemoChatHistoryPayload
        var historyRequestCount = 0
        var sentRunIds: [String] = []

        init(history: LuckyNemoChatHistoryPayload) {
            self.history = history
        }

        func setHistory(_ payload: LuckyNemoChatHistoryPayload) {
            self.history = payload
        }

        func recordHistoryRequest() -> LuckyNemoChatHistoryPayload {
    ...
    func setHistory(_ payload: LuckyNemoChatHistoryPayload) async {
    ...
    // MARK: LuckyNemoChatTransport

    func requestHistory(sessionKey _: String) async throws -> LuckyNemoChatHistoryPayload {
        ...
        attachments _: [LuckyNemoChatAttachmentPayload]) async throws -> LuckyNemoChatSendResponse
    {
        ...
        return LuckyNemoChatSendResponse(runId: idempotencyKey, status: "pending")
    }
    ...
    {
        LuckyNemoChatSessionsListResponse(ts: nil, path: nil, count: 0, defaults: nil, sessions: [])
    }
...
private func replayHistory(
    ...
    messages: [AnyCodable] = []) -> LuckyNemoChatHistoryPayload
{
...
/// `__luckynemo.idempotencyKey` metadata used for turn correlation.
...
private func replaySessionMessageEvent(
    ...
    messageId: String) -> LuckyNemoChatTransportEvent
{
    .sessionMessage(
...
private func replayFinalEvent(
    runId: String,
    text: String,
    timestamp: Double) -> LuckyNemoChatTransportEvent
{
    .chat(
...
private func replayAssistantDeltaEvent(
    ...
    seq: Int) -> LuckyNemoChatTransportEvent
{
    .agent(
...
private struct StreamReplayHarness {
    ...
    static func bootstrapped(
        initialHistory: LuckyNemoChatHistoryPayload = replayHistory()) async throws -> StreamReplayHarness
    {
        let transport = ScriptedChatTransport(history: initialHistory)
        let vm = await MainActor.run {
            LuckyNemoChatViewModel(sessionKey: "main", transport: transport)
        }
    ...
    func converge(
        _ label: String,
        _ condition: @escaping @MainActor @Sendable (LuckyNemoChatViewModel) -> Bool) async throws
    {
        let vm = self.vm
...
extension LuckyNemoChatViewModel {
    fileprivate var replayAssistantRows: [LuckyNemoChatMessage] {
        self.messages.filter { $0.role == "assistant" }
    }

    fileprivate func replayAssistantRows(text: String) -> [LuckyNemoChatMessage] {
        self.replayAssistantRows.filter { message in
            message.content.compactMap(\.text).joined() == text
        }
    }

    fileprivate var replayUserRows: [LuckyNemoChatMessage] {
