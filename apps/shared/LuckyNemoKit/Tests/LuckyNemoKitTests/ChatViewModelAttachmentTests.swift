...
private actor AttachmentSendCapture {
    private(set) var attachments: [LuckyNemoChatAttachmentPayload] = []

    func store(_ attachments: [LuckyNemoChatAttachmentPayload]) {
    ...
    func first() -> LuckyNemoChatAttachmentPayload? {
...
private struct AttachmentProcessingTransport: LuckyNemoChatTransport {
        ...
        attachments: [LuckyNemoChatAttachmentPayload]) async throws -> LuckyNemoChatSendResponse
    {
        ...
        return LuckyNemoChatSendResponse(runId: idempotencyKey, status: self.responseStatus)
    }
...
private func makeAttachmentOutbox() -> LuckyNemoChatSQLiteTranscriptCache {
    LuckyNemoChatSQLiteTranscriptCache(
...
private func makeDurableAttachmentViewModel(
    ...
    outbox: LuckyNemoChatSQLiteTranscriptCache) -> LuckyNemoChatViewModel
{
...
final class ChatViewModelAttachmentTests: XCTestCase {
    func testImageAttachmentsAreProcessedBeforeStaging() async throws {
        ...
        let viewModel = await MainActor.run {
            LuckyNemoChatViewModel(sessionKey: "main", transport: AttachmentProcessingTransport())
        }
    ...
    func testPartialIdentitySyncPreservesTheOtherDeferredComponent() {
        ...
        let contractViewModel = LuckyNemoChatViewModel(
            sessionKey: "main",
            transport: AttachmentProcessingTransport(),
            activeAgentId: "main",
            sessionRoutingContract: oldContract)
        let contractAttachment = LuckyNemoPendingAttachment(
            url: nil,
        ...
        let agentViewModel = LuckyNemoChatViewModel(
            sessionKey: "main",
            transport: AttachmentProcessingTransport(),
            activeAgentId: "main",
            sessionRoutingContract: oldContract)
        let agentAttachment = LuckyNemoPendingAttachment(
            url: nil,
    ...
    func testFailedAttachmentSendWithoutOutboxRestoresDraft() async throws {
        ...
        let attachmentID = await MainActor.run {
            let attachment = LuckyNemoPendingAttachment(
                url: nil,
                data: attachmentData,
                fileName: "retry.m4a",
                mimeType: "audio/mp4",
    ...
    func testVoiceNoteSendKeepsCapturedDurationWhenDraftChangesDuringHealthCheck() async throws {
        ...
        let (viewModel, draftAttachmentID) = await MainActor.run {
            ...
            let draftAttachment = LuckyNemoPendingAttachment(
                url: nil,
    ...
    func testAmbiguousVoiceNoteSurvivesViewModelRecreation() async throws {
        let outbox = makeAttachmentOutbox()
        var firstViewModel: LuckyNemoChatViewModel? = await MainActor.run {
            let viewModel = makeDurableAttachmentViewModel(
    ...
    func testCanonicalVoiceNoteConfirmationPreservesDurationAndDeletesDurableBytes() async throws {
        ...
        let canonical = try JSONDecoder().decode(
            ...
            from: Data(
                """
                {"role":"user","content":"See attached.","__luckynemo":{"idempotencyKey":"\(command
    ...
    func testCanonicalVoiceNotePreservesOptimisticDuration() throws {
        let localAudio = LuckyNemoChatMessageContent(
            type: "file",
            text: nil,
            mimeType: "audio/mp4",
        ...
        let existing = LuckyNemoChatMessage(
            role: "user",
            content: [localAudio],
            timestamp: nil,
        ...
        let incoming = try JSONDecoder().decode(
            LuckyNemoChatMessage.self,
            from: Data(
                #"{"role":"user","content":"See attached.","__luckynemo":{"idempotencyKey":"run:user"},"MediaPaths":["media/inbound/media-1.m4a"],"MediaTypes":["audio/mp4"]}"#
                    .utf8))

        let adopted = LuckyNemoChatViewModel.adoptingCanonicalMessage(incoming, over: existing)
