...
private final class ComposerParityTransport: @unchecked Sendable, LuckyNemoChatTransport {
        ...
        attachments _: [LuckyNemoChatAttachmentPayload]) async throws -> LuckyNemoChatSendResponse
    {
        ...
        return LuckyNemoChatSendResponse(runId: idempotencyKey, status: "started")
    }
...
struct ChatReplyQuoteTests {
    @Test func `single line quote matches web fixture and escapes attribution`() {
        let target = LuckyNemoChatReplyTarget(
            messageID: UUID(),
            text: "quoted body",
            senderLabel: "A *B* [C]")
...
struct ChatComposerStateTests {
    @Test func `late send does not consume newer reply selection for same message`() {
        let vm = LuckyNemoChatViewModel(sessionKey: "main", transport: ComposerParityTransport())
        let messageID = UUID()
    ...
    @Test func `accepted attachment only send quotes and clears reply`() async throws {
        ...
        let vm = LuckyNemoChatViewModel(sessionKey: "main", transport: transport)
        vm.healthOK = true
        vm.attachments = [LuckyNemoPendingAttachment(
            url: nil,
            data: Data([1, 2, 3]),
            fileName: "image.png",
            mimeType: "image/png",
