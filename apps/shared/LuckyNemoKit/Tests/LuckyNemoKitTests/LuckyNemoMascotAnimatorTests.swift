...
struct LuckyNemoMascotAnimatorTests {
    private func makeAnimator(seed: UInt64 = 7, interactive: Bool = false) -> LuckyNemoMascotAnimator {
        LuckyNemoMascotAnimator(seed: seed, hourOfDay: 12, allowsAutoSleep: interactive)
    }

    @Test func `poses stay inside drawable bounds for every mood`() {
        for mood in LuckyNemoMascotMood.allCases {
    ...
    @Test func `sleepy mood wears the nightcap`() {
        ...
        let staticPose = LuckyNemoMascotPose.staticPose(for: .sleepy)
    ...
    @Test func `static poses carry the mood signature`() {
        let sad = LuckyNemoMascotPose.staticPose(for: .sad)
        #expect(sad.antennaDroop > 0)
        #expect(sad.mouthCurve < 0)
        let celebrating = LuckyNemoMascotPose.staticPose(for: .celebrating)
        #expect(celebrating.leftClawDegrees > 0)
        #expect(celebrating.mouthCurve > 0)
        let idle = LuckyNemoMascotPose.staticPose(for: .idle)
        #expect(idle == LuckyNemoMascotPose())
        let working = LuckyNemoMascotPose.staticPose(for: .working)
