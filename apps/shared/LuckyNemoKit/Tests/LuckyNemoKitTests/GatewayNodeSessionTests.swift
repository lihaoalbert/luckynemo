...
struct GatewayNodeSessionTests {
    @Test func `operator canvas refresh uses the operator surface method`() async throws {
        ...
        let options = GatewayConnectOptions(
            ...
            caps: [LuckyNemoGatewayClientCapability.inlineWidgets],
            commands: [],
            permissions: [:],
            clientId: "luckynemo-ios",
            clientMode: "ui",
        ...
        try task.emitResponse(
            ...
            payload: [
                "surface": "canvas",
                "pluginSurfaceUrls": [
                    "canvas": "http://gateway.example.invalid/__luckynemo__/cap/operator-token",
                ],
            ])
    ...
    @Test func `canvas surface refresh is shared across callers with different timeouts`() async throws {
        ...
        task.emitResponse(
            ...
            payload: [
                "surface": "canvas",
                "pluginSurfaceUrls": [
                    "canvas": "http://gateway.example.invalid/__luckynemo__/cap/new-token",
                ],
            ])
    ...
    @Test func `last timed out surface waiter releases stalled rotation for retry`() async throws {
        ...
        try task.emitResponse(
            ...
            payload: [
                "surface": "canvas",
                "pluginSurfaceUrls": [
                    "canvas": "http://gateway.example.invalid/__luckynemo__/cap/retry-token",
                ],
            ])
    ...
    @Test func `lagging canvas refresh reuses the rotated capability`() async throws {
        ...
        try task.emitResponse(
            ...
            payload: [
                "surface": "canvas",
                "pluginSurfaceUrls": [
                    "canvas": "http://gateway.example.invalid/__luckynemo__/cap/old-token",
                ],
            ])
    ...
    func `watch approval warning text is optional and round trips`() throws {
        let legacy = try JSONDecoder().decode(
            LuckyNemoWatchExecApprovalItem.self,
    ...
    func `watch approval recovery schema carries exact resolution attempt identifiers`() throws {
        ...
        let prompt = LuckyNemoWatchExecApprovalPromptMessage(
        ...
        let request = LuckyNemoWatchExecApprovalSnapshotRequestMessage(
            requestId: "request",
            heldApprovals: [LuckyNemoWatchExecApprovalSnapshotRequestItem(
                approvalId: approvalID,
                activeResolutionAttemptId: activeAttemptID)])
        let decoded = try JSONDecoder().decode(
            LuckyNemoWatchExecApprovalSnapshotRequestMessage.self,
            ...
            await gateway.invokeIfCurrentRoute(
                ...
                onInvoke: { request in
                    ...
                    return BridgeInvokeResponse(
                        id: request.id,
                        ok: false,
                        error: LuckyNemoNodeError(code: .unavailable, message: "UNAVAILABLE: route changed"))
                })
        }
    ...
    func `replacement invoke fails promptly while disconnect lifecycle is blocked`() async throws {
        ...
        #expect(error["code"] as? String == LuckyNemoNodeErrorCode.unavailable.rawValue)
    ...
    func `computer invoke receipts deduplicate in flight and after reconnect`() async throws {
        ...
        #expect(mismatchError["code"] as? String == LuckyNemoNodeErrorCode.invalidRequest.rawValue)
        #expect(await probe.count() == 1)
    ...
    func `share extension identity profile uses separate node identity and token store`() async throws {
        ...
        let options = GatewayConnectOptions(
            ...
            clientId: "luckynemo-ios",
            clientMode: "node",
            clientDisplayName: "LuckyNemo Share",
    ...
    func `connect failure preserves protocol mismatch details`() async throws {
        ...
        do {
            ...
            #expect(problem?
                .message == "This app is older than the gateway. Update LuckyNemo on this device, then retry.")
    ...
    func `failed device token write is not reported as an issued role`() async throws {
        ...
        setenv("LUCKYNEMO_STATE_DIR", blocker.path, 1)
    ...
    func `normalize canvas host url preserves explicit secure canvas port`() {
        let normalized = GatewayPluginSurfaceURL.canonicalize(
            raw: "https://canvas.example.com:9443/__luckynemo__/cap/token",
            against: URL(string: "wss://gateway.example.com"))

        #expect(normalized == "https://canvas.example.com:9443/__luckynemo__/cap/token")
    }

    @Test
    func `normalize canvas host url backfills gateway host for loopback canvas`() {
        let normalized = GatewayPluginSurfaceURL.canonicalize(
            raw: "http://127.0.0.1:18789/__luckynemo__/cap/token",
            against: URL(string: "wss://gateway.example.com:7443"))

        #expect(normalized == "https://gateway.example.com:7443/__luckynemo__/cap/token")
    }
    ...
    func `gateway TLS pin applies only to the same canvas endpoint`() {
        ...
        #expect(GatewayPluginSurfaceURL.tlsFingerprintForSurface(
            fingerprint,
            surfaceURL: "https://gateway.example.com:7443/__luckynemo__/cap/token",
            gatewayURL: gateway) == fingerprint)
        #expect(GatewayPluginSurfaceURL.tlsFingerprintForSurface(
            fingerprint,
            surfaceURL: "https://canvas.example.com:7443/__luckynemo__/cap/token",
            gatewayURL: gateway) == nil)
        #expect(GatewayPluginSurfaceURL.tlsFingerprintForSurface(
            fingerprint,
            surfaceURL: "https://gateway.example.com:9443/__luckynemo__/cap/token",
            gatewayURL: gateway) == nil)
    }
