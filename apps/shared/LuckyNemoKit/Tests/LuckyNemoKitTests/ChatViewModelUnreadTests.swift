...
private actor UnreadTestTransportState {
    ...
    var sessionOverride: [LuckyNemoChatSessionEntry]?
...
private final class UnreadTestTransport: @unchecked Sendable, LuckyNemoChatTransport {
    ...
    func setSessions(_ sessions: [LuckyNemoChatSessionEntry]) async {
...
extension UnreadTestTransportState {
    ...
    fileprivate func setSessions(_ sessions: [LuckyNemoChatSessionEntry]) {
...
struct ChatViewModelUnreadTests {
    ...
    @Test func `failed route lease preserves mutation queue ordering`() async throws {
        ...
        let firstLease = LuckyNemoChatSessionMutationRouteLease { _, _, _, _, _, _ in
            await recorder.append("first-start")
            try await Task.sleep(for: .milliseconds(100))
            await recorder.append("first-end")
        }
        let thirdLease = LuckyNemoChatSessionMutationRouteLease { _, _, _, _, _, _ in
            await recorder.append("third")
        }

        let first = queue.reserve(
            routeLease: Task<LuckyNemoChatSessionMutationRouteLease?, Never> { firstLease },
            queueKey: "a",
            routeKey: "a",
            unread: false)
        let second = queue.reserve(
            routeLease: Task<LuckyNemoChatSessionMutationRouteLease?, Never> { nil },
            queueKey: "a",
            routeKey: "a",
            unread: true)
        let third = queue.reserve(
            routeLease: Task<LuckyNemoChatSessionMutationRouteLease?, Never> { thirdLease },
            queueKey: "a",
            routeKey: "a",
            unread: false)

        try await first.value
        await #expect(throws: LuckyNemoChatTransportSendError.self) {
    ...
    private func entry(
        ...
        pinned: Bool? = nil) -> LuckyNemoChatSessionEntry
    {
