...
enum DeviceIdentityPaths {
    ...
    private static let appGroupStateDirAvailable =
        DeviceIdentityPaths.hasAppGroupEntitlement(LuckyNemoAppGroup.identifier)
        ...
        temporaryDirectory: URL) -> URL
    {
        ...
        return temporaryDirectory.appendingPathComponent("luckynemo", isDirectory: true)
    }

    private static func stateDirOverrideURL() -> URL? {
        // Test-scoped stores must win over the process environment. Parallel Swift tests
        // otherwise race whenever another suite temporarily swaps LUCKYNEMO_STATE_DIR.
    ...
    private static func legacyStateDirURL() -> URL? {
        if let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first {
            return appSupport.appendingPathComponent("LuckyNemo", isDirectory: true)
        }
        return nil
    }
    ...
    private static func appGroupStateDirURL() -> URL? {
        guard
            let containerURL = FileManager.default
                .containerURL(forSecurityApplicationGroupIdentifier: LuckyNemoAppGroup.identifier)
        else {
            return nil
        }
        return containerURL.appendingPathComponent("LuckyNemo", isDirectory: true)
    }
    ...
    /// An explicit LUCKYNEMO_STATE_DIR override selects a caller-chosen store, not the legacy
