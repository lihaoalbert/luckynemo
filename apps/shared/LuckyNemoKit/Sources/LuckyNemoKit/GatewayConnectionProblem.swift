            ...
            self.problem(
                .init(
                    ...
                    docsURL: self.docsURL(
                        ...
                        fallback: "https://docs.luckynemo.ai/gateway/tailscale"),
        ...
        case .authRateLimited:
            self.problem(
                .init(
                    ...
                    docsURL: self.docsURL(
                        authError.docsURLString,
                        fallback: "https://docs.luckynemo.ai/gateway/troubleshooting"),
                    requestId: authError.requestId,
                    retryable: false,
                    pauseReconnect: true),
                authError: authError)
        ...
        case .authTokenMissing:
            self.problem(
                .init(
                    ...
                    docsURL: self.docsURL(
                        authError.docsURLString,
                        fallback: "https://docs.luckynemo.ai/gateway/authentication"),
                    requestId: authError.requestId,
                    retryable: false,
                    pauseReconnect: true),
                authError: authError)
        ...
        case .authTokenNotConfigured:
            self.problem(
                .init(
                    ...
                    actionCommand: authError.actionCommand
                        ?? "luckynemo config set gateway.auth.token <new-token>",
                    docsURL: self.docsURL(
                        authError.docsURLString,
                        fallback: "https://docs.luckynemo.ai/gateway/authentication"),
                    requestId: authError.requestId,
        ...
        case .authPasswordNotConfigured:
            self.problem(
                .init(
                    ...
                    actionLabel: authError.actionLabel ?? "Fix on gateway",
                    actionCommand: authError.actionCommand
                        ?? "luckynemo config set gateway.auth.password <new-password>",
                    docsURL: self.docsURL(
                        authError.docsURLString,
                        fallback: "https://docs.luckynemo.ai/gateway/authentication"),
                    requestId: authError.requestId,
        ...
        case .authBootstrapTokenInvalid:
            self.problem(
                .init(
                    ...
                    docsURL: self.docsURL(
                        authError.docsURLString,
                        fallback: "https://docs.luckynemo.ai/platforms/ios"),
                    requestId: authError.requestId,
                    retryable: false,
                    pauseReconnect: true),
                authError: authError)
        case .authDeviceTokenMismatch:
            self.problem(
                .init(
                    ...
                    docsURL: self.docsURL(
                        authError.docsURLString,
                        fallback: "https://docs.luckynemo.ai/gateway/pairing"),
                    requestId: authError.requestId,
                    retryable: false,
                    pauseReconnect: true),
                authError: authError)
...
extension GatewayConnectionProblemMapper {
    ...
    private static func map(_ tlsError: GatewayTLSValidationError) -> GatewayConnectionProblem {
        ...
        case .pinMismatch:
            ...
            return GatewayConnectionProblem(
                ...
                actionCommand: nil,
                docsURL: URL(string: "https://docs.luckynemo.ai/gateway/troubleshooting"),
                retryable: false,
                pauseReconnect: true,
        ...
        case .certificateUnavailable:
            return GatewayConnectionProblem(
                ...
                message: "LuckyNemo could not read the gateway certificate for \(failure.host).",
                actionLabel: "Retry",
                messagePresentation: .localizedFormat(
                    "LuckyNemo could not read the gateway certificate for %@.",
                    [failure.host]),
                actionCommand: nil,
                docsURL: URL(string: "https://docs.luckynemo.ai/gateway/troubleshooting"),
                retryable: true,
        ...
        if nsError.domain == URLError.errorDomain {
            ...
            case .cannotFindHost, .dnsLookupFailed, .notConnectedToInternet, .networkConnectionLost,
                ...
                return GatewayConnectionProblem(
                    ...
                    message: "LuckyNemo could not reach the gateway over the current network.",
    ...
    private static func protocolMismatchProblem(for authError: GatewayConnectAuthError) -> GatewayConnectionProblem {
           ...
           clientMax < expected
        {
            ...
            message = authError.userMessageOverride
                ?? "This app is older than the gateway. Update LuckyNemo on this device, then retry."
            owner = .iphone
                  ...
                  clientMin > expected
        {
            ...
            message = authError.userMessageOverride
                ?? "The gateway is older than this app. Update LuckyNemo on the gateway host, then retry."
            owner = .gateway
            actionLabel = authError.actionLabel ?? "Copy update command"
            actionCommand = authError.actionCommand ?? "luckynemo update"
        } else {
            title = authError.titleOverride ?? "LuckyNemo update required"
            message = authError.userMessageOverride
                ?? "The app and gateway use incompatible protocol versions. Update LuckyNemo on both, then retry."
            owner = .both
            actionLabel = authError.actionLabel ?? "Update LuckyNemo"
            actionCommand = authError.actionCommand
        }
        return self.problem(
            .init(
    ...
    private static func approvalCommand(requestId: String?) -> String {
        if let requestId = self.nonEmpty(requestId) {
            return "luckynemo devices approve \(requestId)"
        }
        return "luckynemo devices list"
    }
