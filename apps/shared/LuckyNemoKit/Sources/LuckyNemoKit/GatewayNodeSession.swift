...
public actor GatewayNodeSession {
    ...
    private let logger = Logger(subsystem: "ai.luckynemo", category: "node.gateway")
    ...
    // periphery:ignore - Shipped public refresh API; keep until a breaking LuckyNemoKit window.
    @discardableResult
...
extension GatewayNodeSession {
        ...
        -> BridgeInvokeResponse
    {
        ...
        let requiresRouteScopedCancellation = request.command == "computer.act" ||
            LuckyNemoTalkCommand(rawValue: request.command) != nil
    ...
    private func decodeInvokeRequest(from payload: LuckyNemoProtocol.AnyCodable) throws -> NodeInvokeRequestPayload {
        try self.decodeEventPayload(from: payload)
    }

    private func decodeEventPayload<T: Decodable>(from payload: LuckyNemoProtocol.AnyCodable) throws -> T {
        do {
