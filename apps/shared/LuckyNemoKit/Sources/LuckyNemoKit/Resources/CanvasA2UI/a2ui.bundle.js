...
var LuckyNemoA2UIHost = class extends i$7 {
	static properties = {
		surfaces: { state: true },
    ...
    :host {
      ...
      position: relative;
      box-sizing: border-box;
      padding: var(--luckynemo-a2ui-inset-top, 0px) var(--luckynemo-a2ui-inset-right, 0px)
        var(--luckynemo-a2ui-inset-bottom, 0px) var(--luckynemo-a2ui-inset-left, 0px);
    }
    ...
    #surfaces {
      ...
      height: 100%;
      overflow: auto;
      padding-bottom: var(--luckynemo-a2ui-scroll-pad-bottom, 0px);
    }
    ...
    .status {
      ...
      top: var(--luckynemo-a2ui-status-top, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .toast {
      ...
      left: 50%;
      transform: translateX(-50%);
      bottom: var(--luckynemo-a2ui-toast-bottom, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .empty {
      ...
      left: 50%;
      transform: translateX(-50%);
      top: var(--luckynemo-a2ui-empty-top, var(--luckynemo-a2ui-status-top, 12px));
      text-align: center;
      opacity: 0.8;
      padding: 10px 12px;
      pointer-events: none;
  ...
  `;
	connectedCallback() {
		...
		for (const eventName of ["luckynemo:a2ui-action-status"]) globalThis.addEventListener(eventName, this.#statusListener);
		this.#syncSurfaces();
	}
	disconnectedCallback() {
		super.disconnectedCallback();
		if (this.#statusListener) {
			for (const eventName of ["luckynemo:a2ui-action-status"]) globalThis.removeEventListener(eventName, this.#statusListener);
	...
	#handleA2UIAction(evt) {
		...
		globalThis["__luckynemoLastA2UIAction"] = userAction;
		const handler = globalThis.webkit?.messageHandlers?.luckynemoCanvasA2UIAction ?? globalThis.luckynemoCanvasA2UIAction;
		if (handler?.postMessage) try {
			if (handler === globalThis.luckynemoCanvasA2UIAction) postNativeMessage(handler, JSON.stringify({ userAction }));
...
if (!customElements.get("luckynemo-a2ui-host")) customElements.define("luckynemo-a2ui-host", LuckyNemoA2UIHost);
