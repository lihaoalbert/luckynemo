...
struct ChatSessionsSheet: View {
    ...
    @State private var scopedSessions: [LuckyNemoChatSessionEntry] = []
    @State private var isLoadingScoped = false
    @State private var renameTarget: LuckyNemoChatSessionEntry?
    ...
    private var displayedSessions: [LuckyNemoChatSessionEntry] {
    ...
    var body: some View {
        NavigationStack {
            ...
            .onAppear {
                self.viewModel.refreshSessions(limit: LuckyNemoChatViewModel.sessionListFetchLimit)
            }
            .alert(
                "Rename Session",
                isPresented: Binding(
    ...
    private func sessionRow(_ session: LuckyNemoChatSessionEntry) -> some View {
            ...
            HStack(spacing: 8) {
                VStack(alignment: .leading, spacing: 4) {
                    Text(session.displayName ?? session.key)
                        .font(LuckyNemoChatTypography.mono(size: 17, relativeTo: .body))
                        .lineLimit(1)
        ...
        .swipeActions(edge: .leading, allowsFullSwipe: true) {
            if !session.isArchived {
                ...
                .tint(LuckyNemoChatTheme.accent)
            }
        }
        .swipeActions(edge: .trailing, allowsFullSwipe: false) {
                ...
                mainSessionKey: self.viewModel.resolvedMainSessionKey)
            {
                ...
                .tint(session.isArchived ? LuckyNemoChatTheme.accent : LuckyNemoChatTheme.danger)
            }
        }
        .contextMenu {
            Button {
