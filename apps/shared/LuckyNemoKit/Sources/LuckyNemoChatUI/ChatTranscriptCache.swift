...
private let cacheLogger = Logger(subsystem: "ai.luckynemo", category: "LuckyNemoChatTranscriptCache")

private final class OutboxChangeHub: @unchecked Sendable {
    private let lock = NSLock()
    private var continuations: [UUID: AsyncStream<LuckyNemoChatOutboxChange>.Continuation] = [:]

    func stream() -> AsyncStream<LuckyNemoChatOutboxChange> {
        let id = UUID()
        let pair = AsyncStream<LuckyNemoChatOutboxChange>.makeStream()
        self.lock.lock()
    ...
    func yield(_ change: LuckyNemoChatOutboxChange) {
        self.lock.lock()
...
public actor LuckyNemoChatSQLiteTranscriptCache: LuckyNemoChatTranscriptCache,
    LuckyNemoChatCanonicalTranscriptMerging,
    LuckyNemoChatCommandOutbox
{
    ...
    public nonisolated static func loadSessionRoutingIdentity(
        databaseURL: URL,
        gatewayID: String) -> LuckyNemoChatSessionRoutingIdentity?
    {
        ...
        return LuckyNemoChatSessionRoutingIdentity(
    ...
    // MARK: - LuckyNemoChatTranscriptCache

    public func loadSessions() async -> [LuckyNemoChatSessionEntry] {
        ...
        guard let decoded = try? JSONDecoder().decode(
            [LuckyNemoChatSessionEntry].self,
    ...
    public func loadTranscript(sessionKey: String) async -> [LuckyNemoChatMessage] {
        await self.loadTranscript(sessionKey: sessionKey, agentID: nil)
    }

    public func loadTranscript(sessionKey: String, agentID: String?) async -> [LuckyNemoChatMessage] {
        guard !self.isRetired else { return [] }
        guard let db = await handle() else { return [] }
    ...
    private func readTranscript(
        _ db: OpaquePointer,
        sessionKey: String,
        agentID: String?) -> [LuckyNemoChatMessage]
    {
    ...
    public func storeSessions(_ sessions: [LuckyNemoChatSessionEntry]) async {
        guard !self.isRetired else { return }
        guard let db = await handle() else { return }
    ...
    public func loadSessionRoutingIdentity() async -> LuckyNemoChatSessionRoutingIdentity? {
    ...
    public func storeSessionRoutingIdentity(_ identity: LuckyNemoChatSessionRoutingIdentity) async {
    ...
    public func storeTranscript(sessionKey: String, messages: [LuckyNemoChatMessage]) async {
    ...
    public func storeTranscript(
        sessionKey: String,
        agentID: String?,
        messages: [LuckyNemoChatMessage]) async
    {
...
extension LuckyNemoChatSQLiteTranscriptCache {
    // MARK: - LuckyNemoChatCommandOutbox

    public nonisolated func changes() -> AsyncStream<LuckyNemoChatOutboxChange> {
        self.outboxChangeHub.stream()
    }

    public func enqueueCommand(_ command: LuckyNemoChatOutboxCommand) async -> Bool {
    ...
    public func loadCommands() async -> [LuckyNemoChatOutboxCommand] {
        await self.loadCommandsIfAvailable() ?? []
    }

    public func loadCommandsIfAvailable() async -> [LuckyNemoChatOutboxCommand]? {
        guard !self.isRetired, let db = await handle() else { return nil }
    ...
    public func claimNextCommand() async -> LuckyNemoChatOutboxCommand? {
        guard !self.isRetired, let db = await handle() else { return nil }
    ...
    public func markCommandAwaitingConfirmation(id: String) async -> LuckyNemoChatOutboxUpdateResult {
        ...
        routingContract: String) async -> LuckyNemoChatOutboxUpdateResult
    {
        ...
        let allowsUntargetedAgent = normalizedRoutingContract == LuckyNemoChatOutboxCommand
    ...
    public func cancelCommand(id: String) async -> LuckyNemoChatOutboxUpdateResult {
        guard !self.isRetired, let db = await handle() else { return .unavailable }
        guard self.execute(db, sql: "BEGIN IMMEDIATE", bindings: []) else { return .unavailable }
        var committed = false
        defer {
        ...
        let result: LuckyNemoChatOutboxUpdateResult
        do {
            let isProven = self.canonicalMessageProofHub.lockProofDecision(for: messageKey)
            defer { self.canonicalMessageProofHub.unlockProofDecision() }
            if isProven {
    ...
    public func confirmCommand(id: String) async -> LuckyNemoChatOutboxUpdateResult {
        guard !self.isRetired, let db = await handle() else { return .unavailable }
        guard self.execute(
            db,
    ...
    private func emitOutboxChange(_ change: LuckyNemoChatOutboxChange) {
...
extension LuckyNemoChatSQLiteTranscriptCache {
    ...
    static func cacheableMessages(_ messages: [LuckyNemoChatMessage]) -> [LuckyNemoChatMessage] {
    ...
    static func boundedSessions(_ sessions: [LuckyNemoChatSessionEntry]) -> [LuckyNemoChatSessionEntry] {
    ...
    private static func attachmentByteCount(_ attachments: [LuckyNemoChatOutboxAttachment]) -> Int? {
        var total = 0
    ...
    private func readCommands(_ db: OpaquePointer) -> [LuckyNemoChatOutboxCommand]? {
        var statement: OpaquePointer?
        let sql = """
        ...
        var commands: [LuckyNemoChatOutboxCommand] = []
        ...
        while step == SQLITE_ROW {
               ...
               let text = sqlite3_column_text(statement, 5)
            {
                ...
                guard let attachments = try? JSONDecoder().decode(
                    [LuckyNemoChatOutboxAttachment].self,
                ...
                if let status = LuckyNemoChatOutboxCommand.Status(rawValue: statusRaw) {
                    commands.append(
