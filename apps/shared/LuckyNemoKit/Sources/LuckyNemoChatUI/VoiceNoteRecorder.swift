...
public struct LuckyNemoVoiceNoteRecording: Equatable, Sendable {
...
@MainActor
@Observable
public final class LuckyNemoVoiceNoteRecorder {
    public enum State: Equatable {
        case idle
        case requestingPermission
        case recording(startedAt: Date, fileURL: URL)
        case finished(recording: LuckyNemoVoiceNoteRecording)
        case staging(recording: LuckyNemoVoiceNoteRecording)
    ...
    public convenience init() {
        self.init(capture: LuckyNemoVoiceNoteAudioCapture())
    }

    /// Creates a recorder with an injectable audio capture seam.
    public init(
        capture: any VoiceNoteAudioCapture,
        durationLimit: TimeInterval = LuckyNemoVoiceNoteRecorder.maximumDurationSeconds,
    ...
    public var completedRecording: LuckyNemoVoiceNoteRecording? {
    ...
    func claimCompletedRecording() -> LuckyNemoVoiceNoteRecording? {
        guard case let .finished(recording) = self.state else { return nil }
    ...
    func completeStaging(_ recording: LuckyNemoVoiceNoteRecording) {
    ...
    @discardableResult
    public func finish() -> LuckyNemoVoiceNoteRecording? {
        guard case let .recording(_, fileURL) = self.state else { return nil }

        self.timerTask?.cancel()
        self.timerTask = nil
        let duration = max(0, self.capture.stop())
        let recording = LuckyNemoVoiceNoteRecording(fileURL: fileURL, durationSeconds: duration)
...
@MainActor
public final class LuckyNemoVoiceNoteAudioCapture: NSObject, VoiceNoteAudioCapture, AVAudioRecorderDelegate {
    ...
    public func start(url: URL) throws {
        do {
            ...
            guard recorder.record() else {
                throw NSError(
                    domain: "LuckyNemoVoiceNoteAudioCapture",
                    code: 1,
...
func luckyNemoVoiceNoteDurationLabel(_ durationSeconds: TimeInterval) -> String {
