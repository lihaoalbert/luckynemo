...
extension LuckyNemoChatViewModel {
    ...
    func replaceMessages(_ messages: [LuckyNemoChatMessage]) {
    ...
    func persistSessionsToCache(_ sessions: [LuckyNemoChatSessionEntry]) {
        guard let transcriptCache else { return }
        let previous = pendingCacheWriteTask
        pendingCacheWriteTask = Task.detached {
            await previous?.value
        ...
        if sessions.isEmpty, !hasAppliedLiveSessions {
            Task { [weak self] in
                ...
                self.sessions = self.applyingLocalUnreadOverrides(
                    to: LuckyNemoChatSessionListOrganizer.organize(cached))
            }
        }
    ...
    static func transcriptCacheAgentID(sessionKey: String, agentID: String?) -> String? {
        guard LuckyNemoChatSessionKey.agentID(from: sessionKey) == nil else { return nil }
