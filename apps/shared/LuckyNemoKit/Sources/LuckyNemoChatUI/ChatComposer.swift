        ...
        content
            .background(
                RoundedRectangle(cornerRadius: self.cornerRadius, style: .continuous)
                    .fill(LuckyNemoChatTheme.composerField))
            .overlay(
                RoundedRectangle(cornerRadius: self.cornerRadius, style: .continuous)
...
private struct CompactChatAttachmentLabel: View {
    var body: some View {
        Image(systemName: "paperclip")
            .font(LuckyNemoChatTypography.display(size: 15, weight: .semibold, relativeTo: .subheadline))
...
@MainActor
struct LuckyNemoChatComposer: View {
    @Bindable var viewModel: LuckyNemoChatViewModel
    let style: LuckyNemoChatView.Style
    let showsSessionSwitcher: Bool
    let userAccent: Color?
    let assistantName: String?
    let assistantAvatarText: String?
    let assistantAvatarTint: Color?
    let composerChrome: LuckyNemoChatView.ComposerChrome
    let isComposerEnabled: Bool
    let isAttachmentInputEnabled: Bool
    let messagePlaceholder: String?
    let talkControl: LuckyNemoChatTalkControl?
    let voiceNoteControl: LuckyNemoChatVoiceNoteControl?
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            if let voiceNoteControl, voiceNoteControl.recorder.isRecording {
                LuckyNemoVoiceNoteRecordingRow(recorder: voiceNoteControl.recorder)
            ...
            if self.composerChrome == .full {
                ...
                if self.style == .standard {
                    let shape = UnevenRoundedRectangle(
                        ...
                        style: .continuous)
                    shape
                        .fill(LuckyNemoChatTheme.composerBackground)
                        .overlay(shape.strokeBorder(LuckyNemoChatTheme.composerBorder, lineWidth: 1))
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 5) {
                    ...
                    if self.viewModel.showsModelPicker {
                        self.modelPicker
                        if self.viewModel.modelSelectionID != LuckyNemoChatViewModel.defaultModelSelectionID {
                            self.modelPinButton
                        }
                    }
                }
    ...
    private func contextUsageIndicator(_ fraction: Double) -> some View {
        ...
        let color = fraction >= 0.8 ? LuckyNemoChatTheme.warning : LuckyNemoChatTheme.muted
        return ZStack {
            Circle()
                .stroke(LuckyNemoChatTheme.muted.opacity(0.2), lineWidth: 2)
            Circle()
    ...
    private var thinkingPicker: some View {
            ...
            set: { next in self.viewModel.selectThinkingLevel(next) }))
        {
            Text(String(localized: "Default (inherited)"))
                ...
                .tag(LuckyNemoChatViewModel.inheritedThinkingSelectionID)
    ...
    private var modelPicker: some View {
            ...
            set: { next in self.viewModel.selectModel(next) }))
        {
            Text(self.viewModel.defaultModelLabel)
                .font(LuckyNemoChatTypography.captionSemiBold)
                .tag(LuckyNemoChatViewModel.defaultModelSelectionID)
    ...
    private func modelOptions(_ models: [LuckyNemoChatModelChoice]) -> some View {
            ...
            set: { next in self.viewModel.switchSession(to: next) }))
        {
            ForEach(self.viewModel.sessionChoices, id: \.key) { session in
                Text(session.displayName ?? session.key)
                    .font(LuckyNemoChatTypography.mono(size: 12, relativeTo: .caption))
                    .tag(session.key)
            }
        } label: {
            Text("Session")
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \LuckyNemoPendingAttachment.id)
                { (att: LuckyNemoPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            LuckyNemoPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
                            ...
                            if let durationSeconds = att.durationSeconds {
                                Text(luckyNemoVoiceNoteDurationLabel(durationSeconds))
                                    .font(LuckyNemoChatTypography.caption)
                                    .foregroundStyle(.secondary)
                            }
                        } else {
                    ...
                    .padding(.vertical, 5)
                    .background(LuckyNemoChatTheme.accent.opacity(0.08))
                    .clipShape(Capsule())
                }
            }
        }
    ...
    private var fullEditor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Rectangle()
                .fill(LuckyNemoChatTheme.divider)
                .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 14, style: .continuous)
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: 14, style: .continuous)
                        .strokeBorder(LuckyNemoChatTheme.composerBorder)))
        .padding(self.editorPadding)
    }
    ...
    private func talkButton(_ talkControl: LuckyNemoChatTalkControl) -> some View {
        Button {
    ...
    private func compactTalkButton(_ talkControl: LuckyNemoChatTalkControl) -> some View {
        Button {
            talkControl.toggle(self.viewModel.sessionKey)
        } label: {
            ChatTalkButtonGlyph(control: talkControl)
                .font(LuckyNemoChatTypography.body(size: 14, weight: .semibold, relativeTo: .subheadline))
                ...
                .background {
                    Circle()
                        .fill(talkControl.isEnabled
                            ...
                            : AnyShapeStyle(LuckyNemoChatTheme.accent))
    ...
    private func talkButtonFill(_ talkControl: LuckyNemoChatTalkControl) -> AnyShapeStyle {
        if talkControl.isEnabled {
            return AnyShapeStyle(LuckyNemoChatTheme.userBubble)
        }
        if !talkControl.isGatewayConnected {
            return AnyShapeStyle(Color.secondary.opacity(0.12))
        }
        return LuckyNemoChatTheme.subtleCard
    }

    private func talkButtonStroke(_ talkControl: LuckyNemoChatTalkControl) -> Color {
        if talkControl.isEnabled {
            return Color.white.opacity(0.18)
        }
        return LuckyNemoChatTheme.composerBorder
    }

    private func talkAccessibilityValue(_ talkControl: LuckyNemoChatTalkControl) -> String {
    ...
    private func talkHelpText(_ talkControl: LuckyNemoChatTalkControl) -> String {
        ...
        ZStack(alignment: self.editorOverlayAlignment) {
            if self.viewModel.input.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                Text(self.placeholderText)
                    .font(LuckyNemoChatTypography.body)
...
extension LuckyNemoChatComposer {
    ...
    private func selectSlashCommand(_ command: LuckyNemoChatCommandChoice) {
...
extension LuckyNemoChatComposer {
        ...
        if self.viewModel.pendingRunCount > 0, !self.viewModel.hasDraftToSend {
            Button {
                ...
                if self.viewModel.isAborting {
                    ...
                    Image(systemName: "stop.fill")
                        .font(LuckyNemoChatTypography.display(size: 13, weight: .semibold, relativeTo: .caption))
                }
            }
            .buttonStyle(.plain)
            .foregroundStyle(.white)
            ...
            .background(
                RoundedRectangle(cornerRadius: self.sendButtonCornerRadius, style: .continuous)
                    .fill(LuckyNemoChatTheme.danger)
    ...
    private var sendButtonFill: Color {
        self.userAccent ?? LuckyNemoChatTheme.userBubble
    }
