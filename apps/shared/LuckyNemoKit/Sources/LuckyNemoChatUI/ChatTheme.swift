...
enum LuckyNemoChatTheme {
    ...
    static let assistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("LuckyNemoChatTheme.assistantBubble"),
        dynamicProvider: resolvedAssistantBubbleColor(for:))

    static let onboardingAssistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("LuckyNemoChatTheme.onboardingAssistantBubble"),
...
enum LuckyNemoPlatformImageFactory {
    static func image(_ image: LuckyNemoPlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
