...
struct ChatTalkActivityStrip: View {
    let control: LuckyNemoChatTalkControl
                        ...
                        VStack(alignment: .leading, spacing: 5) {
                            ...
                            if !model.partialTranscript.isEmpty {
                                Text(model.partialTranscript)
                                    .font(LuckyNemoChatTypography.caption.italic())
    ...
    private func header(model: ChatTalkTranscriptStripModel, showsDisclosure: Bool) -> some View {
        HStack(spacing: 7) {
            Circle()
                .fill(LuckyNemoChatTheme.accent)
            ...
            if let collapsedText = model.collapsedText, !self.isExpanded {
                Text(collapsedText)
                    .font(LuckyNemoChatTypography.caption)
                    .foregroundStyle(model.partialTranscript.isEmpty ? .secondary : LuckyNemoChatTheme.accent)
...
extension View {
    func chatTalkInputDeviceMenu(_ control: LuckyNemoChatTalkControl) -> some View {
