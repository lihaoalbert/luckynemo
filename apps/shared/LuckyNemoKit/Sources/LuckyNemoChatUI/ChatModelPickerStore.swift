...
public struct ChatModelProviderSection: Identifiable, Sendable, Equatable {
    ...
    public let models: [LuckyNemoChatModelChoice]
    public let isDefaultProvider: Bool
}

public struct ChatModelPickerSections: Sendable, Equatable {
    public let pinned: [LuckyNemoChatModelChoice]
    public let recent: [LuckyNemoChatModelChoice]
    public let providers: [ChatModelProviderSection]
}

@MainActor
public final class ChatModelPickerStore {
    private static let favoritesKey = "luckynemo.chat.modelFavorites"
    private static let recentsKey = "luckynemo.chat.modelRecents"
    ...
    public func recordRecent(_ selectionID: String) {
        guard !selectionID.isEmpty, selectionID != LuckyNemoChatViewModel.defaultModelSelectionID else { return }
    ...
    static func isDefaultModel(
        _ choice: LuckyNemoChatModelChoice,
    ...
    private static func effectiveProvider(_ choice: LuckyNemoChatModelChoice) -> String {
        let metadataProvider = self.normalizedProvider(choice.provider)
        if metadataProvider != "other" {
            return metadataProvider
        }
