...
extension LuckyNemoChatViewModel {
    ...
    func performSelectThinkingLevel(_ level: String) {
        ...
        if acceptedThinkingLevelsByTarget[target] == nil {
            ...
            acceptedSettingsPatchResultsByTarget[target] = LuckyNemoChatModelPatchResult(
    ...
    private func mergedThinkingPatchSuccess(
        _ patchResult: LuckyNemoChatModelPatchResult?,
        acceptedLevel: String,
        target: ModelPatchTarget) -> LuckyNemoChatModelPatchResult
    {
    ...
    func updateCurrentSessionThinkingLevels(
        _ thinkingLevels: [LuckyNemoChatThinkingLevelOption],
        sessionKey: String,
    ...
    func inactiveSettingsStateKey(for target: ModelPatchTarget) -> String? {
        if target.agentID != nil || target.sessionRoutingContract != nil {
            guard LuckyNemoChatSessionKey.agentID(from: target.canonicalSessionKey) != nil else { return nil }
        }
        ...
        sessionRoutingContract: String? = nil) -> String
    {
        ...
        let session: LuckyNemoChatSessionEntry?
    ...
    private func thinkingLevelWithoutGatewayMetadata(
        _ level: String,
        target: ModelPatchTarget,
        session: LuckyNemoChatSessionEntry?) -> String?
    {
    ...
    private func thinkingPickerIsAvailable(
        for session: LuckyNemoChatSessionEntry?,
        modelChoice: LuckyNemoChatModelChoice?) -> Bool
    {
        let resolved = self.resolvedThinkingLevelOptions(for: session)
    ...
    private struct ThinkingLevelOptionsResolution {
        let options: [LuckyNemoChatThinkingLevelOption]
        let isGatewayMetadata: Bool
    }

    private func resolvedThinkingLevelOptions(
        for currentSession: LuckyNemoChatSessionEntry?) -> ThinkingLevelOptionsResolution
    {
    ...
    private func selectedModelChoice(
        for currentSession: LuckyNemoChatSessionEntry?) -> LuckyNemoChatModelChoice?
    {
    ...
    private func modelChoice(modelID: String?, provider: String?) -> LuckyNemoChatModelChoice? {
    ...
    private static func sessionModelMatchesDefaults(
        ...
        defaults: LuckyNemoChatSessionsDefaults?) -> Bool
    {
    ...
    private static func normalizedThinkingLevelOptions(
        _ levels: [LuckyNemoChatThinkingLevelOption]?) -> [LuckyNemoChatThinkingLevelOption]?
    {
        ...
        return Self.dedupedThinkingOptions(
            levels.compactMap { level in
                ...
                return LuckyNemoChatThinkingLevelOption(id: id, label: label.isEmpty ? id : label)
            })
    }

    private static func thinkingOptions(from labels: [String]?) -> [LuckyNemoChatThinkingLevelOption]? {
        guard let labels else { return nil }
        return Self.dedupedThinkingOptions(
            labels.compactMap { label in
                guard let id = Self.normalizedThinkingLevel(label) else { return nil }
                let trimmed = label.trimmingCharacters(in: .whitespacesAndNewlines)
                return LuckyNemoChatThinkingLevelOption(id: id, label: trimmed.isEmpty ? id : trimmed)
            })
    }

    static func withCurrentThinkingOption(
        _ options: [LuckyNemoChatThinkingLevelOption],
        current: String) -> [LuckyNemoChatThinkingLevelOption]
    {
        guard !options.contains(where: { $0.id == current }) else { return options }
        return options + [LuckyNemoChatThinkingLevelOption(id: current, label: current)]
    }

    private static func dedupedThinkingOptions(
        _ options: [LuckyNemoChatThinkingLevelOption]) -> [LuckyNemoChatThinkingLevelOption]
    {
        var result: [LuckyNemoChatThinkingLevelOption] = []
        var seen = Set<String>()
    ...
    static func normalizedThinkingLevel(
        _ level: String?,
        options: [LuckyNemoChatThinkingLevelOption],
