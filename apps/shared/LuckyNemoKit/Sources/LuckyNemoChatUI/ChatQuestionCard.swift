...
public enum LuckyNemoQuestionCardStatus: Sendable, Equatable {
    case pending
    case submitting
    case answered
...
@MainActor
@Observable
public final class LuckyNemoQuestionCardModel: Identifiable {
    ...
    public func status(at date: Date = Date()) -> LuckyNemoQuestionCardStatus {
    ...
    public func apply(resolved: LuckyNemoQuestionResolvedEvent, at date: Date = Date()) {
...
struct LuckyNemoQuestionCard: View {
    @Bindable private var model: LuckyNemoQuestionCardModel
    private let onSubmit: @MainActor @Sendable (LuckyNemoQuestionCardModel) async -> Void

    init(
        model: LuckyNemoQuestionCardModel,
        onSubmit: @escaping @MainActor @Sendable (LuckyNemoQuestionCardModel) async -> Void)
    {
        self.model = model
    ...
    var body: some View {
        TimelineView(.periodic(from: .now, by: 1)) { context in
            ...
            .padding(16)
            .background(LuckyNemoChatTheme.subtleCard, in: RoundedRectangle(cornerRadius: 16))
    ...
    private func questionSection(_ question: Question, now: Date) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(question.header.uppercased())
                ...
                .foregroundStyle(LuckyNemoChatTheme.accent)
        ...
        return Button {
            ...
            HStack(alignment: .top, spacing: 10) {
                Image(systemName: selected
                    ...
                    .foregroundStyle(selected ? LuckyNemoChatTheme.accent : .secondary)
                VStack(alignment: .leading, spacing: 2) {
                    Text(option.label).font(LuckyNemoChatTypography.body)
    ...
    private func footer(now: Date) -> some View {
        ...
        if status == .pending || status == .submitting {
            HStack {
                ...
                .font(LuckyNemoChatTypography.body(size: 14, weight: .semibold, relativeTo: .callout))
            ...
            if let errorText = self.model.errorText {
                Text(errorText)
                    .font(LuckyNemoChatTypography.caption)
                    .foregroundStyle(LuckyNemoChatTheme.danger)
            }
        } else {
    ...
    private func terminalText(_ status: LuckyNemoQuestionCardStatus) -> String {
        switch status {
    ...
    private func terminalIcon(_ status: LuckyNemoQuestionCardStatus) -> String {
        switch status {
...
@MainActor
struct LuckyNemoQuestionCards: View {
    let viewModel: LuckyNemoChatViewModel

    var body: some View {
        ForEach(self.viewModel.visibleQuestionCards) { card in
            LuckyNemoQuestionCard(model: card) { [weak viewModel = self.viewModel] model in
...
extension LuckyNemoChatViewModel {
    public var visibleQuestionCards: [LuckyNemoQuestionCardModel] {
    ...
    func refreshQuestions() async {
        ...
        do {
            ...
            self.questionCards = records.map { record in
                ...
                changed = true
                return LuckyNemoQuestionCardModel(record: record)
            } + retainedTerminal
    ...
    func upsertQuestion(_ record: QuestionRecord) {
        if let model = self.questionCards.first(where: { $0.id == record.id }) {
            ...
            self.questionCards.append(LuckyNemoQuestionCardModel(record: record))
        }
        self.questionStateRevision &+= 1
        self.syncQuestionEvictions()
        self.markTimelineChanged()
    ...
    func resolveQuestionEvent(_ event: LuckyNemoQuestionResolvedEvent) {
    ...
    func submitQuestion(_ model: LuckyNemoQuestionCardModel) async {
    ...
    func evictQuestionIfTerminalGraceElapsed(
        _ model: LuckyNemoQuestionCardModel,
        at date: Date = Date())
    {
