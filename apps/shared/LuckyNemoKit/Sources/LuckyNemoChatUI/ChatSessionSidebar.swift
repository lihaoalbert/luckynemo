...
struct ChatSessionSidebar: View {
    ...
    @State private var sessionPendingDeletion: LuckyNemoChatSessionEntry?
    @State private var sessionPendingRename: LuckyNemoChatSessionEntry?
        ...
        return HStack(spacing: 6) {
            VStack(alignment: .leading, spacing: 2) {
                Text(ChatSessionSidebarModel.displayName(for: session))
                    .font(LuckyNemoChatTypography.body(size: 13, weight: .medium, relativeTo: .body))
                    .lineLimit(1)
    ...
    private func badges(for node: ChatSessionSidebarModel.Node) -> some View {
        ...
        if node.badges.failedCount > 0 {
            Image(systemName: "exclamationmark.triangle.fill")
                .foregroundStyle(LuckyNemoChatTheme.warning)
    ...
    @ViewBuilder
    private func contextMenu(for session: LuckyNemoChatSessionEntry) -> some View {
        Button {
    ...
    private func rowSubtitle(for session: LuckyNemoChatSessionEntry) -> String? {
        var parts: [String] = []
