...
extension LuckyNemoChatViewModel {
    static func decodeMessages(_ raw: [AnyCodable]) -> [LuckyNemoChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: LuckyNemoChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    static func stripInboundMetadata(from message: LuckyNemoChatMessage) -> LuckyNemoChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> LuckyNemoChatMessageContent in
    ...
    static func messageContentFingerprint(for message: LuckyNemoChatMessage) -> String {
    ...
    static func finalMessageContentFingerprint(for message: LuckyNemoChatMessage) -> String {
        message.content.map { item in
            let type = (item.type ?? "text").trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
            let text = (item.text ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
    ...
    static func messageIdentityKey(for message: LuckyNemoChatMessage) -> String? {
    ...
    static func userRefreshIdentityKey(for message: LuckyNemoChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    static func finalMessageReconciliationKey(for message: LuckyNemoChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    static func adoptingCanonicalMessage(
        _ incoming: LuckyNemoChatMessage,
        over existing: LuckyNemoChatMessage) -> LuckyNemoChatMessage
    {
    ...
    private static func preservingLocalAudioDurations(
        in incoming: [LuckyNemoChatMessageContent],
        from existing: [LuckyNemoChatMessageContent]) -> [LuckyNemoChatMessageContent]
    {
    ...
    static func messageRange(
        ...
        in messages: [LuckyNemoChatMessage]) -> Range<[LuckyNemoChatMessage].Index>
    {
    ...
    func adoptingProvisionalFinalMessageIDs(
        in incoming: [LuckyNemoChatMessage]) -> [LuckyNemoChatMessage]
    {
    ...
    func clearProvisionalFinalMarkersAdoptedByHistory(_ incoming: [LuckyNemoChatMessage]) {
    ...
    func adoptCorrelatedUserMessage(incoming: LuckyNemoChatMessage) -> Bool {
    ...
    func adoptProvisionalFinalMessage(incoming: LuckyNemoChatMessage) -> Bool {
    ...
    static func reconcileMessageIDs(
        previous: [LuckyNemoChatMessage],
        incoming: [LuckyNemoChatMessage]) -> [LuckyNemoChatMessage]
    {
        guard !previous.isEmpty, !incoming.isEmpty else { return incoming }

        var previousMessagesByKey: [String: [LuckyNemoChatMessage]] = [:]
    ...
    static func reconcileRunRefreshMessages(
        previous: [LuckyNemoChatMessage],
        incoming: [LuckyNemoChatMessage],
        pendingLocalUserEchoIDs: Set<UUID>) -> [LuckyNemoChatMessage]
    {
    ...
    static func dedupeMessages(_ messages: [LuckyNemoChatMessage]) -> [LuckyNemoChatMessage] {
    ...
    static func dedupeKey(for message: LuckyNemoChatMessage) -> String? {
...
extension LuckyNemoChatViewModel {
    ...
    func applyHistoryPayload(
        _ payload: LuckyNemoChatHistoryPayload,
    ...
    private func provisionalFinalMessagesMissing(
        from incoming: [LuckyNemoChatMessage]) -> [LuckyNemoChatMessage]
    {
