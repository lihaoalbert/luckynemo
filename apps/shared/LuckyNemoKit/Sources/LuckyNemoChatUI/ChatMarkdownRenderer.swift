...
struct ChatMarkdownRenderer: View {
    ...
    struct InlineMathTypography {
        static let body = Self(size: LuckyNemoChatTypography.bodySize, relativeTo: .body)
    ...
    private func blockView(_ block: ChatMarkdownRenderedBlock, index: Int) -> some View {
        ...
        case let .heading(level, prose):
            self.proseText(prose, index: index)
                .font(LuckyNemoChatTypography.heading(level: level))
                .foregroundStyle(self.textColor)
                .tint(self.linkColor)
                .textSelection(.enabled)
                .lineSpacing(self.variant == .compact ? 2 : 4)
    ...
    private var linkColor: Color {
        self.context == .user ? self.textColor : LuckyNemoChatTheme.accent
    }
}
