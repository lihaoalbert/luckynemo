...
public enum LuckyNemoMascotMood: String, CaseIterable, Equatable, Sendable {
...
enum LuckyNemoMascotEffect: Equatable {
    case none
    case sparkles
    case hearts
    case zzz
...
final class LuckyNemoMascotAnimator {
    ...
    private(set) var mood: LuckyNemoMascotMood = .idle
    ...
    private var activeGesture: LuckyNemoMascotGesture?
    private var activeGestureStart: TimeInterval = 0
    private var pendingGesture: LuckyNemoMascotGesture?
    ...
    private var lastClickReaction: LuckyNemoMascotGesture?
    ...
    private var requestedAccessory: LuckyNemoMascotAccessory = .none
    ...
    func setMood(_ mood: LuckyNemoMascotMood, at time: TimeInterval) {
    ...
    func setAccessory(_ accessory: LuckyNemoMascotAccessory, at time: TimeInterval) {
    ...
    func handleTap(at time: TimeInterval) {
        ...
        var reactions: [LuckyNemoMascotGesture] = [.hop, .wave, .wink]
    ...
    func pose(at time: TimeInterval) -> LuckyNemoMascotPose {
        ...
        let effectiveMood: LuckyNemoMascotMood = dozing ? .sleepy : self.mood
        ...
        if self.requestedAccessory != .none {
            ...
            pose.accessoryAmount = LuckyNemoMascotGesture.easeOut(progress)
        }
    ...
    private func advanceSchedules(at time: TimeInterval) {
        ...
        self.blinkStarts.removeAll { time - $0 > LuckyNemoMascotGesture.blinkDuration }
    ...
    private func randomQuirk() -> LuckyNemoMascotGesture {
    ...
    private func startGesture(_ gesture: LuckyNemoMascotGesture, at time: TimeInterval) {
        self.activeGesture = gesture
        self.activeGestureStart = time
    }

    private static func entranceGesture(for mood: LuckyNemoMascotMood) -> LuckyNemoMascotGesture? {
        switch mood {
        case .happy: .hop
    ...
    private func basePose(for mood: LuckyNemoMascotMood, at time: TimeInterval) -> LuckyNemoMascotPose {
        ...
        case .working:
            ...
            if phase < 0.05 {
                ...
                pose.rightClawDegrees = -6 - 28 * LuckyNemoMascotGesture.easeInOut((phase - 0.05) / 0.55)
            } else if phase < 0.72 {
                let strike = ((phase - 0.60) / 0.12).clamped(to: 0...1)
                pose.rightClawDegrees = -34 + 46 * strike * strike
            } else {
                pose.rightClawDegrees = 12 - 18 * LuckyNemoMascotGesture.easeInOut((phase - 0.72) / 0.28)
            }
            pose.leftClawDegrees = 4 + 2 * sin(2 * .pi * phase)
            let impact = LuckyNemoMascotGesture.bell(((phase - 0.72) / 0.14).clamped(to: 0...1))
    ...
    private func applyGaze(
        _ pose: inout LuckyNemoMascotPose,
        mood: LuckyNemoMascotMood,
    ...
    private func applyBlinks(_ pose: inout LuckyNemoMascotPose, at time: TimeInterval) {
        guard pose.happyEyes < 0.6 else { return }
        for start in self.blinkStarts {
            let progress = (time - start) / LuckyNemoMascotGesture.blinkDuration
            guard progress >= 0, progress <= 1 else { continue }
            let closure = LuckyNemoMascotGesture.bell(CGFloat(progress))
    ...
    private func applyDizzy(_ pose: inout LuckyNemoMascotPose, at time: TimeInterval) {
...
enum LuckyNemoMascotGesture: Equatable {
    case wave
    case hop
    case wink
    case celebrate
    ...
    func apply(to pose: inout LuckyNemoMascotPose, progress p: CGFloat) {
        switch self {
        case .wave:
        ...
        case .sneeze:
            if p < 0.42 {
                let inhale = LuckyNemoMascotGesture.easeInOut(p / 0.42)
                ...
                let recover = 1 - LuckyNemoMascotGesture.easeInOut((p - 0.58) / 0.42)
        ...
        case .sigh:
            let rise = LuckyNemoMascotGesture.easeInOut((p / 0.3).clamped(to: 0...1))
            let fall = LuckyNemoMascotGesture.easeInOut(((p - 0.3) / 0.45).clamped(to: 0...1))
