...
private let outboxLogger = Logger(subsystem: "ai.luckynemo", category: "LuckyNemoChatOutbox")

/// Display state for a transcript row backed by a durable outbox command.
public enum LuckyNemoChatOutboxMessageState: Equatable, Sendable {
    case queued
    case sending
    case confirming
...
extension LuckyNemoChatViewModel {
    ...
    public func outboxState(for messageID: UUID) -> LuckyNemoChatOutboxMessageState? {
    ...
    func enqueueOutboxCommand(
        ...
        draftAttachments: [LuckyNemoPendingAttachment] = [],
        session: SessionSnapshot) async -> Bool
    {
        ...
        let thinking = self.effectiveThinkingLevelForSend(
            ...
            agentID: agentID,
            sessionRoutingContract: routingContract)
        let command = LuckyNemoChatOutboxCommand(
            id: UUID().uuidString,
            sessionKey: session.key,
            deliverySessionKey: deliverySessionKey,
            routingContract: routingContract,
    ...
    func confirmOutboxCommands(in messages: [LuckyNemoChatMessage]) {
    ...
    func confirmOutboxCommandsNow(in messages: [LuckyNemoChatMessage]) async {
        self.observeCanonicalOutboxMessageKeys(in: messages)
        guard let outbox else { return }
    ...
    private func persistCanonicalOutboxEvidence(
        _ message: LuckyNemoChatMessage,
        for command: LuckyNemoChatOutboxCommand) async
    {
        guard let transcriptCache = transcriptCache as? any LuckyNemoChatCanonicalTranscriptMerging else { return }
    ...
    private func observeCanonicalOutboxMessageKeys(in messages: [LuckyNemoChatMessage]) {
    ...
    private func presentOutboxCommands(_ commands: [LuckyNemoChatOutboxCommand]) {
    ...
    private static func outboxUserMessage(for command: LuckyNemoChatOutboxCommand) -> LuckyNemoChatMessage {
        var content = [
    ...
    private func mapOutboxCommand(_ command: LuckyNemoChatOutboxCommand, to messageID: UUID) {
    ...
    private func deliverOutboxCommand(
        _ command: LuckyNemoChatOutboxCommand,
        outbox: any LuckyNemoChatCommandOutbox,
        routeLease: LuckyNemoChatTransportRouteLease) async -> OutboxFlushDisposition
    {
        do {
            let response = try await routeLease.sendMessage(
                ...
                attachments: command.attachments.map {
                    LuckyNemoChatAttachmentPayload(
                        type: $0.type,
                        mimeType: $0.mimeType,
                        fileName: $0.fileName,
        ...
        } catch is LuckyNemoChatTransportSendError {
            ...
            if error.detailsReason == LuckyNemoChatSessionRoutingContract.changedErrorReason {
    ...
    private func finishAcceptedOutboxCommand(
        _ command: LuckyNemoChatOutboxCommand,
        outbox: any LuckyNemoChatCommandOutbox) async -> OutboxFlushDisposition
    {
    ...
    private static func needsOutboxDeliveryReconciliation(_ command: LuckyNemoChatOutboxCommand) -> Bool {
        command.status == .awaitingConfirmation ||
            (command.status == .failed &&
                command.lastError == LuckyNemoChatSQLiteTranscriptCache.outboxUnconfirmedError)
    }

    private static func deliveryTarget(for command: LuckyNemoChatOutboxCommand) -> OutboxDeliveryTarget {
    ...
    private func parkOutboxCommandWithUnconfirmedDelivery(
        _ command: LuckyNemoChatOutboxCommand,
        outbox: any LuckyNemoChatCommandOutbox) async -> LuckyNemoChatOutboxUpdateResult
    {
        let update = await outbox.markCommandFailedIfPresent(
            id: command.id,
            retryCount: command.retryCount,
            lastError: LuckyNemoChatSQLiteTranscriptCache.outboxUnconfirmedError)
        switch update {
        case .updated:
            self.setOutboxState(
                .failed(reason: LuckyNemoChatSQLiteTranscriptCache.outboxUnconfirmedError),
    ...
    private func parkOutboxCommandForChangedTarget(
        _ command: LuckyNemoChatOutboxCommand,
        outbox: any LuckyNemoChatCommandOutbox) async -> Bool
    {
        let update = await outbox.markCommandFailedIfPresent(
            id: command.id,
            retryCount: command.retryCount,
            lastError: LuckyNemoChatSQLiteTranscriptCache.outboxChangedTargetError)
        guard update != .unavailable else {
            self.applyTransportHealth(false)
            return false
        }
    ...
    private func recordOutboxRejection(
        of command: LuckyNemoChatOutboxCommand,
        outbox: any LuckyNemoChatCommandOutbox,
    ...
    private func spliceSentCommandIntoCachedTranscript(_ command: LuckyNemoChatOutboxCommand) async {
    ...
    private func outboxRoutingContract(for session: SessionSnapshot) -> String? {
        if !self.transport.outboxRequiresSessionRoutingContract {
            return LuckyNemoChatOutboxCommand.legacyUnboundRoutingContract
        }
        ...
        agentID: String?) -> String?
    {
        ...
        if LuckyNemoChatSessionKey.agentID(from: raw) != nil { return raw }
    ...
    private func setOutboxState(_ state: LuckyNemoChatOutboxMessageState, forCommandID commandID: String) {
    ...
    func handleOutboxChange(_ change: LuckyNemoChatOutboxChange) {
    ...
    private static func outboxDisplayState(for command: LuckyNemoChatOutboxCommand)
        -> LuckyNemoChatOutboxMessageState
    {
