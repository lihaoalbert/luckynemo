...
public struct LuckyNemoChatDisplayOptions: OptionSet, Sendable {
...
@MainActor
public struct LuckyNemoChatView: View {
    public enum Style {
        case standard
        case onboarding
    }
    ...
    private let displayOptions: LuckyNemoChatDisplayOptions
    ...
    private let talkControl: LuckyNemoChatTalkControl?
    private let voiceNoteControl: LuckyNemoChatVoiceNoteControl?
    ...
    public init(
        ...
        displayOptions: LuckyNemoChatDisplayOptions? = nil,
        ...
        talkControl: LuckyNemoChatTalkControl? = nil,
        voiceNoteControl: LuckyNemoChatVoiceNoteControl? = nil,
        speech: LuckyNemoChatSpeechController? = nil)
    {
    ...
    private var messageListRows: some View {
        ...
        LuckyNemoQuestionCards(viewModel: self.viewModel)
    ...
    private func isListenable(_ msg: LuckyNemoChatMessage) -> Bool {
    ...
    private func speechChipIsPreparing(
        _ speech: LuckyNemoChatSpeechController,
        messageID: UUID) -> Bool?
    {
        switch speech.phase {
    ...
    private var visibleMessages: [LuckyNemoChatMessage] {
        let base: [LuckyNemoChatMessage]
    ...
    private var jumpToLatestButton: some View {
        Button {
            ...
            Label("Jump to latest", systemImage: "arrow.down")
                .font(LuckyNemoChatTypography.body(size: 16, weight: .semibold, relativeTo: .callout))
...
extension LuckyNemoChatView {
    private func mergeToolResults(in messages: [LuckyNemoChatMessage]) -> [LuckyNemoChatMessage] {
        var result: [LuckyNemoChatMessage] = []
        ...
        for message in messages {
            ...
            let merged = LuckyNemoChatMessage(
                id: last.id,
                role: last.role,
                content: content,
    ...
    private func isToolResultMessage(_ message: LuckyNemoChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: LuckyNemoChatMessage) -> Bool {
    ...
    private func primaryText(in message: LuckyNemoChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: LuckyNemoChatMessage) -> Bool {
    ...
    private func toolCalls(in message: LuckyNemoChatMessage) -> [LuckyNemoChatMessageContent] {
    ...
    private func inlineToolResults(in message: LuckyNemoChatMessage) -> [LuckyNemoChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: LuckyNemoChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: LuckyNemoChatMessage) -> String {
        self.primaryText(in: message)
    }

    @ViewBuilder
    private func copyMessageButton(for message: LuckyNemoChatMessage) -> some View {
    ...
    @ViewBuilder
    private func openFullMessageButton(for message: LuckyNemoChatMessage) -> some View {
    ...
    @ViewBuilder
    private func replyMessageButton(for message: LuckyNemoChatMessage) -> some View {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
...
private struct ChatAssistantIntroCard: View {
    let text: String
    let prompts: [LuckyNemoChatView.StarterPrompt]
    let onPrompt: (LuckyNemoChatView.StarterPrompt) -> Void
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            Text(self.text)
                ...
                .background(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
                        .fill(LuckyNemoChatTheme.assistantBubble))
            ...
            ForEach(self.prompts) { prompt in
                Button {
                    ...
                    HStack(spacing: 8) {
                        Text(prompt.title)
                            .font(LuckyNemoChatTypography.body(size: 15, weight: .semibold, relativeTo: .callout))
                            .multilineTextAlignment(.leading)
                        Spacer(minLength: 8)
    ...
    var body: some View {
        ...
        ContentUnavailableView {
            Label(self.title, systemImage: self.systemImage)
                .font(LuckyNemoChatTypography.headline)
        } description: {
            Text(self.message)
                .font(LuckyNemoChatTypography.body)
        } actions: {
            if let actionTitle, let action {
                Button(action: action) {
                    Text(actionTitle)
                        .font(LuckyNemoChatTypography.body(size: 15, weight: .semibold, relativeTo: .subheadline))
                }
