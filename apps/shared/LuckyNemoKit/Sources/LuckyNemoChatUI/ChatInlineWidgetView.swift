...
enum LuckyNemoChatWidgetSurfaceRole: Sendable, Hashable {
    case node
    case operatorSurface
    case legacy
}

public struct LuckyNemoChatWidgetResource: Sendable, Equatable {
    public let url: URL
    public let tlsFingerprintSHA256: String?
    let surfaceRole: LuckyNemoChatWidgetSurfaceRole
    let attemptedSurfaceRoles: Set<LuckyNemoChatWidgetSurfaceRole>
    ...
    init(
        ...
        surfaceRole: LuckyNemoChatWidgetSurfaceRole,
        attemptedSurfaceRoles: Set<LuckyNemoChatWidgetSurfaceRole>)
    {
        self.url = url
        self.tlsFingerprintSHA256 = tlsFingerprintSHA256
...
public enum LuckyNemoChatWidgetURLResolver {
    private static let documentsPath = "/__luckynemo__/canvas/documents"
    ...
    public static func resolveResource(
        target: String,
        replacing failedResource: LuckyNemoChatWidgetResource?,
    ...
    private static func capabilitySurface(_ rawSurfaceURL: String?) -> URLComponents? {
        ...
        guard segments.count >= 3,
              segments[segments.count - 3] == "__luckynemo__",
    ...
    private static func resolve(
        ...
        role: LuckyNemoChatWidgetSurfaceRole,
        target: String,
        attemptedRoles: Set<LuckyNemoChatWidgetSurfaceRole>) -> LuckyNemoChatWidgetResource?
    {
        guard let url = resolve(surfaceURL: surface.url, target: target) else { return nil }
        let resource = LuckyNemoChatWidgetResource(
            url: url,
    ...
    private static func resolvePreferred(
        ...
        target: String,
        excluding failedResource: LuckyNemoChatWidgetResource?,
        blockedRoles: Set<LuckyNemoChatWidgetSurfaceRole>,
        attemptedRoles: Set<LuckyNemoChatWidgetSurfaceRole>) -> LuckyNemoChatWidgetResource?
    {
        [
            (role: LuckyNemoChatWidgetSurfaceRole.node, surface: surfaces.node),
            (role: LuckyNemoChatWidgetSurfaceRole.operatorSurface, surface: surfaces.operatorSurface),
        ]
            .lazy
    ...
    private static func isReplacement(
        _ candidate: LuckyNemoChatWidgetResource,
        for failedResource: LuckyNemoChatWidgetResource?) -> Bool
    {
...
struct ChatInlineWidgetView: View {
    let preview: LuckyNemoChatCanvasPreview
    ...
    @State private var resolvedResource: LuckyNemoChatWidgetResource?
                ...
                ChatInlineWidgetWebView(
                    ...
                    .overlay {
                        RoundedRectangle(cornerRadius: 10, style: .continuous)
                            .stroke(LuckyNemoChatTheme.muted.opacity(0.24), lineWidth: 1)
                    }
    ...
    private func handleLoadFailure(resource: LuckyNemoChatWidgetResource) {
...
extension LuckyNemoChatWidgetResource {
...
private final class ChatInlineWidgetNavigationDelegate: NSObject, WKNavigationDelegate {
    var resource: LuckyNemoChatWidgetResource {
        didSet {
    ...
    init(resource: LuckyNemoChatWidgetResource, onFailure: @escaping @MainActor @Sendable () -> Void) {
...
private func makeChatInlineWidgetWebView(
    resource: LuckyNemoChatWidgetResource,
    allowsScripts: Bool,
