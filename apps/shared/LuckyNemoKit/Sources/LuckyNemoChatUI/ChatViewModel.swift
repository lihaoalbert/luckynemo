...
private let chatUILogger = Logger(subsystem: "ai.luckynemo", category: "LuckyNemoChatUI")

@MainActor
@Observable
public final class LuckyNemoChatViewModel {
    ...
    public internal(set) var messages: [LuckyNemoChatMessage] = []
    ...
    public internal(set) var replyTarget: LuckyNemoChatReplyTarget?
    @ObservationIgnored
    ...
    /// Setter is module-internal for the thinking-level extension only.
    public internal(set) var thinkingLevelOptions: [LuckyNemoChatThinkingLevelOption]
    /// Setter is module-internal for the thinking-level extension only.
    ...
    public private(set) var modelChoices: [LuckyNemoChatModelChoice] = []
    ...
    public internal(set) var slashCommands: [LuckyNemoChatCommandChoice] = []
    ...
    struct SlashFilterCache {
        let query: String
        let filter: LuckyNemoChatCommandFilter
        let result: [LuckyNemoChatCommandChoice]
    }
    ...
    public var attachments: [LuckyNemoPendingAttachment] = []
    ...
    public internal(set) var questionCards: [LuckyNemoQuestionCardModel] = []
    ...
    public private(set) var pendingToolCalls: [LuckyNemoChatPendingToolCall] = []
    public internal(set) var planSteps: [LuckyNemoChatPlanStep] = []
    ...
    public internal(set) var sessions: [LuckyNemoChatSessionEntry] = [] {
        didSet { syncContextUsageFraction() }
    }
    ...
    let transport: any LuckyNemoChatTransport
    let haptics: LuckyNemoChatHaptics
    let transcriptCache: (any LuckyNemoChatTranscriptCache)?
    let outbox: (any LuckyNemoChatCommandOutbox)?
    @ObservationIgnored
    ...
    public internal(set) var outboxStatesByMessageID: [UUID: LuckyNemoChatOutboxMessageState] = [:]
    @ObservationIgnored
    ...
    var sessionDefaults: LuckyNemoChatSessionsDefaults? {
        didSet { syncContextUsageFraction() }
    }
    ...
    var lastSuccessfulSettingsPatchResultsByTarget: [ModelPatchTarget: LuckyNemoChatModelPatchResult] = [:]
    ...
    var acceptedSettingsPatchResultsByTarget: [ModelPatchTarget: LuckyNemoChatModelPatchResult] = [:]
    ...
    struct FastModeState {
        let override: LuckyNemoChatFastMode?
        let effective: LuckyNemoChatFastMode?
    }
    ...
    private struct SettingsPatchTail {
        ...
        let routeLeaseTask: Task<LuckyNemoChatSessionSettingsRouteLease?, Never>
    ...
    struct HistoryRequest {
        ...
        var visibleMessagesByID: [UUID: LuckyNemoChatMessage]
    ...
    var pendingToolCallsById: [String: LuckyNemoChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        ...
        haptics: LuckyNemoChatHaptics = LuckyNemoChatHaptics(),
    ...
    public func isDefaultModel(_ model: LuckyNemoChatModelChoice) -> Bool {
    ...
    private var usesMutableAgentRouting: Bool {
        LuckyNemoChatSessionKey.agentID(from: self.sessionKey) == nil
    }
    ...
    func usesMutableContractRouting(sessionKey: String, contract: String?) -> Bool {
        if LuckyNemoChatSessionKey.agentID(from: sessionKey) == nil {
            return true
        }
        ...
        let contractMainKey = LuckyNemoChatSessionRoutingContract.parse(contract)?.mainKey ?? ""
    ...
    static let baseThinkingLevelOptions: [LuckyNemoChatThinkingLevelOption] = [
        LuckyNemoChatThinkingLevelOption(id: "off", label: "off"),
        LuckyNemoChatThinkingLevelOption(id: "minimal", label: "minimal"),
        LuckyNemoChatThinkingLevelOption(id: "low", label: "low"),
    ...
    public func removeAttachment(_ id: LuckyNemoPendingAttachment.ID) {
...
extension LuckyNemoChatViewModel {
    // MARK: - Internals
    ...
    func appendMessage(_ message: LuckyNemoChatMessage) {
    ...
    func currentSessionSnapshot() -> SessionSnapshot {
        SessionSnapshot(
            ...
            deliveryAgentID: LuckyNemoChatSessionKey.agentID(from: self.sessionKey) ?? self.activeAgentId,
    ...
    func fetchSessions(limit: Int?, sessionSnapshot: SessionSnapshot? = nil) async {
        ...
        while true {
            ...
            let res: LuckyNemoChatSessionsListResponse
            do {
            ...
            let organized = LuckyNemoChatSessionListOrganizer.organize(res.sessions)
    ...
    private static func isUnsupportedCreateSessionError(_ error: Error) -> Bool {
        ...
        return nsError.domain == "LuckyNemoChatTransport"
    ...
    private func performSelectModel(
        ...
        routeLease: LuckyNemoChatSessionSettingsRouteLease?) async
    {
        do {
            guard let routeLease else { throw LuckyNemoChatTransportSendError.notDispatched }
            let patchResult = try await routeLease.patchSessionSettings(
                ...
                patch: LuckyNemoChatSessionSettingsPatch(model: .some(request.modelRef)))
    ...
    func enqueueSessionSettingsPatch(
        ...
        operation: @escaping @MainActor (LuckyNemoChatSessionSettingsRouteLease?) async -> Void)
    {
    ...
    func placeholderSession(key: String) -> LuckyNemoChatSessionEntry {
    ...
    private func generatedNewSessionKey() -> String {
        ...
        guard let agentID = LuckyNemoChatSessionKey.agentID(from: sessionKey) ??
            activeAgentId ??
            LuckyNemoChatSessionKey.agentID(from: resolvedMainSessionKey) ??
            sessions.lazy.compactMap({ LuckyNemoChatSessionKey.agentID(from: $0.key) }).first
        else {
            return baseKey
        }
    ...
    private func applySuccessfulModelSelection(
        ...
        syncSelection: Bool,
        patchResult: LuckyNemoChatModelPatchResult? = nil)
    {
        self.recordSuccessfulModelPatch(
    ...
    private func recordSuccessfulModelPatch(
        selectionID: String,
        patchResult: LuckyNemoChatModelPatchResult?,
        target: ModelPatchTarget)
    {
        ...
        if let patchResult {
            ...
            self.acceptedSettingsPatchResultsByTarget[target] = LuckyNemoChatModelPatchResult(
