...
struct ChatAgentAvatar: View {
    ...
    var body: some View {
        Text(self.displayText)
            .font(LuckyNemoChatTypography.avatar(size: self.fontSize))
            ...
            .background(
                Circle()
                    .fill(
                        LinearGradient(
                            colors: [
                                (self.tint ?? LuckyNemoChatTheme.accent).opacity(0.95),
            ...
            .shadow(color: (self.tint ?? LuckyNemoChatTheme.accent).opacity(0.18), radius: 8, y: 4)
...
struct ChatMessageBubble: View {
    ...
    let inlineWidgetResourceResolver: @MainActor @Sendable (
        String,
        LuckyNemoChatWidgetResource?) async -> LuckyNemoChatWidgetResource?
...
private struct ChatMessageBody: View {
    @Environment(\.luckyNemoAssistantBubblesInCleanChrome) private var assistantBubblesInClean
    let message: LuckyNemoChatMessage
    let isUser: Bool
    let style: LuckyNemoChatView.Style
    let markdownVariant: ChatMarkdownVariant
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? LuckyNemoChatTheme.userText : LuckyNemoChatTheme.assistantText
    ...
    private var inlineAttachments: [LuckyNemoChatMessageContent] {
    ...
    private var inlineWidgets: [LuckyNemoChatCanvasPreview] {
    ...
    private var toolCalls: [LuckyNemoChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [LuckyNemoChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func usageTint(_ pressure: ChatMessageUsagePresentation.Pressure) -> Color {
        ...
        case .warning:
            LuckyNemoChatTheme.warning
        case .danger:
            LuckyNemoChatTheme.danger
        }
    }
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? LuckyNemoChatTheme.userBubble
        }
        if self.style == .onboarding {
            return LuckyNemoChatTheme.onboardingAssistantBubble
        }
        return LuckyNemoChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return LuckyNemoChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: LuckyNemoChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.isAudio ? "Voice note" : (self.att.fileName ?? "Attachment"))
                ...
                .lineLimit(1)
                .foregroundStyle(self.isUser ? LuckyNemoChatTheme.userText : LuckyNemoChatTheme.assistantText)
            if self.isAudio, let durationSeconds = self.att.durationSeconds {
                Text(luckyNemoVoiceNoteDurationLabel(durationSeconds))
                    .font(LuckyNemoChatTypography.footnote)
                    .foregroundStyle(
                        self.isUser
                            ? LuckyNemoChatTheme.userText.opacity(0.72)
                            : LuckyNemoChatTheme.assistantText.opacity(0.72))
            }
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: LuckyNemoChatMessageContent
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            if let summary = self.summary, !summary.isEmpty {
                Text(summary)
                    .font(LuckyNemoChatTypography.mono(size: 13, relativeTo: .footnote))
...
struct ChatOutboxStatusLabel: View {
    let state: LuckyNemoChatOutboxMessageState
    ...
    var body: some View {
        ...
        .foregroundStyle(self.state.isFailed ? AnyShapeStyle(LuckyNemoChatTheme.danger) : AnyShapeStyle(.secondary))
    ...
    private var title: LocalizedStringResource {
        ...
        case .confirming:
            "Confirming…"
        case let .failed(reason) where reason == LuckyNemoChatSQLiteTranscriptCache.outboxUnconfirmedError:
            "Delivery unknown"
        case .failed:
            "Not sent"
        }
...
private struct LuckyNemoAssistantBubblesInCleanChromeKey: EnvironmentKey {
...
extension EnvironmentValues {
    ...
    public var luckyNemoAssistantBubblesInCleanChrome: Bool {
        get { self[LuckyNemoAssistantBubblesInCleanChromeKey.self] }
        set { self[LuckyNemoAssistantBubblesInCleanChromeKey.self] = newValue }
    }
}
...
private struct AssistantBubbleContainerStyle: ViewModifier {
    ...
    @Environment(\.luckyNemoAssistantBubblesInCleanChrome) private var bubblesInClean
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [LuckyNemoChatPendingToolCall]
    let isClean: Bool
        ...
        return VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                let font = segment.kind == .thinking
                    ? LuckyNemoChatTypography.callout.italic()
                    : LuckyNemoChatTypography.body
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: LuckyNemoChatTheme.assistantText,
