...
struct ChatMathBlockView: View {
    ...
    @ScaledMetric(relativeTo: .body) private var fontSize: CGFloat = LuckyNemoChatTypography.bodySize
...
struct ChatMarkdownTableView: View {
    ...
    private func cell(_ text: String, isHeader: Bool) -> some View {
        Text(self.inlineMarkdown(text))
            .font(isHeader ? LuckyNemoChatTypography.footnoteSemiBold : LuckyNemoChatTypography.footnote)
            .foregroundStyle(LuckyNemoChatTheme.assistantText)
            .textSelection(.enabled)
    }
