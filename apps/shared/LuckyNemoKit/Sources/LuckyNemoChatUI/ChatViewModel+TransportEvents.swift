...
private let transportEventsLogger = Logger(subsystem: "ai.luckynemo", category: "LuckyNemoChatUI")

@MainActor
private final class PendingRunOwnerReference {
    weak var value: LuckyNemoChatViewModel?

    init(_ value: LuckyNemoChatViewModel) {
        self.value = value
    }
}
...
extension LuckyNemoChatViewModel {
    ...
    func handleTransportEvent(_ evt: LuckyNemoChatTransportEvent) {
        switch evt {
        case let .health(ok):
    ...
    private func handleSessionMessageEvent(_ payload: LuckyNemoSessionMessageEventPayload) {
    ...
    private func handleChatEvent(_ chat: LuckyNemoChatEventPayload) {
           ...
           let runId = Self.normalizedRunID(chat.runId)
        {
            ...
            self.adoptRun(
                runId: runId,
                bufferedText: LuckyNemoChatEventText.assistantText(from: chat) ?? "")
            return
        }
        ...
        case "final", "aborted", "error":
            ...
            let hapticEvent: LuckyNemoChatHaptics.Event? = switch chat.state {
    ...
    private func appendFinalChatMessageIfPresent(_ chat: LuckyNemoChatEventPayload) {
        guard chat.state == "final" else { return }
        guard let text = LuckyNemoChatEventText.assistantText(from: chat) else { return }

        let decoded = chat.message.flatMap {
            try? ChatPayloadDecoding.decode($0, as: LuckyNemoChatMessage.self)
        }
    ...
    static func isAssistantMessage(_ message: LuckyNemoChatMessage) -> Bool {
        message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() == "assistant"
    }

    private static func messageWithTimestampIfNeeded(_ message: LuckyNemoChatMessage) -> LuckyNemoChatMessage {
        guard message.timestamp == nil else { return message }
        return LuckyNemoChatMessage(
            id: message.id,
            role: message.role,
    ...
    private func handleAgentEvent(_ evt: LuckyNemoAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = LuckyNemoChatPendingToolCall(
    ...
    private func handleAgentLifecycleEvent(_ evt: LuckyNemoAgentEventPayload, isPendingRun: Bool) {
    ...
    private static func agentLifecycleErrorMessage(_ evt: LuckyNemoAgentEventPayload, aborted: Bool) -> String {
        if aborted {
            return "Run aborted"
        }
    ...
    func finishPendingRunAfterTerminalOkSendAck(_ response: LuckyNemoChatSendResponse) {
    ...
    func finishPendingRunIfTerminalSendAck(_ response: LuckyNemoChatSendResponse) -> Bool {
    ...
    private func refreshIfPending(
        ...
        terminalState: LuckyNemoChatRunTerminalState? = nil,
    ...
    private func finishPendingRun(runId: String, terminalState: LuckyNemoChatRunTerminalState) {
        let hapticEvent: LuckyNemoChatHaptics.Event
    ...
    static func hasUnansweredLatestUser(in messages: [LuckyNemoChatMessage]) -> Bool {
        self.latestUserTurn(in: messages) != nil && !self.hasAssistantMessageAfterLatestUser(in: messages)
    }

    static func latestUserTurn(in messages: [LuckyNemoChatMessage]) -> LatestUserTurn? {
    ...
    static func userTurn(
        at userIndex: [LuckyNemoChatMessage].Index,
        in messages: [LuckyNemoChatMessage]) -> LatestUserTurn?
    {
    ...
    static func hasAnsweredUser(
        ...
        in messages: [LuckyNemoChatMessage])
        -> Bool
    {
        ...
        var occurrence = 0
        var latestMatchingUserIndex: [LuckyNemoChatMessage].Index?
    ...
    private static func hasAssistantMessage(
        after userIndex: [LuckyNemoChatMessage].Index,
        in messages: [LuckyNemoChatMessage]) -> Bool
    {
    ...
    private static func hasAssistantMessageAfterLatestUser(in messages: [LuckyNemoChatMessage]) -> Bool {
        guard let lastUserIndex = messages.lastIndex(where: { $0.role.lowercased() == "user" }) else {
            return false
        }
    ...
    private static func assistantHapticEvent(
        for message: LuckyNemoChatMessage) -> LuckyNemoChatHaptics.Event?
    {
    ...
    func assistantHapticEventAfterLatestUser() -> LuckyNemoChatHaptics.Event? {
    ...
    private func assistantHapticEvent(after timestamp: Double) -> LuckyNemoChatHaptics.Event? {
    ...
    func refreshHistoriesAfterOutboxFlush(
        ...
        routeLease: LuckyNemoChatTransportRouteLease) async
    {
    ...
    private nonisolated static func runPendingRunOwner(
        ...
        armID: UInt64,
        transport: any LuckyNemoChatTransport) async
    {
    ...
    private nonisolated static func observePendingRunCompletion(
        ...
        transport: any LuckyNemoChatTransport) async
    {
        var terminalState: LuckyNemoChatRunTerminalState?
            ...
            armID: armID)
        {
            ...
            let effectiveObservation = terminalState.map(LuckyNemoChatRunObservation.terminal) ?? observation
    ...
    private static func processPendingRunObservation(
        ...
        userMessageTimestamp: Double?,
        armID: UInt64,
        observation: LuckyNemoChatRunObservation,
    ...
    func clearPendingRun(
        _ runId: String,
        hapticEvent: LuckyNemoChatHaptics.Event? = nil)
    {
