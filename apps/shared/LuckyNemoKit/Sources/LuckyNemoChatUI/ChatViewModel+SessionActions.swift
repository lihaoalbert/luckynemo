...
private let chatSessionActionsLogger = Logger(
    subsystem: "ai.luckynemo",
    category: "LuckyNemoChat")
...
extension LuckyNemoChatViewModel {
    ...
    public func fetchSessionList(search: String?, archived: Bool) async -> [LuckyNemoChatSessionEntry] {
        ...
        do {
            let res = try await self.transport.listSessions(
                ...
                search: query,
                archived: archived)
            return LuckyNemoChatSessionListOrganizer.organize(res.sessions)
        } catch {
            ...
            return LuckyNemoChatSessionListOrganizer.filter(self.sessions, search: query)
        }
    }
    ...
    public func setSessionPinned(key: String, pinned: Bool) {
        ...
        if let index = self.sessions.firstIndex(where: { $0.key == key }) {
            ...
            self.sessions = LuckyNemoChatSessionListOrganizer.organize(self.sessions)
        }
        Task {
            do {
                try await self.transport.patchSession(
    ...
    func markCurrentSessionReadAfterActivation(
        ...
        fallbackEntry: LuckyNemoChatSessionEntry?) async
    {
