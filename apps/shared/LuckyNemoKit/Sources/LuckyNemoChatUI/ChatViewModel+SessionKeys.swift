...
extension LuckyNemoChatViewModel {
    nonisolated static func chatContextUsageFraction(for session: LuckyNemoChatSessionEntry?) -> Double? {
    ...
    func currentSessionEntry() -> LuckyNemoChatSessionEntry? {
    ...
    func sessionMutationIdentity(for key: String, listedKey: String? = nil) -> String {
        ...
        return self.modelPatchTarget(
            ...
            agentID: LuckyNemoChatSessionKey.agentID(from: key) ?? self.activeAgentId,
            sessionRoutingContract: nil).canonicalSessionKey
    }

    func applyingLocalUnreadOverrides(
        to sessions: [LuckyNemoChatSessionEntry]) -> [LuckyNemoChatSessionEntry]
    {
        ...
        sessionRoutingContract: String?) -> ModelPatchTarget
    {
        ...
        if LuckyNemoChatSessionKey.agentID(from: candidate) != nil || normalizedCandidate == "unknown" {
            targetKey = candidate
            targetAgentID = nil
    ...
    func sessionEntryForThinking(
        ...
        agentID: String?) -> LuckyNemoChatSessionEntry?
    {
    ...
    func successfulModelPatchResult(
        ...
        session: LuckyNemoChatSessionEntry?) -> LuckyNemoChatModelPatchResult?
    {
        guard let session,
    ...
    public var sessionChoices: [LuckyNemoChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = LuckyNemoChatSessionListOrganizer.organize(sessions)
        let mainSessionKey = resolvedMainSessionKey

        var result: [LuckyNemoChatSessionEntry] = []
