...
extension LuckyNemoChatViewModel {
        ...
        self.enqueueSessionSettingsPatch(requestID: requestID, target: target) { [weak self] routeLease in
            ...
            do {
                ...
                let result = try await routeLease.patchSessionSettings(
                    ...
                    patch: LuckyNemoChatSessionSettingsPatch(verboseLevel: .some(next)))
    ...
    func performSelectFastMode(_ selectionID: String) {
        let next: LuckyNemoChatFastMode?
        switch selectionID {
        ...
        self.enqueueSessionSettingsPatch(requestID: requestID, target: target) { [weak self] routeLease in
            ...
            do {
                ...
                let result = try await routeLease.patchSessionSettings(
                    sessionKey: target.canonicalSessionKey,
                    agentID: target.agentID,
                    patch: LuckyNemoChatSessionSettingsPatch(fastMode: .some(next)))
    ...
    func applyModelControlPatchResult(
        _ result: LuckyNemoChatModelPatchResult,
        sessionKey: String,
    ...
    private func recordModelControlPatchSuccess(
        ...
        fastModeOverride: LuckyNemoChatFastMode?? = nil,
        ...
        verboseLevelOverride: String?? = nil)
    {
        ...
        let recordedFastMode: LuckyNemoChatFastMode? = if let fastModeOverride {
            fastModeOverride
        } else {
        ...
        self.lastSuccessfulSettingsPatchResultsByTarget[target] = LuckyNemoChatModelPatchResult(
    ...
    private func updateCurrentSessionFastMode(
        _ mode: LuckyNemoChatFastMode?,
        effective: LuckyNemoChatFastMode?,
        sessionKey: String,
        exactMatchOnly: Bool = false)
    {
        let index = exactMatchOnly
