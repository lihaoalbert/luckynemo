...
private let chatSendingLogger = Logger(subsystem: "ai.luckynemo", category: "LuckyNemoChatUI")
...
extension LuckyNemoChatViewModel {
    ...
    public func slashCommandMatches(
        query: String,
        filter: LuckyNemoChatCommandFilter) -> [LuckyNemoChatCommandChoice]
    {
    ...
    public func applySlashCommandSelection(_ command: LuckyNemoChatCommandChoice) {
    ...
    private static func isKnownSlashCommandText(
        _ text: String,
        commands: [LuckyNemoChatCommandChoice]) -> Bool
    {
    ...
    private static func commands(
        _ commands: [LuckyNemoChatCommandChoice],
        ...
        filter: LuckyNemoChatCommandFilter) -> [LuckyNemoChatCommandChoice]
    {
        ...
        let effectiveFilter: LuckyNemoChatCommandFilter =
            self.queryTargetsSkills(trimmed) && filter == .all ? .skills : filter
        return commands.enumerated()
            .compactMap { index, command -> (Int, Int, LuckyNemoChatCommandChoice)? in
    ...
    private static func command(
        _ command: LuckyNemoChatCommandChoice,
        isIncludedIn filter: LuckyNemoChatCommandFilter) -> Bool
    {
        switch filter {
        case .all:
            true
    ...
    private struct SendDraft {
        let input: String
        let attachments: [LuckyNemoPendingAttachment]
        let trimmed: String
        let session: SessionSnapshot
        let replyTarget: LuckyNemoChatReplyTarget?
    ...
    private struct LiveSendAttempt {
        ...
        let encodedAttachments: [LuckyNemoChatAttachmentPayload]
        ...
        mustPreserveOutboxOrder: Bool) async -> AttachmentPersistenceDecision
    {
        ...
        guard case let .unavailable(reason) = routeResult,
              reason == LuckyNemoChatTransportUpgradeMessage.routingContract
        else {
    ...
    private static func userContent(
        messageText: String,
        attachments: [LuckyNemoPendingAttachment],
        encodedAttachments: [LuckyNemoChatAttachmentPayload]) -> [LuckyNemoChatMessageContent]
    {
        var content: [LuckyNemoChatMessageContent] = [
    ...
    private func handleLiveSendResponse(
        _ response: LuckyNemoChatSendResponse,
    ...
    private func handleLiveSendFailure(_ error: Error, attempt: LiveSendAttempt) async {
        ...
        if attempt.encodedAttachments.isEmpty, !(error is GatewayResponseError) {
            ...
            let deliveryIsAmbiguous = !(error is LuckyNemoChatTransportSendError)
