<#
.SYNOPSIS
    Build local LuckyNemo Companion Inno installers for quick validation.
...
function Publish-ArchitecturePayload {
    ...
    $trayPublishArgs = @(
        ".\src\LuckyNemo.Tray.WinUI\LuckyNemo.Tray.WinUI.csproj",
        "-c", $Configuration,
        "-r", $RuntimeIdentifier,
        "--self-contained",
        "-o", $publishDir,
...
function Assert-PayloadReady {
    ...
    $publishDir = Join-Path $repoRoot "publish-local-$Architecture"
    $trayExe = Join-Path $publishDir "LuckyNemo.Tray.WinUI.exe"
    ...
    $setupExe = Get-ChildItem -LiteralPath $publishDir -Recurse -File -Filter "LuckyNemo.SetupEngine.UI.exe" -ErrorAction SilentlyContinue | Select-Object -First 1
    if ($setupExe) {
...
if (-not $Version) {
    $versionScript = Join-Path $PSScriptRoot "Get-LuckyNemoVersion.ps1"
...
Get-ChildItem -Path (Join-Path $repoRoot "Output\LuckyNemoCompanion*-Setup-*.exe") |
    Sort-Object Name |
    ForEach-Object {
