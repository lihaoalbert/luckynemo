<#
.SYNOPSIS
    Empirically determines whether LuckyNemoTray MSIX writes user data to real
...
.DESCRIPTION
    LuckyNemoTray declares the runFullTrust restricted capability, which typically
    ...
    -----------------
    PathA-OrphanRisk
        Files land in real %APPDATA%\LuckyNemoTray\ and/or %LOCALAPPDATA%\LuckyNemoTray\.
    ...
    The MSIX is produced by the build-msix CI job.  Download artifact:
        gh run download <run_id> --name luckynemo-msix-win-x64 --dir ./msix-drop/
    Then pass the .msix file path to -MsixPath.

.PARAMETER MsixPath
    Absolute path to the LuckyNemoTray MSIX file (e.g. LuckyNemoTray_1.2.3.0_x64.msix)
...
.EXAMPLE
    ...
    .\validate-msix-storage-paths.ps1 `
        -MsixPath C:\drop\LuckyNemoTray_1.2.3.0_x64.msix `
        -CertPath C:\drop\LuckyNemoTray.cer `
...
.EXAMPLE
    ...
    ---------------
    <EvidenceDir>/
      pre-appdata.txt           — recursive dir listing of %APPDATA%\LuckyNemoTray\ pre-install
      pre-localappdata.txt      — same for %LOCALAPPDATA%\LuckyNemoTray\
      pre-packages.txt          — %LOCALAPPDATA%\Packages\*LuckyNemo* pre-install
      pre-appx.json             — Get-AppxPackage *LuckyNemo* pre-install (JSON)
    ...
    2  Pre-flight blocked: LuckyNemo* process running, or other blocking condition.
...
$PACKAGE_NAME_GLOB  = "*LuckyNemo.Tray*"
...
function Capture-PackagesListing {
    ...
    if (Test-Path -LiteralPath $pkgDir) {
        try {
            $items = Get-ChildItem -LiteralPath $pkgDir -Directory -Filter "*LuckyNemo*" -Force -ErrorAction SilentlyContinue |
                     Select-Object FullName, CreationTimeUtc, LastWriteTimeUtc
            $lines = @("# Captured: $(Get-Date -Format 'o')  Packages filter: *LuckyNemo*")
            foreach ($i in $items) {
...
function Capture-AppxPackage {
    ...
    $dest = Get-EvidencePath $OutFile
    try {
        $pkgs = Get-AppxPackage -Name "*LuckyNemo*" -ErrorAction SilentlyContinue
...
function Capture-AllSnapshots {
    ...
    $appDataDir      = Join-Path $env:APPDATA      "LuckyNemoTray"
    $localAppDataDir = Join-Path $env:LOCALAPPDATA "LuckyNemoTray"
...
function Invoke-Preflight {
    ...
    # 1b. Refuse if any LuckyNemo* process is running
    $running = Get-Process -Name "LuckyNemo*" -ErrorAction SilentlyContinue
    if ($running) {
        $pids = ($running | ForEach-Object { $_.Id }) -join ", "
        Write-Fatal "LuckyNemo* process(es) are running (PIDs: $pids).  Stop them first:"
        foreach ($p in $running) {
            Write-Fatal "  Stop-Process -Id $($p.Id)   # $($p.ProcessName)"
        }
        Add-Step "preflight-process-check" "Failed" "LuckyNemo processes running: PIDs $pids" @{ pids = $pids }
        return $EXIT_PREFLIGHT_BLOCK
    }
    Add-Step "preflight-process-check" "Completed" "No LuckyNemo* processes running."
    ...
    # 1d. Check for pre-existing LuckyNemoTray package
    $existing = Get-AppxPackage -Name "LuckyNemo.Tray" -ErrorAction SilentlyContinue
    if ($existing -and -not $SkipInstall) {
        Add-Step "preflight-existing-package" "Warning" "Pre-existing LuckyNemo.Tray package found: $($existing.PackageFullName).  Will overwrite via Add-AppxPackage." @{
            existingFullName = $existing.PackageFullName
        }
    }
    elseif ($SkipInstall -and -not $existing) {
        Write-Fatal "-SkipInstall set but no LuckyNemo.Tray package is installed.  Nothing to probe."
        Add-Step "preflight-existing-package" "Failed" "No installed package found but -SkipInstall was set."
        return $EXIT_PREFLIGHT_BLOCK
    }
    else {
        Add-Step "preflight-existing-package" "Completed" "No pre-existing LuckyNemo.Tray package found (clean state)."
    }
    ...
    if ($WhatIfPreference) {
        ...
        Write-Host "  3. Write probe files to real %APPDATA%\LuckyNemoTray\ + %LOCALAPPDATA%\LuckyNemoTray\"
...
function Invoke-Install {
    ...
    $pkg = Get-AppxPackage -Name "LuckyNemo.Tray" -ErrorAction SilentlyContinue
    if (-not $pkg) {
        throw "Package LuckyNemo.Tray not found after install step.  Cannot continue."
    }
...
function Invoke-ProbeSetup {
    ...
    if (-not $effectiveAutoSetup) {
        ...
        $instructions = @(
            ...
            "LuckyNemo Setup-Locally flow in the tray UI, then re-run the script to capture",
    ...
    # -AutoSetup path:
    # Write probe marker files to the REAL %APPDATA%\LuckyNemoTray\ and %LOCALAPPDATA%\LuckyNemoTray\
    ...
    $realAppData       = Join-Path $env:APPDATA      "LuckyNemoTray"
    $realLocalAppData  = Join-Path $env:LOCALAPPDATA "LuckyNemoTray"
    ...
    # Wait up to 30 s for a LuckyNemo* process to appear
    Write-StepInfo "Waiting up to 30 s for tray process to start..."
    $deadline     = (Get-Date).AddSeconds(30)
    $trayProcess  = $null
    while ((Get-Date) -lt $deadline) {
        $found = Get-Process -Name "LuckyNemo*" -ErrorAction SilentlyContinue
        if ($found) {
...
# Invoke LuckyNemoTray.exe --uninstall CLI to:
...
function Invoke-CliEngineUninstall {
    ...
    if (-not [string]::IsNullOrEmpty($installLocation)) {
        $candidate = Join-Path $installLocation "LuckyNemo.Tray.WinUI.exe"
    ...
    if ([string]::IsNullOrEmpty($exePath)) {
        Add-Step "cli-engine-uninstall" "Skipped" "LuckyNemo.Tray.WinUI.exe not found in install location '$installLocation'.  cross_check_consistent will be false." @{
...
function Invoke-Verdict {
    ...
    elseif ($newInPackages.Count -gt 0 -and -not $writesToRealAppData -and -not $writesToRealLocalAppData) {
        $verdict   = "PathB-CleanRemove"
        $reasoning = "New package directories found under %LOCALAPPDATA%\Packages (filter *LuckyNemo*) and no real APPDATA growth detected.  Consistent with virtualized storage."
    }
...
function Invoke-Teardown {
    ...
    if (-not $Pkg) {
        Add-Step "teardown-remove-appx" "Skipped" "No LuckyNemo.Tray package found to remove."
    }
    else {
