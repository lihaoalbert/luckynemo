<#
.SYNOPSIS
    Removes the LuckyNemo local WSL gateway during app uninstall.
...
.DESCRIPTION
    ...
    launching LuckyNemo binaries from the install directory, so the app payload is
...
param(
    ...
    [string]$DataDirectoryName = 'LuckyNemoTray',
    [string]$AutoStartName = 'LuckyNemoTray',
    [string]$StartupTaskName = 'LuckyNemo Companion',
...
function Resolve-AppDataDir {
        if ($env:LUCKYNEMO_TRAY_DATA_DIR) {
            return $env:LUCKYNEMO_TRAY_DATA_DIR
        }
    ...
    function Resolve-LocalDataDir {
        if ($env:LUCKYNEMO_TRAY_LOCALAPPDATA_DIR) {
            return Join-Path $env:LUCKYNEMO_TRAY_LOCALAPPDATA_DIR $DataDirectoryName
        }

        if ($env:LUCKYNEMO_TRAY_LOCAL_DATA_DIR) {
            return $env:LUCKYNEMO_TRAY_LOCAL_DATA_DIR
        }
    ...
    function Complete-GatewayCleanup {
        ...
        Write-Host "LuckyNemo local WSL gateway removed successfully."
        exit 0
}
