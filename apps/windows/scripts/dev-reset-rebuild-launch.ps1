...
.DESCRIPTION
    Consolidates the full dev-reset cycle used during LuckyNemo tray development.
...
.PARAMETER WipeWslDistro
    Also unregister the LuckyNemoGateway WSL distro (wsl --unregister).
    Default: off (preserve the distro).

.PARAMETER CaptureDir
    If set, exports LUCKYNEMO_VISUAL_TEST=1 and LUCKYNEMO_VISUAL_TEST_DIR=<path>
...
.EXAMPLE
    ...
    Full clean slate: also unregister the LuckyNemoGateway WSL distro.
...
.EXAMPLE
    ...
    Reset + build + launch with LUCKYNEMO_VISUAL_TEST capture enabled.
#>
...
$TrayProject     = Join-Path $WorktreePath "src\LuckyNemo.Tray.WinUI\LuckyNemo.Tray.WinUI.csproj"
$AppDataDir      = Join-Path $env:APPDATA "LuckyNemoTray"
$LocalAppDataDir = Join-Path $env:LOCALAPPDATA "LuckyNemoTray"
$timestamp       = (Get-Date).ToString("yyyy-MM-ddTHH-mm-ss")
$BackupRoot      = Join-Path $env:TEMP "luckynemo-test-backup-$timestamp"
...
function Get-LuckyNemoProcesses {
    @(Get-Process -ErrorAction SilentlyContinue | Where-Object { $_.ProcessName -like "LuckyNemo*" })
}
...
Write-Host ""
Write-Host "============================================================"
Write-Host "     LuckyNemo Dev Loop -- Reset / Rebuild / Launch"
Write-Host "============================================================"
...
# =============================================================================
# STEP 1 -- Kill LuckyNemo* processes (by PID; name-based kills are forbidden)
# =============================================================================

Write-Host "STEP 1: Kill LuckyNemo* processes"
$procs = @(Get-LuckyNemoProcesses)

if ($procs.Count -eq 0) {
    Write-Skip "No LuckyNemo* processes running"
}
else {
    foreach ($p in $procs) {
...
Invoke-StateDirReset -Path $AppDataDir      -Label "AppData_LuckyNemoTray"
Invoke-StateDirReset -Path $LocalAppDataDir -Label "LocalAppData_LuckyNemoTray"
...
if ($DontLaunch) {
    ...
    if ($PSCmdlet.ShouldProcess($TrayProject, "dotnet run -p:Platform=x64")) {
        if ($CaptureDir) {
            ...
            $env:LUCKYNEMO_VISUAL_TEST     = "1"
            $env:LUCKYNEMO_VISUAL_TEST_DIR = $captureAbs
            Write-Verbose "Set LUCKYNEMO_VISUAL_TEST=1  LUCKYNEMO_VISUAL_TEST_DIR=$captureAbs"
        }
        ...
        if ($CaptureDir) {
            Write-Skip "WhatIf: would also set LUCKYNEMO_VISUAL_TEST=1 and LUCKYNEMO_VISUAL_TEST_DIR=$CaptureDir"
        }
    }
}
