...
if ($shouldProbeNativeLoad -and
    -not ([System.Management.Automation.PSTypeName]"LuckyNemoNativeDependencyProbe").Type) {
    Add-Type @"
using System;
using System.Runtime.InteropServices;

public static class LuckyNemoNativeDependencyProbe {
...
function Add-NativeLoadProbeErrors {
    ...
    [LuckyNemoNativeDependencyProbe]::SetDllDirectory($File.DirectoryName) | Out-Null
    Push-Location $File.DirectoryName
    try {
        $handle = [LuckyNemoNativeDependencyProbe]::LoadLibrary($File.Name)
        $lastError = [Runtime.InteropServices.Marshal]::GetLastWin32Error()
    } finally {
        Pop-Location
        [LuckyNemoNativeDependencyProbe]::SetDllDirectory($null) | Out-Null
    }
    ...
    [LuckyNemoNativeDependencyProbe]::FreeLibrary($handle) | Out-Null
}
...
'@
    ...
    $probeDir = Join-Path ([System.IO.Path]::GetTempPath()) ("luckynemo-tts-native-probe-{0}" -f [Guid]::NewGuid().ToString("N"))
    try {
        New-Item -ItemType Directory -Path $probeDir | Out-Null
        $projectPath = Join-Path $probeDir "LuckyNemo.TtsNativeProbe.csproj"
