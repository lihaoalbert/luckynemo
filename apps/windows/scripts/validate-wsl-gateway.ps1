<#
.SYNOPSIS
    Validate the LuckyNemo WSL gateway local-setup product code path end-to-end.
...
.DESCRIPTION
    ...
    so the *product* code path that runs
        ...
        wsl --install Ubuntu-24.04 --name LuckyNemoGateway --location <path> --no-launch --version 2
...
.PARAMETER Scenario
    ...
    UpstreamInstall - Build/test, drive tray onboarding to install LuckyNemoGateway,
    ...
    FreshMachine    - Like UpstreamInstall, but unregisters any existing
                      LuckyNemoGateway distro first (simulates a clean machine).
...
param(
    ...
    [string]$DistroName = "LuckyNemoGateway",
...
$trayProject = Join-Path $repoRoot "src\LuckyNemo.Tray.WinUI\LuckyNemo.Tray.WinUI.csproj"
...
$trayExe = Join-Path $repoRoot "src\LuckyNemo.Tray.WinUI\bin\Debug\$trayTfm\$runtimeIdentifier\LuckyNemo.Tray.WinUI.exe"
$cliProject = Join-Path $repoRoot "src\LuckyNemo.Cli\LuckyNemo.Cli.csproj"
...
$setupStatePath = Join-Path $validationLocalAppDataRoot "LuckyNemoTray\setup-state.json"
...
function Test-IsLuckyNemoOwnedDistroName {
    param([string]$Name)
    return $Name -eq "LuckyNemoGateway"
}
...
function Assert-DestructiveSafety {
    ...
    if ($Scenario -in @("FreshMachine", "Recreate") -and -not (Test-IsLuckyNemoOwnedDistroName -Name $DistroName)) {
        throw "Refusing destructive action for non-LuckyNemo distro '$DistroName'. Distro name must be exactly 'LuckyNemoGateway'."
    }
}
...
function Write-Summary {
    ...
    $lines = @(
        "# LuckyNemo WSL gateway validation",
        "",
        "- Scenario: $Scenario",
...
function Invoke-RepositoryValidation {
    ...
    Invoke-LoggedProcess "test-shared" "dotnet" @("test", ".\tests\LuckyNemo.Shared.Tests\LuckyNemo.Shared.Tests.csproj", "--no-restore")
    Invoke-LoggedProcess "test-tray"   "dotnet" @("test", ".\tests\LuckyNemo.Tray.Tests\LuckyNemo.Tray.Tests.csproj", "--no-restore")
}
...
function Invoke-RelayPrototypeProbe {
    $probeUri = if (-not [string]::IsNullOrWhiteSpace($RelayProbeUri)) { $RelayProbeUri } else { [Environment]::GetEnvironmentVariable("LUCKYNEMO_RELAY_PROBE_URI", "Process") }
    if ([string]::IsNullOrWhiteSpace($probeUri)) {
        $msg = "No relay probe endpoint was supplied. Set -RelayProbeUri or LUCKYNEMO_RELAY_PROBE_URI."
...
function Save-DiagnosticsSnapshot {
    ...
    $identityPath = Join-Path $validationAppDataRoot "LuckyNemoTray\device-key-ed25519.json"
...
function Get-ValidationAppEnvironment {
    return @{
        LUCKYNEMO_TRAY_DATA_DIR = $validationAppDataRoot
        LUCKYNEMO_TRAY_APPDATA_DIR = $validationAppDataRoot
        LUCKYNEMO_TRAY_LOCALAPPDATA_DIR = $validationLocalAppDataRoot
    }
}
...
function Convert-SetupPhase {
    ...
    if ($v -match '^\d+$') {
        ...
        $names = @(
            ...
            "InstallLuckyNemoCli", "PrepareGatewayConfig", "InstallGatewayService",
...
function Stop-ExistingTrayProcesses {
    ...
    $procs = Get-Process -Name "LuckyNemo.Tray.WinUI" -ErrorAction SilentlyContinue |
        Where-Object {
...
function Start-TrayForLocalSetup {
    ...
    $env = @{
        LUCKYNEMO_SKIP_UPDATE_CHECK = "1"
        LUCKYNEMO_FORCE_ONBOARDING = "1"
        LUCKYNEMO_WSL_DISTRO_NAME = $DistroName
        # TODO: LUCKYNEMO_WSL_INSTALL_LOCATION was removed (commit: remove LUCKYNEMO_WSL_INSTALL_LOCATION env-var binding).
        ...
        LUCKYNEMO_WSL_ALLOW_EXISTING_DISTRO = if ($Scenario -eq "UpstreamInstall") { "1" } else { "0" }
        LUCKYNEMO_TRAY_DATA_DIR = $validationAppDataRoot
        LUCKYNEMO_TRAY_APPDATA_DIR = $validationAppDataRoot
        LUCKYNEMO_TRAY_LOCALAPPDATA_DIR = $validationLocalAppDataRoot
        LUCKYNEMO_VISUAL_TEST = "1"
        LUCKYNEMO_VISUAL_TEST_DIR = $screenshotsRoot
    }
...
function Save-RedactedDeviceIdentityShape {
    $idp = Join-Path $validationAppDataRoot "LuckyNemoTray\device-key-ed25519.json"
...
function Invoke-BootstrapHandoffProbe {
    ...
    $args = @("-d", $DistroName, "--", "/opt/luckynemo/bin/luckynemo", "qr", "--json", "--url", $GatewayUrl)
    ...
    if ($exitCode -ne 0) {
        ...
        throw "BootstrapTokenCommandFailed: luckynemo qr --json failed. See $stdout and $stderr."
    }
    ...
    try {
        ...
        throw "BootstrapTokenJsonInvalid: luckynemo qr --json did not produce valid JSON: $($_.Exception.Message)"
    }
    ...
    if ($RequireRealGatewayBootstrap -and -not $hasSetupCode) {
        throw "RealGatewayBootstrapRequired: expected upstream setupCode bootstrap handoff, but luckynemo qr --json returned $shape."
    }
}
...
function Invoke-SmokeChecks {
    ...
    Invoke-LoggedProcess "wsl-luckynemo-version" "wsl.exe" @(
        "-d", $DistroName, "-u", "luckynemo", "--", "/opt/luckynemo/bin/luckynemo", "--version")
    Invoke-LoggedProcess "wsl-luckynemo-config-validate" "wsl.exe" @(
        "-d", $DistroName, "-u", "luckynemo", "--", "/opt/luckynemo/bin/luckynemo", "config", "validate")
    ...
    try {
        ...
        $isDev = $version -like "*-dev*" -or $displayName -like "Dev LuckyNemo*"
    ...
    $args = @(
        ...
        "--message", "luckynemo validation ping",
        "--connect-timeout-ms", "15000"
    )
    if ($RequireOperatorPairing) { $args += "--require-stored-device-token" }
    Invoke-LoggedProcess "luckynemo-cli-probe" "dotnet" $args -Environment (Get-ValidationAppEnvironment) -SensitiveOutput
}
