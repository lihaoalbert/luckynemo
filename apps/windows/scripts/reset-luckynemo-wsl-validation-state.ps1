# reset-luckynemo-wsl-validation-state.ps1
#
# Exact-target destructive cleanup for LuckyNemo-owned WSL validation state.
#
...
$script:LuckyNemoDistroName = "LuckyNemoGateway"
...
$result = [ordered]@{
    script = "reset-luckynemo-wsl-validation-state"
    startedAt = $startedAt.ToString("o")
    finishedAt = $null
    outputDir = $OutputDir
    backupRoot = $BackupRoot
    distroName = $script:LuckyNemoDistroName
...
function Assert-DestructiveTargetIsAllowed {
    # Hard-lock: this script will only ever touch the production LuckyNemoGateway distro.
    # No override flag exists. If $script:LuckyNemoDistroName is ever something else,
    # the script must refuse to run regardless of dry-run mode.
    if ($script:LuckyNemoDistroName -ne "LuckyNemoGateway") {
        throw "Refusing to run: distro name is locked to 'LuckyNemoGateway' but resolved to '$($script:LuckyNemoDistroName)'."
    }
}
...
function Get-LuckyNemoProcesses {
    return @(Get-Process | Where-Object { $_.ProcessName -like "LuckyNemo*" })
}
...
function Add-TargetSummary {
    ...
    $script:result.targets = [ordered]@{
        ...
        distroExists = ($Distros -contains $script:LuckyNemoDistroName)
        distroName = $script:LuckyNemoDistroName
    ...
    Add-ResetStep "target-summary" "Completed" "Captured LuckyNemo-owned reset targets." @{
...
function Assert-CleanPostCondition {
    ...
    $remainingProcesses = @(Get-LuckyNemoProcesses)
    if (-not $KeepRunningProcesses -and $remainingProcesses.Count -gt 0) {
        throw "LuckyNemo processes are still running after reset: $(@($remainingProcesses | ForEach-Object { $_.Id }) -join ', ')"
    }

    $remainingDistros = @(Get-WslDistros)
    if ($remainingDistros -contains $script:LuckyNemoDistroName) {
        throw "WSL distro '$($script:LuckyNemoDistroName)' is still registered after reset."
    }
    ...
    Add-ResetStep "postconditions" "Passed" "LuckyNemo-owned state reset postconditions passed." @{
...
try {
    ...
    $appData = Join-Path $AppDataRoot "LuckyNemoTray"
    $localAppData = Join-Path $LocalAppDataRoot "LuckyNemoTray"
    $processes = @(Get-LuckyNemoProcesses)
    ...
    if ($result.dryRun) {
        Add-ResetStep "mode" "DryRun" "No state will be changed. Pass -ConfirmDestructiveClean to reset LuckyNemo-owned state."
        Write-Host "DRY-RUN: pass -ConfirmDestructiveClean to actually reset LuckyNemo-owned state."
    }
    else {
        Add-ResetStep "mode" "Confirmed" "LuckyNemo-owned state reset is enabled for this run."
        Write-Host "Backups will be written under: $BackupRoot"
    }

    if ($processes.Count -eq 0) {
        Add-ResetStep "stop-luckynemo-processes" "Skipped" "No LuckyNemo processes are running."
    }
    elseif ($KeepRunningProcesses) {
        Add-ResetStep "stop-luckynemo-processes" "Skipped" "Keeping running LuckyNemo processes because -KeepRunningProcesses was set." @{
            pids = @($processes | ForEach-Object { $_.Id })
        }
    }
    elseif ($result.dryRun) {
        Add-ResetStep "stop-luckynemo-processes" "DryRun" "Would stop running LuckyNemo processes by PID." @{
            pids = @($processes | ForEach-Object { $_.Id })
        }
    }
    else {
        ...
        Add-ResetStep "stop-luckynemo-processes" "Completed" "Stopped running LuckyNemo processes by PID." @{
            pids = @($processes | ForEach-Object { $_.Id })
        }
    }

    $hasGatewayDistro = $distros -contains $script:LuckyNemoDistroName
    ...
    if (-not $hasGatewayDistro) {
        Add-ResetStep "unregister-$($script:LuckyNemoDistroName)" "Skipped" "WSL distro '$($script:LuckyNemoDistroName)' is not registered."
    }
    elseif ($result.dryRun) {
        Add-ResetStep "unregister-$($script:LuckyNemoDistroName)" "DryRun" "Would terminate and unregister only the '$($script:LuckyNemoDistroName)' WSL distro." @{ distroName = $script:LuckyNemoDistroName }
    }
    else {
        # Exact-target only: --terminate <name>, never --shutdown.
        Invoke-CapturedCommand "wsl-terminate-$($script:LuckyNemoDistroName)" "wsl.exe" @("--terminate", $script:LuckyNemoDistroName) -IgnoreExitCode
        Invoke-CapturedCommand "wsl-unregister-$($script:LuckyNemoDistroName)" "wsl.exe" @("--unregister", $script:LuckyNemoDistroName)
    }
