...
.DESCRIPTION
    ...
    Source of truth for steps:
        src/LuckyNemo.Tray.WinUI/Services/LocalGatewaySetup/LocalGatewayUninstall.cs

    Shared helpers dot-sourced from:
        scripts/_uninstall-helpers.ps1
        (Test-IsLuckyNemoOwnedDistroName, Invoke-WslCommand, Stop-LuckyNemoProcessByPid,
...
.PARAMETER DistroName
    WSL distro name to target.  Default: LuckyNemoGateway.
    Must be exactly "LuckyNemoGateway" (no prefix variants).
...
param(
    ...
    # LuckyNemoTray.exe is not available (e.g. standalone script on bare machine).
    [switch]$NoCli,

    # Path to LuckyNemoTray.exe.  Auto-detected from common locations when empty.
    [string]$ExePath = "",
...
if ($Help -or [string]::IsNullOrEmpty($Mode)) {
    Write-Host @'
validate-wsl-gateway-uninstall.ps1  —  LuckyNemo WSL gateway uninstall validator
...
OPTIONS:
  ...
  -DistroName <name>      Default: LuckyNemoGateway (must be exact match)
  ...
  -ExePath <path>         Explicit path to LuckyNemoTray.exe (auto-detected when empty).
  -Help                   Show this help.
...
if (-not (Test-IsLuckyNemoOwnedDistroName -Name $DistroName)) {
    Write-Host "ERROR: Refusing to operate on distro '$DistroName': name must be exactly 'LuckyNemoGateway'. Pass -DistroName LuckyNemoGateway." -ForegroundColor Red
    exit 2
}
...
$setupStatePath  = Join-Path $localAppData "LuckyNemoTray\setup-state.json"
$deviceKeyPath   = Join-Path $appData      "LuckyNemoTray\device-key-ed25519.json"
$mcpTokenPath    = Join-Path $appData      "LuckyNemoTray\mcp-token.txt"
$settingsPath    = Join-Path $appData      "LuckyNemoTray\settings.json"
$logsDir         = Join-Path $localAppData "LuckyNemoTray\Logs"
$execPolicyPath  = Join-Path $localAppData "LuckyNemoTray\exec-policy.json"
$vhdDirPath      = Join-Path $localAppData "LuckyNemoTray\wsl\$DistroName"
$wslParentDirPath = Join-Path $localAppData "LuckyNemoTray\wsl"

$autoStartRegKey  = "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Run"
$autoStartAppName = "LuckyNemoTray"
...
function Get-LuckyNemoProcessSnapshot {
    try {
        $procs = Get-Process -ErrorAction SilentlyContinue |
            Where-Object { $_.Name -like 'LuckyNemo*' }
...
function Get-StateSnapshot {
    ...
    $luckyNemoDistros = @($allDistros | Where-Object { $_ -like '*LuckyNemo*' })
    ...
    $snapshot = [ordered]@{
        ...
        wsl_distros_luckynemo   = $luckyNemoDistros
    ...
    $snapshot.processes_luckynemo = Get-LuckyNemoProcessSnapshot
...
function Get-TrayExePath {
    ...
    $candidate = Join-Path $PSScriptRoot 'LuckyNemo.Tray.WinUI.exe'
    if (Test-Path -LiteralPath $candidate) { return $candidate }

    # 3. Parent directory (repo layout: scripts\ sibling of publish\)
    $candidate = Join-Path (Split-Path $PSScriptRoot -Parent) 'publish\LuckyNemo.Tray.WinUI.exe'
    if (Test-Path -LiteralPath $candidate) { return $candidate }
...
function Get-Postconditions {
    ...
    # No LuckyNemo keepalive processes running.
...
function Invoke-UninstallSteps {
    ...
    try {
        ...
        if ($kPids.Count -eq 0) {
            ...
            foreach ($pid in $kPids) {
                try {
                    Stop-LuckyNemoProcessByPid -ProcessId $pid -Force
                    $stopped++
                } catch {
    ...
    # Safety guard: only "LuckyNemoGateway" may be unregistered (mirrors C# AllowedDistroName).
    # wsl --terminate first to cleanly stop the distro before unregistering.
    # ------------------------------------------------------------------ Step 5
    try {
        if ($DistroName -ne 'LuckyNemoGateway') {
            $guard = "Refused to unregister '$DistroName': only 'LuckyNemoGateway' is allowed."
...
function Write-SummaryMarkdown {
    ...
    $lines = @(
        "# LuckyNemo WSL Gateway Uninstall Validation",
        "",
        "| Field       | Value |",
        "|-------------|-------|",
        "| Mode        | $Mode |",
        ...
        # ===================================================================
            ...
            if ($useCliPath) {
                ...
                if ($NoCli.IsPresent) {
                    ...
                    Write-Host "LuckyNemoTray.exe not found; falling back to inline PS replication." `
                        -ForegroundColor Yellow
                }
