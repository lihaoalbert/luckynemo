...
$winUiRoot = Join-Path $repoRoot 'src\LuckyNemo.Tray.WinUI'
...
if (-not $SkipDotNetTests) {
    ...
    dotnet test (Join-Path $repoRoot 'tests\LuckyNemo.Tray.Tests\LuckyNemo.Tray.Tests.csproj') --filter 'FullyQualifiedName~LocalizationValidationTests|FullyQualifiedName~CapabilitiesPageLocalizationCoverageTests' --no-restore
