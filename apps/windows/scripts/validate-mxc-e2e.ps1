...
try {
    Set-ProcessEnv -Name "LUCKYNEMO_REPO_ROOT" -Value $repoRoot
    Set-ProcessEnv -Name "LUCKYNEMO_RUN_E2E" -Value "1"
    Set-ProcessEnv -Name "LUCKYNEMO_RUN_MXC_E2E" -Value "1"

    Write-Host "LuckyNemo MXC validation"
    ...
    if (-not $NoBuild) {
        ...
        Invoke-Checked -Name "Build tray app for $RuntimeIdentifier" -Command {
            & dotnet build ".\src\LuckyNemo.Tray.WinUI\LuckyNemo.Tray.WinUI.csproj" -c $Configuration -r $RuntimeIdentifier
        }

        Invoke-Checked -Name "Build E2E tests for $RuntimeIdentifier" -Command {
            & dotnet build ".\tests\LuckyNemo.E2ETests\LuckyNemo.E2ETests.csproj" -c $Configuration -r $RuntimeIdentifier
        }
    }

    $e2eTrx = Join-Path $ResultsDirectory "LuckyNemo.E2ETests.Mxc.trx"
    $e2eConsoleLog = Join-Path $ResultsDirectory "LuckyNemo.E2ETests.Mxc.console.log"
    Invoke-Checked -Name "Run Gateway MXC E2E proofs" -Command {
        & dotnet test ".\tests\LuckyNemo.E2ETests\LuckyNemo.E2ETests.csproj" `
            ...
            --logger "trx;LogFileName=LuckyNemo.E2ETests.Mxc.trx" `
            --logger "console;verbosity=detailed" `
            --filter "FullyQualifiedName~LuckyNemo.E2ETests.Setup.MxcSetupAndConnectTests" `
