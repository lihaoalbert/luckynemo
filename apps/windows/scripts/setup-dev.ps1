<#
.SYNOPSIS
    Prepares a Windows checkout for LuckyNemo developer and agent work.
...
if (-not (Test-WindowsHost)) {
    throw "LuckyNemo Windows development requires Windows."
}

Write-Header "LuckyNemo developer setup"
if ($CheckOnly) {
...
if ($RunValidation) {
    ...
    $env:LUCKYNEMO_REPO_ROOT = $repoRoot

    & "$repoRoot\build.ps1"
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

    dotnet build "$repoRoot\tests\LuckyNemo.Shared.Tests\LuckyNemo.Shared.Tests.csproj"
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

    dotnet build "$repoRoot\tests\LuckyNemo.Tray.Tests\LuckyNemo.Tray.Tests.csproj"
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

    dotnet test "$repoRoot\tests\LuckyNemo.Shared.Tests\LuckyNemo.Shared.Tests.csproj" --no-restore
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

    dotnet test "$repoRoot\tests\LuckyNemo.Tray.Tests\LuckyNemo.Tray.Tests.csproj" --no-restore
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
}

Write-Success "LuckyNemo developer setup is ready."
