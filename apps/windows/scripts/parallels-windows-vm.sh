...
# Monorepo: the gateway repo is the checkout root two levels above apps/windows.
LUCKYNEMO_REPO="${LUCKYNEMO_REPO:-$WINDOWS_ROOT/../..}"
BASE_CONTROLLER="$LUCKYNEMO_REPO/scripts/e2e/parallels-windows-prepare.sh"

COMMAND="${1:-help}"
[[ $# -gt 0 ]] && shift
VM="Windows 11"
TODAY="$(date +%F)"
APP_SNAPSHOT="pre-luckynemo-windows-app-e2e-${TODAY}"
APP_SNAPSHOT_EXPLICIT=0
SELECTED_SNAPSHOT=""
GUEST_CHECKOUT=""
# Monorepo: the Windows app sources live under apps/windows in the main repo.
REPO_URL="https://github.com/lihaoalbert/LuckyNemo.git"
REPO_REF=""
SKIP_RESTORE=0
...
Commands:
  inventory   Delegate base VM and snapshot inventory to the LuckyNemo controller.
  prepare     Prepare the LuckyNemo base, then add Windows app prerequisites and snapshot them.
  verify      Verify both the LuckyNemo base and Windows app development prerequisites.
...
The general Parallels, WSL, Git/Node, snapshot, and transport implementation lives in the
monorepo root (two levels above apps/windows). Set LUCKYNEMO_REPO to override.
EOF
}
...
[[ -f "$BASE_CONTROLLER" ]] || {
  printf 'error: LuckyNemo Parallels controller not found: %s\n' "$BASE_CONTROLLER" >&2
  printf 'expected the monorepo root two levels above apps/windows; set LUCKYNEMO_REPO to override\n' >&2
  exit 1
}
...
set --
LUCKYNEMO_PARALLELS_WINDOWS_LIBRARY_ONLY=1 source "$BASE_CONTROLLER"
[[ "${LUCKYNEMO_PARALLELS_WINDOWS_API:-0}" == "1" ]] || {
  printf 'error: incompatible LuckyNemo Parallels Windows controller API\n' >&2
  exit 1
}
COMMAND="$APP_COMMAND"
VM_NAME="$VM"
BASELINE_SNAPSHOT="pre-luckynemo-native-e2e-${TODAY}"
...
matches = [
    ...
    if item.get("name", "").startswith("pre-luckynemo-windows-app-e2e-")
]
if not matches:
...
set_app_paths() {
  ...
  if [[ -z "$GUEST_CHECKOUT" ]]; then
    GUEST_CHECKOUT="${GUEST_PROFILE}/github/LuckyNemo/apps/windows"
  fi
...
run_app_tests() {
  ...
  helper="${GUEST_PROFILE}/AppData/Local/Temp/luckynemo-windows-validation-${run_id}.ps1"
  log_path="${GUEST_PROFILE}/AppData/Local/Temp/luckynemo-windows-validation-${run_id}.log"
  done_path="${GUEST_PROFILE}/AppData/Local/Temp/luckynemo-windows-validation-${run_id}.done"
  pid_path="${GUEST_PROFILE}/AppData/Local/Temp/luckynemo-windows-validation-${run_id}.pid"
