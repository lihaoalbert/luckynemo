...
.DESCRIPTION
    ...
    3. [VERIFY]   Assert that {app}\LuckyNemo.Tray.WinUI.exe and
    ...
    When Inno runs with /LOG=<path> it writes a plain-text log with entries like:
        ...
        ...
        Dir: C:\...\LuckyNemoTray (directory): deleted.
...
.PARAMETER InstallerPath
    ...
    Output\LuckyNemoTray-Setup-x64.exe).  If still not found the test exits
    with SKIP.
...
.EXAMPLE
    ...
    .\Test-InnoUninstallOrdering.ps1 -InstallerPath C:\build\LuckyNemoTray-Setup-x64.exe

.EXAMPLE
    # Keep temp dir for debugging:
    .\Test-InnoUninstallOrdering.ps1 -InstallerPath C:\build\LuckyNemoTray-Setup-x64.exe -KeepTempDir
...
function Find-Installer {
    ...
    $candidates = @(
        (Join-Path $repoRoot "Output\LuckyNemoTray-Setup-x64.exe"),
        (Join-Path $repoRoot "installer-output\LuckyNemoTray-Setup-x64.exe"),
        (Join-Path $repoRoot "publish-x64\installer\LuckyNemoTray-Setup-x64.exe")
    )
    ...
    foreach ($searchRoot in @((Join-Path $repoRoot "Output"), (Join-Path $repoRoot "installer-output"))) {
        if (Test-Path -LiteralPath $searchRoot) {
            $found = Get-ChildItem -LiteralPath $searchRoot -Recurse -Filter "LuckyNemoTray-Setup*.exe" -ErrorAction SilentlyContinue |
    ...
    for ($i = 0; $i -lt $lines.Count; $i++) {
        ...
        if ($dirDelIdx -eq -1) {
            if (($line -match '(?i)deleting directory') -or
                ...
                if ($line -match 'LuckyNemoTray' -or $line -match [regex]::Escape($script:appDirPattern)) {
                    $dirDelIdx = $i
                }
            }
        }
...
function Invoke-WslCleanupCheck {
    ...
    $luckyNemoDistros = @($wslLines | Where-Object { $_ -like '*LuckyNemoGateway*' })
    if ($luckyNemoDistros.Count -gt 0) {
        Add-Step "wsl-cleanup-check" "Warning" "$($luckyNemoDistros.Count) LuckyNemoGateway distro(s) still registered after uninstall.  Unexpected for a fresh-install test." @{
            distros = $luckyNemoDistros
        }
    }
    else {
        Add-Step "wsl-cleanup-check" "Passed" "No LuckyNemoGateway WSL distros registered (expected for a fresh-install test)."
    }
}
...
try {
    ...
    if ([string]::IsNullOrEmpty($foundInstaller)) {
        Add-Step "locate-installer" "Skipped" "No Inno installer found.  Pass -InstallerPath to specify one explicitly." @{
            searchedPaths = @("Output\LuckyNemoTray-Setup-x64.exe",
                              "installer-output\LuckyNemoTray-Setup-x64.exe",
                              "publish-x64\installer\LuckyNemoTray-Setup-x64.exe")
        }
    ...
    # =====================================================================
    $exePath          = Join-Path $tempInstallPath "LuckyNemo.Tray.WinUI.exe"
    ...
    if (-not $exeExists -or -not $hookScriptExists) {
        Add-Step "verify-post-install-files" "Failed" "Expected files missing post-install." @{
            "LuckyNemo.Tray.WinUI.exe exists"    = $exeExists
