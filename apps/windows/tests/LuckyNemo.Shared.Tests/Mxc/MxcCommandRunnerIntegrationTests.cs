...
/// LUCKYNEMO_RUN_INTEGRATION=1 so it doesn't run by default on CI; matches the
...
public class MxcCommandRunnerIntegrationTests
{
    private static MxcCommandRunner? TryBuildRunner(bool sandboxEnabled = true, Action<SettingsData>? configure = null)
    {
        ...
        return new MxcCommandRunner(
            ...
            hostFallback,
            () => settings,
            () => Path.Combine(Path.GetTempPath(), "luckynemo-mxc-smoke-test-settings"),
    ...
    {
        var dir = Directory.CreateDirectory(Path.Combine(Path.GetTempPath(), "luckynemo-mxc-grant-smoke-" + Guid.NewGuid().ToString("N"))).FullName;
    ...
    {
        var root = Directory.CreateDirectory(Path.Combine(AppContext.BaseDirectory, "luckynemo-mxc-fs-matrix-" + Guid.NewGuid().ToString("N"))).FullName;
    ...
    public async Task SystemRun_CustomEnv_DeniesBeforeMxcUnavailableHostFallback()
    {
        ...
        var result = await runner.RunAsync(new CommandRequest
        {
            Command = "echo %LUCKYNEMO_MXC_ENV_FALLBACK_MARKER%",
            Shell = "cmd",
            Env = new Dictionary<string, string>
            {
                ["LUCKYNEMO_MXC_ENV_FALLBACK_MARKER"] = "LUCKYNEMO_ENV_FALLBACK_SHOULD_NOT_RUN",
            },
            TimeoutMs = 30_000,
        });
        ...
        Console.WriteLine(
            ...
            $"exitCode={result.ExitCode}; " +
            $"fallbackMarkerSeen={result.Stdout.Contains("LUCKYNEMO_ENV_FALLBACK_SHOULD_NOT_RUN", StringComparison.Ordinal)}; " +
        ...
        Assert.DoesNotContain("LUCKYNEMO_ENV_FALLBACK_SHOULD_NOT_RUN", result.Stdout);
