...
public class MxcPolicyBuilderTests
{
    ...
    {
        var settings = new SettingsData();
        var missingSettingsDir = Path.Combine(Path.GetTempPath(), "luckynemo-missing-settings-" + Guid.NewGuid().ToString("N"));
    ...
    {
        var parent = Directory.CreateDirectory(Path.Combine(Path.GetTempPath(), "luckynemo-settings-parent-" + Guid.NewGuid().ToString("N"))).FullName;
        var missingSettingsDir = Path.Combine(parent, "LuckyNemoTray");
        var settings = new SettingsData
        {
            SandboxCustomFolders = new()
            {
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), "luckynemo-denied-" + Guid.NewGuid().ToString("N"));
