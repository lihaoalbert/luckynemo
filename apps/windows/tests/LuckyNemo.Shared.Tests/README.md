...
Unit test suite for the LuckyNemo.Shared library.

## Overview

This test project provides comprehensive coverage of the LuckyNemo.Shared library, focusing on:
...
$env:LUCKYNEMO_RUN_INTEGRATION=1
dotnet test
...
- ✅ Notify defaults title to "LuckyNemo"
...
### LuckyNemoGatewayClientTests.cs (20 tests)
...
Enable locally by setting `LUCKYNEMO_RUN_INTEGRATION=1` before running `dotnet test`.
...
When adding new functionality to `LuckyNemo.Shared`:
