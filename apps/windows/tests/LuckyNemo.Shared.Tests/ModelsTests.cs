...
public class CommandCenterModelTests
{
    ...
    public void NodeCapabilityHealthInfo_PendingReapprovalKeepsDeclarationsSeparateAndActionable()
    {
        ...
        Assert.Contains(info.Warnings, warning =>
            ...
            warning.CopyText == "luckynemo nodes approve request-123" &&
    ...
    public void NodeCapabilityHealthInfo_PendingApprovalWithUnsafeRequestIdFallsBackToPendingList()
    {
        ...
        Assert.Contains(info.Warnings, warning =>
            ...
            warning.CopyText == "luckynemo nodes pending" &&
    ...
    public void NodeCapabilityHealthInfo_SeparatesSafeAndDangerousPolicyBlocks()
    {
        ...
        Assert.Contains(info.Warnings, w =>
            ...
            !w.CopyText.Contains("luckynemo config set", StringComparison.OrdinalIgnoreCase));
        Assert.Contains(info.Warnings, w =>
    ...
    public void NodeCapabilityHealthInfo_WarnsSpecificallyForBlockedBrowserProxy()
    {
        ...
        Assert.Contains(info.Warnings, w =>
            ...
            w.CopyText == "luckynemo config set gateway.nodes.allowCommands '[\"browser.proxy\"]'");
    ...
    public void BuildAllowCommandsRepairCommand_IsStableAndDeduplicated()
    {
        ...
        Assert.Equal("luckynemo config set gateway.nodes.allowCommands '[\"canvas.present\",\"screen.snapshot\"]'", command);
    }

    [Theory]
    [InlineData("request-123", "luckynemo nodes approve request-123")]
    [InlineData(" request:123 ", "luckynemo nodes approve request:123")]
    [InlineData(null, "luckynemo nodes pending")]
    [InlineData("", "luckynemo nodes pending")]
    [InlineData("request-1;whoami", "luckynemo nodes pending")]
    [InlineData("<requestId>", "luckynemo nodes pending")]
    ...
    [Theory]
    [InlineData("request-123", "luckynemo devices approve request-123")]
    [InlineData(" request:123 ", "luckynemo devices approve request:123")]
    [InlineData(null, "luckynemo devices list")]
    [InlineData("", "luckynemo devices list")]
    [InlineData("request-1;whoami", "luckynemo devices list")]
    [InlineData("<requestId>", "luckynemo devices list")]
    ...
    public void BuildUnknownPairingDiscoveryCommands_IncludesBothApprovalQueues()
    {
        ...
        Assert.Equal(
            string.Join(Environment.NewLine, "luckynemo nodes pending", "luckynemo devices list"),
            commands);
    ...
    public void TryBuildNodeApprovalCommand_DistinguishesApprovalFromDiscovery()
    {
        ...
        Assert.Equal("luckynemo nodes approve request-123", approvalCommand);
    ...
    public void BuildDangerousCommandOptInGuidance_IsStableAndDoesNotEmitRepairCommand()
    {
        ...
        Assert.DoesNotContain("luckynemo config set", guidance, StringComparison.OrdinalIgnoreCase);
    }
    ...
    [InlineData("ws://luckynemo.wsl:18789", false, "", GatewayKind.Wsl)]
    [InlineData("ws://192.168.1.20:18789", false, "", GatewayKind.RemoteLan)]
    [InlineData("wss://luckynemo.local:18789", false, "", GatewayKind.RemoteLan)]
