...
/// connect the <b>real</b> <see cref="LuckyNemoGatewayClient"/> over a <b>real</b>
...
///      (proving it matches the upstream luckynemo/luckynemo schema — param names,
...
public sealed class GatewayProtocolLiveRoundTripTests : IDisposable
{
    ...
    {
        _output = output;
        _identityDir = Path.Combine(Path.GetTempPath(), "luckynemo-rt-" + Guid.NewGuid().ToString("N"));
    ...
    private static async Task ConnectAndWaitAsync(LuckyNemoGatewayClient client, LoopbackGatewayServer server)
    {
