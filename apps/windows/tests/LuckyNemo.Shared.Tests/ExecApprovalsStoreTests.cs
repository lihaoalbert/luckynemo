...
public class ExecApprovalsStoreTests : IDisposable
{
    ...
    [Fact]
    public async Task ResolveAsync_HomeRelativeStateDir_UsesLuckyNemoHome()
    {
        WriteFile(MinimalFileWithAgent("main", "full"));
        var luckyNemoHome = Path.Combine(_dir, "effective-home");
        var stateDirOverride = $"~{Path.DirectorySeparatorChar}custom-state";
        var store = new ExecApprovalsStore(
            _dir,
            _log,
            stateDirOverride,
            luckyNemoHomeOverride: luckyNemoHome,
            osHomeOverride: Path.Combine(_dir, "os-home"));

        var resolved = await store.ResolveAsync("main");

        Assert.Equal(ExecSecurity.Full, resolved.Defaults.Security);
        Assert.True(File.Exists(Path.Combine(luckyNemoHome, "custom-state", "exec-approvals.json")));
    }
    ...
    public void Normalize_SocketPathPreserved()
    {
        ...
        {
          "version": 1,
          "socket": { "path": "/run/luckynemo.sock", "token": "tok" },
          "agents": {}
        }
        """);
    ...
    public async Task ResolveAsync_TildeOnlyStateDir_ResolvesToEffectiveHome()
    {
        ...
        Assert.Equal(ExecSecurity.Full, resolved.Defaults.Security);
        Assert.True(File.Exists(Path.Combine(luckyNemoHome, "exec-approvals.json")),
            "exec-approvals.json should have been migrated to the effective-home state dir");
    }

    /// <summary>
    /// When luckyNemoHomeOverride itself starts with "~/" it is expanded relative to
    /// osHomeOverride before being used as the base for further tilde expansion in
    /// stateDirOverride.
    /// </summary>
    [Fact]
    public async Task ResolveAsync_TildePrefixedLuckyNemoHome_ExpandsRelativeToOsHome()
    {
        WriteFile(MinimalFileWithAgent("main", "allowlist"));
        var osHome = Path.Combine(_dir, "os-home");
        ...
        var store = new ExecApprovalsStore(
            ...
            luckyNemoHomeOverride:  $"~{sep}.luckynemo",   // should expand to osHome/.luckynemo
            osHomeOverride:        osHome);

        var resolved = await store.ResolveAsync("main");

        // effectiveHome = osHome/.luckynemo; stateDir = osHome/.luckynemo/custom-state
        var expectedFile = Path.Combine(osHome, ".luckynemo", "custom-state", "exec-approvals.json");
        Assert.Equal(ExecSecurity.Allowlist, resolved.Defaults.Security);
...
internal sealed class CapturingLogger : ILuckyNemoLogger
{
