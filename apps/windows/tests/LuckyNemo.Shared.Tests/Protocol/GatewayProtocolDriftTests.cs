...
namespace LuckyNemo.Shared.Tests.Protocol;

/// <summary>
/// Protocol drift guard for the LuckyNemo gateway <c>sessions</c> / <c>files</c> /
/// <c>commands</c> / compaction surface.
///
/// The Windows companion can silently drift from the upstream
/// <c>luckynemo/luckynemo</c> gateway protocol
...
/// the typed client (<c>LuckyNemoGatewayClient</c> + its <c>.Protocol</c> partial,
...
/// clone of the upstream LuckyNemo repository or any network access. When upstream
...
public class GatewayProtocolDriftTests
{
    ...
    /// <see cref="LuckyNemo.Shared.PatchField{T}"/> (unset → omitted, value → sent
    ...
    /// not contain a closing quote inside the hole; the scanned LuckyNemo methods
    /// satisfy this.
    /// </summary>
    ...
    /// <summary>The client dispatch surface: the LuckyNemoGatewayClient partial class files.</summary>
    private static Source LoadClientSource() =>
        ConcatSources(f => FileNameMatches(f.Path, "LuckyNemoGatewayClient", ".cs"),
            "Could not locate LuckyNemoGatewayClient*.cs under src/.");
    ...
    private static Source LoadProtocolSource() =>
        ConcatSources(
            f => FileNameMatches(f.Path, "LuckyNemoGatewayClient", ".cs") || EndsWith(f.Path, "GatewayProtocolModels.cs"),
    ...
    {
        var path = Path.Combine(ProductionSourceFiles.FindRepoRoot(), "tests", "LuckyNemo.Shared.Tests", "Protocol", "gateway-protocol-snapshot.json");
