...
public class SystemCapabilityTests
{
    ...
    public async Task Notify_DefaultsTitle_WhenMissing()
    {
        ...
        var res = await cap.ExecuteAsync(req);
        Assert.True(res.Ok);
        Assert.Equal("LuckyNemo", received!.Title);
    }
...
public class BrowserProxyCapabilityTests
{
    ...
    public async Task BrowserProxy_ForwardsToLocalControlPortWithBearerAuth()
    {
        ...
        var res = await cap.ExecuteAsync(new NodeInvokeRequest
        {
            ...
            Args = Parse("""{"method":"POST","path":"/snapshot","query":{"format":"aria"},"profile":"luckynemo","body":{"limit":1},"timeoutMs":5000}""")
        });

        Assert.True(res.Ok);
        Assert.NotNull(handler.LastRequest);
        Assert.Equal(HttpMethod.Post, handler.LastRequest!.Method);
        Assert.Equal("http://127.0.0.1:18791/snapshot?format=aria&profile=luckynemo", handler.LastRequest.RequestUri!.ToString());
    ...
    public async Task BrowserProxy_RetriesUnauthorizedWithPasswordAuth()
    {
        ...
        Assert.True(handler.Requests[1].Headers.TryGetValues("x-luckynemo-password", out var passwordValues));
    ...
    private sealed class BrowserProxyAuthFallbackHandler : HttpMessageHandler
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            var hasPasswordHeader =
                request.Headers.TryGetValues("x-luckynemo-password", out var passwordValues) &&
