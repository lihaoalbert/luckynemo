...
using LuckyNemo.Shared.ExecApprovals;
...
public sealed class NodeToolTelemetryTests
{
    ...
    {
        Assert.Equal("luckynemo.node.tool.invoke", NodeToolInvocation.InvokeSpanName);
        Assert.Equal("luckynemo.node.tool.execute", NodeToolInvocation.ExecuteSpanName);
        Assert.Equal("luckynemo.node.tool.system_run.authorize", NodeToolInvocation.SystemRunAuthorizeSpanName);
        Assert.Equal("luckynemo.node.tool.system_run.run", NodeToolInvocation.SystemRunRunSpanName);
        Assert.Equal("luckynemo.node.tool.invocations", NodeToolInvocation.InvocationsMetricName);
        Assert.Equal("luckynemo.node.tool.duration", NodeToolInvocation.DurationMetricName);
        Assert.Equal(
            "luckynemo.node.tool.system_run.approval.pipeline",
    ...
    public void Invocation_CreatesDetachedRootAndExplicitChild_AndCompletesOnce()
    {
        ...
        Assert.Equal("command_failed", root.GetTagItem(LuckyNemoTelemetryTagKey.ErrorCategory.ToTelemetryName()));
        ...
        [
            ...
            LuckyNemoTelemetryTagKey.Outcome.ToTelemetryName(),
    ...
    public async Task SystemRun_NonzeroSandboxExit_EmitsNestedRunSpanAndSandboxAttributes()
    {
        ...
        Assert.Equal("failure", run.GetTagItem(LuckyNemoTelemetryTagKey.Outcome.ToTelemetryName()));
        Assert.Equal("command_failed", run.GetTagItem(LuckyNemoTelemetryTagKey.ErrorCategory.ToTelemetryName()));
        ...
        NodeToolErrorCategory expectedCategory)
    {
        ...
        Assert.Equal(expectedCategory.ToTelemetryValue(),
            approval.GetTagItem(LuckyNemoTelemetryTagKey.ErrorCategory.ToTelemetryName()));
    }
    ...
    public async Task SystemRun_V2Allow_EmitsAuthorizeAndRunSpans()
    {
        ...
        Assert.Equal(executeActivity.SpanId, run.ParentSpanId);
        Assert.Equal("success", authorize.GetTagItem(LuckyNemoTelemetryTagKey.Outcome.ToTelemetryName()));
        ...
        Assert.Equal(
            "command_failed",
            run.GetTagItem(LuckyNemoTelemetryTagKey.ErrorCategory.ToTelemetryName()));
    }
    ...
    public async Task SystemRun_V2DirectArgvGate_ReportsCapabilityUnavailable()
    {
        ...
        var authorize = Assert.Single(
            activities.Stopped,
            activity => activity.OperationName == NodeToolInvocation.SystemRunAuthorizeSpanName);
        Assert.Equal("failure", authorize.GetTagItem(LuckyNemoTelemetryTagKey.Outcome.ToTelemetryName()));
        Assert.Equal("v2", authorize.GetTagItem(NodeToolInvocation.ApprovalPipelineTag));
        Assert.Equal(
            "capability_unavailable",
            authorize.GetTagItem(LuckyNemoTelemetryTagKey.ErrorCategory.ToTelemetryName()));
        Assert.DoesNotContain(
            activities.Stopped,
            activity => activity.OperationName == NodeToolInvocation.SystemRunRunSpanName);
    }
