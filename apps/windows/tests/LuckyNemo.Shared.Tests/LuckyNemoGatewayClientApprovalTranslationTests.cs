...
/// </summary>
public class LuckyNemoGatewayClientApprovalTranslationTests
{
    private static LuckyNemoGatewayClient NewClient() =>
        new("ws://localhost:18789", "test-token", new TestLogger());

    private static void InvokeHandleEvent(LuckyNemoGatewayClient client, string json)
    {
        ...
        const string json = """
            {
              ...
              "payload": {
                "id": "4d6a4c38-5226-4ffe-a0e1-fb4acff1181d",
                "decision": "deny",
                "resolvedBy": "luckynemo-control-ui",
                "ts": 1780691511789,
                "request": {
                  "command": "luckynemo nodes invoke",
                  "host": "gateway",
                  "sessionKey": "agent:main:main",
                  "agentId": "main"
                }
    ...
    public void ExecApprovalResolved_AllowOnceDecision_MapsToResolvedTerminalPhase()
    {
        ...
        Assert.NotNull(observed);
        // Must be ``resolved`` (terminal in LuckyNemoChatDataProvider.IsTerminalApprovalPhase)
    ...
    private static TaskCompletionSource<bool> RegisterPendingApprovalResolve(LuckyNemoGatewayClient client, string requestId)
    {
        var fieldInfo = typeof(LuckyNemoGatewayClient).GetField(
        ...
        var trackMethod = typeof(LuckyNemoGatewayClient).GetMethod(
            "TrackPendingRequest",
            BindingFlags.NonPublic | BindingFlags.Instance);
    ...
    private static void InvokeHandleResponse(LuckyNemoGatewayClient client, string json)
    {
        using var doc = JsonDocument.Parse(json);
        var method = typeof(LuckyNemoGatewayClient).GetMethod(
            "HandleResponse",
