...
public class DeepLinkParserTests
{
    ...
    [InlineData("LUCKYNEMO//send")]
    public void ParseDeepLink_ReturnsNull_ForNonLuckyNemoScheme(string uri)
    {
        Assert.Null(DeepLinkParser.ParseDeepLink(uri));
    }

    [Theory]
    [InlineData("luckynemo://send", "send")]
    [InlineData("LUCKYNEMO://send", "send")]
    [InlineData("luckynemo://send/", "send")]
    [InlineData("luckynemo://send/?text=hello", "send")]
    [InlineData("luckynemo://send?text=hello", "send")]
    [InlineData("luckynemo://pair/setup", "pair/setup")]
    [InlineData("luckynemo://", "")]
    ...
    {
        var result = DeepLinkParser.ParseDeepLink("luckynemo://send?text=hello&target=channel");
    ...
    {
        var result = DeepLinkParser.ParseDeepLink("luckynemo://send?text=hello");
    ...
    {
        var result = DeepLinkParser.ParseDeepLink("luckynemo://send?text=hello&target=channel&urgent=true");
    ...
    {
        var result = DeepLinkParser.ParseDeepLink("luckynemo://send?text=hello%20world&key=a%2Bb");
    ...
    {
        var result = DeepLinkParser.ParseDeepLink("luckynemo://send");
    ...
    {
        // Windows may canonicalize luckynemo://send/?args=... — path should be "send", not "send/"
        var result = DeepLinkParser.ParseDeepLink("luckynemo://send/?text=hello");
    ...
    {
        var result = DeepLinkParser.ParseDeepLink("luckynemo-dev://settings", "luckynemo-dev");

        Assert.NotNull(result);
        Assert.Equal("settings", result.Path);
        Assert.Null(DeepLinkParser.ParseDeepLink("luckynemo://settings", "luckynemo-dev"));
        Assert.Null(DeepLinkParser.ParseDeepLink("luckynemo-dev://settings"));
    }
