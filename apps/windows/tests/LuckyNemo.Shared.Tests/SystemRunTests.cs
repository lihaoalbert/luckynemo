...
public class LocalCommandRunnerTests
{
    ...
    {
        var tempDir = Directory.CreateDirectory(Path.Combine(Path.GetTempPath(), "luckynemo-pwsh-path-" + Guid.NewGuid().ToString("N"))).FullName;
...
/// Gated by LUCKYNEMO_RUN_INTEGRATION=1.
/// </summary>
public class LocalCommandRunnerIntegrationTests
{
    ...
    public async Task Run_WithEnvVars()
    {
        ...
        var result = await runner.RunAsync(new CommandRequest
        {
            Command = "echo %TEST_LUCKYNEMO_VAR%",
            Shell = "cmd",
            TimeoutMs = 10000,
            Env = new() { { "TEST_LUCKYNEMO_VAR", "hello_from_test" } }
        });
