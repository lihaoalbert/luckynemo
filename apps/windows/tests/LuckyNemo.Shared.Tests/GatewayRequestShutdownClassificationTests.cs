...
public sealed class GatewayRequestShutdownClassificationTests
{
    ...
    public async Task ResolveGatewayResponse_ResponseWinsWhenTerminalSignalCompletedFirst()
    {
        ...
        var result = await LuckyNemoGatewayClient.ResolveGatewayResponseAsync(
            responseTask,
            cancellationTask,
            cancellationTask,
            "must not time out");
    ...
    public async Task ResolveGatewayResponse_TimeoutWinnerStaysTimeoutAfterLaterCancellation()
    {
        ...
        var exception = await Assert.ThrowsAsync<TimeoutException>(() =>
            LuckyNemoGatewayClient.ResolveGatewayResponseAsync(
                response.Task,
                timeoutTask,
                cancellationTask,
                "expected timeout"));
    ...
    private static LuckyNemoGatewayClient CreateClient(LoopbackWebSocketServer server, string identityPath)
        => new(server.WebSocketUrl, "test-token", new TestLogger(), identityPath: identityPath);

    private static async Task ConnectAsync(LuckyNemoGatewayClient client, LoopbackWebSocketServer server)
    {
    ...
    private static void CancelLifetime(LuckyNemoGatewayClient client)
    {
