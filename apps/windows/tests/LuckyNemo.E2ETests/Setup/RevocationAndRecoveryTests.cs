...
public class RevocationAndRecoveryTests
{
    ...
    public async Task RealGateway_DeviceRemoval_RecoversThroughSharedTokenReconnect()
    {
        ...
        try
        {
            var removeDevice = await _fixture.RunInWslAsync(
                $"luckynemo devices remove {ShellSingleQuote(deviceId)} --json",
            ...
            var pendingDevices = await _fixture.RunInWslAsync("luckynemo devices list --json", TimeSpan.FromSeconds(30), env);
            ...
            var approve = await _fixture.RunInWslAsync(
                $"luckynemo devices approve {ShellSingleQuote(requestId)} --json",
                TimeSpan.FromSeconds(30),
                env);
        ...
        Dictionary<string, string> env)
    {
        try
        {
            var pending = await _fixture.RunInWslAsync("luckynemo devices list --json", TimeSpan.FromSeconds(30), env);
