...
public sealed class E2ESetupFixture : IAsyncLifetime
{
    ...
    /// Set as LUCKYNEMO_TRAY_DATA_DIR env var.
    /// </summary>
    ...
    internal E2ESetupFixture(Action<Dictionary<string, object>>? settingsPatch)
    {
        ...
        {
            _distroName = "LuckyNemoE2E-disabled";
        ...
        _distroName = $"LuckyNemoE2E-{runId}";

        // Data dir in temp — this is what the tray and setup engine use
        DataDir = Path.Combine(Path.GetTempPath(), $"luckynemo-e2e-{runId}");
        LocalAppDataRoot = Path.Combine(Path.GetTempPath(), $"luckynemo-e2e-localappdata-{runId}");
    ...
    public async Task InitializeAsync()
    {
        ...
        Environment.SetEnvironmentVariable("LUCKYNEMO_TRAY_DATA_DIR", DataDir);
        Environment.SetEnvironmentVariable("LUCKYNEMO_TRAY_APPDATA_DIR", DataDir);
        Environment.SetEnvironmentVariable("LUCKYNEMO_TRAY_LOCALAPPDATA_DIR", LocalAppDataRoot);
    ...
    private Process SpawnTray(string exePath)
    {
        ...
        psi.Environment["LUCKYNEMO_TRAY_DATA_DIR"] = DataDir;
        psi.Environment["LUCKYNEMO_TRAY_APPDATA_DIR"] = DataDir;
        psi.Environment["LUCKYNEMO_TRAY_LOCALAPPDATA_DIR"] = LocalAppDataRoot;
        psi.Environment["LUCKYNEMO_MCP_PORT"] = McpPort.ToString();
        psi.Environment["LUCKYNEMO_SUPPRESS_EXTERNAL_BROWSER"] = "1";
    ...
    public async Task<LuckyNemo.SetupEngine.CommandResult> RunInWslAsync(
        string command,
        ...
        bool inputViaStdin = false)
    {
        ...
        var result = new LuckyNemo.SetupEngine.CommandResult(timedOut ? -1 : process.ExitCode, stdout, stderr, sw.Elapsed, timedOut);
    ...
    {
        var logPath = Path.Combine(DataDir, "luckynemo-tray.log");
    ...
    private static string LocateTrayExe()
    {
        ...
        var exe = Path.Combine(
            repoRoot,
            "src", "LuckyNemo.Tray.WinUI", "bin", configuration,
            targetFramework, rid, "LuckyNemo.Tray.WinUI.exe");

        if (!File.Exists(exe))
        {
            throw new FileNotFoundException(
                $"Tray exe not found at {exe}. Build it first: " +
                $"`dotnet build src/LuckyNemo.Tray.WinUI/LuckyNemo.Tray.WinUI.csproj -c {configuration} -r {rid}`");
        }
        return exe;
    }
    ...
    {
        var projectPath = Path.Combine(repoRoot, "src", "LuckyNemo.Tray.WinUI", "LuckyNemo.Tray.WinUI.csproj");
    ...
    private static string FindRepoRoot()
    {
        ...
        {
            if (File.Exists(Path.Combine(dir, "luckynemo-windows-node.slnx")))
                return dir;
            var parent = Directory.GetParent(dir)?.FullName;
            if (parent == dir || parent == null) break;
            dir = parent;
        }
        throw new DirectoryNotFoundException("Could not locate repo root (luckynemo-windows-node.slnx) from " + AppContext.BaseDirectory);
    }
