...
using LuckyNemo.E2ETests;
using LuckyNemo.SetupEngine;

namespace LuckyNemo.E2ETests.Setup;
...
public class SetupAndConnectTests
{
    ...
    public async Task FullSetup_WslAndGatewayConfiguration_FilesValidated()
    {
        ...
        Assert.Contains("default=luckynemo", wslConf.Stdout);
        Assert.Contains("useWindowsTimezone=true", wslConf.Stdout);

        var agentsContext = await _fixture.RunInWslAsync(
            "cat /home/luckynemo/.luckynemo/workspace/AGENTS.md",
            TimeSpan.FromSeconds(15));
        AssertCommandSucceeded(agentsContext, "read managed Windows node context");
        Assert.Contains("<!-- BEGIN LUCKYNEMO WINDOWS NODE CONTEXT: managed by LuckyNemo Windows setup -->", agentsContext.Stdout);
        Assert.Contains("exec host=node", agentsContext.Stdout);
        Assert.Contains("<!-- END LUCKYNEMO WINDOWS NODE CONTEXT -->", agentsContext.Stdout);
        Assert.DoesNotContain("tools.exec.security full", agentsContext.Stdout);

        var luckyNemoJsonProbe = await _fixture.RunInWslAsync(
            "paths=$(find /home/luckynemo/.luckynemo /opt/luckynemo /etc/luckynemo -type f -name luckynemo.json 2>/dev/null | sort); if [ -z \"$paths\" ]; then echo 'LUCKYNEMO_JSON_PATH:<not-found>'; else for path in $paths; do echo LUCKYNEMO_JSON_PATH:$path; cat \"$path\"; done; fi",
            TimeSpan.FromSeconds(15));
        AssertCommandSucceeded(luckyNemoJsonProbe, "probe WSL luckynemo.json");
        Console.WriteLine($"[E2E] WSL luckynemo.json probe:\n{luckyNemoJsonProbe.Stdout}");

        if (luckyNemoJsonProbe.Stdout.Contains('{', StringComparison.Ordinal))
        {
            var jsonStart = luckyNemoJsonProbe.Stdout.IndexOf('{');
            using var configDoc = JsonDocument.Parse(luckyNemoJsonProbe.Stdout[jsonStart..]);
        ...
        var gatewayPort = await _fixture.RunInWslAsync("luckynemo config get gateway.port", TimeSpan.FromSeconds(15));
        AssertCommandSucceeded(gatewayPort, "read gateway.port");
        Assert.Contains(_fixture.GatewayPort.ToString(), gatewayPort.Stdout);

        var gatewayBind = await _fixture.RunInWslAsync("luckynemo config get gateway.bind", TimeSpan.FromSeconds(15));
        AssertCommandSucceeded(gatewayBind, "read gateway.bind");
        Assert.Contains("loopback", gatewayBind.Stdout);

        var gatewayAuthMode = await _fixture.RunInWslAsync("luckynemo config get gateway.auth.mode", TimeSpan.FromSeconds(15));
        ...
        var cliAllowCommands = await _fixture.RunInWslAsync(
            "luckynemo config get gateway.nodes.allowCommands",
            TimeSpan.FromSeconds(15));
    ...
    {
        const string agents = "/home/luckynemo/.luckynemo/workspace/AGENTS.md";
        const string beginMarker = "<!-- BEGIN LUCKYNEMO WINDOWS NODE CONTEXT: managed by LuckyNemo Windows setup -->";
        const string endMarker = "<!-- END LUCKYNEMO WINDOWS NODE CONTEXT -->";
    ...
    public async Task FullSetup_GatewayCliShowsPairedDeviceAndNode()
    {
        ...
        var devices = await _fixture.RunInWslAsync("luckynemo devices list --json", TimeSpan.FromSeconds(30), env);
        AssertCommandSucceeded(devices, "list gateway devices");
        Console.WriteLine($"[E2E] luckynemo devices list --json:\n{devices.Stdout}");
        AssertNoPendingRequests(devices.Stdout);

        var nodes = await _fixture.RunInWslAsync("luckynemo nodes list --json", TimeSpan.FromSeconds(30), env);
        AssertCommandSucceeded(nodes, "list gateway nodes");
        Console.WriteLine($"[E2E] luckynemo nodes list --json:\n{nodes.Stdout}");
    ...
    public async Task FullSetup_GatewayRestart_ReconnectsTrayAndNode()
    {
        ...
        var restart = await _fixture.RunInWslAsync(
            "luckynemo gateway restart || (systemctl --user restart openclaw-gateway.service && echo restarted-via-systemctl)",
    ...
    public async Task RealGateway_InvalidSetupCode_DoesNotDestroyExistingPairing()
    {
        ...
        using var applyDoc = await _fixture.Client!.CallToolExpectSuccessAsync(
            "app.connection.applySetupCode",
            new { setupCode = "this-is-not-a-valid-luckynemo-setup-code" });
        var apply = applyDoc.RootElement;
    ...
    [E2EFact]
    public async Task FullSetup_LuckyNemoCommand_IsOnDefaultWslPath()
    {
        var loginShell = await _fixture.RunInWslAsync("bash -lc 'luckynemo --version'", TimeSpan.FromSeconds(15));
        AssertCommandSucceeded(loginShell, "luckynemo --version in login shell");
        Console.WriteLine($"[E2E] login shell luckynemo --version: {loginShell.Stdout}");

        var systemPath = await _fixture.RunInWslAsync(
            "env -i HOME=/home/luckynemo USER=luckynemo PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin luckynemo --version",
            TimeSpan.FromSeconds(15));
        AssertCommandSucceeded(systemPath, "luckynemo --version on default system PATH");
        Console.WriteLine($"[E2E] system PATH luckynemo --version: {systemPath.Stdout}");
    }
    ...
    private static void AssertCommandSucceeded(LuckyNemo.SetupEngine.CommandResult result, string description)
    {
    ...
    {
        var qr = await _fixture.RunInWslAsync("luckynemo qr --json", TimeSpan.FromSeconds(30), env);
