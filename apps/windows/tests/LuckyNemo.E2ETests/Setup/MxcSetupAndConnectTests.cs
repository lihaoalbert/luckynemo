...
public sealed class MxcSetupAndConnectTests
{
    ...
    {
        const string marker = "LUCKYNEMO_GATEWAY_SYSTEM_RUN_MXC_OK";
        ...
        var invoke = await _fixture.RunInWslAsync(
            $"luckynemo gateway call node.invoke --params {ShellSingleQuote(invokeParams)} --json --timeout {GatewayCliProofTimeoutMs}",
    ...
    {
        const string sourcePayload = "LUCKYNEMO_GATEWAY_SYSTEM_RUN_MXC_DENIED_PAYLOAD";
        const string sourceReadyMarker = "LUCKYNEMO_GATEWAY_SYSTEM_RUN_MXC_SOURCE_READY";
        ...
        var sourcePathForCmd = "%TEMP%/luckynemo-mxc-denied-source.txt";
    ...
    private async Task AssertPrimaryTrayReadyAndGatewayCliHealthyAsync()
    {
        ...
        var allowCommands = await _fixture.RunInWslAsync(
            "luckynemo config get gateway.nodes.allowCommands --json",
            TimeSpan.FromSeconds(30),
            env);
    ...
    private async Task SetExecApprovalForSystemRunProofAsync()
    {
        ...
        using var updated = await _fixture.Client!.CallToolExpectSuccessAsync("system.execApprovals.set", new
        {
            ...
            rules = new object[]
            {
                new
                {
                    pattern = "echo LUCKYNEMO_GATEWAY_SYSTEM_RUN_MXC_OK",
                    action = "allow",
                    shells = new[] { "cmd" },
                ...
                new
                {
                    pattern = "echo LUCKYNEMO_GATEWAY_SYSTEM_RUN_MXC_DENIED_PAYLOAD > *luckynemo-mxc-denied-source.txt\"" +
                              " && echo LUCKYNEMO_GATEWAY_SYSTEM_RUN_MXC_SOURCE_READY" +
                              " && copy /Y *luckynemo-mxc-denied-source.txt\" *mxc-denied-write-*.txt\"",
                    action = "allow",
                    shells = new[] { "cmd" },
                ...
                new
                {
                    pattern = "echo LUCKYNEMO_GATEWAY_SYSTEM_RUN_MXC_SOURCE_READY",
                    action = "allow",
                    shells = new[] { "cmd" },
                ...
                new
                {
                    pattern = "copy /Y *luckynemo-mxc-denied-source.txt\" *mxc-denied-write-*.txt\"",
                    action = "allow",
                    shells = new[] { "cmd" },
    ...
    {
        var logPath = Path.Combine(_fixture.DataDir, "luckynemo-tray.log");
