...
internal sealed class IsolatedTrayInstance : IAsyncDisposable
{
    ...
    {
        var dataDir = Path.Combine(Path.GetTempPath(), $"luckynemo-e2e-{name}-{Guid.NewGuid():N}"[..36]);
        var localAppDataRoot = Path.Combine(Path.GetTempPath(), $"luckynemo-e2e-local-{name}-{Guid.NewGuid():N}"[..42]);
    ...
    private static string LocateTrayExe()
    {
        ...
        var binRoot = Path.Combine(repoRoot, "src", "LuckyNemo.Tray.WinUI", "bin");
        var candidates = Directory.Exists(binRoot)
            ? Directory.GetFiles(binRoot, "LuckyNemo.Tray.WinUI.exe", SearchOption.AllDirectories)
            : [];
        var exe = candidates
            .Where(path => path.Contains(rid, StringComparison.OrdinalIgnoreCase))
            .OrderByDescending(File.GetLastWriteTimeUtc)
            .FirstOrDefault();
        return exe ?? throw new FileNotFoundException("Tray exe not found. Build LuckyNemo.Tray.WinUI first.");
    }
