...
/// LUCKYNEMO_RUN_E2E=1 so these still run before merge.
/// </summary>
...
internal static class MxcE2ETestGate
{
    ...
    private const string ExplicitGitHubActionsOptInEnvVar = "LUCKYNEMO_RUN_MXC_E2E";
    ...
    private static bool TryFindE2EWxcExec(out string? path)
    {
        ...
        foreach (var repoRoot in CandidateRepoRoots())
        {
            ...
            var trayBin = Path.Combine(repoRoot, "src", "LuckyNemo.Tray.WinUI", "bin");
    ...
    private static IEnumerable<string> CandidateRepoRoots()
    {
        ...
        {
            Environment.GetEnvironmentVariable("LUCKYNEMO_REPO_ROOT"),
        ...
        })
        {
            ...
            while (dir is not null)
            {
                if (File.Exists(Path.Combine(dir.FullName, "package.json"))
                    && Directory.Exists(Path.Combine(dir.FullName, "src", "LuckyNemo.Tray.WinUI")))
                {
...
{
    public const string EnvVar = "LUCKYNEMO_RUN_E2E";
