...
public sealed class TrayAppFixture : IAsyncLifetime
{
    ...
    {
        DataDir = Path.Combine(Path.GetTempPath(), "luckynemo-tray-it-" + Guid.NewGuid().ToString("N").Substring(0, 8));
        ...
        while (DateTime.UtcNow < deadline)
        {
            if (_process.HasExited)
            {
                throw new InvalidOperationException(
                    ...
                    $"Logs: {Path.Combine(DataDir, "luckynemo-tray.log")}");
            }
            try
            {
                if (token is null)
