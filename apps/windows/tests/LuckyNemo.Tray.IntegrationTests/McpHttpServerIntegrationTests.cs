...
///
/// Runs only with LUCKYNEMO_RUN_INTEGRATION=1 on Windows. See IntegrationFactAttribute.
/// </summary>
public class McpHttpServerIntegrationTests : IClassFixture<TrayAppFixture>
{
    ...
    public async Task Initialize_ReturnsProtocolVersionAndServerInfo()
    {
        ...
        Assert.Equal("luckynemo-tray-mcp", serverInfo.GetProperty("name").GetString());
    }
    ...
    // ---- screen.* ----
    // Canonical LuckyNemo protocol: screen.snapshot + screen.record only.
