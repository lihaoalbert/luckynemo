...
namespace LuckyNemo.Tray.UITests;

/// <summary>
/// Owns one isolated LuckyNemo process for the accessibility test collection.
...
public sealed class AccessibilityAppFixture : IDisposable
{
    ...
    {
        _executablePath = Path.Combine(AppContext.BaseDirectory, "LuckyNemo.Tray.WinUI.exe");
        ...
        _dataDirectory = Path.Combine(
            Path.GetTempPath(),
            $"LuckyNemo.Tray.Axe.{Guid.NewGuid():N}");
        ...
        _process = StartProcess($"{LuckyNemoTray.AppIdentity.ProtocolScheme}://hub/connection");
    ...
    public async Task NavigateAsync(string pageTag, string pageMarkerAutomationId)
    {
        ...
        using var sender = StartProcess($"{LuckyNemoTray.AppIdentity.ProtocolScheme}://hub/{pageTag}");
    ...
    {
        var configuredPath = Environment.GetEnvironmentVariable("LUCKYNEMO_UI_SCREENSHOT_PATH");
    ...
    private Process StartProcess(string deepLink)
    {
        ...
        startInfo.Environment["LUCKYNEMO_TRAY_DATA_DIR"] = _dataDirectory;
        startInfo.Environment["LUCKYNEMO_SKIP_UPDATE_CHECK"] = "1";
        startInfo.Environment["LUCKYNEMO_FORCE_ONBOARDING"] = "0";
        startInfo.Environment["LUCKYNEMO_LANGUAGE"] = "en-US";
        startInfo.Environment["LUCKYNEMO_ACCESSIBILITY_TEST_CHAT"] = "1";
        startInfo.Environment["LUCKYNEMO_ACCESSIBILITY_TEST_SESSIONS"] = "1";

        return Process.Start(startInfo)
            ?? throw new InvalidOperationException("Failed to start the LuckyNemo tray executable.");
    }
    ...
    private IntPtr WaitForHubWindow()
    {
        ...
        throw new TimeoutException(
            $"LuckyNemo did not expose its Hub window within {StartupTimeout.TotalSeconds:0} seconds.");
    }
    ...
    private void EnsureTargetIsAlive()
    {
        ...
        throw new InvalidOperationException(
            $"LuckyNemo exited unexpectedly with code {_process.ExitCode}.{crashLog}");
    }
