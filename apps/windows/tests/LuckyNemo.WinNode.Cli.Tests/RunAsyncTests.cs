...
{
    // Tests run on developer machines where %APPDATA%\LuckyNemoTray\mcp-token.txt
    ...
    // FakeMcpServer ignores it. Redirect via LUCKYNEMO_TRAY_DATA_DIR (same
    ...
    private Func<string, string?> EmptyEnv => key =>
        key == "LUCKYNEMO_TRAY_DATA_DIR" ? _sandboxDataDir : null;
    ...
    [Fact]
    public async Task Endpoint_resolves_from_LUCKYNEMO_MCP_PORT_when_no_overrides()
    {
        using var server = new FakeMcpServer();
        var env = (string key) => key == "LUCKYNEMO_MCP_PORT" ? server.Port.ToString() : null;
