...
using LuckyNemo.WinNode.Cli;

namespace LuckyNemo.WinNode.Cli.Tests;

/// <summary>
/// Tests covering bearer-token resolution: --mcp-token > $LUCKYNEMO_MCP_TOKEN >
/// on-disk mcp-token.txt under $LUCKYNEMO_TRAY_DATA_DIR (or %APPDATA%\LuckyNemoTray).
...
public class AuthTokenTests : IDisposable
{
    ...
    {
        "LUCKYNEMO_TRAY_DATA_DIR" => _sandboxDir,
        "LUCKYNEMO_MCP_TOKEN" => mcpToken,
        _ => null,
    };
    ...
    [Fact]
    public async Task LUCKYNEMO_MCP_TOKEN_env_var_sets_bearer_header()
    {
        using var server = new FakeMcpServer();
        var (o, e) = Buffers();
    ...
    [Fact]
    public async Task Token_file_under_LUCKYNEMO_TRAY_DATA_DIR_is_loaded_automatically()
    {
        // Mirrors the live flow: the tray writes mcp-token.txt to the sandbox
        // dir, the CLI launched with the same LUCKYNEMO_TRAY_DATA_DIR finds it.
    ...
    [Fact]
    public void ResolveTokenPath_uses_LUCKYNEMO_TRAY_DATA_DIR_when_set()
    {
        Func<string, string?> env = k => k == "LUCKYNEMO_TRAY_DATA_DIR" ? @"C:\sandbox" : null;
        var path = CliRunner.ResolveTokenPath(env);
        Assert.Equal(Path.Combine(@"C:\sandbox", "mcp-token.txt"), path);
    }

    [Fact]
    public void ResolveTokenPath_falls_back_to_AppData_LuckyNemoTray()
    {
    ...
    [Fact]
    public void ResolveTokenPath_uses_LUCKYNEMO_APP_IDENTITY_dev_when_set()
    {
        Func<string, string?> env = key =>
            key == LuckyNemoAppIdentity.IdentityEnvironmentVariable ? LuckyNemoAppIdentity.DevIdentity : null;
        var path = CliRunner.ResolveTokenPath(env);
        var expected = Path.Combine(
            Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData),
            "LuckyNemoTray-Dev",
    ...
    public void ResolveTokenPath_explicit_identity_overrides_environment()
    {
        Func<string, string?> env = key =>
            key == LuckyNemoAppIdentity.IdentityEnvironmentVariable ? LuckyNemoAppIdentity.DevIdentity : null;
        var path = CliRunner.ResolveTokenPath(env, LuckyNemoAppIdentity.ReleaseIdentity);
        var expected = Path.Combine(
            Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData),
            "LuckyNemoTray",
            "mcp-token.txt");
    ...
    public void ResolveTokenPath_data_dir_override_wins_over_identity()
    {
        ...
        {
            LuckyNemoAppIdentity.DataDirectoryOverrideEnvironmentVariable => @"C:\sandbox",
    ...
    public void ParseArgs_accepts_identity_dev()
    {
        ...
        Assert.Equal(LuckyNemoAppIdentity.DevIdentity, options.Identity);
    }
    ...
    [Fact]
    public async Task LUCKYNEMO_APP_IDENTITY_invalid_value_returns_argument_error()
    {
        using var server = new FakeMcpServer();
        var (o, e) = Buffers();
        ...
        var exit = await CliRunner.RunAsync(
            ...
            o,
            e,
            key => key == LuckyNemoAppIdentity.IdentityEnvironmentVariable ? "staging" : null);
