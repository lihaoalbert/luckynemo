...
public class SetupStepsTests : IDisposable
{
    ...
    public SetupStepsTests(ITestOutputHelper output)
    {
        ...
        _prevDataDir = Environment.GetEnvironmentVariable("LUCKYNEMO_TRAY_DATA_DIR");
        _prevLocalDataDir = Environment.GetEnvironmentVariable("LUCKYNEMO_TRAY_LOCAL_DATA_DIR");
        Environment.SetEnvironmentVariable("LUCKYNEMO_TRAY_DATA_DIR", _tempDir);
        Environment.SetEnvironmentVariable("LUCKYNEMO_TRAY_LOCAL_DATA_DIR", _localTempDir);
    }

    public void Dispose()
    {
        Environment.SetEnvironmentVariable("LUCKYNEMO_TRAY_DATA_DIR", _prevDataDir);
        Environment.SetEnvironmentVariable("LUCKYNEMO_TRAY_LOCAL_DATA_DIR", _prevLocalDataDir);
    ...
    public void DistroInstallPathPolicy_ResolvesImmediateChild()
    {
        ...
        Assert.Equal(
            Path.GetFullPath(Path.Combine(_localTempDir, "wsl", "LuckyNemoGateway-Dev")),
            installPath);
        Assert.Equal(
    ...
    [Theory]
    [InlineData("LuckyNemo Gateway")]
    [InlineData("LuckyNemo-网关")]
    ...
    public void DistroInstallPathPolicy_AddsRecoveryOnlyForLegacyTeardownNames()
    {
        ...
        Assert.Contains(
            ...
            DistroInstallPathPolicy.WithLegacyReplacementGuidance("LuckyNemo Gateway", error));
        Assert.Equal(
            error,
            DistroInstallPathPolicy.WithLegacyReplacementGuidance("LuckyNemoGateway", error));
        Assert.Equal(
            error,
    ...
    {
        var legacyPath = Path.Combine(_localTempDir, "wsl", "LuckyNemo Gateway");
        Directory.CreateDirectory(legacyPath);
        var sentinel = Path.Combine(legacyPath, "keep.txt");
        File.WriteAllText(sentinel, "keep");
        var commands = new FakeCommandRunner(_ => Ok("LuckyNemo Gateway"));
        var ctx = CreateContext(
            new SetupConfig { CleanBeforeRun = true, DistroName = "LuckyNemo Gateway" },
            commands);
    ...
    {
        const string legacyName = "LuckyNemo Gateway";
        var legacyPath = Path.Combine(_localTempDir, "wsl", legacyName);
        Directory.CreateDirectory(legacyPath);
    ...
    public void DistroInstallPathPolicy_RejectsWhenWslRootIsJunction()
    {
        ...
        try
        {
            ...
            var candidate = Path.Combine(wslRoot, "LuckyNemoGateway");
    ...
    public void DistroInstallPathPolicy_RejectsWhenManagedChildIsJunction()
    {
        ...
        var wslRoot = Path.Combine(_localTempDir, "wsl");
        Directory.CreateDirectory(wslRoot);
        var managedChild = Path.Combine(wslRoot, "LuckyNemo Gateway");
    ...
    {
        var wslRoot = Path.Combine(_localTempDir, "wsl");
        Directory.CreateDirectory(Path.Combine(wslRoot, "LuckyNemo-é"));
        Directory.CreateDirectory(Path.Combine(wslRoot, "LuckyNemo-e\u0301"));
    ...
    public void DistroInstallPathPolicy_RejectsCaseCollision()
    {
        ...
        Directory.CreateDirectory(Path.Combine(wslRoot, "LuckyNemo Gateway"));
        Directory.CreateDirectory(Path.Combine(wslRoot, "luckynemo gateway"));
    ...
    {
        var command = InstallCliStep.BuildInstallCommand("https://luckynemo.ai/install-cli.sh", null);

        Assert.Equal("curl -fsSL --proto '=https' --tlsv1.2 'https://luckynemo.ai/install-cli.sh' | bash", command);
    }

    [Fact]
    public void InstallCli_BuildInstallCommand_AppendsVersionWhenConfigured()
    {
        var command = InstallCliStep.BuildInstallCommand("https://luckynemo.ai/install-cli.sh", "2026.5.22");

        Assert.Equal("curl -fsSL --proto '=https' --tlsv1.2 'https://luckynemo.ai/install-cli.sh' | bash -s -- --version '2026.5.22'", command);
    }

    [Fact]
    public void InstallCli_BuildInstallCommand_EscapesSingleQuotesInUrlAndVersion()
    {
        var command = InstallCliStep.BuildInstallCommand("https://luckynemo.ai/install-cli's.sh", "2026.5.22'a");

        Assert.Equal("curl -fsSL --proto '=https' --tlsv1.2 'https://luckynemo.ai/install-cli'\\''s.sh' | bash -s -- --version '2026.5.22'\\''a'", command);
    }
    ...
    public async Task CreateWslInstance_UsesDirectFreshInstallAndDoesNotExportBaseDistro()
    {
        ...
        {
            if (args.SequenceEqual(["--list", "--quiet"]))
                return Ok(installed ? "LuckyNemoGateway\n" : "");
            ...
            if (args.SequenceEqual(["--list", "--verbose"]))
                return Ok("  NAME              STATE           VERSION\n* LuckyNemoGateway   Stopped         2\n");
            if (args.SequenceEqual(["-d", "LuckyNemoGateway", "-u", "root", "--", "sh", "-lc", "id -u && test -d / && echo LUCKYNEMO_FRESH_WSL_READY"]))
                return Ok("0\nLUCKYNEMO_FRESH_WSL_READY\n");
        ...
        Assert.Contains("LuckyNemoGateway", installCall.Arguments);
        Assert.Contains("--location", installCall.Arguments);
        Assert.Contains(Path.Combine(ctx.LocalDataDir, "wsl", "LuckyNemoGateway"), installCall.Arguments);
    ...
    public async Task CreateWslInstance_PartialCleanupAvoidsGlobalShutdownWhenUnregisterSucceeds()
    {
        ...
        var commands = new FakeCommandRunner(args =>
        {
            ...
            {
                listCalls++;
                return Ok(listCalls == 1 ? "" : "LuckyNemoGateway\n");
            }
            if (args.Contains("--install"))
                return Fail("download failed");
            if (args.SequenceEqual(["--terminate", "LuckyNemoGateway"]))
                return Ok();
            if (args.SequenceEqual(["--unregister", "LuckyNemoGateway"]))
                return Ok();
    ...
    public async Task CreateWslInstance_PartialCleanupSkipsInstallPathDeleteWhenDistroStateIsUnknown()
    {
        ...
        var ctx = CreateContext(commands: commands);
        installPath = Path.Combine(ctx.LocalDataDir, "wsl", "LuckyNemoGateway");

        var result = await new CreateWslInstanceStep().ExecuteAsync(ctx, CancellationToken.None);

        Assert.Equal(StepOutcome.Failed, result.Outcome);
        Assert.Contains("download failed", result.Message);
        Assert.Contains("could not confirm whether distro 'LuckyNemoGateway' is still registered", result.Message);
    ...
    public async Task CreateWslInstance_PartialCleanupDeletesInstallPathWhenListFailsButDistroIsAlreadyGone()
    {
        ...
        var commands = new FakeCommandRunner(args =>
        {
            ...
            if (args.SequenceEqual(["--terminate", "LuckyNemoGateway"]) ||
                args.SequenceEqual(["--unregister", "LuckyNemoGateway"]))
            {
    ...
    public async Task CreateWslInstance_FailsWhenInstallDirectoryIsDirty()
    {
        var commands = new FakeCommandRunner(args =>
            args.SequenceEqual(["--list", "--quiet"])
                ...
                : Fail($"unexpected args: {string.Join(' ', args)}"));
        var ctx = CreateContext(commands: commands);
        var installPath = Path.Combine(ctx.LocalDataDir, "wsl", "LuckyNemoGateway");
        Directory.CreateDirectory(installPath);
    ...
    public void WslInstallSupport_TryGetEnvironmentIssue_ReturnsFalseForHealthyStatus()
    {
        Assert.False(WslInstallSupport.TryGetEnvironmentIssue(
            "Default Distribution: LuckyNemoGateway\nDefault Version: 2\n",
            out var message));
    ...
    public async Task PreflightWsl_SucceedsWhenStatusOutputIsHealthy()
    {
        var commands = new FakeCommandRunner(args =>
        {
            if (args is ["--version"])
                return Ok("WSL version: 2.7.3.0\n");
            if (args is ["--status"])
                return Ok("Default Distribution: LuckyNemoGateway\nDefault Version: 2\n");
            return Ok();
        });
        var ctx = CreateContext(commands: commands);
    ...
    {
        Assert.True(WslConfig.IsValidLinuxUserName("luckynemo"));
        Assert.True(WslConfig.IsValidLinuxUserName("_luckynemo"));
        Assert.True(WslConfig.IsValidLinuxUserName("luckynemo-user_1"));
    }
    ...
    public void ConfigureGateway_AddsDevicePairPublicUrlForLoopbackGateway()
    {
        ...
        Assert.Contains(
            "luckynemo config set plugins.entries.device-pair.config.publicUrl 'http://127.0.0.1:18789'",
            commands);
    }
    ...
    public void ConfigureGateway_EnablesDevicePairPluginForLoopbackGateway()
    {
        ...
        Assert.Contains(
            "luckynemo config set plugins.entries.device-pair.enabled true",
            commands);
    }
    ...
    public void ConfigureGateway_TailscaleServeIsRootOwnedAndKeepsTailscaleAuthOptIn()
    {
        ...
        Assert.Contains("luckynemo config set gateway.tailscale.mode off", commands);
        Assert.DoesNotContain("gateway.tailscale.mode serve", commands);
        Assert.DoesNotContain("gateway.tailscale.resetOnExit", commands);
        Assert.Contains("luckynemo config set gateway.auth.allowTailscale false", commands);
    ...
    public void ConfigureGateway_EnablesTailscaleIdentityAuthOnlyWhenRequested()
    {
        ...
        Assert.Contains("luckynemo config set gateway.auth.allowTailscale true", commands);
    }
    ...
    public async Task TailscaleTransportWithoutIdentityTrust_PreservesTokenAndDeviceCredentialsForPairing()
    {
        ...
        Assert.Equal("Tailscale (LuckyNemoGateway)", record.FriendlyName);
    ...
    public void TailscalePolicy_ParsesAuthorizationUrlsAndOnlyAcceptsGatewayServeProxy()
    {
        ...
        const string funnelStatus = """
            {
              "AllowFunnel": { "luckynemo.example.ts.net:443": true },
              "Web": {
                "luckynemo.example.ts.net:443": {
                  "Handlers": {
                    "/": { "Proxy": "http://127.0.0.1:18789" }
    ...
    public void ConfigureGateway_DoesNotEnableDevicePairWhenNoPublicUrlAvailable()
    {
        ...
        Assert.DoesNotContain(
            "luckynemo config set plugins.entries.device-pair.enabled",
            commands);
    }
    ...
    public void ConfigureGateway_RespectsExplicitDevicePairEnabledOverride()
    {
        ...
        Assert.Contains(
            "luckynemo config set plugins.entries.device-pair.enabled 'false'",
            commands);
        Assert.DoesNotContain(
            "luckynemo config set plugins.entries.device-pair.enabled true",
            commands);
    ...
    public void ConfigureGateway_DoesNotOverrideExplicitDevicePairPublicUrl()
    {
        ...
        Assert.DoesNotContain("'http://127.0.0.1:18789'", commands);
        Assert.Contains(
            "luckynemo config set plugins.entries.device-pair.config.publicUrl 'https://gateway.example.test'",
            commands);
    }
    ...
    public void ConfigureGateway_QuotesExtraConfigValueWithEmbeddedSingleQuote()
    {
        ...
        Assert.Contains(
            "luckynemo config set gateway.custom.note 'a'\\''b'",
            commands);
    }
    ...
    {
        // Each `luckynemo config set` pays a cold Node start inside WSL. As more keys are
    ...
    public void ComputeConfigurationTimeout_NeverBelowFloor()
    {
        ...
        var timeout = ConfigureGatewayStep.ComputeConfigurationTimeout(
            "luckynemo config set gateway.mode local");
    ...
    public void BuildReplacementSummary_LocalGatewayAndDistro_MentionsReplacement()
    {
        ...
        Assert.Contains("WSL distro 'LuckyNemo' will be deleted and recreated", summary);
    ...
    public void TryGetExistingKeepalive_ReturnsFalseForCorruptMarker()
    {
        ...
        var result = StartKeepaliveStep.TryGetExistingKeepalive(markerPath, "LuckyNemoGateway", out var pid);
    ...
    {
        var script = WindowsNodeBootstrapContextStep.BuildApplyScript("/home/luckynemo/.luckynemo/workspace");

        Assert.Contains("set -o pipefail", script);
        Assert.Contains("workspace='/home/luckynemo/.luckynemo/workspace'", script);
        ...
        Assert.Contains("mktemp \"$workspace/.AGENTS.md.luckynemo.XXXXXX\"", script);
        ...
        Assert.DoesNotContain("luckynemo setup", script);
        Assert.DoesNotContain("luckynemo config get", script);
    ...
    {
        var script = WindowsNodeBootstrapContextStep.BuildRollbackScript("/home/luckynemo/.luckynemo/workspace");
    ...
    {
        var script = WindowsNodeBootstrapContextStep.BuildApplyScript("/home/o'brien/.luckynemo/workspace");

        Assert.Contains("workspace='/home/o'\\''brien/.luckynemo/workspace'", script);
    }

    [Fact]
    public void WindowsNodeContext_BuildRollbackScript_QuotesWorkspacePathWithEmbeddedSingleQuote()
    {
        var script = WindowsNodeBootstrapContextStep.BuildRollbackScript("/home/o'brien/.luckynemo/workspace");

        Assert.Contains("workspace='/home/o'\\''brien/.luckynemo/workspace'", script);
    }

    [Theory]
    [InlineData("/home/luckynemo/.luckynemo/workspace", "/home/luckynemo", "/home/luckynemo/.luckynemo/workspace")]
    [InlineData("~", "/home/luckynemo", "/home/luckynemo")]
    [InlineData("~/.luckynemo/custom workspace", "/home/luckynemo", "/home/luckynemo/.luckynemo/custom workspace")]
    [InlineData("relative/path", "/home/luckynemo", "/home/luckynemo/relative/path")]
    [InlineData("", "/home/luckynemo", "/home/luckynemo/.luckynemo/workspace")]
    [InlineData("null", "/home/luckynemo", "/home/luckynemo/.luckynemo/workspace")]
    [InlineData("undefined", "/home/luckynemo", "/home/luckynemo/.luckynemo/workspace")]
    [InlineData("/abs/path", "/home/luckynemo/", "/abs/path")]
    [InlineData("~/x", "/home/luckynemo/", "/home/luckynemo/x")]
    ...
    [Theory]
    [InlineData("\"/home/luckynemo/.luckynemo/workspace\"\n", "/home/luckynemo/.luckynemo/workspace")]
    [InlineData("Config warnings:\n- plugins.entries.device-pair\n\"/home/luckynemo/.luckynemo/workspace\"\n", "/home/luckynemo/.luckynemo/workspace")]
    [InlineData("\"~/.luckynemo/workspace\"\n", "~/.luckynemo/workspace")]
    [InlineData("/home/luckynemo/.luckynemo/workspace\n", "/home/luckynemo/.luckynemo/workspace")]
    [InlineData("null\n", null)]
    [InlineData("", null)]
    ...
    [Theory]
    [InlineData("[{\"id\":\"main\",\"workspace\":\"/home/luckynemo/main\",\"isDefault\":true}]", "/home/luckynemo/main")]
    [InlineData("Warning\n[\n  {\"id\":\"other\",\"workspace\":\"/home/luckynemo/other\",\"isDefault\":false},\n  {\"id\":\"primary\",\"workspace\":\"/home/luckynemo/primary\",\"isDefault\":true}\n]\n", "/home/luckynemo/primary")]
    ...
    public async Task WindowsNodeContext_Execute_RunsInWslAsConfiguredUserAndResolvesWorkspace()
    {
        var commands = new FakeCommandRunner(
            ...
            (_, command, _) =>
            {
                if (command.Contains("getent passwd"))
                    return Ok("/home/luckynemo\n");
                if (command.Contains("luckynemo agents list --json"))
                    return Ok(AgentsListJson("/home/luckynemo/.luckynemo/workspace"));
                if (command.Contains("luckynemo setup"))
                    return Ok("");
                if (command.Contains("luckynemo config get agents.defaults.workspace"))
                    return Ok("\"~/.luckynemo/workspace\"\n");
                if (command.Contains("WINDOWS_NODE_CONTEXT_READY"))
                    return Ok(string.Join("\n",
                        "WINDOWS_NODE_CONTEXT_BOOTSTRAP_FALLBACK:/home/luckynemo/.luckynemo/workspace/AGENTS.md",
                        "WINDOWS_NODE_CONTEXT_WORKSPACE:/home/luckynemo/.luckynemo/workspace",
        ...
        {
            Assert.Equal("LuckyNemoGateway", c.DistroName);
            Assert.Equal("luckynemo", c.User);
        });
        Assert.Contains("getent passwd", commands.WslCalls[0].Command);
        Assert.Contains("luckynemo agents list --json", commands.WslCalls[1].Command);
        Assert.Contains("luckynemo config get agents.defaults.workspace", commands.WslCalls[2].Command);
        Assert.Contains("luckynemo setup --help", commands.WslCalls[3].Command);
        Assert.Contains("luckynemo setup --baseline --workspace '/home/luckynemo/.luckynemo/workspace'", commands.WslCalls[3].Command);
        Assert.Contains("luckynemo setup --workspace '/home/luckynemo/.luckynemo/workspace'", commands.WslCalls[3].Command);
        Assert.Contains("workspace='/home/luckynemo/.luckynemo/workspace'", commands.WslCalls[4].Command);
        // getent uses $(id -un) command-substitution and no $vars, so argv path is safe.
        Assert.False(commands.WslCalls[0].InputViaStdin);
        // agents list + config get + luckynemo setup scripts reference $PATH via WslPathPrefix,
        ...
        Assert.Contains(state.Targets, target =>
            target.DistroName == "LuckyNemoGateway" &&
            target.User == "luckynemo" &&
            target.WorkspacePath == "/home/luckynemo/.luckynemo/workspace");
    }
    ...
    public async Task WindowsNodeContext_Execute_ResolvesRelativeConfiguredWorkspaceFromGatewayUserHome()
    {
        var commands = new FakeCommandRunner(
            ...
            (_, command, _) =>
            {
                if (command.Contains("getent passwd"))
                    return Ok("/home/luckynemo\n");
                if (command.Contains("luckynemo agents list --json"))
                    return Ok(AgentsListJson("/home/luckynemo/relative/workspace"));
                if (command.Contains("luckynemo setup"))
                    return Ok("");
                if (command.Contains("luckynemo config get agents.defaults.workspace"))
                    return Ok("\"relative/workspace\"\n");
                if (command.Contains("workspace='/home/luckynemo/relative/workspace'"))
        ...
        Assert.Contains(commands.WslCalls,
            c => c.Command.Contains("luckynemo setup --workspace '/home/luckynemo/relative/workspace'"));
        Assert.Contains(commands.WslCalls,
            c => c.Command.Contains("workspace='/home/luckynemo/relative/workspace'"));
    ...
    public async Task WindowsNodeContext_Execute_UsesDefaultOnlyWhenWorkspaceKeyIsAbsent()
    {
            ...
            (_, command, _) =>
            {
                ...
                if (command.Contains("luckynemo config get agents.defaults.workspace"))
                    return new CommandResult(
                        1,
                        "",
                        "Config path not found: agents.defaults.workspace. Run luckynemo config validate to inspect config shape.",
                        TimeSpan.Zero,
                        TimedOut: false);
                if (command.Contains("luckynemo setup --workspace '/home/luckynemo/.luckynemo/workspace'"))
                    return Ok();
                if (command.Contains("workspace='/home/luckynemo/.luckynemo/workspace'"))
                    return Ok("WINDOWS_NODE_CONTEXT_READY\n");
                return Fail($"unexpected wsl command: {command}");
            });
        var ctx = CreateContext(commands: commands);
        ...
        Assert.True(result.IsSuccess, result.Message);
        Assert.Contains(commands.WslCalls,
            c => c.Command.Contains("luckynemo setup --workspace '/home/luckynemo/.luckynemo/workspace'"));
    }
    ...
    public async Task WindowsNodeContext_Execute_DoesNotPersistDefaultWhenWorkspaceLookupFails()
    {
        ...
        Assert.Equal(StepOutcome.Failed, result.Outcome);
        Assert.Contains("Could not resolve LuckyNemo default workspace path", result.Message);
        Assert.DoesNotContain(commands.WslCalls, c => c.Command.Contains("luckynemo setup"));
    }
    ...
    public async Task WindowsNodeContext_Execute_UsesEffectiveDefaultAgentWorkspaceWithoutRewritingDefaults()
    {
        var commands = new FakeCommandRunner(
            ...
            (_, command, _) =>
            {
                if (command.Contains("getent passwd"))
                    return Ok("/home/luckynemo\n");
                if (command.Contains("luckynemo agents list --json"))
                    return Ok(AgentsListJson("/home/luckynemo/main-agent"));
                if (command.Contains("luckynemo config get agents.defaults.workspace"))
                    return Ok("\"~/.luckynemo/workspace\"\n");
                if (command.Contains("workspace='/home/luckynemo/main-agent'"))
                    return Ok("WINDOWS_NODE_CONTEXT_READY\n");
                return Fail($"unexpected wsl command: {command}");
            });
        var ctx = CreateContext(commands: commands);
        ...
        Assert.True(result.IsSuccess, result.Message);
        Assert.DoesNotContain(commands.WslCalls, c => c.Command.Contains("luckynemo setup"));
        Assert.Contains(commands.WslCalls, c => c.Command.Contains("workspace='/home/luckynemo/main-agent'"));
    }
    ...
    public async Task WindowsNodeContext_Execute_FailsWhenEffectiveAgentWorkspaceLookupFails()
    {
        ...
        var ctx = CreateContext(commands: commands);
        var priorTarget = new WindowsNodeContextTarget("prior-distro", "luckynemo", "/prior/workspace");
        ...
        Assert.Equal(StepOutcome.Failed, result.Outcome);
        Assert.Contains("Could not resolve LuckyNemo agent workspace path", result.Message);
        Assert.DoesNotContain(commands.WslCalls, c => c.Command.Contains("luckynemo config get"));
        Assert.DoesNotContain(commands.WslCalls, c => c.Command.Contains("luckynemo setup"));
    ...
    public async Task WindowsNodeContext_Execute_UsesExplicitWorkspaceOverride()
    {
        var commands = new FakeCommandRunner(
            ...
            (_, command, _) =>
            {
                if (command.Contains("getent passwd"))
                    return Ok("/home/luckynemo\n");
                if (command.Contains("luckynemo setup --workspace"))
                    return Ok("");
        ...
        Assert.Contains(commands.WslCalls, c => c.Command.Contains("luckynemo setup --workspace '/custom/abs/path'"));
    }
    ...
    public async Task WindowsNodeContext_Execute_OverrideWithTilde_ExpandsBeforePassingToSetup()
    {
        ...
        // absolute path goes to `luckynemo setup --workspace` and the apply script.
        var commands = new FakeCommandRunner(
            _ => Fail("unexpected RunAsync"),
            (_, command, _) =>
            {
                ...
                if (command.Contains("luckynemo setup --workspace"))
                    return Ok("");
                if (command.Contains("workspace='/home/luckynemo/custom-ws'"))
                    return Ok("WINDOWS_NODE_CONTEXT_READY\n");
                return Fail($"unexpected wsl command: {command}");
            });
        var ctx = CreateContext(new SetupConfig
        ...
        Assert.True(result.IsSuccess, result.Message);
        Assert.Contains(commands.WslCalls,
            c => c.Command.Contains("luckynemo setup --workspace '/home/luckynemo/custom-ws'"));
    ...
    public async Task WindowsNodeContext_Rollback_TreatsMissingRecordedDistroAsCleaned()
    {
        ...
        await WindowsNodeBootstrapContextStep.RecordAppliedTargetAsync(
            ctx,
            new WindowsNodeContextTarget("missing-distro", "luckynemo", "/recorded/workspace"),
            CancellationToken.None);
    ...
    public async Task WindowsNodeContext_Rollback_CleansLegacyEffectiveWorkspaceWithoutStateFile()
    {
        var commands = new FakeCommandRunner(
            ...
            (_, command, _) =>
            {
                if (command.Contains("getent passwd"))
                    return Ok("/home/luckynemo\n");
                if (command.Contains("luckynemo agents list --json"))
                    return Ok(AgentsListJson("/home/luckynemo/legacy-main"));
                if (command.Contains("WINDOWS_NODE_CONTEXT_REMOVED"))
                    return Ok("WINDOWS_NODE_CONTEXT_REMOVED\n");
                return Fail($"unexpected wsl command: {command}");
            });
        ...
        Assert.Contains(commands.WslCalls, call => call.Command.Contains("luckynemo agents list --json"));
        Assert.Contains(commands.WslCalls, call => call.Command.Contains("workspace='/home/luckynemo/legacy-main'"));
    }
    ...
    public async Task AuthorizeTailscale_AuthKeyUsesTransientEnvironmentAndDerivesMagicDnsName()
    {
        ...
        var commands = new FakeCommandRunner(
            ...
            (_, command, _) => command.Contains("tailscale status --json")
                ? Ok("""{"BackendState":"Running","Self":{"DNSName":"luckynemo.tailnet.ts.net"}}""")
                : Ok());
        var ctx = CreateContext(config, commands);
        ...
        Assert.True(result.IsSuccess);
        Assert.Equal("luckynemo.tailnet.ts.net", ctx.TailscaleDnsName);
    ...
    public async Task FinalizeTailscaleServe_RootOwnsServeAndPublishesOnlyWssAfterHealthCheck()
    {
        const string expectedStatus = """
            { "Web": { "luckynemo.tailnet.ts.net:443": { "Handlers": { "/": { "Proxy": "http://127.0.0.1:18789" } } } } }
            """;
        ...
        var ctx = CreateContext(config, commands);
        ctx.DistroName = "test-distro";
        ctx.TailscaleDnsName = "luckynemo.tailnet.ts.net";
        ...
        Assert.True(result.IsSuccess, result.Message);
        Assert.Equal("wss://luckynemo.tailnet.ts.net", ctx.GatewayUrl);
        Assert.Equal("https://luckynemo.tailnet.ts.net", probe.Endpoint!.AbsoluteUri.TrimEnd('/'));
        ...
        Assert.DoesNotContain(commands.WslCalls, call => call.Command.Contains("tailscale serve", StringComparison.Ordinal) && call.User == "luckynemo");
    }
    ...
    public async Task TailscaleHealthFailure_RollsBackServeAndDaemonBeforePairing()
    {
        ...
        Assert.NotEqual("wss://luckynemo.tailnet.ts.net", ctx.GatewayUrl);
    ...
    public async Task FinalizeTailscaleServe_RejectsFunnelAndRollbackResetsServeBeforeDistroCleanup()
    {
        const string funnelStatus = """
            {
              "AllowFunnel": { "luckynemo.tailnet.ts.net:443": true },
              "Web": { "luckynemo.tailnet.ts.net:443": { "Handlers": { "/": { "Proxy": "http://127.0.0.1:18789" } } } }
            }
            """;
        var commands = new FakeCommandRunner(
