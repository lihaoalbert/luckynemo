...
public sealed class TrayExecutableResolverTests : IDisposable
{
    ...
    {
        var setupEngineDir = Path.Combine(_tempDir, "LuckyNemoTray", "SetupEngine");
        Directory.CreateDirectory(setupEngineDir);

        var expectedTrayPath = Path.Combine(_tempDir, "LuckyNemoTray", "LuckyNemo.Tray.WinUI.exe");
    ...
    {
        var trayPath = Path.Combine(_tempDir, "LuckyNemo Tray", "LuckyNemo.Tray.WinUI.exe");
        ...
        Assert.Contains("LuckyNemo Companion", psi.ArgumentList);
