...
public class WizardDefaultAnswerMatrixTests
{
    ...
    {
        var configPath = Path.Combine(RepositoryRoot(), "src", "LuckyNemo.SetupEngine", "default-config.json");
    ...
    {
        if (Environment.GetEnvironmentVariable("LUCKYNEMO_REPO_ROOT") is { Length: > 0 } configured)
            return configured;

        var directory = AppContext.BaseDirectory;
        while (!string.IsNullOrWhiteSpace(directory))
        {
            if (File.Exists(Path.Combine(directory, "src", "LuckyNemo.SetupEngine", "default-config.json")))
                return directory;
