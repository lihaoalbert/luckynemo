...
public class SetupConfigTests : IDisposable
{
    ...
    public void Defaults_AreReasonable()
    {
        ...
        Assert.Equal("LuckyNemoGateway", config.DistroName);
    ...
    public void TailscaleConfig_NormalizesHostnameAndRejectsIncompatibleGatewaySettings()
    {
        var config = new SetupConfig
        {
            ...
            Tailscale = new TailscaleConfig { Enabled = true, Hostname = "LuckyNemo !!! Gateway" }
        };

        Assert.Equal("luckynemo-gateway", config.Tailscale.EffectiveHostname);
    ...
    public void TailscaleAuthKey_IsRuntimeOnlyAndStatusParsesMagicDns()
    {
        ...
        Assert.True(TailscaleSetupPolicy.TryParseStatus("""{"BackendState":"Running","Self":{"DNSName":"luckynemo.tailnet.ts.net."}}""", out var status));
        Assert.True(status.IsRunning);
        Assert.Equal("tailnet.ts.net", TailscaleSetupPolicy.GetTailnetDnsSuffix(status.DnsName));
        Assert.Equal("luckynemo-gateway", TailscaleSetupPolicy.NormalizeHostname("luckynemo_gateway", "ignored"));
    ...
    public void FromEnvironment_OverridesDefaults()
    {
        ...
        var prevDistro = Environment.GetEnvironmentVariable("LUCKYNEMO_SETUP_DISTRO");
        var prevPort = Environment.GetEnvironmentVariable("LUCKYNEMO_SETUP_PORT");
        var prevHeadless = Environment.GetEnvironmentVariable("LUCKYNEMO_SETUP_HEADLESS");
        var prevTrustTailscaleAuth = Environment.GetEnvironmentVariable("LUCKYNEMO_SETUP_TAILSCALE_TRUST_AUTH");
        try
        {
            Environment.SetEnvironmentVariable("LUCKYNEMO_SETUP_DISTRO", "EnvDistro");
            Environment.SetEnvironmentVariable("LUCKYNEMO_SETUP_PORT", "9876");
            Environment.SetEnvironmentVariable("LUCKYNEMO_SETUP_HEADLESS", "true");
            Environment.SetEnvironmentVariable("LUCKYNEMO_SETUP_TAILSCALE_TRUST_AUTH", "true");
        ...
        {
            Environment.SetEnvironmentVariable("LUCKYNEMO_SETUP_DISTRO", prevDistro);
            Environment.SetEnvironmentVariable("LUCKYNEMO_SETUP_PORT", prevPort);
            Environment.SetEnvironmentVariable("LUCKYNEMO_SETUP_HEADLESS", prevHeadless);
            Environment.SetEnvironmentVariable("LUCKYNEMO_SETUP_TAILSCALE_TRUST_AUTH", prevTrustTailscaleAuth);
        }
    }
    ...
    public void FromEnvironment_InvalidPort_KeepsDefault()
    {
        ...
        try
        {
            Environment.SetEnvironmentVariable("LUCKYNEMO_SETUP_PORT", "notanumber");
            var config = SetupConfig.FromEnvironment();
    ...
    {
        var oldData = Environment.GetEnvironmentVariable("LUCKYNEMO_TRAY_DATA_DIR");
        var oldLocalData = Environment.GetEnvironmentVariable("LUCKYNEMO_TRAY_LOCAL_DATA_DIR");
        try
        {
            Environment.SetEnvironmentVariable("LUCKYNEMO_TRAY_DATA_DIR", Path.Combine(_tempDir, "roaming"));
            Environment.SetEnvironmentVariable("LUCKYNEMO_TRAY_LOCAL_DATA_DIR", Path.Combine(_tempDir, "local"));
            var config = new SetupConfig
            {
        ...
        {
            Environment.SetEnvironmentVariable("LUCKYNEMO_TRAY_DATA_DIR", oldData);
            Environment.SetEnvironmentVariable("LUCKYNEMO_TRAY_LOCAL_DATA_DIR", oldLocalData);
        }
    }
    ...
    public void SetupReviewSummary_UsesDiscoveredTailnetSuffix()
    {
        ...
        Assert.Equal("wss://luckynemo-test.example.ts.net", summary.GatewayEndpoint);
        ...
        Assert.Equal("LuckyNemoGateway · wss://luckynemo-test.example.ts.net", summary.CompletionGatewaySummary);
    }
    ...
    public void WslConfig_Defaults()
    {
        ...
        Assert.Equal("luckynemo", wsl.User);
