...
public class SetupContextTests
{
    ...
    {
        var prev = Environment.GetEnvironmentVariable("LUCKYNEMO_TRAY_DATA_DIR");
        try
        {
            Environment.SetEnvironmentVariable("LUCKYNEMO_TRAY_DATA_DIR", @"C:\custom\data");
            var ctx = CreateContext();
            Assert.Equal(@"C:\custom\data", ctx.DataDir);
        }
        finally
        {
            Environment.SetEnvironmentVariable("LUCKYNEMO_TRAY_DATA_DIR", prev);
        }
    }
    ...
    public void Constructor_UsesAppDataWhenNoEnvVar()
    {
        ...
        try
        {
            Environment.SetEnvironmentVariable("LUCKYNEMO_TRAY_DATA_DIR", null);
            var ctx = CreateContext();
            Assert.Contains("LuckyNemoTray", ctx.DataDir);
        }
        finally
        {
            Environment.SetEnvironmentVariable("LUCKYNEMO_TRAY_DATA_DIR", prev);
    ...
    {
        var prev = Environment.GetEnvironmentVariable("LUCKYNEMO_TRAY_LOCAL_DATA_DIR");
        var prevRoot = Environment.GetEnvironmentVariable("LUCKYNEMO_TRAY_LOCALAPPDATA_DIR");
        try
        {
            Environment.SetEnvironmentVariable("LUCKYNEMO_TRAY_LOCALAPPDATA_DIR", null);
            Environment.SetEnvironmentVariable("LUCKYNEMO_TRAY_LOCAL_DATA_DIR", @"C:\custom\local");
            var ctx = CreateContext();
            Assert.Equal(@"C:\custom\local", ctx.LocalDataDir);
        }
        finally
        {
            Environment.SetEnvironmentVariable("LUCKYNEMO_TRAY_LOCAL_DATA_DIR", prev);
            Environment.SetEnvironmentVariable("LUCKYNEMO_TRAY_LOCALAPPDATA_DIR", prevRoot);
        }
    }
    ...
    {
        var prev = Environment.GetEnvironmentVariable("LUCKYNEMO_TRAY_LOCALAPPDATA_DIR");
        try
        {
            Environment.SetEnvironmentVariable("LUCKYNEMO_TRAY_LOCALAPPDATA_DIR", @"C:\custom\localappdata");
            var ctx = CreateContext();
            Assert.Equal(@"C:\custom\localappdata\LuckyNemoTray", ctx.LocalDataDir);
        }
        finally
        {
            Environment.SetEnvironmentVariable("LUCKYNEMO_TRAY_LOCALAPPDATA_DIR", prev);
        }
    }
