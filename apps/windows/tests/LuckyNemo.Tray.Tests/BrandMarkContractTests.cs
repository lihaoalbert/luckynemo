...
public sealed class BrandMarkContractTests
{
    ...
    private static string ReadTrayFile(params string[] pathParts)
        => File.ReadAllText(Path.Combine(new[] { RepoRoot, "src", "LuckyNemo.Tray.WinUI" }.Concat(pathParts).ToArray()));
    ...
    {
        var sourceRoot = Path.Combine(RepoRoot, "src", "LuckyNemo.Tray.WinUI");
