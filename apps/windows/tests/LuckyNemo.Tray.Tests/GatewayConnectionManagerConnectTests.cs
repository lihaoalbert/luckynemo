...
public sealed class GatewayConnectionManagerConnectTests : IDisposable
{
    ...
    {
        _tempDir = Path.Combine(Path.GetTempPath(), "luckynemo-connmgr-test-" + Guid.NewGuid().ToString("N"));
    ...
    {
        public LuckyNemoGatewayClient DataClient { get; } = new(gatewayUrl, "mock-token", NullLogger.Instance);
