...
/// <c>LuckyNemo.Tray.Tests</c> is a pure net10.0 project that doesn't
...
public sealed class FluentIconCatalogTests
{
    private static readonly string[] ExpectedConstants =
    {
        ...
        "ChevronR", "Check",
        // Diagnostics surface (see src/LuckyNemo.Tray.WinUI/Pages/DebugPage.xaml).
        "Bug", "Briefcase", "Folder", "Copy", "Document", "Refresh", "Reset", "Clear", "Develop", "Telemetry",
        // Workspace surface (see src/LuckyNemo.Tray.WinUI/Pages/WorkspacePage.xaml).
        "Workspace",
    };
    ...
    private static string ReadCatalogSource()
    {
        var path = Path.Combine(
            ...
            "src", "LuckyNemo.Tray.WinUI", "Helpers", "FluentIconCatalog.cs");
    ...
    public void NativeWinUiSources_DoNotHardcodeSegoeFluentIcons()
    {
        ...
        var sourceRoots = new[]
        {
            Path.Combine(repositoryRoot, "src", "LuckyNemo.Tray.WinUI"),
            Path.Combine(repositoryRoot, "src", "LuckyNemo.SetupEngine.UI"),
        };
...
public sealed class TrayMenuPopupCompositionTests
{
    ...
    private static string ReadStateBuilder()
    {
        var path = Path.Combine(
            TestRepositoryPaths.GetRepositoryRoot(),
            "src", "LuckyNemo.Tray.WinUI", "Services", "TrayMenuStateBuilder.cs");
        return File.ReadAllText(path);
    }
