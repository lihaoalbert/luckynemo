...
/// <summary>
/// Unit tests for <see cref="LuckyNemoChatDataProvider"/>'s
...
public class FlattenedToolOutputDetectionTests
{
    ...
    public void DetectsExecTerminatorMarkers(string text)
    {
        ...
        Assert.True(LuckyNemoChatDataProvider.LooksLikeFlattenedToolOutput(text),
            $"Expected to detect: {text}");
    }

    [Theory]
    [InlineData("\\\\wsl.localhost\\Ubuntu\\home\\user\\foo.wav -rw-r--r-- 1 user user 12K May 7 12:00 foo")]
    [InlineData("/usr/lib/node_modules/luckynemo/dist/foo.js exists")]
    ...
    [InlineData("/tmp/luckynemo-foo.lock present, age 14m, owner regisb     ")]
    ...
    [Theory]
    [InlineData("LuckyNemo 2026.4.23 (a979721) — iMessage green bubble energy, but for everyone.\nUsage: luckynemo nodes invoke")]
    [InlineData("LuckyNemo v1.5.0 — gateway test build")]
    [InlineData("luckynemo nodes invoke --node main --command canvas.eval")]
    public void DetectsLuckyNemoCliBanner(string text)
    {
        text = text.PadRight(50, '.');
        Assert.True(LuckyNemoChatDataProvider.LooksLikeFlattenedToolOutput(text),
            $"Expected to detect: {text}");
    ...
    {
        Assert.False(LuckyNemoChatDataProvider.LooksLikeFlattenedToolOutput(text),
    ...
    public void DoesNotMatchShortText()
    {
        ...
        Assert.False(LuckyNemoChatDataProvider.LooksLikeFlattenedToolOutput("Process exited"));
    }
    ...
    {
        Assert.Equal(expected, LuckyNemoChatDataProvider.LooksLikeSystemControlNote(text));
    }
    ...
    {
        Assert.True(LuckyNemoChatDataProvider.LooksLikeSystemControlNote(text));
    }
    ...
    {
        Assert.False(LuckyNemoChatDataProvider.LooksLikeSystemControlNote(text));
    }
    ...
    public void TruncateChatEvent_ChatModelChangedEvent_TruncatesModelField()
    {
        ...
        var truncated = (ChatModelChangedEvent)LuckyNemoChatDataProvider.TruncateChatEvent(
    ...
    public void TruncateChatEvent_ChatIntentEvent_TruncatesIntentField()
    {
        ...
        var truncated = (ChatIntentEvent)LuckyNemoChatDataProvider.TruncateChatEvent(
    ...
    public void TruncateChatEvent_ChatPermissionRequestEvent_TruncatesAllTextFields()
    {
        ...
        var truncated = (ChatPermissionRequestEvent)LuckyNemoChatDataProvider.TruncateChatEvent(
    ...
    public void TruncateChatEvent_ChatToolStartEvent_TruncatesLabelAndToolName()
    {
        ...
        var truncated = (ChatToolStartEvent)LuckyNemoChatDataProvider.TruncateChatEvent(
    ...
    public void TruncateChatEvent_AllRenderedTextFieldsRespectByteCap()
    {
        ...
        AssertCapped(((ChatUserMessageEvent)LuckyNemoChatDataProvider.TruncateChatEvent(new ChatUserMessageEvent(huge))).Text);
        AssertCapped(((ChatThinkingEvent)LuckyNemoChatDataProvider.TruncateChatEvent(new ChatThinkingEvent(huge))).Text);
        AssertCapped(((ChatReasoningEvent)LuckyNemoChatDataProvider.TruncateChatEvent(new ChatReasoningEvent(huge))).Text);
        AssertCapped(((ChatReasoningDeltaEvent)LuckyNemoChatDataProvider.TruncateChatEvent(new ChatReasoningDeltaEvent(huge))).Text);

        var message = (ChatMessageEvent)LuckyNemoChatDataProvider.TruncateChatEvent(new ChatMessageEvent(huge, huge));
        AssertCapped(message.Text);
        AssertCapped(message.ReasoningText!);

        AssertCapped(((ChatMessageDeltaEvent)LuckyNemoChatDataProvider.TruncateChatEvent(new ChatMessageDeltaEvent(huge))).Text);
        AssertCapped(((ChatToolStartEvent)LuckyNemoChatDataProvider.TruncateChatEvent(new ChatToolStartEvent(huge, huge))).Text);
        AssertCapped(((ChatToolStartEvent)LuckyNemoChatDataProvider.TruncateChatEvent(new ChatToolStartEvent(huge, huge))).ToolName);
        AssertCapped(((ChatToolOutputEvent)LuckyNemoChatDataProvider.TruncateChatEvent(new ChatToolOutputEvent(huge))).Text);
        AssertCapped(((ChatToolErrorEvent)LuckyNemoChatDataProvider.TruncateChatEvent(new ChatToolErrorEvent(huge))).Text);
        AssertCapped(((ChatStatusEvent)LuckyNemoChatDataProvider.TruncateChatEvent(new ChatStatusEvent(huge, ChatTone.Dim))).Text);
        AssertCapped(((ChatErrorEvent)LuckyNemoChatDataProvider.TruncateChatEvent(new ChatErrorEvent(huge))).Text);
        AssertCapped(((ChatRestoredEvent)LuckyNemoChatDataProvider.TruncateChatEvent(new ChatRestoredEvent(huge))).Text);
        AssertCapped(((ChatRawEvent)LuckyNemoChatDataProvider.TruncateChatEvent(new ChatRawEvent("raw", huge))).Text!);
        AssertCapped(((ChatModelChangedEvent)LuckyNemoChatDataProvider.TruncateChatEvent(new ChatModelChangedEvent(huge))).Model);
        AssertCapped(((ChatIntentEvent)LuckyNemoChatDataProvider.TruncateChatEvent(new ChatIntentEvent(huge))).Intent);

        var permission = (ChatPermissionRequestEvent)LuckyNemoChatDataProvider.TruncateChatEvent(
            new ChatPermissionRequestEvent("req-1", huge, huge, huge));
        ...
        static void AssertCapped(string text)
        {
            ...
            Assert.True(bytes <= LuckyNemoChatDataProvider.MaxEntryTextBytes,
                $"Expected <= {LuckyNemoChatDataProvider.MaxEntryTextBytes} bytes but got {bytes}.");
    ...
    [InlineData("LuckyNemo 2026.4.23 — Usage: luckynemo help", "exec")]
    ...
    {
        Assert.Equal(expected, LuckyNemoChatDataProvider.ClassifyFlattenedToolOutput(text));
    }
    ...
    public void ToolresultRoleAlwaysClassified(string text)
    {
        ...
        var kind = LuckyNemoChatDataProvider.ClassifyFlattenedToolOutput(text);
