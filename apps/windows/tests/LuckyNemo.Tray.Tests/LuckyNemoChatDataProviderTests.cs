...
public class LuckyNemoChatDataProviderTests
{
    private sealed class ChatActivityCollector : IDisposable
    {
        ...
        public ChatActivityCollector()
        {
            ...
            {
                ShouldListenTo = source => source.Name == LuckyNemoActivitySourceName.LuckyNemo.ToTelemetryName(),
    ...
    private sealed class ChatMetricCollector : IDisposable
    {
        ...
        public ChatMetricCollector()
        {
            ...
            {
                if (instrument.Meter.Name == LuckyNemoMeterName.LuckyNemo.ToTelemetryName() &&
                    instrument.Name.StartsWith("luckynemo.chat.", StringComparison.Ordinal))
                {
    ...
    private static (FakeBridge bridge, LuckyNemoChatDataProvider provider, List<ChatDataSnapshot> snapshots, List<ChatProviderNotification> notifications)
        CreateProvider(
            ...
            Action? historyFailureReservedForTesting = null)
    {
        ...
        var provider = toolMetaCachePath is null && attachmentMetaCachePath is null && lastChatStatePath is null &&
            ...
            ? new LuckyNemoChatDataProvider(bridge)
            : new LuckyNemoChatDataProvider(
                bridge,
                post: null,
    ...
    public void AuthoritativeReload_PreservesMetadataLessAndPostRequestEntries()
    {
        ...
        Assert.True(LuckyNemoChatDataProvider.ShouldPreserveLiveEntryDuringAuthoritativeReload(
            metadata: null,
        ...
        Assert.True(LuckyNemoChatDataProvider.ShouldPreserveLiveEntryDuringAuthoritativeReload(
            ...
            maxHistorySequence: 10,
            requestStartedAt));
        Assert.False(LuckyNemoChatDataProvider.ShouldPreserveLiveEntryDuringAuthoritativeReload(
            ...
            return Task.FromResult(new ChatHistoryInfo
            {
                ...
                [
                    new ChatMessageInfo
                    {
                        ...
                        LuckyNemoKind = historyCall == 1 ? null : "compaction",
                        LuckyNemoSeq = historyCall
                    }
                ]
            });
        };
    ...
    public async Task LiveCompactionMessage_PreservesStructuredMetadata()
    {
        ...
        await using (provider)
        {
            ...
            Assert.Equal("compaction", metadata.LuckyNemoKind);
    ...
    public async Task Telemetry_LocalSendAndLifecycle_EmitCorrelatedAllowlistedSpans()
    {
        ...
        Assert.Equal("local", turn.GetTagItem(LuckyNemoTelemetryTagKey.Source.ToTelemetryName()));
        Assert.Equal("success", turn.GetTagItem(LuckyNemoTelemetryTagKey.Outcome.ToTelemetryName()));
        Assert.Equal("lifecycle_end", turn.GetTagItem(LuckyNemoTelemetryTagKey.Reason.ToTelemetryName()));
    ...
    public async Task Telemetry_RemoteLifecycle_EmitsRemoteTurn()
    {
        ...
        var turn = Assert.Single(
            activities.Stopped,
            activity => activity.OperationName == ChatTelemetryTracker.TurnSpanName);
        Assert.Equal("remote", turn.GetTagItem(LuckyNemoTelemetryTagKey.Source.ToTelemetryName()));
        Assert.Equal("lifecycle_end", turn.GetTagItem(LuckyNemoTelemetryTagKey.Reason.ToTelemetryName()));
        Assert.Single(
            activities.Stopped,
            activity => activity.OperationName == ChatTelemetryTracker.ResponseWaitSpanName);
    ...
    public async Task Telemetry_LoadHistory_EmitsBoundedHistorySpan()
    {
        ...
        Assert.Equal("forced", history.GetTagItem(LuckyNemoTelemetryTagKey.Source.ToTelemetryName()));
        Assert.Equal("success", history.GetTagItem(LuckyNemoTelemetryTagKey.Outcome.ToTelemetryName()));
        Assert.Equal(
            ["luckynemo.outcome", "luckynemo.source"],
    ...
    public async Task Telemetry_AbortIntent_CompletesTurnAsCanceled()
    {
        ...
        var turn = Assert.Single(
            activities.Stopped,
            activity => activity.OperationName == ChatTelemetryTracker.TurnSpanName);
        Assert.Equal("canceled", turn.GetTagItem(LuckyNemoTelemetryTagKey.Outcome.ToTelemetryName()));
        Assert.Equal("abort_requested", turn.GetTagItem(LuckyNemoTelemetryTagKey.Reason.ToTelemetryName()));
        await provider.DisposeAsync();
    }
    ...
    public async Task Telemetry_Disconnect_CompletesOutstandingTurn()
    {
        ...
        var turn = Assert.Single(
            ...
            activity => activity.OperationName == ChatTelemetryTracker.TurnSpanName);
        Assert.Equal("canceled", turn.GetTagItem(LuckyNemoTelemetryTagKey.Outcome.ToTelemetryName()));
        Assert.Equal("disconnected", turn.GetTagItem(LuckyNemoTelemetryTagKey.Reason.ToTelemetryName()));
        await provider.DisposeAsync();
    }
    ...
    public async Task Telemetry_Reset_CompletesQueuedAndActiveTurns()
    {
        ...
        {
            Assert.Equal("canceled", turn.GetTagItem(LuckyNemoTelemetryTagKey.Outcome.ToTelemetryName()));
            Assert.Equal("reset", turn.GetTagItem(LuckyNemoTelemetryTagKey.Reason.ToTelemetryName()));
        });
        await provider.DisposeAsync();
    }
    ...
    public async Task SendMessageAsync_AckDoesNotQueueStaleSnapshotThatCanResurrectClearedQueuedMessage()
    {
        ...
        var provider = new LuckyNemoChatDataProvider(bridge, post: posted.Add);
        var snapshots = new List<ChatDataSnapshot>();
        provider.Changed += (_, e) => snapshots.Add(e.Snapshot);
        await provider.LoadAsync();
        posted.Clear();
    ...
    public async Task ChatMessageReceived_UserEcho_AttachesGatewayIdentityToExactLocalRow()
    {
        ...
        Assert.Equal(7, meta.LuckyNemoSeq);
    ...
    public async Task ChatMessageReceived_UserEcho_ReconcilesExistingQueuedPromotionAfterPendingEchoExpires()
    {
        ...
        Assert.Null(beforeMeta[promoted.Id].LuckyNemoSeq);
        snapshots.Clear();
        ...
        Assert.Equal(9, afterMeta[user.Id].LuckyNemoSeq);
    ...
    public async Task ChatMessageReceived_UserEcho_DoesNotReconcileStaleIdenticalConfirmedRemoteMessage()
    {
        ...
        Assert.Contains(users, user =>
            ...
            afterMeta[user.Id].LuckyNemoSeq is null);
        Assert.Contains(users, user =>
            afterMeta[user.Id].GatewayMessageId == "remote-later" &&
            afterMeta[user.Id].LuckyNemoSeq == 50 &&
            !afterMeta[user.Id].IsLocalQueuedSend);
    }
    ...
    [Fact]
    public async Task LoadHistoryAsync_OrdersMessagesByLuckyNemoSequenceBeforeTimestamp()
    {
        var (bridge, provider, snapshots, _) = CreateProvider(new[] { MainSession() });
        bridge.HistoryBehavior = _ => Task.FromResult(new ChatHistoryInfo
        {
            ...
            Messages = new[]
            {
                new ChatMessageInfo { SessionKey = "main", Role = "user", Text = "c", Ts = 1_000, LuckyNemoSeq = 3 },
                new ChatMessageInfo { SessionKey = "main", Role = "user", Text = "a", Ts = 3_000, LuckyNemoSeq = 1 },
                new ChatMessageInfo { SessionKey = "main", Role = "user", Text = "b", Ts = 2_000, LuckyNemoSeq = 2 },
            }
        });
        await provider.LoadAsync();
        snapshots.Clear();
    ...
    [Fact]
    public async Task LoadHistoryAsync_KeepsGatewayOrderWhenOnlySomeRowsHaveLuckyNemoSequence()
    {
        var (bridge, provider, snapshots, _) = CreateProvider(new[] { MainSession() });
        bridge.HistoryBehavior = _ => Task.FromResult(new ChatHistoryInfo
        {
            ...
            Messages = new[]
            {
                ...
                new ChatMessageInfo { SessionKey = "main", Role = "user", Text = "t", Ts = 500, LuckyNemoSeq = 1 },
                new ChatMessageInfo { SessionKey = "main", Role = "user", Text = "u", Ts = 600, LuckyNemoSeq = 2 },
            }
        });
        await provider.LoadAsync();
        snapshots.Clear();
    ...
    public async Task PostDelegate_IsUsedForChangedAndNotifications()
    {
        ...
        var provider = new LuckyNemoChatDataProvider(bridge, post: a => queued.Add(a));
        var snapshots = new List<ChatDataSnapshot>();
        provider.Changed += (_, e) => snapshots.Add(e.Snapshot);
    ...
    public async Task Reconnect_FromConnectingToConnected_DoesNotReload()
    {
        ...
        var provider = new LuckyNemoChatDataProvider(bridge);
        bridge.HistoryBehavior = _ => { historyCalls++; return Task.FromResult(new ChatHistoryInfo { SessionKey = "main" }); };
        await provider.LoadAsync();
        await provider.LoadHistoryAsync("main");
    ...
    public async Task Reconnect_IgnoresHistoryResponseFromPreviousConnection()
    {
        ...
        Assert.Single(historySpans, activity =>
            Equals("canceled", activity.GetTagItem(LuckyNemoTelemetryTagKey.Outcome.ToTelemetryName())));
        Assert.Single(historySpans, activity =>
            Equals("success", activity.GetTagItem(LuckyNemoTelemetryTagKey.Outcome.ToTelemetryName())));
        Assert.All(historySpans, activity =>
            Assert.Equal("initial", activity.GetTagItem(LuckyNemoTelemetryTagKey.Source.ToTelemetryName())));
        await provider.DisposeAsync();
    }
    ...
    public async Task Disconnect_CancelsInFlightHistoryWithoutPublishingStaleContent()
    {
        ...
        var history = Assert.Single(
            activities.Stopped,
            activity => activity.OperationName == ChatTelemetryTracker.HistoryLoadSpanName);
        Assert.Equal("canceled", history.GetTagItem(LuckyNemoTelemetryTagKey.Outcome.ToTelemetryName()));
        Assert.Null(history.GetTagItem(LuckyNemoTelemetryTagKey.ErrorType.ToTelemetryName()));
        await provider.DisposeAsync();
    }
    ...
    public async Task Disconnect_DropsHistoryDeliveryQueuedAfterNewerStatusSnapshot()
    {
        ...
        var provider = new LuckyNemoChatDataProvider(
            bridge,
    ...
    public async Task OnChatMessageReceived_OversizedContent_IsTruncated()
    {
        ...
        Assert.True(bytes <= LuckyNemoChatDataProvider.MaxEntryTextBytes,
            $"entry was {bytes} bytes; cap is {LuckyNemoChatDataProvider.MaxEntryTextBytes}");
    ...
    public void TruncateForChatEntry_BelowCap_ReturnsInputUnchanged()
    {
        ...
        Assert.Same(small, LuckyNemoChatDataProvider.TruncateForChatEntry(small));
    }

    [Fact]
    public void TruncateForChatEntry_AboveCap_RespectsByteCap()
    {
        var big = new string('Z', LuckyNemoChatDataProvider.MaxEntryTextBytes + 50_000);
        var truncated = LuckyNemoChatDataProvider.TruncateForChatEntry(big);
    ...
    public void TruncateForChatEntry_DoesNotSplitSurrogatePair()
    {
        ...
        var sb = new System.Text.StringBuilder(LuckyNemoChatDataProvider.MaxEntryTextBytes);
        for (var i = 0; i < LuckyNemoChatDataProvider.MaxEntryTextBytes / 4 + 10; i++)
            sb.Append(emoji);
        var truncated = LuckyNemoChatDataProvider.TruncateForChatEntry(sb.ToString());
    ...
    public async Task LoadHistoryAsync_DoesNotRetryFailureFromPreviousConnection()
    {
        ...
        var history = Assert.Single(
            activities.Stopped,
            activity => activity.OperationName == ChatTelemetryTracker.HistoryLoadSpanName);
        Assert.Equal("failure", history.GetTagItem(LuckyNemoTelemetryTagKey.Outcome.ToTelemetryName()));
        Assert.Equal("initial", history.GetTagItem(LuckyNemoTelemetryTagKey.Source.ToTelemetryName()));
        await provider.DisposeAsync();
    }
    ...
    public async Task LoadHistoryAsync_ReconnectDuringFailureNotification_PreservesNewGenerationRetryBudget()
    {
        ...
        var staleFailure = Assert.Single(
            activities.Stopped,
            activity => activity.OperationName == ChatTelemetryTracker.HistoryLoadSpanName);
        Assert.Equal("canceled", staleFailure.GetTagItem(LuckyNemoTelemetryTagKey.Outcome.ToTelemetryName()));
        Assert.Null(staleFailure.GetTagItem(LuckyNemoTelemetryTagKey.ErrorType.ToTelemetryName()));
        Assert.Equal(1, calls);
    ...
    public async Task Dispose_CancelsInFlightHistoryAndDelayedRetry()
    {
        ...
        Assert.Contains(historySpans, history =>
            Equals("failure", history.GetTagItem(LuckyNemoTelemetryTagKey.Outcome.ToTelemetryName())));
        Assert.Contains(historySpans, history =>
            Equals("canceled", history.GetTagItem(LuckyNemoTelemetryTagKey.Outcome.ToTelemetryName())));
        Assert.Single(historySpans, history =>
            history.GetTagItem(LuckyNemoTelemetryTagKey.ErrorType.ToTelemetryName()) is not null);
    }
    ...
    //  See LuckyNemoChatDataProvider.BuildSnapshotLocked for the design.
    // ─────────────────────────────────────────────────────────────────────

    private static (FakeBridge bridge, LuckyNemoChatDataProvider provider, List<ChatDataSnapshot> snapshots)
    ...
    public async Task SendMessageAsync_PreHandshake_GatewayClientRefusesViaProvider()
    {
        ...
        //  LuckyNemoGatewayClientSessionKeyTests in LuckyNemo.Shared.Tests.)
        var (bridge, provider, _, _) = CreateProvider();
    ...
    public async Task RememberSelectedThread_CancelsPendingDefaultStateSave()
    {
        ...
        var persisted = LuckyNemoChatDataProvider.LoadLastChatState(statePath);
    ...
    public async Task RememberSelectedThread_ModelsBeforeSessionsKeepsSavedSelectionPending()
    {
        using var temp = new TempDirectory();
        var statePath = Path.Combine(temp.DirectoryPath, "last-chat-state.json");
        await File.WriteAllTextAsync(statePath, JsonSerializer.Serialize(new LuckyNemoChatDataProvider.LastChatState
        {
        ...
        await Task.Delay(150);
        var persistedBeforeSessions = LuckyNemoChatDataProvider.LoadLastChatState(statePath);
        ...
        var json = $$"""
            {
              ...
              "host": "gateway",
              "command": "luckynemo nodes invoke --node \"Windows Node\" --command system.run",
              "title": "{{title}}",
    ...
    public async Task RespondToPermissionAsync_RpcThrows_BannerPreservedForRetry()
    {
        ...
        // disconnected, see LuckyNemoGatewayClient.ResolveExecApprovalAsync's
    ...
    {
        // Mirrors the flat envelope that LuckyNemoGatewayClient.HandleExecApprovalEvent
    ...
    public void LoadLastChatState_WithCorruptedJson_ReturnsNull()
    {
        ...
        var state = LuckyNemoChatDataProvider.LoadLastChatState(path);
    ...
    private static ISet<string> GetQueuedDrainScheduledThreads(LuckyNemoChatDataProvider provider)
    {
        var field = typeof(LuckyNemoChatDataProvider).GetField(
    ...
    private sealed class TestLogger : LuckyNemo.Shared.ILuckyNemoLogger
    {
    ...
    {
        public string DirectoryPath { get; } = Path.Combine(Path.GetTempPath(), "luckynemo-chat-attachments-" + Guid.NewGuid().ToString("N"));
