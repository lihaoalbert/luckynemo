...
public sealed class ConnectionPageTailscaleRecoveryTests
{
    ...
    public void NetworkFailure_ForManagedTailscaleGateway_UsesDedicatedRecoveryPlan()
    {
        var snapshot = new GatewayConnectionSnapshot
        {
            ...
            GatewayUrl = "wss://luckynemo.tailnet.ts.net",
        };
        var record = new GatewayRecord
        {
            Id = "tailscale",
            Url = "wss://luckynemo.tailnet.ts.net",
            FriendlyName = "Tailscale (LuckyNemoGateway)",
            IsLocal = true,
        ...
        var source = File.ReadAllText(Path.Combine(TestRepositoryPaths.GetRepositoryRoot(), "src", "LuckyNemo.Tray.WinUI", "Pages", "ConnectionPage.xaml.cs"));
