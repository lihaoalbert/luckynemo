...
[CollectionDefinition(LuckyNemoTrayDataDirEnvironmentCollection.Name, DisableParallelization = true)]
public sealed class LuckyNemoTrayDataDirEnvironmentCollection
{
    public const string Name = "LuckyNemoTrayDataDirEnvironment";
}

[Collection(LuckyNemoTrayDataDirEnvironmentCollection.Name)]
public sealed class SettingsManagerIsolationTests
{
    [Fact]
    public void LuckyNemoTrayDataDirRedirectsSettingsAwayFromRealAppData()
    {
        var previousOverride = Environment.GetEnvironmentVariable("LUCKYNEMO_TRAY_DATA_DIR");
        var isolatedDirectory = Path.Combine(Path.GetTempPath(), "LuckyNemoTray.Tests", Guid.NewGuid().ToString("N"));
        ...
        try
        {
            ...
            Environment.SetEnvironmentVariable("LUCKYNEMO_TRAY_DATA_DIR", isolatedDirectory);
        ...
        {
            Environment.SetEnvironmentVariable("LUCKYNEMO_TRAY_DATA_DIR", previousOverride);
