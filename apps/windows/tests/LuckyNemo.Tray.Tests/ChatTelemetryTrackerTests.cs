...
public sealed class ChatTelemetryTrackerTests
{
    ...
    {
        Assert.Equal("luckynemo.chat.turn", ChatTelemetryTracker.TurnSpanName);
        Assert.Equal("luckynemo.chat.queue.wait", ChatTelemetryTracker.QueueWaitSpanName);
        Assert.Equal("luckynemo.chat.send", ChatTelemetryTracker.SendSpanName);
        Assert.Equal("luckynemo.chat.response.wait", ChatTelemetryTracker.ResponseWaitSpanName);
        Assert.Equal("luckynemo.chat.response.receive", ChatTelemetryTracker.ResponseReceiveSpanName);
        Assert.Equal("luckynemo.chat.history.load", ChatTelemetryTracker.HistoryLoadSpanName);
        Assert.Equal("luckynemo.chat.history.backfill", ChatTelemetryTracker.HistoryBackfillSpanName);
        Assert.Equal("luckynemo.chat.turns", ChatTelemetryTracker.TurnsMetricName);
        Assert.Equal("luckynemo.chat.response.wait.duration", ChatTelemetryTracker.ResponseWaitDurationMetricName);
        Assert.Equal("luckynemo.chat.response.receive.duration", ChatTelemetryTracker.ResponseReceiveDurationMetricName);
        Assert.Equal("luckynemo.chat.remote_turns.dropped", ChatTelemetryTracker.DroppedRemoteTurnsMetricName);
        Assert.Equal("luckynemo.chat.terminal_events.dropped", ChatTelemetryTracker.DroppedTerminalEventsMetricName);
    ...
    public void LocalTurn_ParentsSendAndCompletesExactlyOnce()
    {
        ...
        Assert.Equal("assistant_final", turn.GetTagItem(LuckyNemoTelemetryTagKey.Reason.ToTelemetryName()));
    ...
    public void TerminalBeforeOutput_RecordsWaitOnly()
    {
        ...
        Assert.Equal("failure", wait.GetTagItem(LuckyNemoTelemetryTagKey.Outcome.ToTelemetryName()));
    ...
    public void DeferredSend_AccumulatesQueueSegmentsAndRecordsEachAttempt()
    {
        ...
        Assert.All(attempts, measurement =>
            Assert.Equal("success", measurement.Tag(LuckyNemoTelemetryTagKey.Outcome.ToTelemetryName())));
        ...
        Assert.All(queueWaits, queueWait =>
        {
            ...
            Assert.Equal("local", queueWait.GetTagItem(LuckyNemoTelemetryTagKey.Source.ToTelemetryName()));
            Assert.Equal("success", queueWait.GetTagItem(LuckyNemoTelemetryTagKey.Outcome.ToTelemetryName()));
        });
    }
    ...
    public void QueuedTurnCanceledBeforeDispatch_ClosesQueueWaitWithTurnOutcome()
    {
        ...
        Assert.Equal(turn.TraceId, queueWait.TraceId);
        Assert.Equal(turn.SpanId, queueWait.ParentSpanId);
        Assert.Equal("canceled", queueWait.GetTagItem(LuckyNemoTelemetryTagKey.Outcome.ToTelemetryName()));
        var queueMetric = Assert.Single(metrics.For(ChatTelemetryTracker.QueueWaitDurationMetricName));
        Assert.Equal("canceled", queueMetric.Tag(LuckyNemoTelemetryTagKey.Outcome.ToTelemetryName()));
    }
    ...
    public void PreparedCompletion_ReservesUnderLockAndEmitsAfterward()
    {
        ...
        Assert.Equal("send_rejected", turn.Tag(LuckyNemoTelemetryTagKey.Reason.ToTelemetryName()));
    }
    ...
    public void HistoryOperations_RecordOnlyAllowlistedTags()
    {
        ...
        Assert.Equal(["luckynemo.outcome", "luckynemo.source"], loadSpan.Tags.Select(tag => tag.Key).Order().ToArray());
        ...
        Assert.Equal(
            ["error.type", "luckynemo.chat.backfill.reason", "luckynemo.outcome", "luckynemo.source"],
