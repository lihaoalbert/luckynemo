...
public class DeepLinkSecurityPolicyTests
{
    ...
    [Theory]
    [InlineData("luckynemo://agent?message=hello%20world&token=secret", "luckynemo://agent?<redacted>")]
    [InlineData("luckynemo://send/private-message?message=secret", "luckynemo://send/...?<redacted>")]
    [InlineData("luckynemo://dashboard/sessions?filter=abc", "luckynemo://dashboard/...?<redacted>")]
    [InlineData("luckynemo://settings", "luckynemo://settings")]
    ...
    public void RequiresConfirmation_OnlyForStateChangingActions()
    {
        Assert.True(DeepLinkSecurityPolicy.RequiresConfirmation(
            DeepLinkParser.ParseDeepLink("luckynemo://agent?message=ping")));
        Assert.False(DeepLinkSecurityPolicy.RequiresConfirmation(
            DeepLinkParser.ParseDeepLink("luckynemo://dashboard/sessions")));
    }

    [Fact]
    public void BuildPipeName_IsDeterministicAndScoped()
    {
        var first = DeepLinkSecurityPolicy.BuildPipeName(@"C:\Users\alice\AppData\Local\LuckyNemoTray", "S-1-5-21-alice", 2);
        var second = DeepLinkSecurityPolicy.BuildPipeName(@"C:\Users\alice\AppData\Local\LuckyNemoTray", "S-1-5-21-alice", 2);
        var differentUser = DeepLinkSecurityPolicy.BuildPipeName(@"C:\Users\alice\AppData\Local\LuckyNemoTray", "S-1-5-21-bob", 2);
        var differentSession = DeepLinkSecurityPolicy.BuildPipeName(@"C:\Users\alice\AppData\Local\LuckyNemoTray", "S-1-5-21-alice", 3);
        var differentDataDir = DeepLinkSecurityPolicy.BuildPipeName(@"D:\isolated\LuckyNemoTray", "S-1-5-21-alice", 2);

        Assert.Equal(first, second);
        Assert.NotEqual(first, differentUser);
        Assert.NotEqual(first, differentSession);
        Assert.NotEqual(first, differentDataDir);
        Assert.StartsWith("LuckyNemoTray-DeepLink-", first);
