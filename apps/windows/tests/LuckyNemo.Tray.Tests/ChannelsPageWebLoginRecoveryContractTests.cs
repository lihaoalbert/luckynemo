...
public sealed class ChannelsPageWebLoginRecoveryContractTests
{
    private static string ReadChannelsPage() =>
        Read("src", "LuckyNemo.Tray.WinUI", "Pages", "ChannelsPage.xaml.cs");
    ...
    public void LinkingFlow_ShowsActionableRecoveryCommand()
    {
        ...
        Assert.Contains("luckynemo plugins install @luckynemo/", source, StringComparison.Ordinal);
    ...
    public void GatewayClient_DoesNotLeakStackTraceIntoWebLoginRawResponse()
    {
        ...
        // affect the result.
        var source = Read("src", "LuckyNemo.Shared", "LuckyNemoGatewayClient.cs");
    ...
    private static string ExtractWebLoginRegion(string source)
    {
        ...
        Assert.True(start >= 0, "Could not find WebLoginStartAsync in LuckyNemoGatewayClient.cs");
