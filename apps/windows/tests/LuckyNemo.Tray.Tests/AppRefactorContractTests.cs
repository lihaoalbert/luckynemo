...
public sealed class AppRefactorContractTests
{
    ...
    public void Startup_UsesConnectionManagerAsOnlyGatewayClientOwner()
    {
        ...
        Assert.DoesNotMatch(new Regex(@"\bnew\s+LuckyNemoGatewayClient\s*\(", RegexOptions.Multiline), source);
    ...
    public void CommandCenter_UsesOverallStateBeforeLegacyStatus()
    {
        ...
        var source = File.ReadAllText(Path.Combine(
            root, "src", "LuckyNemo.Tray.WinUI", "Services", "CommandCenterStateBuilder.cs"));

        AssertInOrder(
            source,
            "if (overallState == LuckyNemo.Connection.OverallConnectionState.Degraded)",
    ...
    {
        var root = TestRepositoryPaths.GetRepositoryRoot();
        var source = File.ReadAllText(Path.Combine(root, "src", "LuckyNemo.Tray.WinUI", "Pages", "ChatPage.xaml.cs"));
    ...
    {
        var root = TestRepositoryPaths.GetRepositoryRoot();
        var source = File.ReadAllText(Path.Combine(root, "src", "LuckyNemo.Tray.WinUI", "Pages", "PermissionsPage.xaml.cs"));
    ...
    {
        var root = TestRepositoryPaths.GetRepositoryRoot();
        var xaml = File.ReadAllText(Path.Combine(root, "src", "LuckyNemo.Tray.WinUI", "Pages", "PermissionsPage.xaml"));
        var codeBehind = File.ReadAllText(Path.Combine(root, "src", "LuckyNemo.Tray.WinUI", "Pages", "PermissionsPage.xaml.cs"));
    ...
    public void Setup_IsHostedInTrayAndUsesSelfRestartAfterCompletion()
    {
        var source = ReadAppSources();
        var root = TestRepositoryPaths.GetRepositoryRoot();
        var setupWindow = File.ReadAllText(Path.Combine(root, "src", "LuckyNemo.SetupEngine.UI", "SetupWindow.xaml.cs"));
        var welcomePage = File.ReadAllText(Path.Combine(root, "src", "LuckyNemo.SetupEngine.UI", "Pages", "WelcomePage.xaml.cs"));
        var wizardPage = File.ReadAllText(Path.Combine(root, "src", "LuckyNemo.SetupEngine.UI", "Pages", "WizardPage.xaml.cs"));
        var progressPage = File.ReadAllText(Path.Combine(root, "src", "LuckyNemo.SetupEngine.UI", "Pages", "ProgressPage.xaml.cs"));
        var updateCoordinator = File.ReadAllText(Path.Combine(root, "src", "LuckyNemo.Tray.WinUI", "Services", "UpdateCoordinator.cs"));
        var cliUninstall = File.ReadAllText(Path.Combine(root, "src", "LuckyNemo.Tray.WinUI", "CliUninstallHandler.cs"));
        var setupProgram = File.ReadAllText(Path.Combine(root, "src", "LuckyNemo.SetupEngine", "Program.cs"));
        var settingsPage = File.ReadAllText(Path.Combine(root, "src", "LuckyNemo.Tray.WinUI", "Pages", "SettingsPage.xaml.cs"));
        var settingsManager = File.ReadAllText(Path.Combine(root, "src", "LuckyNemo.Tray.WinUI", "Services", "SettingsManager.cs"));
        var keepAlivePolicy = File.ReadAllText(Path.Combine(root, "src", "LuckyNemo.Tray.WinUI", "Services", "WslKeepAlivePolicy.cs"));
        var setupClassifier = File.ReadAllText(Path.Combine(root, "src", "LuckyNemo.Tray.WinUI", "Services", "SetupExistingGatewayClassifier.cs"));
        ...
        Assert.DoesNotContain("LuckyNemo.SetupEngine.UI.exe", source);
        Assert.DoesNotContain("Process.GetProcessesByName(\"LuckyNemo.SetupEngine.UI\")", source);
        Assert.False(File.Exists(Path.Combine(root, "src", "LuckyNemo.Tray.WinUI", "Windows", "SetupWizardWindow.cs")));
    }
    ...
    {
        var root = TestRepositoryPaths.GetRepositoryRoot();
        var xaml = File.ReadAllText(Path.Combine(root, "src", "LuckyNemo.SetupEngine.UI", "Pages", "ProgressPage.xaml"));
        var code = File.ReadAllText(Path.Combine(root, "src", "LuckyNemo.SetupEngine.UI", "Pages", "ProgressPage.xaml.cs"));
    ...
    {
        var root = TestRepositoryPaths.GetRepositoryRoot();
        var source = File.ReadAllText(Path.Combine(root, "src", "LuckyNemo.SetupEngine.UI", "SetupWindow.xaml.cs"));
    ...
    {
        var root = TestRepositoryPaths.GetRepositoryRoot();
        var app = File.ReadAllText(Path.Combine(root, "src", "LuckyNemo.Tray.WinUI", "App.xaml.cs"));
        var setupWindow = File.ReadAllText(Path.Combine(root, "src", "LuckyNemo.SetupEngine.UI", "SetupWindow.xaml.cs"));
    ...
    public void CompletePage_UsesCompletionArgsForStartupPreference()
    {
        var root = TestRepositoryPaths.GetRepositoryRoot();
        var setupWindow = File.ReadAllText(Path.Combine(root, "src", "LuckyNemo.SetupEngine.UI", "SetupWindow.xaml.cs"));
        var complete = File.ReadAllText(Path.Combine(root, "src", "LuckyNemo.SetupEngine.UI", "Pages", "CompletePage.xaml.cs"));
    ...
    {
        var root = TestRepositoryPaths.GetRepositoryRoot();
        var source = File.ReadAllText(Path.Combine(root, "src", "LuckyNemo.SetupEngine.UI", "Pages", "CapabilitiesPage.xaml.cs"));
    ...
    public void CapabilitiesPage_ExposesExplicitCustomCapabilitySetsForReview()
    {
        var root = TestRepositoryPaths.GetRepositoryRoot();
        var source = File.ReadAllText(Path.Combine(root, "src", "LuckyNemo.SetupEngine.UI", "Pages", "CapabilitiesPage.xaml.cs"));
        var xaml = File.ReadAllText(Path.Combine(root, "src", "LuckyNemo.SetupEngine.UI", "Pages", "CapabilitiesPage.xaml"));
    ...
    {
        var root = TestRepositoryPaths.GetRepositoryRoot();
        var source = File.ReadAllText(Path.Combine(root, "src", "LuckyNemo.SetupEngine.UI", "Pages", "WizardPage.xaml.cs"));
    ...
    public void WizardConnect_UsesActiveGatewayRecordUrl()
    {
        ...
        Assert.Contains("new LuckyNemoGatewayClient(gatewayUrl, token", method);
    ...
    {
        var root = TestRepositoryPaths.GetRepositoryRoot();
        var xaml = File.ReadAllText(Path.Combine(root, "src", "LuckyNemo.Tray.WinUI", "Pages", "SettingsPage.xaml"));
        var code = File.ReadAllText(Path.Combine(root, "src", "LuckyNemo.Tray.WinUI", "Pages", "SettingsPage.xaml.cs"));

        Assert.Contains("x:Name=\"LuckyNemoOnboardCard\"", xaml);
        ...
        Assert.Contains("LuckyNemoOnboardCard.Visibility = activeGatewayAccess.CanControlWslGateway", code);
        Assert.Contains("CurrentApp.Registry?.Load();", code);
        Assert.Contains("LuckyNemoOnboardCard.Visibility = Visibility.Collapsed;", code);
    }
    ...
    {
        var root = TestRepositoryPaths.GetRepositoryRoot();
        var source = File.ReadAllText(Path.Combine(root, "src", "LuckyNemo.Tray.WinUI", "Windows", "HubWindow.xaml.cs"));
    ...
    {
        var root = TestRepositoryPaths.GetRepositoryRoot();
        var setupUiDir = Path.Combine(root, "src", "LuckyNemo.SetupEngine.UI");
        ...
        Assert.DoesNotContain("LuckyNemo.Tray.WinUI", source);
    }
    ...
    {
        var root = TestRepositoryPaths.GetRepositoryRoot();
        var source = File.ReadAllText(Path.Combine(root, "src", "LuckyNemo.Tray.WinUI", "Pages", "SettingsPage.xaml.cs"));
        ...
        Assert.DoesNotContain("LuckyNemo.SetupEngine.Program.Main(setupArgs)", source);
        Assert.DoesNotContain("LuckyNemo.SetupEngine.UI.exe", source);
    }
    ...
    public void SetupUiImages_UseLibraryQualifiedAssetUris()
    {
        ...
        Assert.Contains("ms-appx:///LuckyNemo.SetupEngine.UI/Assets/Setup/LuckyNemoMascot.png", xaml);
    ...
    {
        var root = TestRepositoryPaths.GetRepositoryRoot();
        var source = File.ReadAllText(Path.Combine(root, "src", "LuckyNemo.SetupEngine.UI", "Pages", "WelcomePage.xaml.cs"));
    ...
    {
        var root = TestRepositoryPaths.GetRepositoryRoot();
        var xaml = File.ReadAllText(Path.Combine(root, "src", "LuckyNemo.SetupEngine.UI", "Pages", "WelcomePage.xaml"));
        var welcomePage = File.ReadAllText(Path.Combine(root, "src", "LuckyNemo.SetupEngine.UI", "Pages", "WelcomePage.xaml.cs"));
        var setupWindow = File.ReadAllText(Path.Combine(root, "src", "LuckyNemo.SetupEngine.UI", "SetupWindow.xaml.cs"));
    ...
    public void HubWindow_DesktopIconMirrorsStatusAccent()
    {
        var root = TestRepositoryPaths.GetRepositoryRoot();
        var source = File.ReadAllText(Path.Combine(
            root, "src", "LuckyNemo.Tray.WinUI", "Windows", "HubWindow.xaml.cs"));
    ...
    {
        var source = ReadLuckyNemoComposerSource();
    ...
    private static string ReadCoordinatorSource()
    {
        ...
        return File.ReadAllText(Path.Combine(
            root, "src", "LuckyNemo.Tray.WinUI", "Services", "TrayIconCoordinator.cs"));
    }
    ...
    private static string ReadWslKeepAliveServiceSource()
    {
        var root = TestRepositoryPaths.GetRepositoryRoot();
        return File.ReadAllText(Path.Combine(
            root, "src", "LuckyNemo.Tray.WinUI", "Services", "WslGatewayKeepAliveService.cs"));
    }

    private static string ReadAppSources()
    {
        var root = TestRepositoryPaths.GetRepositoryRoot();
        var appDir = Path.Combine(root, "src", "LuckyNemo.Tray.WinUI");
        return string.Join(
            "\n",
            Directory
    ...
    private static string ReadSandboxPageSource()
    {
        var root = TestRepositoryPaths.GetRepositoryRoot();
        return File.ReadAllText(Path.Combine(
            root, "src", "LuckyNemo.Tray.WinUI", "Pages", "SandboxPage.xaml.cs"));
    }

    private static string ReadLuckyNemoComposerSource()
    {
        var root = TestRepositoryPaths.GetRepositoryRoot();
        return File.ReadAllText(Path.Combine(
            root, "src", "LuckyNemo.Tray.WinUI", "Chat", "LuckyNemoComposer.cs"));
    }
    ...
    private static string ExtractMethod(string source, string methodName)
    {
        var match = Regex.Match(
            source,
            $@"(?m)^\s*(?:private|protected|public|internal)\s+(?:static\s+)?(?:async\s+)?(?:Task(?:<[^>]+>)?|System\.Threading\.Tasks\.Task|void|bool|int|string\??|object\??|IntPtr|LuckyNemo\.Connection\.GatewayCredential\?)\s+{Regex.Escape(methodName)}\s*\(");
