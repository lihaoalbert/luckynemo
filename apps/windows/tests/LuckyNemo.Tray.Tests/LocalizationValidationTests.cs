...
public class LocalizationValidationTests
{
    ...
    private static readonly HashSet<string> InvariantOrDeferredResourceKeys = new(StringComparer.Ordinal)
    {
        ...
        // Product/feature name — "LuckyNemo Onboard" is kept identical across
    ...
    private static string GetStringsDirectory() =>
        Path.Combine(TestRepositoryPaths.GetRepositoryRoot(), "src", "LuckyNemo.Tray.WinUI", "Strings");
    ...
    private static bool IsInvariantValue(string value) =>
        ...
        Regex.IsMatch(value, @"^(v\d|[A-Z0-9._%+-]+://|~?/|\.NET|WinUI|WinAppSDK|LuckyNemo$|GitHub|MCP|JSON|API|HTTP|HTTPS|SSH|TLS|WebView2|OAuth|QR$|Cron$|main$|user$|machine-name$)", RegexOptions.CultureInvariant) ||
        value.Contains("luckynemo://", StringComparison.Ordinal) ||
        value.Contains("github.com", StringComparison.Ordinal) ||
        value.Contains("luckynemo.ai", StringComparison.Ordinal) ||
    ...
    {
        var winUiRoot = Path.Combine(TestRepositoryPaths.GetRepositoryRoot(), "src", "LuckyNemo.Tray.WinUI");
