...
public class TrayMenuWindowMarkupTests
{
    ...
    {
        var source = Read("src", "LuckyNemo.Tray.WinUI", "Windows", "CanvasWindow.xaml.cs");

        Assert.Contains("BridgeMessageReceived", source);
        Assert.Contains("IsTrustedBridgeSource(e.Source)", source);
        Assert.Contains("luckynemo-canvas.local", source);
    ...
    {
        var source = Read("src", "LuckyNemo.Tray.WinUI", "Windows", "TrayMenuWindow.xaml.cs");
