...
public sealed class AssistantBridgeServiceTests
{
    ...
    public void ParseStatus_ReadsListenServiceAndRecentTurns()
    {
        const string json = """
        {
          ...
          "assistant": {
            "listen_service": {
              ...
              "log_file": "C:\\Users\\RipauvGohil\\AppData\\Local\\LuckyNemo\\runtime\\assistant-listen-owner.log",
              "pid": 20656,
              "speak_aloud": true,
              "status": "running",
            ...
            "recent_turns": [
              {
                ...
                "input_text": "LuckyNemo, say only live voice smoke ok.",
        ...
        Assert.Equal("LuckyNemo, say only live voice smoke ok.", snapshot.RecentTurns[0].InputText);
        Assert.Equal("live voice smoke ok", snapshot.RecentTurns[0].ResponseText);
        Assert.Equal(23502, snapshot.RecentTurns[0].TotalMs);
    }

    [Fact]
    public void ResolveLuckyNemoCli_PrefersBackendRootEnvironmentOverride()
    {
        var oldValue = Environment.GetEnvironmentVariable("LUCKYNEMO_BACKEND_ROOT");
        ...
        try
        {
            ...
            var exe = Path.Combine(scripts, "luckynemo.exe");
            File.WriteAllText(exe, "");

            Environment.SetEnvironmentVariable("LUCKYNEMO_BACKEND_ROOT", root);

            var launcher = AssistantBridgeService.ResolveLuckyNemoCli();
        ...
        {
            Environment.SetEnvironmentVariable("LUCKYNEMO_BACKEND_ROOT", oldValue);
            try { Directory.Delete(root, recursive: true); } catch { }
        }
    }

    [Fact]
    public void ResolveLuckyNemoCli_IgnoresBackendRootOutsideTrustedParents()
    {
        var oldValue = Environment.GetEnvironmentVariable("LUCKYNEMO_BACKEND_ROOT");
        var root = Path.Combine(Path.GetTempPath(), "LuckyNemo.Tray.Tests", Guid.NewGuid().ToString("N"));
        try
        {
            var scripts = Path.Combine(root, ".venv", "Scripts");
            Directory.CreateDirectory(scripts);
            ...
            Assert.True(
                ...
                "LUCKYNEMO_BACKEND_ROOT should not execute arbitrary binaries outside trusted checkout parents.");
        }
        finally
        {
            Environment.SetEnvironmentVariable("LUCKYNEMO_BACKEND_ROOT", oldValue);
    ...
    {
        Assert.False(AssistantBridgeService.TryNormalizeBackendRoot("LuckyNemo", out _));
    }
    ...
    public void BuildBackendNotFoundMessage_ListsSearchedLocations()
    {
        ...
        Assert.Contains(@"D:\Projects\LuckyNemo", message);
        Assert.Contains("LUCKYNEMO_BACKEND_ROOT", message);
    }
    ...
    private static LuckyNemoCliLauncher PowerShellScriptLauncher(string scriptPath, string workingDirectory)
    {
        ...
        return new LuckyNemoCliLauncher(
            exe,
            workingDirectory,
