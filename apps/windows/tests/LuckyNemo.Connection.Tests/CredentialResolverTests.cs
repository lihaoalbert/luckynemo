...
public class CredentialResolverTests
{
    ...
    {
        var tempDir = Path.Combine(Path.GetTempPath(), "luckynemo-corrupt-token-" + Guid.NewGuid().ToString("N"));
    ...
    {
        var tempDir = Path.Combine(Path.GetTempPath(), "luckynemo-wrong-shape-token-" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(tempDir);
        try
        {
    ...
    {
        var tempDir = Path.Combine(Path.GetTempPath(), "luckynemo-wrong-shape-fallback-" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(tempDir);
        try
        {
    ...
    {
        var tempDir = Path.Combine(Path.GetTempPath(), "luckynemo-fallback-token-" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(tempDir);
        try
        {
            File.WriteAllText(Path.Combine(tempDir, "device-key-ed25519.json"), "{ broken json");
