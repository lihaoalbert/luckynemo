...
public class ConnectionStateMachineTests
{
    ...
    public void NodePairingRequired_WithOperatorConnected_DerivesPairingRequired()
    {
        ...
        Assert.Equal(OverallConnectionState.PairingRequired, _sm.Current.OverallState);
        Assert.Equal(LuckyNemo.Shared.PairingStatus.Pending, _sm.Current.NodePairingStatus);
    }
    ...
    public void SetNodeInfo_PendingWithoutRequestId_ClearsStaleRequestIdAndKind()
    {
        _sm.SetNodeInfo(
            "node-1",
            LuckyNemo.Shared.PairingStatus.Pending,
            "req-1",
            LuckyNemo.Shared.PairingApprovalKind.DevicePair);

        _sm.SetNodeInfo("node-1", LuckyNemo.Shared.PairingStatus.Pending);

        Assert.Null(_sm.Current.NodePairingRequestId);
        Assert.Equal(LuckyNemo.Shared.PairingApprovalKind.Unknown, _sm.Current.NodePairingApprovalKind);
    }
    ...
    public void SetNodeInfo_UnknownKindForSameRequestId_PreservesKnownKind()
    {
        ...
        Assert.Equal(LuckyNemo.Shared.PairingApprovalKind.DevicePair, _sm.Current.NodePairingApprovalKind);
    }
    ...
    public void NodePaired_TransitionsToConnected()
    {
        ...
        Assert.Equal(RoleConnectionState.Connected, _sm.Current.NodeState);
        Assert.Equal(LuckyNemo.Shared.PairingStatus.Paired, _sm.Current.NodePairingStatus);
    }
