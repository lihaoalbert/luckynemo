...
/// <summary>
/// Self-tests for the shared <c>LuckyNemo.TestSupport</c> fixtures. These are the
...
public sealed class TestSupportFixtureTests
{
    ...
    {
        const string name = "LUCKYNEMO_TESTSUPPORT_PROBE";
    ...
    public void CliHarness_CapturesAndLooksUp()
    {
        using var harness = new CliHarness()
            .WithEnv("LUCKYNEMO_ENDPOINT", "http://localhost:1234/");

        harness.Out.Write("stdout-line");
        harness.Err.Write("stderr-line");

        Assert.Equal("http://localhost:1234/", harness.EnvLookup("LUCKYNEMO_ENDPOINT"));
        Assert.Null(harness.EnvLookup("LUCKYNEMO_MISSING"));
