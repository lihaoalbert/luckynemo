...
1. Run full repo build:
   - `./build.ps1`
2. Run shared tests:
   - `dotnet test ./tests/LuckyNemo.Shared.Tests/LuckyNemo.Shared.Tests.csproj --no-restore`
3. Run tray tests:
   - `dotnet test ./tests/LuckyNemo.Tray.Tests/LuckyNemo.Tray.Tests.csproj --no-restore`

This is the required local closeout subset for agents. CI also builds and runs additional connection, setup, CLI, UI, accessibility, integration, and E2E suites; see `docs/TEST_COVERAGE.md` for the broader test inventory and `.github/workflows/windows-app-ci.yml` (monorepo root) for the workflow source of truth.
...
- In linked git worktrees, set `LUCKYNEMO_REPO_ROOT` to the worktree path before running tests that discover the repository root, for example:
  - `$env:LUCKYNEMO_REPO_ROOT='D:\github\luckynemo-windows-node.<worktree-name>'`
- Tray tests must isolate `SettingsManager` from real user settings. Do not use `new SettingsManager()` in tests unless the test intentionally reads `%APPDATA%\LuckyNemoTray\settings.json`; pass a temp settings directory or set `LUCKYNEMO_TRAY_DATA_DIR` before the test process starts.
...
When changing MXC sandboxing, `system.run`, exec approvals, Windows node command execution, gateway setup/connect E2E behavior, or files under `src\LuckyNemo.Shared\Mxc`, run:

```powershell
.\scripts\validate-mxc-e2e.ps1
```

The script sets `LUCKYNEMO_RUN_E2E` and `LUCKYNEMO_RUN_MXC_E2E` itself, then runs the real WSL Gateway -> Windows node -> `system.run` MXC E2E proofs. It fails if the MXC proof skips. Use `-AllowSkip` only to document that the current host is not MXC-capable; do not report an `-AllowSkip` run as merge validation for MXC-related work.

## UI, MCP, and PR Proof

Use `.agents/skills/luckynemo-proof-validation/SKILL.md` when a change touches tray UX, Settings, onboarding, chat/canvas, Command Center, Windows node capabilities, MCP, gateway connection/pairing, permissions, diagnostics, or agent-facing instructions.
...
3. Update `src/LuckyNemo.WinNode.Cli/skill.md`.
4. Add/update capability, MCP bridge, `winnode`, and UI/gateway tests as appropriate.
5. Run required validation plus `dotnet test .\tests\LuckyNemo.WinNode.Cli.Tests\LuckyNemo.WinNode.Cli.Tests.csproj --no-restore` when `winnode`, MCP output, or command docs change.
...
- Do not add OpenTelemetry SDK/exporter package references to `LuckyNemo.Shared`; shared helpers may use `System.Diagnostics.ActivitySource`, `Activity`, and `System.Diagnostics.Metrics` only.
...
`src\LuckyNemo.Tray.WinUI\App.xaml.cs` and `src\LuckyNemo.Tray.WinUI\Pages\ConnectionPage.xaml.cs` are active god-file reduction targets. When touching either file:
...
- Active gateway records live in `%APPDATA%\LuckyNemoTray\gateways.json`; per-gateway identity files live under `%APPDATA%\LuckyNemoTray\gateways\<gateway-id>\device-key-ed25519.json`.
