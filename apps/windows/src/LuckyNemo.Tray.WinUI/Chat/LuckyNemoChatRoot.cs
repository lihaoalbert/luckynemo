...
using LuckyNemoTray.FunctionalUI;
using LuckyNemoTray.FunctionalUI.Core;
using LuckyNemoTray.Services;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;
using static LuckyNemoTray.FunctionalUI.Factories;
using static LuckyNemoTray.FunctionalUI.Core.Theme;

namespace LuckyNemoTray.Chat;

/// <summary>
    /// FunctionalUI root component used to render the LuckyNemo chat surface (Header
...
/// </summary>
public sealed class LuckyNemoChatRoot : Component
{
    ...
    /// <see cref="LuckyNemoChatRoot"/> so its timeline re-renders.
    /// </summary>
    ...
    public override Element Render()
    {
        ...
        var initialSelectedId = _initialThreadId ?? (_provider as LuckyNemoChatDataProvider)?.CachedLastChatState?.DefaultThreadId;
        ...
        var entryMetaSnapshot = UseMemo<IReadOnlyDictionary<string, ChatEntryMetadata>?>(() =>
        {
            ...
            {
                LuckyNemoChatDataProvider nativeForMeta => nativeForMeta.GetEntryMetadata(selectedIdForMetadata),
                _ => null
            };
        ...
        if (selectedThread is null && composeKey is not null)
        {
            ...
            var lastState = (_provider as LuckyNemoChatDataProvider)?.CachedLastChatState;
            composeOnlyThread = new ChatThread
            {
                ...
                Title = _pendingSelectedThreadId is not null
                    ...
                    : lastState?.ThreadTitle ?? "LuckyNemo Windows Tray",
        ...
        if (hostConnected &&
            ...
            _provider is LuckyNemoChatDataProvider native)
        {
        ...
        // Per-entry metadata for the LuckyNemo timeline footer (sender · time · model).
        ...
        // unloaded timeline. See LuckyNemoChatDataProvider.HistoryLoaded
        ...
        UseEffect((Func<Action>)(() =>
        {
            ...
            _ = Task.Run(async () =>
            {
                ...
                catch (Exception ex) { LuckyNemoTray.Services.Logger.Debug($"ChatRoot: welcome settle race: {ex.Message}"); }
            });
        ...
        {
            body = Component<LuckyNemoChatTimeline, LuckyNemoChatTimelineProps>(new(
                ...
                UserSenderLabel: "LuckyNemo Windows Tray",
            ...
            && !ChannelGroupsContain(channelGroups, effectiveThread.Id))
        {
            ...
            var syntheticGroup = new ChannelGroup(
                ...
                Sessions: new[] { (Id: effectiveThread.Id!, Title: effectiveThread.Title ?? "LuckyNemo Windows Tray", Model: effectiveThread.Model, ModelProvider: effectiveThread.ModelProvider) });
        ...
        Element composer = effectiveThread is { } composerThread
            ? Component<LuckyNemoComposer, LuckyNemoComposerProps>(new(
                ConnectionState: connState,
                TurnActive: turnActiveOverride,
                ChannelLabel: composerThread.Title ?? "LuckyNemo Windows Tray",
                ...
                OnChannelChanged: id =>
                {
                    ...
                    if (_provider is LuckyNemoChatDataProvider nativeProvider)
    ...
    private static Action<FrameworkElement> MakeShimmer(double beginOffsetMs)
    {
        return fe =>
        {
            ...
            {
                LuckyNemoTray.Services.Logger.Debug($"ChatRoot: skeleton storyboard animation failed (non-essential): {ex.Message}");
            }
        };
    }
    ...
    {
        var welcomeTitle = LocalizedOrDefault("Chat_ZeroState_WelcomeTitle", "Welcome to LuckyNemo");
        ...
        var suggestions = new[]
        {
            "Say hi 👋",
            "What can you do?",
            "Give me a quick tour of LuckyNemo",
        };
    ...
    private static Element PlaceholderEmptyThreadState(string connectionState)
    {
        ...
        var msg = isConnected
            ? "Start a new LuckyNemo chat from the composer below."
        ...
        IReadOnlyList<ChatAttachment> attachments)
    {
        ...
        if (_provider is LuckyNemoChatDataProvider native &&
