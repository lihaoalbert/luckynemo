...
// LuckyNemo.Tray.WinUI and depends on Microsoft.Windows.ApplicationModel.Resources
...
/// <see cref="LuckyNemoGatewayClient"/>) into the
...
/// </para>
/// </remarks>
public sealed class LuckyNemoChatDataProvider : IChatDataProvider
{
    ...
    private Dictionary<string, HashSet<string>> _persistedAbortedIds;    // threadId → set of __luckynemo.id values (loaded from disk)
    ...
    public string DisplayName => "LuckyNemo gateway";
    ...
    /// </param>
    public LuckyNemoChatDataProvider(IChatGatewayBridge bridge, Action<Action>? post = null)
        : this(bridge, post, DefaultToolMetaCacheFilePath)
    {
    }

    internal LuckyNemoChatDataProvider(
        ...
        bool authoritative = false)
    {
        ...
        try
        {
            ...
            lock (_gate)
            {
                ...
                // don't reflect the actual processing order; __luckynemo.seq is
                ...
                foreach (var msg in ordered)
                {
                    ...
                    var msgMeta = new ChatEntryMetadata(
                        ...
                        msg.ResponseTokens,
                        msg.ContextPercent,
                        GatewayMessageId: msg.LuckyNemoId,
                        LuckyNemoSeq: msg.LuckyNemoSeq,
                        LuckyNemoKind: msg.LuckyNemoKind,
                    ...
                    // Check if this user message was aborted (persisted __luckynemo.id match)
                    if (roleLower == "user")
                    {
                        Logger.Debug($"[ChatHistory] user msg LuckyNemoId='{msg.LuckyNemoId ?? "(null)"}' seq={msg.LuckyNemoSeq}");
                        if (IsMessageAborted(threadId, msg.LuckyNemoId))
                ...
                if (prior.Entries.Count > 0)
                {
                    ...
                    var authoritativeMaxHistorySequence = authoritative
                        ? history.Messages
                            .Where(message => message.LuckyNemoSeq is not null)
                            .Select(message => message.LuckyNemoSeq!.Value)
    ...
    internal static bool ShouldPreserveLiveEntryDuringAuthoritativeReload(
        ...
        metadata is null ||
        metadata.LuckyNemoSeq is null ||
        metadata.LuckyNemoSeq is { } liveSequence && liveSequence > maxHistorySequence ||
    ...
    private void OnChatMessageReceived(object? sender, ChatMessageInfo message)
    {
        ...
        if (roleLower == "system" &&
            string.Equals(message.LuckyNemoKind, "compaction", StringComparison.OrdinalIgnoreCase) &&
            !string.IsNullOrEmpty(message.Text))
        {
            ...
            lock (_gate)
            {
                compactionMeta = BuildLiveMetaLocked(
                    ...
                    message.LuckyNemoId,
                    message.LuckyNemoSeq,
                    luckyNemoKind: message.LuckyNemoKind,
    ...
    private void OnAgentEventReceived(object? sender, AgentEventInfo evt)
    {
        ...
        // AgentEventInfo.Ts is a double of unix-epoch ms (per LuckyNemoGatewayClient).
    ...
    private static bool HasGatewayIdentity(ChatEntryMetadata meta)
        => !string.IsNullOrEmpty(meta.GatewayMessageId) || meta.LuckyNemoSeq is not null;
        ...
        try
        {
            ...
            lock (_gate)
            {
                ...
                var meta = BuildLiveMetaLocked(
                    threadId,
                    lastUser.Ts,
                    lastUser.LuckyNemoId,
                    lastUser.LuckyNemoSeq);
    ...
    // upstream (see LuckyNemoGatewayClient.HandleExecApprovalEvent), so it maps to
    ...
    ///     "title": "exec run command luckynemo → ..."
    ///   }
    /// }
    /// </code>
    ///
    ...
    // content blocks ({text}, {tool_use}, {text}, …). The LuckyNemo gateway
    ...
    ///   <item>Opens with the LuckyNemo CLI version banner
    ///     (<c>"LuckyNemo 2026.4.23 ..."</c>) — these are <c>--help</c>
    ...
    internal static bool LooksLikeFlattenedToolOutput(string text)
    {
        ...
        // ── LuckyNemo / common CLI tool version banner. Catches ``luckynemo
        // help``, ``luckynemo nodes invoke --help``, etc.
        var trimmed = text.AsSpan().TrimStart();
        if (trimmed.StartsWith("LuckyNemo 20") ||
            trimmed.StartsWith("LuckyNemo v") ||
            trimmed.StartsWith("luckynemo ")) return true;
    ...
    private AssistantQueueFrameDisposition ClassifyAssistantQueueFrameLocked(
        ...
        int? luckyNemoSeq)
    {
        if ((!string.IsNullOrEmpty(gatewayMessageId) || luckyNemoSeq is not null) &&
        ...
        for (var i = timeline.Entries.Count - 1; i >= 0; i--)
        {
            ...
            if (entry.IsStreaming || !string.Equals(entry.Text, assistantText, StringComparison.Ordinal))
                return AssistantQueueFrameDisposition.Render;
            if (string.IsNullOrEmpty(gatewayMessageId) && luckyNemoSeq is null)
            ...
            var sameGatewayIdentity =
                ...
                (luckyNemoSeq is not null && existing.LuckyNemoSeq == luckyNemoSeq);
        ...
        int? luckyNemoSeq)
    {
        ...
        for (var i = timeline.Entries.Count - 1; i >= 0; i--)
        {
            ...
            if (!bothHaveGatewayIds &&
                luckyNemoSeq is not null &&
                existing.LuckyNemoSeq == luckyNemoSeq &&
        ...
        long? compactionTokensAfter = null)
    {
        ...
        return new ChatEntryMetadata(
            ...
            LuckyNemoSeq: luckyNemoSeq,
            LuckyNemoKind: luckyNemoKind,
    ...
    private static List<ChatMessageInfo> OrderHistoryMessages(List<(ChatMessageInfo Message, int Index)> messages)
    {
        ...
        var sequencedCount = messages.Count(item => item.Message.LuckyNemoSeq is not null);
        if (sequencedCount == messages.Count)
        {
            return messages
                .OrderBy(item => item.Message.LuckyNemoSeq)
    ...
    private ChatDataSnapshot BuildSnapshotLocked()
    {
            ...
            && !_sessions.Any(s => string.Equals(s.Key, ck, StringComparison.Ordinal)))
        {
            threadList.Add(new ChatThread
            {
                ...
                Title = _lastChatState?.ThreadTitle ?? "LuckyNemo Windows Tray",
    ...
    /// <summary>
    /// After a successful abort, reload chat.history to capture the __luckynemo.id
        ...
        try
        {
            ...
            for (int i = 0; i < msgs.Count; i++)
            {
                ...
                if (!string.Equals(msg.Role, "user", StringComparison.OrdinalIgnoreCase))
                    continue;
                if (msg.LuckyNemoId is null) continue;
                if (IsMessageAborted(threadId, msg.LuckyNemoId)) continue;
                if (newAbortedIds.Contains(msg.LuckyNemoId)) continue;
                ...
                {
                    newAbortedIds.Add(msg.LuckyNemoId);
                }
    ...
    /// <summary>Check if a user message's __luckynemo.id is in the persisted aborted set.</summary>
    private bool IsMessageAborted(string threadId, string? luckyNemoId)
    {
        if (luckyNemoId is null) return false;
        lock (_gate)
        {
            var found = _persistedAbortedIds.TryGetValue(threadId, out var set);
            var contains = found && set!.Contains(luckyNemoId);
            Logger.Debug($"[IsMessageAborted] thread='{threadId}' id='{luckyNemoId}' dictHasThread={found} setCount={set?.Count ?? 0} match={contains}");
            return contains;
        }
    }
}
