...
/// <summary>
/// Per-entry metadata maintained by <see cref="LuckyNemoChatDataProvider"/>
/// in parallel to the vendored <see cref="LuckyNemo.Chat.ChatTimelineItem"/>.
...
/// taken from <see cref="LuckyNemo.Shared.SessionInfo.Model"/>). <c>null</c>
...
/// Gateway-assigned stable message id from <c>__luckynemo.id</c>, when known.
/// Used to reconcile live entries with later <c>chat.history</c> rows.
/// </param>
/// <param name="LuckyNemoSeq">
/// Monotonic per-session sequence from <c>__luckynemo.seq</c>, when known.
