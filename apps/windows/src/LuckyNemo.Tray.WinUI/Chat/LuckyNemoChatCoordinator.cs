...
/// </summary>
public sealed class LuckyNemoChatCoordinator : IDisposable
{
    ...
    private LuckyNemoChatDataProvider? _provider;
    ...
    public bool IsMuted
    {
        ...
        set
        {
            ...
            if (value)
            {
                ...
                catch (Exception ex) { _logger.Debug($"LuckyNemoChatCoordinator: StopSpeaking during mute failed: {ex.Message}"); }
            }
        }
    }

    public LuckyNemoChatCoordinator(
    ...
    public LuckyNemoChatDataProvider? Provider
    {
        get
        {
            lock (_gate)
    ...
    public void SetOperatorClient(LuckyNemoGatewayClient? client)
    {
        LuckyNemoChatDataProvider? oldProvider;
        ...
        var newProvider = new LuckyNemoChatDataProvider(new GatewayClientChatBridge(client), _post);
        lock (_gate)
        {
            if (_disposed)
            {
    ...
    {
        try { (_nodeServiceAccessor()?.TextToSpeech ?? GetFallbackTextToSpeechService()).StopSpeaking(); }
        catch (Exception ex) { _logger.Debug($"LuckyNemoChatCoordinator.StopSpeaking failed: {ex.Message}"); }
    }
    ...
    public void Dispose()
    {
        LuckyNemoChatDataProvider? provider;
