...
{
    internal const string TurnSpanName = "luckynemo.chat.turn";
    internal const string QueueWaitSpanName = "luckynemo.chat.queue.wait";
    internal const string SendSpanName = "luckynemo.chat.send";
    internal const string ResponseWaitSpanName = "luckynemo.chat.response.wait";
    internal const string ResponseReceiveSpanName = "luckynemo.chat.response.receive";
    internal const string HistoryLoadSpanName = "luckynemo.chat.history.load";
    internal const string HistoryBackfillSpanName = "luckynemo.chat.history.backfill";

    internal const string TurnsMetricName = "luckynemo.chat.turns";
    internal const string TurnDurationMetricName = "luckynemo.chat.turn.duration";
    internal const string QueueWaitDurationMetricName = "luckynemo.chat.queue.wait.duration";
    internal const string SendAttemptsMetricName = "luckynemo.chat.send.attempts";
    internal const string SendDurationMetricName = "luckynemo.chat.send.duration";
    internal const string ResponseWaitDurationMetricName = "luckynemo.chat.response.wait.duration";
    internal const string ResponseReceiveDurationMetricName = "luckynemo.chat.response.receive.duration";
    internal const string HistoryLoadsMetricName = "luckynemo.chat.history.loads";
    internal const string HistoryLoadDurationMetricName = "luckynemo.chat.history.load.duration";
    internal const string HistoryBackfillsMetricName = "luckynemo.chat.history.backfills";
    internal const string HistoryBackfillDurationMetricName = "luckynemo.chat.history.backfill.duration";
    internal const string DroppedRemoteTurnsMetricName = "luckynemo.chat.remote_turns.dropped";
    internal const string DroppedTerminalEventsMetricName = "luckynemo.chat.terminal_events.dropped";

    internal const string AdmissionStatusTag = "luckynemo.chat.admission.status";
    internal const string BackfillReasonTag = "luckynemo.chat.backfill.reason";
    internal const string DroppedRemoteTurnReasonTag = "luckynemo.chat.remote_turn.drop.reason";
    internal const string DroppedTerminalEventReasonTag = "luckynemo.chat.terminal_event.drop.reason";
    internal const string FirstOutputKindTag = "luckynemo.chat.response.first_output";
    ...
    private static readonly Counter<long> Turns = LuckyNemoTelemetry.CreateCounter(
        TurnsMetricName,
        unit: "{turn}",
        description: "Number of observed LuckyNemo chat turns.");
    private static readonly Histogram<double> TurnDuration = LuckyNemoTelemetry.CreateHistogram(
        TurnDurationMetricName,
        unit: "ms",
        description: "Duration of observed LuckyNemo chat turns.");
    private static readonly Histogram<double> QueueWaitDuration = LuckyNemoTelemetry.CreateHistogram(
        QueueWaitDurationMetricName,
        unit: "ms",
        description: "Cumulative local queue dwell for completed LuckyNemo chat turns.");
    private static readonly Counter<long> SendAttempts = LuckyNemoTelemetry.CreateCounter(
        SendAttemptsMetricName,
        unit: "{attempt}",
        description: "Number of LuckyNemo chat.send RPC attempts.");
    private static readonly Histogram<double> SendDuration = LuckyNemoTelemetry.CreateHistogram(
        SendDurationMetricName,
        unit: "ms",
        description: "Duration of LuckyNemo chat.send RPC attempts.");
    private static readonly Histogram<double> ResponseWaitDuration = LuckyNemoTelemetry.CreateHistogram(
    ...
    private static readonly Histogram<double> ResponseReceiveDuration = LuckyNemoTelemetry.CreateHistogram(
        ResponseReceiveDurationMetricName,
        unit: "ms",
        description: "Duration from the first accepted inbound output to terminal chat completion.");
    private static readonly Counter<long> HistoryLoads = LuckyNemoTelemetry.CreateCounter(
        HistoryLoadsMetricName,
        unit: "{load}",
        description: "Number of full LuckyNemo chat history loads.");
    private static readonly Histogram<double> HistoryLoadDuration = LuckyNemoTelemetry.CreateHistogram(
        HistoryLoadDurationMetricName,
        unit: "ms",
        description: "Duration of full LuckyNemo chat history loads.");
    private static readonly Counter<long> HistoryBackfills = LuckyNemoTelemetry.CreateCounter(
        HistoryBackfillsMetricName,
        unit: "{backfill}",
        description: "Number of targeted LuckyNemo chat history backfills.");
    private static readonly Histogram<double> HistoryBackfillDuration = LuckyNemoTelemetry.CreateHistogram(
        HistoryBackfillDurationMetricName,
        unit: "ms",
        description: "Duration of targeted LuckyNemo chat history backfills.");
    private static readonly Counter<long> DroppedRemoteTurns = LuckyNemoTelemetry.CreateCounter(
        DroppedRemoteTurnsMetricName,
        unit: "{turn}",
        description: "Number of remote LuckyNemo chat turns not traced because safe correlation was unavailable.");
    private static readonly Counter<long> DroppedTerminalEvents = LuckyNemoTelemetry.CreateCounter(
        DroppedTerminalEventsMetricName,
        unit: "{event}",
        description: "Number of terminal LuckyNemo chat events not applied because safe correlation was unavailable.");
    ...
    public void StartLocalTurn(string messageId, string threadId, bool queued)
    {
        ...
        lock (_gate)
        {
            ...
            var state = new TurnState(
                ...
                threadId,
                SourceLocal,
                LuckyNemoTelemetry.StartDetachedActivity(
                    TurnSpanName,
                    default(ActivityContext),
                    [LuckyNemoTelemetryTag.String(LuckyNemoTelemetryTagKey.Source, SourceLocal)]),
    ...
    public void ObserveLifecycleStart(string threadId, string? runId, bool allowRemoteTurn = true)
    {
        ...
        if (string.IsNullOrWhiteSpace(runId))
        {
            ...
            LuckyNemoTelemetry.Add(
                DroppedRemoteTurns,
                tags: [LuckyNemoTelemetryTag.String(DroppedRemoteTurnReasonTag, MissingRunId)]);
            return;
        }
        ...
        lock (_gate)
        {
            ...
            var remote = new TurnState(
                ...
                LuckyNemoTelemetry.StartDetachedActivity(
                    TurnSpanName,
                    default(ActivityContext),
                    [LuckyNemoTelemetryTag.String(LuckyNemoTelemetryTagKey.Source, SourceRemote)]),
                Stopwatch.GetTimestamp());
        ...
        Exception? exception = null)
    {
        ...
        var tags = new[]
        {
            LuckyNemoTelemetryTag.String(LuckyNemoTelemetryTagKey.Outcome, ToTelemetryValue(outcome)),
            LuckyNemoTelemetryTag.String(AdmissionStatusTag, ToTelemetryValue(status)),
        };
        FinishActivity(operation.Activity, outcome, tags, exception);
        LuckyNemoTelemetry.Add(SendAttempts, tags: tags);
    ...
    public ChatTelemetryOperation StartHistoryLoad(ChatHistoryTelemetrySource source)
    {
        var tags = new[]
        {
            LuckyNemoTelemetryTag.String(LuckyNemoTelemetryTagKey.Source, ToTelemetryValue(source)),
        };
        ...
        Exception? exception = null)
    {
        ...
        FinishActivity(operation.Activity, outcome, tags, exception);
        LuckyNemoTelemetry.Add(HistoryLoads, tags: tags);
        LuckyNemoTelemetry.Record(
            HistoryLoadDuration,
            Stopwatch.GetElapsedTime(operation.StartTimestamp).TotalMilliseconds,
            tags);
    ...
    public ChatTelemetryOperation StartHistoryBackfill(ChatBackfillTelemetryReason reason)
    {
        ...
        {
            LuckyNemoTelemetryTag.String(LuckyNemoTelemetryTagKey.Source, SourceRemote),
            LuckyNemoTelemetryTag.String(BackfillReasonTag, ToTelemetryValue(reason)),
        };
        return new ChatTelemetryOperation(
            LuckyNemoTelemetry.StartDetachedActivity(
        ...
        Exception? exception = null)
    {
        ...
        var tags = AppendOutcome(operation.Tags, outcome);
        FinishActivity(operation.Activity, outcome, tags, exception);
        LuckyNemoTelemetry.Add(HistoryBackfills, tags: tags);
        LuckyNemoTelemetry.Record(
    ...
    private static void FinishTurn(PreparedTurnCompletion completion)
    {
        ...
        var tags = new[]
        {
            LuckyNemoTelemetryTag.String(LuckyNemoTelemetryTagKey.Source, completion.Source),
            LuckyNemoTelemetryTag.String(LuckyNemoTelemetryTagKey.Outcome, ToTelemetryValue(completion.Outcome)),
            LuckyNemoTelemetryTag.String(LuckyNemoTelemetryTagKey.Reason, ToTelemetryValue(completion.Reason)),
        };
        FinishActivity(completion.Activity, completion.Outcome, tags);
        LuckyNemoTelemetry.Add(Turns, tags: tags);
        LuckyNemoTelemetry.Record(
            TurnDuration,
    ...
    private static LuckyNemoTelemetryTag[] ResponseMetricTags(PreparedTurnCompletion completion) =>
    [
        LuckyNemoTelemetryTag.String(LuckyNemoTelemetryTagKey.Source, completion.Source),
        LuckyNemoTelemetryTag.String(LuckyNemoTelemetryTagKey.Outcome, ToTelemetryValue(completion.Outcome)),
        LuckyNemoTelemetryTag.String(FirstOutputKindTag, ToTelemetryValue(completion.FirstOutputKind)),
    ];
        ...
        completion.Complete(() =>
        {
            ...
            FinishActivity(
                ...
                [
                    LuckyNemoTelemetryTag.String(LuckyNemoTelemetryTagKey.Source, completion.Source),
                    LuckyNemoTelemetryTag.String(
                        LuckyNemoTelemetryTagKey.Outcome,
    ...
    private static void FinishActivity(
        Activity? activity,
        ChatTelemetryOutcome outcome,
        IEnumerable<LuckyNemoTelemetryTag> tags,
        Exception? exception = null)
    {
        if (activity is null)
            return;
        ...
        switch (outcome)
        {
            ...
            case ChatTelemetryOutcome.Failure:
                ...
                {
                    activity.SetTag(
                        LuckyNemoTelemetryTagKey.ErrorType.ToTelemetryName(),
    ...
    private static LuckyNemoTelemetryTag[] AppendOutcome(
        IReadOnlyList<LuckyNemoTelemetryTag> tags,
        ChatTelemetryOutcome outcome) =>
        [.. tags, LuckyNemoTelemetryTag.String(LuckyNemoTelemetryTagKey.Outcome, ToTelemetryValue(outcome))];
        ...
        long startTimestamp)
    {
        var tags = new[]
        {
            LuckyNemoTelemetryTag.String(LuckyNemoTelemetryTagKey.Source, state.Source),
        };
        var activity = state.Activity is not null
            ? LuckyNemoTelemetry.StartDetachedActivity(spanName, state.Activity.Context, tags)
            : LuckyNemoTelemetry.StartDetachedActivity(spanName, default(ActivityContext), tags);
...
internal sealed class ChatTelemetryOperation(
    Activity? activity,
    long startTimestamp,
    IReadOnlyList<LuckyNemoTelemetryTag>? tags = null)
{
    private int _finished;

    public Activity? Activity { get; } = activity;
    public long StartTimestamp { get; } = startTimestamp;
    public IReadOnlyList<LuckyNemoTelemetryTag> Tags { get; } = tags ?? [];
