...
internal sealed class TrayDashboardSummaryBuilder
{
    ...
    private (TrayHealthSeverity, string) ClassifyHealth()
    {
        ...
        if (_snapshot.OverallState is { } overall)
        {
            ...
            {
                LuckyNemo.Connection.OverallConnectionState.Connected or
                LuckyNemo.Connection.OverallConnectionState.Ready => (TrayHealthSeverity.Ok, "Connected"),
                LuckyNemo.Connection.OverallConnectionState.Connecting => (TrayHealthSeverity.Caution, "Connecting…"),
                LuckyNemo.Connection.OverallConnectionState.Degraded => (TrayHealthSeverity.Caution, "Connection degraded"),
                LuckyNemo.Connection.OverallConnectionState.PairingRequired => (TrayHealthSeverity.Caution, "Pairing required"),
                LuckyNemo.Connection.OverallConnectionState.Error => (TrayHealthSeverity.Critical, "Connection error"),
    ...
    private bool IsStandaloneMcpOnly() =>
        ...
        (_snapshot.OverallState is null or LuckyNemo.Connection.OverallConnectionState.Idle) &&
