...
public static class Logger
{
    ...
    static Logger()
    {
        // LUCKYNEMO_TRAY_DATA_DIR keeps test instances out of the user's log file.
        ...
        _logFilePath = Path.Combine(_logDirectory, "luckynemo-tray.log");
    ...
    // LUCKYNEMO_TRAY_TRACE=1 surfaces verbose protocol traces that are normally
    ...
    private static readonly bool s_traceEnabled =
        string.Equals(Environment.GetEnvironmentVariable("LUCKYNEMO_TRAY_TRACE"), "1", StringComparison.Ordinal) ||
        string.Equals(Environment.GetEnvironmentVariable("LUCKYNEMO_TRAY_TRACE"), "true", StringComparison.OrdinalIgnoreCase);
    ...
    private static void TryRotate()
    {
        try
        {
            ...
            {
                var backupPath = Path.Combine(_logDirectory, "luckynemo-tray.log.old");
    ...
    private static void ReportWriteFailure(string detail)
    {
        ...
        try { System.Diagnostics.Trace.WriteLine($"[LuckyNemo Logger] {detail}"); }
        catch (Exception) { /* fallback diagnostics in the logger itself — cannot recurse into Log(). */ }
#if DEBUG
        try { System.Diagnostics.Debug.WriteLine($"[LuckyNemo Logger] {detail}"); }
