...
internal sealed class OpenTelemetryOtlpProbeSink : IOpenTelemetryProbeSink
{
    ...
    private const string ExporterTagKey = "luckynemo.exporter";
    private const string ExporterProtocolTagKey = "luckynemo.exporter.protocol";
    private const string SignalTagKey = "luckynemo.signal";
    ...
    private static readonly Counter<long> ExporterProbeCounter = LuckyNemoTelemetry.CreateCounter(
        "luckynemo.telemetry.exporter.probes",
        unit: "{probe}",
        description: "Number of LuckyNemo telemetry exporter probe metrics sent.");
    ...
    public static IOpenTelemetryProbeSink Create(OpenTelemetryEndpointOptions options)
    {
        ...
        try
        {
            tracerProvider = Sdk.CreateTracerProviderBuilder()
                ...
                .AddSource(LuckyNemoActivitySourceName.LuckyNemo.ToTelemetryName())
                .AddOtlpExporter(exporter => ConfigureExporter(exporter, endpoint, options.Protocol, OpenTelemetryOtlpSignal.Traces))
                .Build();

            meterProvider = Sdk.CreateMeterProviderBuilder()
                .SetResourceBuilder(CreateResourceBuilder())
                .AddMeter(LuckyNemoMeterName.LuckyNemo.ToTelemetryName())
    ...
    public void SendProbe(OpenTelemetryEndpointOptions options)
    {
        LuckyNemoTelemetry.Trace(
            "luckynemo.telemetry.exporter.probe",
            static () => { },
        ...
        _probeLogger.Log(
            ...
            null,
            static (_, _) => "LuckyNemo telemetry exporter probe log sent.");
    }
    ...
    public void SendConnectionState(OpenTelemetryConnectionState state)
    {
        ...
        [
            new("luckynemo.connection.event", state.EventName),
            new("luckynemo.connection.state.overall", state.OverallState),
            new("luckynemo.connection.state.operator", state.OperatorState),
            new("luckynemo.connection.state.node", state.NodeState)
        ];
        _connectionLogger.Log(
            ...
            attributes,
            null,
            static (_, _) => "LuckyNemo gateway connection state changed.");
    }
    ...
    public void SendNodeToolCompletion(NodeToolTelemetryCompletion completion)
    {
        ...
        _nodeToolLogger.Log(
            ...
            null,
            static (_, _) => "LuckyNemo node tool invocation did not succeed.");
    }
    ...
    private static ResourceBuilder CreateResourceBuilder() =>
        ResourceBuilder.CreateDefault()
            .AddService(
                serviceName: LuckyNemoResourceName.WindowsTray.ToServiceName(),
    ...
    private static LuckyNemoTelemetryTag[] CreateProbeTags(OpenTelemetryEndpointOptions options, string signal) =>
    [
        ...
        NodeToolTelemetryCompletion completion)
    {
        var attributes = new List<KeyValuePair<string, object?>>
        {
            ...
            new(LuckyNemoTelemetryTagKey.Outcome.ToTelemetryName(), completion.Outcome.ToTelemetryValue()),
            new(LuckyNemoTelemetryTagKey.ErrorCategory.ToTelemetryName(), completion.ErrorCategory.ToTelemetryValue()),
            new("luckynemo.node.tool.duration_ms", completion.DurationMilliseconds),
        };
