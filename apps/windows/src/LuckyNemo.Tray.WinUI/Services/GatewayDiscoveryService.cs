...
/// <summary>
/// Discovers LuckyNemo gateways on the local network via mDNS/Bonjour.
/// Browses for _luckynemo-gw._tcp services and resolves SRV records for endpoints.
...
{
    private const string ServiceType = "_luckynemo-gw._tcp.local.";
    ...
    /// <summary>
    /// Discovers LuckyNemo gateways on localhost by enumerating listening TCP ports
        ...
        try
        {
            ...
            var tasks = ports.Select(async port =>
            {
                try
                {
                    ...
                    // Match the gateway's specific HTML title — not generic "LuckyNemo" which matches MCP too
                    if (response.Contains("<title>LuckyNemo Control</title>", StringComparison.OrdinalIgnoreCase))
                    {
