...
internal sealed class AssistantBridgeService
{
    ...
    private readonly LuckyNemoCliLauncher? _launcher;
    private readonly TimeSpan _timeout;

    public AssistantBridgeService(ILuckyNemoLogger logger, string userId = "owner")
        : this(logger, userId, ResolveLuckyNemoCli)
    {
    }
    ...
    internal AssistantBridgeService(
        ...
        string userId,
        Func<LuckyNemoCliLauncher?> launcherResolver,
        TimeSpan? commandTimeout = null)
    {
        ...
        var overrideRoot = Environment.GetEnvironmentVariable("LUCKYNEMO_BACKEND_ROOT");
        if (!string.IsNullOrWhiteSpace(overrideRoot))
        {
            if (TryNormalizeBackendRoot(overrideRoot, out var normalizedOverrideRoot))
            {
                ...
                {
                    _logger.Info($"Assistant bridge using LUCKYNEMO_BACKEND_ROOT launcher at '{_launcher.WorkingDirectory}'.");
                }
            }
            else
            {
                _logger.Warn("Assistant bridge ignored LUCKYNEMO_BACKEND_ROOT because it is not a fully qualified local checkout under a trusted parent.");
            }
        }
    }
    ...
    {
        var result = await RunLuckyNemoAsync(["dashboard", "bridge", "status", "--user", _userId, "--json"], cancellationToken);
        if (!result.Success)
        ...
        catch (JsonException ex)
        {
            ...
            return AssistantBridgeSnapshot.Unavailable("LuckyNemo returned status JSON the Companion could not read.");
        }
    }
    ...
    {
        var result = await RunLuckyNemoAsync(args, cancellationToken);
        return new AssistantCommandResult(result.Success, result.ErrorMessage);
    }

    private async Task<AssistantProcessResult> RunLuckyNemoAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (!process.Start())
                return AssistantProcessResult.Failed("Could not start the LuckyNemo backend command.");
            ...
            if (timedOut)
                return AssistantProcessResult.Failed("LuckyNemo backend command timed out.");

            if (process.ExitCode != 0)
            {
                var detail = string.IsNullOrWhiteSpace(stderr) ? "LuckyNemo backend command failed." : stderr.Trim();
        ...
        catch (Exception ex)
        {
            ...
            return AssistantProcessResult.Failed("LuckyNemo backend command could not be started.");
        }
    }
    ...
    internal static LuckyNemoCliLauncher? ResolveLuckyNemoCli()
    {
        foreach (var root in CandidateBackendRoots())
        {
            ...
            var luckynemoExe = Path.Combine(normalizedRoot, ".venv", "Scripts", "luckynemo.exe");
            if (File.Exists(luckynemoExe))
                return new LuckyNemoCliLauncher(luckynemoExe, normalizedRoot, []);

            var pythonExe = Path.Combine(normalizedRoot, ".venv", "Scripts", "python.exe");
            if (File.Exists(pythonExe))
                return new LuckyNemoCliLauncher(pythonExe, normalizedRoot, ["-m", "luckynemo.cli"]);
        }
    ...
    internal static string BuildBackendNotFoundMessage()
    {
        ...
        return "LuckyNemo backend checkout was not found. Set LUCKYNEMO_BACKEND_ROOT to a fully qualified local checkout under D:\\Projects, %USERPROFILE%\\Projects, or %USERPROFILE%\\source\\repos. Searched: " +
    ...
    internal static IReadOnlyList<string> CandidateBackendRoots()
    {
        ...
        candidates.Add(@"D:\Projects\LuckyNemo");
...
internal sealed record LuckyNemoCliLauncher(
