...
{
    // LUCKYNEMO_TRAY_DATA_DIR overrides both this and App.DataPath so an isolated test
    ...
    private static readonly byte[] ProtectedSecretEntropy = Encoding.UTF8.GetBytes("LuckyNemoTray.Settings.v1");
    ...
    public List<LuckyNemo.Shared.UserNotificationRule> UserRules
    {
    ...
    public bool ShowDiagnosticsEffective => _data.ShowDiagnostics ?? LuckyNemoTray.Helpers.DiagnosticsGate.BuildDefault;
    ...
    public void Save()
    {
        lock (_saveLock)
        {
            try
            {
                ...
                LuckyNemo.Shared.Mcp.McpAuthToken.TryRestrictDataDirectoryAcl(_settingsDirectory);
    ...
    internal static bool CanProtectSettingSecretsForCurrentUser()
    {
        ...
        try
        {
            var bytes = Encoding.UTF8.GetBytes("luckynemo-dpapi-probe");
            var protectedBytes = ProtectedData.Protect(bytes, ProtectedSecretEntropy, DataProtectionScope.CurrentUser);
