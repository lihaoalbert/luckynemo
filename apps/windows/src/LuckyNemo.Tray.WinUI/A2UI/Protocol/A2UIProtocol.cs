...
namespace LuckyNemoTray.A2UI.Protocol;
...
public static class A2UIMessageParser
{
    ...
    public static IEnumerable<A2UIMessage> Parse(string jsonl, LuckyNemo.Shared.ILuckyNemoLogger? logger)
    {
            ...
            if (lineLen > 0)
            {
                ...
                if (trimmed.Length > 0)
                {
                    ...
                    else
                    {
                        ...
                        {
                            logger?.Warn($"[A2UI] dropping malformed JSONL line: {LuckyNemo.Shared.TokenSanitizer.Sanitize(Truncate(trimmed, 200))} — {ex.Message}");
                        }
