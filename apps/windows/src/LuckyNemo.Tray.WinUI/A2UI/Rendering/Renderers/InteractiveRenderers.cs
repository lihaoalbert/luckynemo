...
public sealed class DateTimeInputRenderer : IComponentRenderer
{
    ...
    public FrameworkElement Render(A2UIComponentDef c, RenderContext ctx)
    {
        ...
        if (enableDate)
        {
            datePicker = new CalendarDatePicker { PlaceholderText = LuckyNemoTray.Helpers.LocalizationHelper.GetString("A2UI_DateTimeSelectDate") };
...
public sealed class MultipleChoiceRenderer : IComponentRenderer
{
    ...
    public FrameworkElement Render(A2UIComponentDef c, RenderContext ctx)
    {
        ...
        if (single)
        {
            var combo = new ComboBox { PlaceholderText = LuckyNemoTray.Helpers.LocalizationHelper.GetString("A2UI_MultipleChoiceSelect") };
