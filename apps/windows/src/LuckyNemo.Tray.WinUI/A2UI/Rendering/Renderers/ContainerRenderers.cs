...
using LuckyNemoTray.A2UI.Protocol;

namespace LuckyNemoTray.A2UI.Rendering.Renderers;
    ...
    public FrameworkElement Render(A2UIComponentDef c, RenderContext ctx)
    {
        ...
        if (c.Properties["tabItems"] is JsonArray arr)
        {
            ...
            foreach (var node in arr)
            {
                ...
                var titleText = ctx.ResolveString(titleVal) ?? LuckyNemoTray.Helpers.LocalizationHelper.GetString("A2UI_TabFallback");
                ...
                if (titleVal?.HasPath == true)
                {
                    ...
                    void Update() => tab.Header = ctx.ResolveString(titleVal) ?? LuckyNemoTray.Helpers.LocalizationHelper.GetString("A2UI_TabFallback");
    ...
    public FrameworkElement Render(A2UIComponentDef c, RenderContext ctx)
    {
        ...
        trigger.Click += async (_, _) =>
        {
            ...
            var dialog = new ContentDialog
            {
                ...
                CloseButtonText = LuckyNemoTray.Helpers.LocalizationHelper.GetString("A2UI_ModalClose"),
            };
