...
namespace LuckyNemoTray.A2UI.Rendering;
...
public sealed class MediaResolver : IDisposable
{
    ...
    public MediaResolver(ILuckyNemoLogger logger) : this(logger, handler: null) { }

    /// <summary>
    /// Test seam: pass a custom <see cref="HttpMessageHandler"/> to stub the
    /// network. Production callers use the parameterless overload.
    /// </summary>
    public MediaResolver(ILuckyNemoLogger logger, HttpMessageHandler? handler)
    {
        _logger = logger;
        _http = handler != null
    ...
    /// </summary>
    private static SocketsHttpHandler BuildSafeHandler(ILuckyNemoLogger logger)
    {
