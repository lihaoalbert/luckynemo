...
using LuckyNemoTray.A2UI.DataModel;
using LuckyNemoTray.A2UI.Hosting;
...
namespace LuckyNemoTray.Windows;
...
public sealed partial class A2UICanvasWindow : WindowEx
{
    ...
    /// </summary>
    public A2UICanvasWindow(IActionSink actions, MediaResolver media, ILuckyNemoLogger logger)
    {
        InitializeComponent();
        this.SetIcon("Assets\\luckynemo.ico");
        ...
        Title = LuckyNemoTray.Helpers.LocalizationHelper.GetString("A2UI_CanvasTitle");
        WaitingForContentText.Text = LuckyNemoTray.Helpers.LocalizationHelper.GetString("Canvas_WaitingForContent");
        ...
        Closed += (_, _) =>
        {
            ...
            catch (Exception ex) { LuckyNemoTray.Services.Logger.Debug($"A2UICanvasWindow: unsubscribe SurfaceCreated failed: {ex.Message}"); }
            try { _router.SurfaceDeleted -= OnSurfaceDeleted; }
            catch (Exception ex) { LuckyNemoTray.Services.Logger.Debug($"A2UICanvasWindow: unsubscribe SurfaceDeleted failed: {ex.Message}"); }
            try { _router.ResetAll(); }
            catch (Exception ex) { LuckyNemoTray.Services.Logger.Debug($"A2UICanvasWindow: router ResetAll on close failed: {ex.Message}"); }
    ...
    private void UpdateLayout()
    {
        ...
        if (surfaces.Count == 1)
        {
            ...
            Title = string.IsNullOrWhiteSpace(s.Title)
                ? LuckyNemoTray.Helpers.LocalizationHelper.GetString("A2UI_CanvasTitle")
                : s.Title!;
            return;
        }
    ...
    private void EnterFullScreen()
    {
        ...
        catch (Exception ex) { LuckyNemoTray.Services.Logger.Debug($"A2UICanvasWindow: EnterFullScreen failed: {ex.Message}"); }
    }
    ...
    private void ExitFullScreen()
    {
        ...
        catch (Exception ex) { LuckyNemoTray.Services.Logger.Debug($"A2UICanvasWindow: ExitFullScreen failed: {ex.Message}"); }
    }
    ...
    public void BringToFront(bool keepTopMost = false)
    {
        ...
        catch (Exception ex) { LuckyNemoTray.Services.Logger.Debug($"A2UICanvasWindow: best-effort foreground/topmost adjust failed: {ex.Message}"); }
    }
}
