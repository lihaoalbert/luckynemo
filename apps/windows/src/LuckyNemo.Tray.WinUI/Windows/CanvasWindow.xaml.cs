...
public sealed partial class CanvasWindow : WindowEx
{
    ...
    private bool IsUrlSafe(string url)
    {
        ...
        if (url.StartsWith("https://luckynemo-canvas.local/", StringComparison.OrdinalIgnoreCase) ||
            url.Equals("https://luckynemo-canvas.local", StringComparison.OrdinalIgnoreCase))
        {
            return true;
        }
        ...
        if (Uri.TryCreate(url, UriKind.Absolute, out var parsedUri) &&
            LuckyNemo.Shared.CanvasUrlSafety.IsPrivateOrLoopbackHost(parsedUri.Host))
        {
            return false;
        }
    ...
    private string RewriteGatewayUrl(string url)
    {
        ...
        try
        {
            ...
            if (url.Contains("/__luckynemo__/canvas/documents/") && !string.IsNullOrEmpty(_canvasDir))
            {
                var pathPart = new Uri(url).AbsolutePath;
                var localRelative = pathPart.Replace("/__luckynemo__/canvas/documents/", "");
                ...
                {
                    var localUrl = $"https://luckynemo-canvas.local/{localRelative}";
    ...
    public CanvasWindow()
    {
        ...
        Title = AppIdentity.DecorateWindowTitle("LuckyNemo Canvas");
    ...
    private async Task InitializeWebViewCoreAsync()
    {
        try
        {
            ...
            // Files in %LOCALAPPDATA%/LuckyNemoTray/canvas/ are served at
            // https://luckynemo-canvas.local/
            Directory.CreateDirectory(_canvasDir);
            CanvasWebView.CoreWebView2.SetVirtualHostNameToFolderMapping(
                "luckynemo-canvas.local",
                _canvasDir,
                CoreWebView2HostResourceAccessKind.Allow);
            Logger.Info($"[Canvas] Virtual host mapped: luckynemo-canvas.local → {_canvasDir}");
    ...
    private static bool IsTrustedA2UIUrl(string url)
    {
        ...
        return uri.AbsolutePath.StartsWith("/__luckynemo__/a2ui/", StringComparison.OrdinalIgnoreCase);
    }
    ...
    private bool IsTrustedBridgeSource(string? source)
    {
        ...
        if (string.Equals(sourceOrigin.Scheme, "https", StringComparison.OrdinalIgnoreCase) &&
            string.Equals(sourceOrigin.IdnHost, "luckynemo-canvas.local", StringComparison.OrdinalIgnoreCase))
            return true;
