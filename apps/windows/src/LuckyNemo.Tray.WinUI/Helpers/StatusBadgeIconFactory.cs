...
internal static class StatusBadgeIconFactory
{
    ...
    private static readonly string OutputDir = Path.Combine(Path.GetTempPath(), "LuckyNemoTray", "StatusIcons");
    ...
    {
        var fallback = Path.Combine(AssetsPath, "luckynemo.ico");
        try
        {
            Directory.CreateDirectory(OutputDir);
            var outputPath = Path.Combine(OutputDir, $"luckynemo-{accent}".ToLowerInvariant() + ".ico");
    ...
    private static Bitmap LoadBaseImage()
    {
        ...
        var ico = Path.Combine(AssetsPath, "luckynemo.ico");
