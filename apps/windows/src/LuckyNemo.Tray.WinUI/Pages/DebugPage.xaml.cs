...
public sealed partial class DebugPage : Page
{
    ...
    private IGatewayTerminalLauncher TerminalLauncher =>
        _terminalLauncher ??= new GatewayTerminalLauncher(new LuckyNemoTray.AppLogger());

    private static readonly string LocalAppData = AppIdentity.ResolveLocalDataDirectory();
    private static readonly string LogPath = Path.Combine(LocalAppData, "luckynemo-tray.log");
    ...
    private void OnRunGatewayDoctor(object sender, RoutedEventArgs e)
    {
        ...
        {
            LuckyNemoTray.Services.Logger.Warn($"[DebugPage] Failed to launch gateway doctor: {ex.Message}");
        }
    }
    ...
    private async Task ShowDiagnosticsBundlePreviewAsync()
    {
        ...
        try
        {
            ...
            await ShowBundlePreviewAsync(
                ...
                suggestedFileName: $"luckynemo-diagnostics-{DateTime.Now:yyyyMMdd-HHmmss}.txt",
