...
public sealed partial class SettingsPage : Page
{
    ...
    private const string DocumentationUrl = "https://docs.luckynemo.ai/platforms/windows";
    private const string GitHubUrl = "https://github.com/lihaoalbert/LuckyNemo";
    ...
    private void WireAutoSaveHandlers()
    {
        ...
        ShowToolCallsToggle.Toggled += (_, _) =>
        {
            ...
            LuckyNemoTray.Chat.LuckyNemoChatRoot.SetToolCallsVisible(ShowToolCallsToggle.IsOn);
        };
    }
    ...
    private void LoadGatewaySection(SettingsManager settings)
    {
        ...
        LuckyNemoOnboardCard.Visibility = activeGatewayAccess.CanControlWslGateway
    ...
    private void OnTestNotification(object sender, RoutedEventArgs e)
    {
        try
        {
            new ToastContentBuilder()
                ...
                .AddText("This is a test notification from LuckyNemo settings.")
                .Show();
        }
        catch (Exception ex)
        {
    ...
    {
        var channel = Environment.GetEnvironmentVariable("LUCKYNEMO_UPDATE_CHANNEL");
    ...
    private async Task OnRemoveGatewayAsync()
    {
        ...
        try
        {
            var exePath = ResolveCurrentExecutablePath()
                ?? throw new FileNotFoundException("LuckyNemo tray executable could not be resolved for local gateway removal.");

            jsonOutput = Path.Combine(Path.GetTempPath(), $"luckynemo-uninstall-{Guid.NewGuid():N}.json");
            ...
            proc = Process.Start(psi) ?? throw new InvalidOperationException("Failed to start LuckyNemo uninstall process.");
            await proc.WaitForExitAsync(_uninstallCts.Token);

            if (proc.ExitCode == 0)
            {
                CurrentApp.Registry?.Load();
                LuckyNemoOnboardCard.Visibility = Visibility.Collapsed;
