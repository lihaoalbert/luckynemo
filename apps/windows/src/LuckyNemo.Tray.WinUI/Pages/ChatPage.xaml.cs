...
public sealed partial class ChatPage : Page
{
    ...
    private void OnUnloaded(object sender, RoutedEventArgs e)
    {
        ...
        LuckyNemoTray.Chat.DebugChatSurfaceOverrides.Changed -= OnDebugOverrideChanged;
    }
    ...
    private IChatDataProvider? ResolveChatProvider(App? app)
    {
        ...
        if (Environment.GetEnvironmentVariable("LUCKYNEMO_ACCESSIBILITY_TEST_CHAT") == "1"
            && Environment.GetEnvironmentVariable("LUCKYNEMO_TRAY_DATA_DIR") is { Length: > 0 })
        {
    ...
    {
        if (Environment.GetEnvironmentVariable("LUCKYNEMO_VISUAL_TEST") != "1") return;
        if (WebView.CoreWebView2 == null) return;

        try
        {
            await Task.Delay(5000);
            var outputDir = Environment.GetEnvironmentVariable("LUCKYNEMO_VISUAL_TEST_DIR");
