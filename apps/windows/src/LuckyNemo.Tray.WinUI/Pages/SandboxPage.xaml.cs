...
public sealed partial class SandboxPage : Page
{
    ...
    /// </summary>
    private LuckyNemo.Shared.Mxc.MxcAvailability? _cachedAvailability;
    ...
    private async System.Threading.Tasks.Task RefreshAvailabilityAsync()
    {
        ...
        try
        {
            _cachedAvailability = await System.Threading.Tasks.Task.Run(
                static () => LuckyNemo.Shared.Mxc.MxcAvailability.Probe());
        }
        catch (Exception)
        {
            ...
            _cachedAvailability = new LuckyNemo.Shared.Mxc.MxcAvailability(
    ...
    /// </summary>
    private void UpdateUnavailableActionBar(LuckyNemo.Shared.Mxc.MxcAvailability? availability, bool sandboxEnabled)
    {
    ...
    private async Task OnSandboxEnabledToggledAsync()
    {
        ...
        if (!newValue && oldValue)
        {
            ...
            var dialog = new ContentDialog
            {
                ...
                Content = "Agent-started Windows commands will run as you and may access your files, network, clipboard, and LuckyNemo settings.\n\nThis is the high-risk mode. Only do this if you trust the agent and need it for debugging or performance.",
