...
internal sealed record ConnectionPagePlan
{
    ...
    // ─── Status strip ───
    public string StripGlyph { get; init; } = LuckyNemoTray.Helpers.FluentIconCatalog.System; // PC1 default — "no gateway yet"
        ...
        string displayName)
    {
        ...
        return snap.OverallState switch
        {
            ...
            OverallConnectionState.Disconnecting => new ConnectionPagePlan
            {
                ...
                StripGlyph = LuckyNemoTray.Helpers.FluentIconCatalog.Sync,
        ...
        SettingsManager? settings)
    {
        ...
        if (savedCount == 0)
        {
            return new ConnectionPagePlan
            {
                ...
                StripGlyph = LuckyNemoTray.Helpers.FluentIconCatalog.System,
                StripAccent = ConnectionAccent.Neutral,
        ...
        GatewayConnectionSnapshot snap, GatewayRecord? rec, string name)
    {
        ...
        {
            Mode = ConnectionPageMode.Cockpit,
            StripGlyph = LuckyNemoTray.Helpers.FluentIconCatalog.Sync, // replaced visually by ProgressRing
        ...
        string name)
    {
        ...
        {
            Mode = ConnectionPageMode.Cockpit,
            StripGlyph = LuckyNemoTray.Helpers.FluentIconCatalog.StatusOk,
        ...
        string name)
    {
        ...
        {
            Mode = ConnectionPageMode.Cockpit,
            StripGlyph = LuckyNemoTray.Helpers.FluentIconCatalog.StatusWarn,
            StripAccent = ConnectionAccent.Caution,
        ...
        string name)
    {
        ...
        if (operatorPairing)
        {
            ...
            return new ConnectionPagePlan
            {
                ...
                StripGlyph = LuckyNemoTray.Helpers.FluentIconCatalog.Lock,
                StripAccent = ConnectionAccent.Caution,
        ...
        GatewayConnectionSnapshot snap, GatewayRecord? rec, string name)
    {
            ...
            IsManagedTailscaleGateway(rec, rec?.Url ?? snap.GatewayUrl))
        {
            return new ConnectionPagePlan
            {
                ...
                StripGlyph = LuckyNemoTray.Helpers.FluentIconCatalog.StatusErr,
    ...
    private static string? BuildDevicePairingApproveCommand(GatewayConnectionSnapshot snap)
    {
        ...
        var reqId = !string.IsNullOrEmpty(snap.OperatorPairingRequestId)
            ...
            : null;
        // Noun-first per luckynemo/src/cli/devices-cli.ts:
        // `luckynemo devices approve <requestId>`. Mirror BuildNodeApproveCommand:
        // single discovery command in the clipboard, no shell-hostile suffix.
        return reqId != null
            ? $"luckynemo devices approve {reqId}"
            : "luckynemo devices list";
    }
    ...
    private static RecoveryCategory ClassifyError(string err)
    {
        ...
        return LuckyNemo.Shared.GatewayErrorClassifier.Classify(err) switch
        {
            LuckyNemo.Shared.GatewayErrorKind.ScopeMismatch => RecoveryCategory.Scope,
            LuckyNemo.Shared.GatewayErrorKind.TokenDrift => RecoveryCategory.TokenDrift,
            LuckyNemo.Shared.GatewayErrorKind.Auth => RecoveryCategory.Auth,
            LuckyNemo.Shared.GatewayErrorKind.Tls => RecoveryCategory.Tls,
            LuckyNemo.Shared.GatewayErrorKind.Tunnel => RecoveryCategory.Tunnel,
            LuckyNemo.Shared.GatewayErrorKind.Server => RecoveryCategory.Server,
            LuckyNemo.Shared.GatewayErrorKind.RateLimited => RecoveryCategory.RateLimited,
            LuckyNemo.Shared.GatewayErrorKind.PairingRejected => RecoveryCategory.Auth,
            ...
            LuckyNemo.Shared.GatewayErrorKind.PairingRequired => RecoveryCategory.Auth,
