...
using LuckyNemo.Shared.Capabilities;
using LuckyNemo.Shared.Sessions;
using LuckyNemo.Shared.Mxc;
using LuckyNemo.Shared.Telemetry;
using LuckyNemoTray.Dialogs;
using LuckyNemoTray.Helpers;
using LuckyNemoTray.Services;
using LuckyNemoTray.Windows;
using LuckyNemo.Connection;
using Microsoft.Extensions.DependencyInjection;
using LuckyNemoTray.Presentation;
using LuckyNemoTray.Presentation.Adapters;
using System;
...
using Updatum;
using WinUIEx;
using SetupCompletedEventArgs = LuckyNemo.SetupEngine.UI.SetupCompletedEventArgs;
using SetupWindow = LuckyNemo.SetupEngine.UI.SetupWindow;

namespace LuckyNemoTray;

public partial class App : Application, LuckyNemoTray.Services.IAppCommands
{
    // Monorepo: Windows app releases ship as windows-v* tags in the main repo
    // (see REBRAND-NOTES.md). TODO(monorepo): FetchOnlyLatestRelease only works
    // when the Windows app has the latest release in the repo; with mixed
    // release streams (gateway vYYYY.M.PATCH vs windows-v*) the updater must
    // instead pick the newest windows-v* release (Updatum has no tag filter —
    // verify on Windows whether to fetch the release list manually or move
    // Windows releases back to a dedicated repository).
    internal static readonly UpdatumManager AppUpdater = new("lihaoalbert", "LuckyNemo")
    {
        FetchOnlyLatestRelease = true,
        InstallUpdateSingleFileExecutableName = "LuckyNemo.Tray.WinUI",
    };
    ...
    private LuckyNemoTray.Chat.LuckyNemoChatCoordinator? _chatCoordinator;
    ...
    public LuckyNemoTray.Chat.LuckyNemoChatDataProvider? ChatProvider => _chatCoordinator?.Provider;
    ...
    private LuckyNemoTray.Dialogs.PairingApprovalDialog? _pairingApprovalDialog;
    ...
    // LUCKYNEMO_TRAY_DATA_DIR isolates a test instance: settings, logs, run marker,
    ...
    private static readonly string? DataDirOverride =
        Environment.GetEnvironmentVariable("LUCKYNEMO_TRAY_DATA_DIR") is { Length: > 0 } v ? v : null;
    ...
    // Isolated test/dev runs set LUCKYNEMO_TRAY_DATA_DIR to the direct LuckyNemo data
    ...
    private static readonly string IdentityDataPath = DataDirOverride
        ?? Path.Combine(
            Environment.GetEnvironmentVariable("LUCKYNEMO_TRAY_APPDATA_DIR")
    ...
    public App()
    {
        ...
        // Language override for localization testing (e.g., LUCKYNEMO_LANGUAGE=zh-CN)
        var langOverride = Environment.GetEnvironmentVariable("LUCKYNEMO_LANGUAGE");
        if (!string.IsNullOrEmpty(langOverride))
        {
            ...
            if (allowedLocales.Contains(langOverride.ToLowerInvariant()))
                ...
                Logger.Warn($"[App] Ignoring invalid LUCKYNEMO_LANGUAGE value: {langOverride}");
        }
    ...
    private void InitializeServiceProvider()
    {
        ...
        services.AddLuckyNemoTrayCore(context);
    ...
    private async Task OnLaunchedAsync(LaunchActivatedEventArgs args)
    {
        ...
        LuckyNemoTray.Chat.LuckyNemoChatRoot.SetToolCallsVisible(_settings.ShowChatToolCalls);
        ...
        _chatCoordinator = new LuckyNemoTray.Chat.LuckyNemoChatCoordinator(
            _settings,
            () => _nodeService,
            new AppLogger(),
            _dispatcherQueue is null
                ? null
                : LuckyNemoTray.Chat.FunctionalChatHostExtensions.AsPost(_dispatcherQueue));
        ...
        if (DataDirOverride is null &&
            Environment.GetEnvironmentVariable("LUCKYNEMO_SKIP_UPDATE_CHECK") != "1")
        {
        ...
        try
        {
            if ((!_isPostSetupRestart && RequiresSetup(_settings)) ||
                Environment.GetEnvironmentVariable("LUCKYNEMO_FORCE_ONBOARDING") == "1")
            {
    ...
    */
    ...
    private async Task<bool> ConfirmDeepLinkActionAsync(DeepLinkResult result)
    {
        ...
        var dialog = new ContentDialog
        {
            Title = "Confirm LuckyNemo action",
    ...
    /// <summary>
    /// Opt-in design preview: when the <c>LUCKYNEMO_TRAY_PREVIEW_DATA</c>
    ...
    {
        var flag = Environment.GetEnvironmentVariable("LUCKYNEMO_TRAY_PREVIEW_DATA");
    ...
    private LuckyNemo.Connection.GatewayCredential? ResolveStartupOperatorCredential(
    ...
    private LuckyNemo.Connection.GatewayCredential? ResolveStartupNodeCredential(
        GatewayRecord record,
        CredentialResolver resolver,
        string identityDir)
    {
    ...
    private void TryMigrateLegacyGatewaySettings(string gatewayUrl, ILuckyNemoLogger logger)
    {
    ...
    private void OnOperatorClientChanged(object? sender, OperatorClientChangedEventArgs e)
    {
        ...
        if (e.NewClient is { } client)
        {
            ...
            var concreteClient = client as LuckyNemoGatewayClient;
            if (concreteClient == null)
                Logger.Warn("[ConnMgr] NewClient is not LuckyNemoGatewayClient — chat coordinator disabled");
    ...
    private void OnPairingStatusChanged(object? sender, LuckyNemo.Shared.PairingStatusEventArgs args)
    {
        ...
        try
        {
            if (args.Status == LuckyNemo.Shared.PairingStatus.Pending)
            {
                var approvalCommand = args.ApprovalKind switch
                {
                    LuckyNemo.Shared.PairingApprovalKind.DevicePair => BuildPairingApprovalCommand(args.DeviceId),
                    LuckyNemo.Shared.PairingApprovalKind.NodePair => CommandCenterDiagnostics.BuildNodeApprovalRepairCommand(args.RequestId),
            ...
            else if (args.Status == LuckyNemo.Shared.PairingStatus.Paired)
            {
            ...
            else if (args.Status == LuckyNemo.Shared.PairingStatus.Rejected)
            {
    ...
    public static string BuildPairingApprovalCommand(string deviceId) =>
        $"luckynemo devices approve {deviceId}";
    ...
    private void OnNodeNotificationRequested(object? sender, LuckyNemo.Shared.Capabilities.SystemNotifyArgs args)
    {
    ...
    private void OnGatewayNotificationReceived(object? sender, LuckyNemoNotification notification)
    {
        ...
        try
        {
            var builder = new ToastContentBuilder()
                .AddText(notification.Title ?? "LuckyNemo")
    ...
    private static string? GetNotificationIcon(string? type)
    {
        ...
        var iconPath = System.IO.Path.Combine(appDir, "Assets", "luckynemo.ico");
        return System.IO.File.Exists(iconPath) ? iconPath : null;
    }

    private bool ShouldShowNotification(LuckyNemoNotification notification)
    {
    ...
    private void ShowPairingApprovalDialog(bool bringToFront)
    {
        ...
        _pairingApprovalDialog = new LuckyNemoTray.Dialogs.PairingApprovalDialog(_pairingApprovalCoordinator);
    ...
    private async Task<(SetupWindow? Window, bool CreatedNew)> EnsureSetupWindowAsync(bool startAtGatewayInstalledMilestone = false)
    {
        ...
        try
        {
            ...
            setupWindow.Title = AppIdentity.DecorateWindowTitle("LuckyNemo Setup");
    ...
    private async Task ShowGatewayWizardAsync()
    {
        ...
        if (!createdNew)
        {
            if (setupWindow.TryNavigateToGatewayInstalledMilestone())
                Logger.Info("Setup window already open; switched to direct LuckyNemo onboard handoff");
            else
    ...
    private async Task RestartAfterSetupAsync(bool enableAutoStart)
    {
        ...
        {
            await ShowSetupRestartErrorAsync("LuckyNemo setup finished, but the tray executable could not be found for restart.");
            return;
        }
        ...
        catch (Exception ex)
        {
            ...
            await ShowSetupRestartErrorAsync("LuckyNemo setup finished, but restarting the tray failed. The current tray will keep running; please exit and reopen LuckyNemo.");
        }
    }
