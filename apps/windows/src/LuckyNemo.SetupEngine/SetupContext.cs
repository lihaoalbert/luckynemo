...
{
    public string DistroName { get; set; } = "LuckyNemoGateway";
    ...
    public static SetupConfig FromEnvironment(SetupConfig? baseConfig = null)
    {
        ...
        if (Environment.GetEnvironmentVariable("LUCKYNEMO_SETUP_DISTRO") is { Length: > 0 } distro)
            config.DistroName = distro;
        if (Environment.GetEnvironmentVariable("LUCKYNEMO_SETUP_PORT") is { Length: > 0 } port && int.TryParse(port, out var p))
            config.GatewayPort = p;
        if (Environment.GetEnvironmentVariable("LUCKYNEMO_SETUP_HEADLESS") is "1" or "true")
            config.Headless = true;
        if (Environment.GetEnvironmentVariable("LUCKYNEMO_SETUP_LOG_PATH") is { Length: > 0 } logPath)
            config.LogPath = logPath;
        if (Environment.GetEnvironmentVariable("LUCKYNEMO_SETUP_TAILSCALE") is "1" or "true")
            config.Tailscale.Enabled = true;
        if (Environment.GetEnvironmentVariable("LUCKYNEMO_SETUP_TAILSCALE_TRUST_AUTH") is "1" or "true")
        {
            config.Tailscale.Enabled = true;
            config.Tailscale.TrustTailscaleAuth = true;
        }
        if (Environment.GetEnvironmentVariable("LUCKYNEMO_SETUP_TAILSCALE_AUTH") is { Length: > 0 } authMode &&
            TailscaleConfig.TryParseAuthMode(authMode, out var parsedAuthMode))
            config.Tailscale.AuthMode = parsedAuthMode;
        if (Environment.GetEnvironmentVariable("LUCKYNEMO_SETUP_TAILSCALE_HOSTNAME") is { Length: > 0 } hostname)
            config.Tailscale.Hostname = hostname;
        if (Environment.GetEnvironmentVariable("LUCKYNEMO_SETUP_TAILSCALE_AUTH_KEY") is { Length: > 0 } authKey)
...
public sealed class WslConfig
{
    ...
    public string User { get; set; } = "luckynemo";
...
public sealed class SetupContext
{
    ...
    public static string ResolveDataDir()
        => Environment.GetEnvironmentVariable("LUCKYNEMO_TRAY_DATA_DIR")
            ?? Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "LuckyNemoTray");

    public static string ResolveLocalDataDir()
    {
        if (Environment.GetEnvironmentVariable("LUCKYNEMO_TRAY_LOCALAPPDATA_DIR") is { Length: > 0 } localAppDataRoot)
            return Path.Combine(localAppDataRoot, "LuckyNemoTray");

        // Compatibility alias used by early SetupEngine tests/builds. Unlike
        // LOCALAPPDATA_DIR, this points directly at the LuckyNemoTray data folder.
        if (Environment.GetEnvironmentVariable("LUCKYNEMO_TRAY_LOCAL_DATA_DIR") is { Length: > 0 } localDataDir)
            return localDataDir;

        return Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "LuckyNemoTray");
    }
}
