...
public static class SetupReviewSummaryBuilder
{
    ...
    {
        var distroName = Display(config.DistroName, "LuckyNemoGateway");
        ...
        var installerDescription = installerHost is null
            ...
            : $"Fetched over HTTPS from {installerHost}; runs as a non-root {Display(config.Wsl.User, "luckynemo")} user inside the instance.";
        ...
        return new SetupReviewSummary(
            ...
            ExactCommands: string.Join(
                ...
                new[]
                {
                    wslCommand,
                    installCommand,
                    $"luckynemo config set gateway.bind {gatewayBind} · port {gatewayPort}",
                    tailscaleEnabled
                        ? config.Tailscale.TrustTailscaleAuth
                        ...
                        : null,
                    "luckynemo gateway install --force   (systemd --user service)",
                    $"writes -> {installPath}",
