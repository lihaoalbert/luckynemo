...
{
    private const string DefaultHostnamePrefix = "luckynemo-";
    ...
    public static string? ValidateConfig(SetupConfig config)
    {
        ...
        if (tailscale.AuthMode == TailscaleAuthMode.AuthKey && string.IsNullOrWhiteSpace(tailscale.AuthKey))
            return "Tailscale auth-key mode requires LUCKYNEMO_SETUP_TAILSCALE_AUTH_KEY.";
    ...
    public static string NormalizeHostname(string? requestedHostname, string machineName)
    {
        ...
        return string.IsNullOrWhiteSpace(normalized) ? "luckynemo-gateway" : normalized;
    }
    ...
    /// loopback HTTP port proves that Companion will reach LuckyNemo.
    /// </summary>
...
public sealed class InstallTailscaleStep : SetupStep
{
    ...
    internal static string BuildInstallScript() => $"""
        ...
        if [ "$ID" != "ubuntu" ] || [ "$VERSION_ID" != "24.04" ] || [ "$VERSION_CODENAME" != "noble" ]; then
            echo "LuckyNemo's generated Tailscale gateway requires Ubuntu 24.04 (noble); found $ID $VERSION_ID $VERSION_CODENAME" >&2
            exit 1
        fi
        install -d -m 0755 /usr/share/keyrings
        curl -fsSL --proto '=https' --tlsv1.2 {NobleKeyringUrl} -o /tmp/luckynemo-tailscale-keyring.gpg
        install -m 0644 /tmp/luckynemo-tailscale-keyring.gpg /usr/share/keyrings/tailscale-archive-keyring.gpg
        rm -f /tmp/luckynemo-tailscale-keyring.gpg
...
public sealed class AuthorizeTailscaleStep : SetupStep
{
        ...
        CancellationToken ct)
    {
        ...
        await presenter.PresentAsync(
            new ExternalAuthorizationRequest("Tailscale", authorizationUrl, "Authorize the generated LuckyNemo gateway in your Tailscale tailnet:"),
            ct);
    }
...
public sealed class FinalizeTailscaleServeStep : SetupStep
{
    ...
    public override async Task<StepResult> ExecuteAsync(SetupContext ctx, CancellationToken ct)
    {
        ...
        var configurePairUrl = await ctx.Commands.RunInWslAsync(
            ctx.DistroName!,
            $"{ctx.WslPathPrefix} && luckynemo config set {ConfigureGatewayStep.DevicePairPublicUrlKey} {ShellEscape(devicePairPublicUrl)} && luckynemo config set {ConfigureGatewayStep.DevicePairEnabledKey} true",
    ...
    private async Task<StepResult?> WaitForServeRouteAsync(SetupContext ctx, CancellationToken ct)
    {
        ...
        while (_clock.UtcNow < deadline)
        {
            ...
            // Root is the durable owner of the Serve configuration. The LuckyNemo
