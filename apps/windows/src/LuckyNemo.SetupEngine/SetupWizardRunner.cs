...
public sealed class SetupWizardRunner
{
    ...
    {
        var registry = new GatewayRegistry(_ctx.DataDir, logger: new SetupLuckyNemoLogger(_ctx.Logger));
        registry.Load();
        ...
        var storedDeviceToken = DeviceIdentity.TryReadStoredDeviceToken(identityPath, new SetupLuckyNemoLogger(_ctx.Logger));
        ...
        var wsLogger = new SetupLuckyNemoLogger(_ctx.Logger);
    ...
    private LuckyNemoGatewayClient CreateWizardClient(string credential, string identityPath, ILuckyNemoLogger wsLogger)
    {
        return new LuckyNemoGatewayClient(_ctx.GatewayUrl!, credential, logger: wsLogger, identityPath: identityPath)
        {
            UseV2Signature = true
        };
    }

    private async Task TryCancelWizardAsync(LuckyNemoGatewayClient client, string sessionId)
    {
        try
        {
    ...
    private async Task TryResetReloadModeAsync()
    {
        try
        {
            var result = await _ctx.Commands.RunInWslAsync(
                _ctx.DistroName!,
                $"{_ctx.WslPathPrefix} && luckynemo config set gateway.reload.mode hybrid",
