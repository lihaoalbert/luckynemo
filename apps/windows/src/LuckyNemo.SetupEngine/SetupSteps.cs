...
// PATH prefix for all luckynemo CLI commands in WSL
internal static class WslConstants
{
    public static string GetPathPrefix(string user) =>
        $"""export PATH="/home/{user}/.luckynemo/bin:/opt/luckynemo/bin:/usr/local/bin:$PATH" """;
    ...
    public const string PathPrefix = """export PATH="/home/luckynemo/.luckynemo/bin:/opt/luckynemo/bin:/usr/local/bin:$PATH" """;
}
...
// Adapter to bridge SetupLogger → ILuckyNemoLogger for WebSocket clients
internal sealed class SetupLuckyNemoLogger(SetupLogger logger) : ILuckyNemoLogger
{
    ...
    // is no LUCKYNEMO_TRAY_TRACE-style opt-in gate available here. Letting the
...
public sealed class CleanupStaleDistroStep : SetupStep
{
        ...
        CancellationToken ct)
    {
        ...
        return StepResult.Fail(
            $"Failed to remove app-owned WSL directory '{deletePath}'. Close any process using the LuckyNemo WSL distro and retry setup."
...
public sealed class CleanupStaleGatewayStep : SetupStep
{
    ...
    public override async Task<StepResult> ExecuteAsync(SetupContext ctx, CancellationToken ct)
    {
        ...
        var registry = new GatewayRegistry(ctx.DataDir, logger: new SetupLuckyNemoLogger(ctx.Logger));
        registry.Load();
...
public sealed class PreflightWslStep : SetupStep
{
    ...
    public override async Task<StepResult> ExecuteAsync(SetupContext ctx, CancellationToken ct)
    {
        ...
        if (!WslInstallSupport.SupportsDirectNamedInstall(wslVersion))
            return StepResult.Terminal($"WSL {wslVersion} cannot create a clean app-owned LuckyNemo gateway distro. {WslInstallSupport.UpdateInstructions}");
...
public sealed class CreateWslInstanceStep : SetupStep
{
    ...
    private static async Task<StepResult> VerifyFreshDistro(SetupContext ctx, string distro, string installPath, CancellationToken ct)
    {
        ...
        var probe = await ctx.Commands.RunAsync(
            WslConstants.WslExePath,
            ["-d", distro, "-u", "root", "--", "sh", "-lc", "id -u && test -d / && echo LUCKYNEMO_FRESH_WSL_READY"],
            TimeSpan.FromSeconds(30),
            ct: ct);

        if (probe.ExitCode != 0 || !probe.Stdout.Contains("LUCKYNEMO_FRESH_WSL_READY", StringComparison.Ordinal))
...
""";
        ...
        var script = $"""
            ...
            mkdir -p /home/{wsl.User}/.luckynemo
            mkdir -p /var/lib/luckynemo
            mkdir -p /var/log/luckynemo
            mkdir -p /opt/luckynemo
            
            chown -R {wsl.User}:{wsl.User} /home/{wsl.User}/.luckynemo
            chown -R {wsl.User}:{wsl.User} /var/lib/luckynemo
            chown -R {wsl.User}:{wsl.User} /var/log/luckynemo
            chown -R {wsl.User}:{wsl.User} /opt/luckynemo
...
public sealed class InstallCliStep : SetupStep
{
    ...
    public override string DisplayName => "Install LuckyNemo CLI";
    ...
    public override async Task<StepResult> ExecuteAsync(SetupContext ctx, CancellationToken ct)
    {
        ...
        {
            ("luckynemo --version", null),
            ($"/home/{user}/.luckynemo/bin/luckynemo --version", $"/home/{user}/.luckynemo/bin/luckynemo"),
            ("/opt/luckynemo/bin/luckynemo --version", "/opt/luckynemo/bin/luckynemo"),
            ("/usr/local/bin/luckynemo --version", "/usr/local/bin/luckynemo")
        };
        ...
        foreach (var (cmd, executablePath) in verifyCommands)
        {
            ...
            if (verify.ExitCode == 0 && !string.IsNullOrWhiteSpace(verify.Stdout))
            {
                ...
                ctx.Logger.Info($"LuckyNemo CLI version: {verify.Stdout.Trim()}");
        ...
        CancellationToken ct)
    {
        ...
        if (!executablePath.StartsWith("/", StringComparison.Ordinal) ||
            ...
            executablePath.Contains('\n'))
        {
            return StepResult.Fail($"Refusing to create luckynemo PATH symlink for unexpected install path: {executablePath}");
        }

        if (!string.Equals(executablePath, "/usr/local/bin/luckynemo", StringComparison.Ordinal))
        {
            var linkCommand = $"""
                set -e
                ln -sfn {executablePath} /usr/local/bin/luckynemo
                echo LUCKYNEMO_PATH_READY
                """;
            ...
            if (link.ExitCode != 0 || !link.Stdout.Contains("LUCKYNEMO_PATH_READY", StringComparison.Ordinal))
                return StepResult.Fail($"Failed to make luckynemo available on default PATH: {link.Stderr}");
        }

        var bareVerify = await ctx.Commands.RunInWslAsync(
            distro,
            $"env -i HOME=/home/{user} USER={user} PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin luckynemo --version",
            TimeSpan.FromSeconds(15),
            ct: ct);

        if (bareVerify.ExitCode != 0 || string.IsNullOrWhiteSpace(bareVerify.Stdout))
            return StepResult.Fail($"luckynemo PATH symlink verification failed: {bareVerify.Stderr}");

        ctx.Logger.Info($"LuckyNemo CLI available on default PATH: {bareVerify.Stdout.Trim()}");
        return StepResult.Ok();
    }
    ...
    {
        var user = ctx.Config.Wsl.User;
        await ctx.Commands.RunInWslAsync(ctx.DistroName!, $"rm -rf /opt/luckynemo /home/{user}/.luckynemo /usr/local/bin/luckynemo", TimeSpan.FromSeconds(30), ct: ct, user: "root");
    }
}
...
public sealed class ConfigureGatewayStep : SetupStep
{
    ...
    // Each `luckynemo config set` emitted below spawns the Node CLI fresh inside WSL; on a
    ...
    public override async Task<StepResult> ExecuteAsync(SetupContext ctx, CancellationToken ct)
    {
        ...
        // Gateway env contract: the LuckyNemo gateway CLI still reads OPENCLAW_GATEWAY_TOKEN
        // (main repo keeps the OPENCLAW_* env prefix). Do not rename to LUCKYNEMO_*.
        ...
        if (result.ExitCode != 0 || !result.Stdout.Contains("GATEWAY_CONFIGURED"))
        {
            if (result.TimedOut)
                return StepResult.Fail(
                    $"Gateway configuration timed out after {timeout.TotalSeconds:0}s while running luckynemo config inside WSL.");
        ...
        TailscaleConfig? tailscale = null)
    {
        var configCommands = $"""
            ...
            luckynemo config set gateway.port {port}
            luckynemo config set gateway.bind {gw.Bind}
            luckynemo config set gateway.auth.mode {gw.AuthMode}
            luckynemo config set gateway.auth.token "$OPENCLAW_GATEWAY_TOKEN"
            luckynemo config set gateway.reload.mode {gw.ReloadMode}
            luckynemo config set gateway.nodes.allowCommands {escapedAllowedCommands}
            """;
        ...
        if (tailscale?.Enabled == true)
        {
            ...
            configCommands += $"""
                ...
                luckynemo config set gateway.tailscale.mode off
                luckynemo config set gateway.auth.allowTailscale {trustTailscaleAuth}
                """;
        }
        ...
        if (GetDefaultDevicePairPublicUrl(gw, port, tailscale?.Enabled == true) is { } defaultPublicUrl &&
            gw.ExtraConfig?.ContainsKey(DevicePairPublicUrlKey) != true)
        {
            configCommands += $"\n            luckynemo config set {DevicePairPublicUrlKey} {WslShellQuoting.QuotePosixSingleQuote(defaultPublicUrl)}";
        }
        ...
        {
            configCommands += $"\n            luckynemo config set {DevicePairEnabledKey} true";
        }
        ...
        if (gw.ExtraConfig is { Count: > 0 })
        {
            foreach (var (key, value) in gw.ExtraConfig)
            {
                ...
                configCommands += $"\n            luckynemo config set {key} {escapedValue}";
            }
        }
    ...
    // `luckynemo config set` invocations the step emits so it cannot silently regress as
    ...
    private static int CountConfigSetCommands(string configCommands)
    {
        ...
        {
            if (line.Contains("luckynemo config set", StringComparison.Ordinal))
                count++;
        }
...
public sealed class InstallGatewayServiceStep : SetupStep
{
    ...
    public override async Task<StepResult> ExecuteAsync(SetupContext ctx, CancellationToken ct)
    {
        ...
        var result = await ctx.Commands.RunInWslAsync(
            distro, $"{ctx.WslPathPrefix} && luckynemo gateway install --force", TimeSpan.FromSeconds(60), ct: ct);
    ...
    public override async Task RollbackAsync(SetupContext ctx, CancellationToken ct)
    {
        await ctx.Commands.RunInWslAsync(ctx.DistroName!, $"{ctx.WslPathPrefix} && luckynemo gateway uninstall", TimeSpan.FromSeconds(30), ct: ct);
    }
}
...
public sealed class StartGatewayStep : SetupStep
{
    ...
    public override async Task<StepResult> ExecuteAsync(SetupContext ctx, CancellationToken ct)
    {
        ...
        {
            if (!portCheck.Stdout.Contains("luckynemo", StringComparison.OrdinalIgnoreCase))
            {
            ...
            ctx.Logger.Info($"Port {ctx.Config.GatewayPort} appears to be in use by luckynemo — proceeding");
        }

        // Start the service
        var start = await ctx.Commands.RunInWslAsync(
            distro, $"{pathCmd} && luckynemo gateway start", TimeSpan.FromSeconds(30), ct: ct);
        ...
        if (start.ExitCode != 0)
        {
            ...
            if (start.Stderr.Contains("start-limit", StringComparison.OrdinalIgnoreCase))
            {
                ...
                // Gateway product contract: `luckynemo gateway install` still creates the
                // systemd unit "openclaw-gateway.service" (main repo src/daemon/constants.ts).
                ...
                start = await ctx.Commands.RunInWslAsync(distro, $"{pathCmd} && luckynemo gateway start", TimeSpan.FromSeconds(30), ct: ct);
                if (start.ExitCode != 0)
...
public sealed class MintBootstrapTokenStep : SetupStep
{
    ...
    public override async Task<StepResult> ExecuteAsync(SetupContext ctx, CancellationToken ct)
    {
        ...
        var mint = await ctx.Commands.RunInWslAsync(
            distro, $"{ctx.WslPathPrefix} && luckynemo qr --json", TimeSpan.FromSeconds(30), env, ct);
...
public sealed class PairOperatorStep : SetupStep
{
    ...
    public override async Task<StepResult> ExecuteAsync(SetupContext ctx, CancellationToken ct)
    {
        ...
        var wsLogger = new SetupLuckyNemoLogger(ctx.Logger);
        LuckyNemoGatewayClient? client = null;

        try
        {
            // Phase 1: Initial connect (may get PAIRING_REQUIRED)
            client = new LuckyNemoGatewayClient(gatewayUrl, token, logger: wsLogger, identityPath: identityPath);
    ...
    private static async Task<StepResult> FinalizeWithDeviceToken(
        SetupContext ctx, string gatewayUrl, string identityPath, ILuckyNemoLogger wsLogger, CancellationToken ct)
    {
        ...
        var finalClient = new LuckyNemoGatewayClient(gatewayUrl, deviceToken, logger: wsLogger, identityPath: identityPath);
        ...
        try
        {
            ...
            if (result == ConnectionOutcome.PairingRequired)
            {
                ...
                finalClient = new LuckyNemoGatewayClient(gatewayUrl, deviceToken, logger: wsLogger, identityPath: identityPath);
                finalClient.UseV2Signature = true;
    ...
    internal static async Task<StepResult> AutoApprovePairing(SetupContext ctx, string? requestId, CancellationToken ct)
    {
        ...
        if (string.IsNullOrWhiteSpace(requestId))
        {
            var preview = await ctx.Commands.RunInWslAsync(
                distro,
                $"""{ctx.WslPathPrefix} && luckynemo devices approve --latest --json""",
    ...
    internal static async Task<ConnectionOutcome> WaitForConnectionOrPairing(
        LuckyNemoGatewayClient client, SetupContext ctx, TimeSpan timeout, CancellationToken ct)
    {
    ...
    internal static async Task<StepResult> AutoApproveNodePairing(SetupContext ctx, string? requestId, CancellationToken ct)
    {
        ...
        if (string.IsNullOrWhiteSpace(requestId))
        {
            ...
            var pending = await ctx.Commands.RunInWslAsync(
                distro,
                $"""{ctx.WslPathPrefix} && luckynemo nodes list --json""",
                TimeSpan.FromSeconds(30), env, ct);
...
public sealed class WindowsNodeBootstrapContextStep : SetupStep
{
    ...
    public override async Task<StepResult> ExecuteAsync(SetupContext ctx, CancellationToken ct)
    {
        ...
        if (home is null)
            return StepResult.Fail("Could not resolve Linux home directory for luckynemo user");
        ...
        if (string.IsNullOrWhiteSpace(workspace))
            return StepResult.Fail("Could not resolve LuckyNemo agent workspace path");
        ...
        if (!runBaselineSetup)
        {
            ...
            if (string.IsNullOrWhiteSpace(defaultWorkspace))
                return StepResult.Fail("Could not resolve LuckyNemo default workspace path");
        ...
        if (runBaselineSetup)
        {
            var setupResult = await RunLuckyNemoSetupAsync(ctx, distro, user, workspace, ct);
    ...
    internal static async Task<StepResult> RunLuckyNemoSetupAsync(SetupContext ctx, string distro, string user, string workspaceAbsolute, CancellationToken ct)
    {
        var workspaceArg = WslShellQuoting.QuotePosixSingleQuote(workspaceAbsolute);

        // CLI contract differs across versions: older CLIs use plain `setup` for
        // baseline initialization; newer ones require `--baseline`. Detect the
        // installed contract instead of trusting the pinned LKG version.
        var script = $"""
            set -e
            {ctx.WslPathPrefix}
            if luckynemo setup --help 2>&1 | grep -q -- '--baseline'; then
                luckynemo setup --baseline --workspace {workspaceArg} >/dev/null
            else
                luckynemo setup --workspace {workspaceArg} >/dev/null
            fi
            """;
        ...
        if (result.ExitCode != 0)
            return StepResult.Fail($"luckynemo setup failed (exit {result.ExitCode}): {FirstNonEmpty(result.Stderr, result.Stdout)}");
    ...
    internal static async Task<string?> ResolveConfiguredDefaultWorkspacePathAsync(
        ...
        CancellationToken ct)
    {
        var script = $"{ctx.WslPathPrefix}\nluckynemo config get agents.defaults.workspace --json";
        var result = await ctx.Commands.RunInWslAsync(
            distro,
            script,
            TimeSpan.FromSeconds(15),
        ...
        if (result.ExitCode != 0)
        {
            // The pinned gateway CLI reports an absent key with exit 1. Only that
            ...
            raw = $"{home.TrimEnd('/')}/.luckynemo/workspace";
        }
        else if (string.IsNullOrWhiteSpace(raw))
        {
            // A present JSON null uses LuckyNemo's default. Empty or malformed
    ...
    internal static string? ExtractWorkspaceFromConfigOutput(string stdout)
    {
        ...
        // luckynemo config get --json prints a JSON value; warnings may be on stderr (suppressed)
    ...
    internal static string ExpandLinuxPath(string path, string home)
    {
        ...
        if (string.IsNullOrWhiteSpace(trimmed) || trimmed == "null" || trimmed == "undefined")
            return $"{home.TrimEnd('/')}/.luckynemo/workspace";
    ...
    internal static string BuildApplyScript(string absoluteWorkspacePath)
        => $$"""
            ...
            tmp=$(mktemp "$workspace/.AGENTS.md.luckynemo.XXXXXX")
            trap 'rm -f -- "$tmp"' EXIT
...
public sealed class VerifyEndToEndStep : SetupStep
{
    ...
    public override async Task<StepResult> ExecuteAsync(SetupContext ctx, CancellationToken ct)
    {
        ...
        var distro = ctx.DistroName!;
        var status = await ctx.Commands.RunInWslAsync(
            distro, $"{ctx.WslPathPrefix} && luckynemo gateway status --json", TimeSpan.FromSeconds(15), ct: ct);
    ...
    internal static async Task<StepResult> DrainPendingDeviceApprovalsAsync(SetupContext ctx, CancellationToken ct)
    {
        ...
        for (var i = 0; i < maxDrainIterations; i++)
        {
            var preview = await ctx.Commands.RunInWslAsync(
                distro,
                $"""{pathPrefix} && luckynemo devices approve --latest --json""",
    ...
    private static async Task<StepResult> DrainPendingApprovalsAsync(SetupContext ctx, CancellationToken ct)
    {
        ...
        for (var i = 0; i < maxDrainIterations; i++)
        {
            var nodeList = await ctx.Commands.RunInWslAsync(
                distro,
                $"""{pathPrefix} && luckynemo nodes list --json""",
                TimeSpan.FromSeconds(15), env, ct);
        ...
        SetupContext ctx, string gatewayUrl, string identityPath, ILuckyNemoLogger wsLogger, CancellationToken ct)
    {
        ...
        var client = new LuckyNemoGatewayClient(gatewayUrl, deviceToken, logger: wsLogger, identityPath: identityPath);
        client.UseV2Signature = true;
        ...
        try
        {
            ...
            if (result == PairOperatorStep.ConnectionOutcome.PairingRequired)
            {
                ...
                // try to use it (LuckyNemoGatewayClient prefers stored DeviceToken over constructor token).
                ...
                client = new LuckyNemoGatewayClient(gatewayUrl, ctx.SharedGatewayToken!, logger: wsLogger, identityPath: identityPath);
                client.UseV2Signature = true;
...
public sealed class StartKeepaliveStep : SetupStep
{
    ...
    public override Task<StepResult> ExecuteAsync(SetupContext ctx, CancellationToken ct)
    {
        ...
        if (TryGetExistingKeepalive(markerPath, distro, out var existingPid, new SetupLuckyNemoLogger(ctx.Logger)))
        {
    ...
    internal static bool TryGetExistingKeepalive(string markerPath, string distro, out int pid, ILuckyNemoLogger? logger = null)
    {
        pid = 0;
        if (!File.Exists(markerPath))
            return false;
        ...
        try
        {
            ...
            foreach (var proc in procs)
            {
                try
                {
                    ...
                    var cmdLine = GetProcessCommandLine(proc.Id, new SetupLuckyNemoLogger(ctx.Logger));
    ...
    private static string? GetProcessCommandLine(int pid, ILuckyNemoLogger? logger = null)
    {
        try
        {
