...
{
    internal const string OperatorConnectSpanName = "luckynemo.connection.operator.connect";
    internal const string OperatorReconnectSpanName = "luckynemo.connection.operator.reconnect";
    internal const string OperatorPrepareSpanName = "luckynemo.connection.operator.prepare";
    internal const string OperatorTransportSpanName = "luckynemo.connection.operator.transport";
    internal const string OperatorHandshakeSpanName = "luckynemo.connection.operator.handshake";
    internal const string NodeConnectSpanName = "luckynemo.connection.node.connect";
    internal const string NodeReconnectSpanName = "luckynemo.connection.node.reconnect";
    internal const string NodePrepareSpanName = "luckynemo.connection.node.prepare";
    internal const string NodeTransportSpanName = "luckynemo.connection.node.transport";
    internal const string NodeHandshakeSpanName = "luckynemo.connection.node.handshake";
    internal const string AttemptsMetricName = "luckynemo.connection.attempts";
    internal const string AttemptDurationMetricName = "luckynemo.connection.attempt.duration";
    internal const string StateTransitionsMetricName = "luckynemo.connection.state.transitions";

    private const string RoleTag = "luckynemo.connection.role";
    private const string OperationTag = "luckynemo.connection.operation";
    private const string StateScopeTag = "luckynemo.connection.state.scope";
    private const string StateFromTag = "luckynemo.connection.state.from";
    private const string StateToTag = "luckynemo.connection.state.to";
    private static readonly Counter<long> ConnectionAttempts = LuckyNemoTelemetry.CreateCounter(
        AttemptsMetricName,
        unit: "{attempt}",
        description: "Number of LuckyNemo gateway connection attempts.");
    private static readonly Histogram<double> ConnectionAttemptDuration = LuckyNemoTelemetry.CreateHistogram(
        AttemptDurationMetricName,
        unit: "ms",
        description: "Duration of LuckyNemo gateway connection attempts.");
    private static readonly Counter<long> ConnectionStateTransitions = LuckyNemoTelemetry.CreateCounter(
        StateTransitionsMetricName,
        unit: "{transition}",
        description: "Number of LuckyNemo gateway connection state transitions.");
    ...
    internal LuckyNemoGatewayClient? ConcreteOperatorClient => _activeLifecycle?.DataClient;
    ...
    public async Task<SetupCodeResult> ApplySetupCodeAsync(string setupCode, SshTunnelConfig? sshTunnel = null)
    {
        ...
        try
        {
            ...
            // Setup codes from `luckynemo qr` always provide bootstrap tokens.
        ...
        GatewayRecord existing)
    {
        Directory.CreateDirectory(identityDir);
        var diagLogger = new DiagnosticTeeLogger(_logger, _diagnostics);
        using var client = new LuckyNemoGatewayClient(
            gatewayUrl,
            token,
            diagLogger,
    ...
    private void StartOperatorTelemetryAttempt(string operation, long generation)
    {
        var tags = new[]
        {
            LuckyNemoTelemetryTag.String(RoleTag, "operator"),
            LuckyNemoTelemetryTag.String(OperationTag, operation),
            LuckyNemoTelemetryTag.String(LuckyNemoTelemetryTagKey.Source, "gateway_connection")
        };
        var rootActivity = LuckyNemoTelemetry.StartDetachedActivity(
        ...
        LuckyNemoTelemetry.Add(ConnectionAttempts, tags: tags);
    }
    ...
    private void TransitionOperatorTelemetryPhase(long generation, string spanName)
    {
        ...
        var nextPhase = LuckyNemoTelemetry.StartDetachedActivity(
            spanName,
            parentContext,
            [
                LuckyNemoTelemetryTag.String(RoleTag, "operator"),
        ...
        string initialPhaseSpanName)
    {
        ...
        var tags = new[]
        {
            LuckyNemoTelemetryTag.String(RoleTag, "node"),
            LuckyNemoTelemetryTag.String(OperationTag, operation),
            LuckyNemoTelemetryTag.String(LuckyNemoTelemetryTagKey.Source, "gateway_connection")
        };
        var rootActivity = LuckyNemoTelemetry.StartDetachedActivity(
    ...
    private static void RecordNodePreflightTelemetryFailure(ConnectionErrorCategory errorCategory)
    {
        ...
        {
            LuckyNemoTelemetryTag.String(RoleTag, "node"),
            LuckyNemoTelemetryTag.String(OperationTag, "connect"),
            LuckyNemoTelemetryTag.String(LuckyNemoTelemetryTagKey.Source, "gateway_connection")
        };
        var rootActivity = LuckyNemoTelemetry.StartDetachedActivity(NodeConnectSpanName, tags);
        var attempt = new TelemetryAttempt(
            Generation: 0,
            Operation: "connect",
    ...
    private static void FinishConnectionTelemetryAttempt(
        ...
        ConnectionErrorCategory? errorCategory)
    {
        var tags = new List<LuckyNemoTelemetryTag>
        {
            LuckyNemoTelemetryTag.String(RoleTag, role),
            LuckyNemoTelemetryTag.String(OperationTag, attempt.Operation),
            LuckyNemoTelemetryTag.String(LuckyNemoTelemetryTagKey.Outcome, outcome)
        };
        if (errorCategory.HasValue)
        {
            tags.Add(LuckyNemoTelemetryTag.String(
    ...
    private static void FinishTelemetryActivity(
        ...
        string outcome,
        ConnectionErrorCategory? errorCategory,
        IEnumerable<LuckyNemoTelemetryTag>? tags = null)
    {
        if (activity == null)
            return;
        ...
        {
            activity.SetTag(LuckyNemoTelemetryTagKey.Outcome.ToTelemetryName(), outcome);
            if (errorCategory.HasValue)
            {
                activity.SetTag(
                    LuckyNemoTelemetryTagKey.ErrorCategory.ToTelemetryName(),
                    errorCategory.Value.ToString().ToLowerInvariant());
            }
        }
        ...
        where TState : struct, Enum
    {
        ...
        LuckyNemoTelemetry.Add(
            ...
            tags:
            [
                LuckyNemoTelemetryTag.String(StateScopeTag, scope),
                LuckyNemoTelemetryTag.String(StateFromTag, from.ToString().ToLowerInvariant()),
                LuckyNemoTelemetryTag.String(StateToTag, to.ToString().ToLowerInvariant())
            ]);
    }
...
/// </summary>
internal sealed class DiagnosticTeeLogger : ILuckyNemoLogger
{
    private readonly ILuckyNemoLogger _inner;
    private readonly ConnectionDiagnostics _diagnostics;

    public DiagnosticTeeLogger(ILuckyNemoLogger inner, ConnectionDiagnostics diagnostics)
    {
        _inner = inner;
