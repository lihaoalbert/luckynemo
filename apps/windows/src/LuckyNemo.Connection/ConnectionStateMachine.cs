...
internal sealed class ConnectionStateMachine
{
    ...
    internal void SetNodeInfo(
        string? deviceId,
        LuckyNemo.Shared.PairingStatus pairingStatus,
        string? pairingRequestId = null,
        LuckyNemo.Shared.PairingApprovalKind? pairingApprovalKind = null)
    {
        var requestId = pairingStatus == LuckyNemo.Shared.PairingStatus.Pending
            ? pairingRequestId
            : null;
        var explicitApprovalKind = pairingApprovalKind is { } kind && kind != LuckyNemo.Shared.PairingApprovalKind.Unknown
            ? kind
            : (LuckyNemo.Shared.PairingApprovalKind?)null;
        var approvalKind = pairingStatus == LuckyNemo.Shared.PairingStatus.Pending && !string.IsNullOrWhiteSpace(requestId)
            ? explicitApprovalKind ??
              (string.Equals(requestId, Current.NodePairingRequestId, StringComparison.Ordinal)
                  ...
                  : LuckyNemo.Shared.PairingApprovalKind.Unknown)
            : LuckyNemo.Shared.PairingApprovalKind.Unknown;
    ...
    private void RebuildSnapshot()
    {
        Current = Current with
        {
            ...
            {
                RoleConnectionState.PairingRequired => LuckyNemo.Shared.PairingStatus.Pending,
                RoleConnectionState.PairingRejected => LuckyNemo.Shared.PairingStatus.Rejected,
                RoleConnectionState.Connected => LuckyNemo.Shared.PairingStatus.Paired,
                _ => LuckyNemo.Shared.PairingStatus.Unknown
            }
        };
    }
}
