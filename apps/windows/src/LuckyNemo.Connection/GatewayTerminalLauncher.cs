...
public static class GatewayTerminalLaunchCommandBuilder
{
    ...
    // Runs `luckynemo doctor` in the app-managed WSL gateway, then `exec bash`
    ...
    // inside a quoted argument, so `luckynemo doctor; exec bash` breaks (wt tries to
...
public sealed class GatewayTerminalLauncher(ILuckyNemoLogger logger) : IGatewayTerminalLauncher
{
