...
{
    internal const string LuckyNemoWslPathPrefix = "export PATH=\"/home/luckynemo/.luckynemo/bin:/opt/luckynemo/bin:/usr/local/bin:$PATH\"";
    ...
    public static IReadOnlyList<string> Build(WslGatewayControlAction action)
    {
        return new ReadOnlyCollection<string>([
            "bash",
            "-lc",
            $"{LuckyNemoWslPathPrefix} && luckynemo gateway {ToVerb(action)}"
        ]);
    }
...
public sealed class WslGatewayController(IWslCommandRunner commandRunner, ILuckyNemoLogger logger)
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        logger.Info($"Running LuckyNemo gateway {WslGatewayControlCommandBuilder.ToVerb(action)} in WSL distro '{normalizedDistroName}'.");
