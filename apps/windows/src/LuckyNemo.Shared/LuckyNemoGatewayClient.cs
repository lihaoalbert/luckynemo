...
public partial class LuckyNemoGatewayClient : WebSocketClientBase, IOperatorGatewayClient
{
    // Gateway wire-protocol contract: client id/mode must stay "cli" (brand-neutral),
    // matching the gateway's closed GATEWAY_CLIENT_IDS registry. Do not rebrand.
    private const string OperatorClientId = "cli";
    private const string OperatorClientDisplayName = "LuckyNemo Windows Tray";
    ...
    /// (through <see cref="LuckyNemoChatDataProvider"/>).
    /// </remarks>
    ...
    /// as <see cref="LuckyNemo.Chat.ChatComposeTarget.IsReady"/>.
    /// </summary>
    ...
    // Events
    public event EventHandler<LuckyNemoNotification>? NotificationReceived;
    ...
    public LuckyNemoGatewayClient(string gatewayUrl, string token, ILuckyNemoLogger? logger = null, bool tokenIsBootstrapToken = false, bool bootstrapPairAsNode = false, string? identityPath = null, bool ignoreStoredDeviceToken = false)
        : base(gatewayUrl, token, logger)
    {
        ...
        var dataPath = identityPath ?? LuckyNemoAppIdentity.ResolveRoamingDataDirectory(
    ...
    private static ChatHistoryInfo ParseChatHistory(JsonElement payload, string sessionKey)
    {
        ...
        foreach (var m in msgs.EnumerateArray())
        {
            ...
            var luckyNemoMetadata = ExtractLuckyNemoMetadata(m);
            ...
            list.Add(new ChatMessageInfo
            {
                ...
                State = "final",
                Ts = ts,
                LuckyNemoId = luckyNemoMetadata.Id,
                LuckyNemoSeq = luckyNemoMetadata.Seq,
                LuckyNemoKind = luckyNemoMetadata.Kind,
                CompactionTokensBefore = luckyNemoMetadata.TokensBefore,
                CompactionTokensAfter = luckyNemoMetadata.TokensAfter,
    ...
    private async Task SendConnectMessageAsync(string? nonce = null)
    {
        ...
        var msg = new
        {
            ...
            @params = new
            {
                ...
                locale = "en-US",
                userAgent = $"luckynemo-windows-tray/{appVersion}",
                device = new
                {
    ...
    private void ClearPendingRequests()
    {
        ...
        // (LuckyNemoChatRoot) silently swallows OperationCanceledException —
        ...
        // Today LuckyNemoChatDataProvider.RespondToPermissionAsync correctly
    ...
    /// <c>LuckyNemoGatewayClientSessionKeyTests</c>.
    /// </remarks>
    ...
    private void HandleEvent(JsonElement root, int rawMessageLength)
    {
        ...
        switch (eventType)
        {
            ...
            case "exec.approval.resolved":
                ...
                // envelope. See LuckyNemoChatDataProvider.MapApprovalEvent.
    ...
    private void HandleExecApprovalEvent(JsonElement root, string topLevelEventType)
    {
        ...
        // LuckyNemoChatDataProvider.IsTerminalApprovalPhase, so the banner
    ...
    private void HandleChatEvent(JsonElement root, int rawMessageLength)
    {
        ...
        if (payload.TryGetProperty("message", out var message))
        {
            ...
            var messageLuckyNemoMetadata = ExtractLuckyNemoMetadata(message);
            var payloadLuckyNemoMetadata = ExtractLuckyNemoMetadata(payload);
            EmitChatMessageReceived(
                ...
                respTok,
                ctxPct,
                messageLuckyNemoMetadata.Id ?? payloadLuckyNemoMetadata.Id,
                messageLuckyNemoMetadata.Seq ?? payloadLuckyNemoMetadata.Seq,
                messageLuckyNemoMetadata.Kind ?? payloadLuckyNemoMetadata.Kind,
                messageLuckyNemoMetadata.TokensBefore ?? payloadLuckyNemoMetadata.TokensBefore,
                messageLuckyNemoMetadata.TokensAfter ?? payloadLuckyNemoMetadata.TokensAfter);
        ...
        else if (payload.TryGetProperty("text", out var textProp))
        {
            ...
            if (!string.IsNullOrEmpty(text))
            {
                ...
                var luckyNemoMetadata = ExtractLuckyNemoMetadata(payload);
                EmitChatMessageReceived(
                    sessionKey,
                    role,
                    text,
                    ...
                    respTok,
                    ctxPct,
                    luckyNemoMetadata.Id,
                    luckyNemoMetadata.Seq,
                    luckyNemoMetadata.Kind,
                    luckyNemoMetadata.TokensBefore,
                    luckyNemoMetadata.TokensAfter);
    ...
    private static (
        ...
        string? Kind,
        long? TokensBefore,
        long? TokensAfter) ExtractLuckyNemoMetadata(JsonElement node)
    {
        if (node.ValueKind != JsonValueKind.Object ||
            !node.TryGetProperty("__luckynemo", out var luckyNemo) ||
            luckyNemo.ValueKind != JsonValueKind.Object)
        {
            return (null, null, null, null, null);
        }

        var id = luckyNemo.TryGetProperty("id", out var idProp) && idProp.ValueKind == JsonValueKind.String
            ? idProp.GetString()
            : null;
        var seq = luckyNemo.TryGetProperty("seq", out var seqProp) &&
                  seqProp.ValueKind == JsonValueKind.Number &&
                  seqProp.TryGetInt32(out var parsedSeq)
            ? parsedSeq
            : (int?)null;
        var kind = luckyNemo.TryGetProperty("kind", out var kindProp) &&
                   kindProp.ValueKind == JsonValueKind.String
            ? kindProp.GetString()
            : null;
        var tokensBefore = luckyNemo.TryGetProperty("tokensBefore", out var beforeProp) &&
                           beforeProp.ValueKind == JsonValueKind.Number &&
                           beforeProp.TryGetInt64(out var parsedBefore)
            ? parsedBefore
            : (long?)null;
        var tokensAfter = luckyNemo.TryGetProperty("tokensAfter", out var afterProp) &&
    ...
    private void EmitChatNotification(string text, string? sessionKey = null)
    {
        ...
        var notification = new LuckyNemoNotification
        {
            Message = displayText,
            FullMessage = text,
            IsChat = true,
