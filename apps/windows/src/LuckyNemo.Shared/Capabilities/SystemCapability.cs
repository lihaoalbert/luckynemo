...
namespace LuckyNemo.Shared.Capabilities;
...
public class SystemCapability : NodeCapabilityBase
{
    ...
    /// </param>
    public SystemCapability(ILuckyNemoLogger logger, bool includeRunCommands = true) : base(logger)
    {
    ...
    {
        var title = GetStringArg(request.Args, "title", "LuckyNemo");
    ...
    /// Matches LuckyNemo upstream behavior: rejects paths with separators, checks PATHEXT on Windows.
    /// </summary>
    ...
    private async Task<NodeInvokeResponse> HandleRunAsync(NodeInvokeRequest request)
    {
        ...
        // Per LuckyNemo spec, "command" is an argv array (e.g. ["echo","Hello"]).
