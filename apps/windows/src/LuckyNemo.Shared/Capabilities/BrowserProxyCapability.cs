...
public class BrowserProxyCapability : NodeCapabilityBase
{
    ...
    private HttpRequestMessage CreateHttpRequest(string method, Uri uri, JsonElement args, bool usePasswordAuth)
    {
        ...
        if (!string.IsNullOrWhiteSpace(_bearerToken))
        {
            if (usePasswordAuth)
            {
                httpRequest.Headers.TryAddWithoutValidation("x-luckynemo-password", _bearerToken);
    ...
    private static string BuildReachabilityGuidance(int localControlPort, int? sshRemoteGatewayPort)
    {
        ...
        return $"Start the local LuckyNemo browser control host on gateway port + 2 ({localControlPort}). If the gateway is reached through SSH, also forward the browser-control port with: {sshForward}";
    }
