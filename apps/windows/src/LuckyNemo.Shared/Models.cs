...
public class LuckyNemoNotification
{
...
public static class CommandCenterDiagnostics
{
    ...
    public static string BuildAllowCommandsRepairCommand(IEnumerable<string> commands)
    {
        ...
        return $"luckynemo config set gateway.nodes.allowCommands '{json}'";
    }
    ...
    public static string BuildDangerousCommandOptInGuidance(IEnumerable<string> commands)
    {
        ...
        return string.Join(Environment.NewLine, [
            "Privacy-sensitive LuckyNemo command opt-in guidance",
    ...
    public static List<GatewayDiagnosticWarning> BuildNodeWarnings(NodeCapabilityHealthInfo node)
    {
        ...
        if (isPendingApproval)
        {
            ...
            var repairCommand = hasSafeRequestId ? approvalCommand : "luckynemo nodes pending";
            ...
            warnings.Add(new GatewayDiagnosticWarning
            {
                ...
                Detail = hasSafeRequestId
                    ? approvalDetail
                    : $"{approvalDetail} The gateway did not provide a safe request ID to copy. Run luckynemo nodes pending to discover the request, then approve it explicitly.",
...
public class ChatMessageInfo
{
    ...
    /// <summary>Gateway-assigned unique message ID from the <c>__luckynemo.id</c> field.</summary>
    public string? LuckyNemoId { get; set; }

    /// <summary>Monotonic sequence number within the session from <c>__luckynemo.seq</c>.</summary>
    public int? LuckyNemoSeq { get; set; }

    /// <summary>Structured gateway message kind from <c>__luckynemo.kind</c>.</summary>
    public string? LuckyNemoKind { get; set; }
