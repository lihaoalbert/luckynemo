...
public class WindowsNodeClient : WebSocketClientBase
{
    ...
    public WindowsNodeClient(string gatewayUrl, string token, string dataPath, ILuckyNemoLogger? logger = null, string? bootstrapToken = null)
    ...
    private static string ResolveRequiredCredential(string? token, string? bootstrapToken, string dataPath, ILuckyNemoLogger? logger)
    {
    ...
    public static bool HasStoredNodeDeviceToken(string dataPath, ILuckyNemoLogger? logger = null)
    {
        return !string.IsNullOrWhiteSpace(TryLoadStoredNodeToken(dataPath, logger));
    }

    private static string? TryLoadStoredNodeToken(string dataPath, ILuckyNemoLogger? logger)
    {
        try
        {
    ...
    private string BuildNodeConnectMessage(string? nonce, long ts)
    {
        ...
        var msg = new
        {
            ...
            @params = new
            {
                ...
                auth,
                locale = "en-US",
                userAgent = $"luckynemo-windows-node/{_registration.Version}",
                device = new
                {
                    id = _deviceIdentity.DeviceId,
            ...
            if (!gotNewToken)
            {
                    if (string.IsNullOrEmpty(_deviceIdentity.NodeDeviceToken))
                {
                    ...
                    else
                    {
                        ...
                        _isPaired = false;
                        _pairingBlocked = true;
                        _logger.Info("Not yet paired - check 'luckynemo devices list' for pending approval");
                        _logger.Info($"To approve, run: luckynemo devices approve {_deviceIdentity.DeviceId}");
                        EmitPairingStatusOnTransition(new PairingStatusEventArgs(
                            PairingStatus.Pending, 
                            _deviceIdentity.DeviceId,
                            $"Run: luckynemo devices approve {ShortDeviceId}..."));
                    }
                }
                else
                {
