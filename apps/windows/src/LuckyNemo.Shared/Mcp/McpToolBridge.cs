...
namespace LuckyNemo.Shared.Mcp;
...
public class McpToolBridge
{
    ...
    public McpToolBridge(
        ...
        ILuckyNemoLogger? logger = null,
        string serverName = "luckynemo-tray-mcp",
    ...
    /// the LuckyNemo docs (docs/nodes/index.md, docs/platforms/mac/canvas.md) and
    /// the capability implementations under <c>LuckyNemo.Shared.Capabilities</c>.
    ...
    private static readonly Dictionary<string, string> CommandDescriptions = new(StringComparer.Ordinal)
    {
        // system.*
        ["system.notify"] =
            "Show a Windows toast notification on the node. Args: title (string, default 'LuckyNemo'), body (string), subtitle (string), sound (bool, default true). Returns { sent: true }.",
        ["system.run"] =
        ...
        // screen.* — names match the canonical LuckyNemo protocol
        // (apps/shared/LuckyNemoKit/Sources/LuckyNemoKit/ScreenCommands.swift).
