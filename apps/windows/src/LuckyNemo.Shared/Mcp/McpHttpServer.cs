...
/// but anything sandboxed away from <c>%APPDATA%\LuckyNemoTray\</c> cannot.
///
...
public sealed class McpHttpServer : IDisposable, IAsyncDisposable
{
    ...
    public McpHttpServer(McpToolBridge bridge, int port, ILuckyNemoLogger logger, string? authToken = null)
        ...
        try
        {
            ...
            if (ctx.Request.HttpMethod == "GET")
            {
                ...
                _writeText(ctx.Response, HttpStatusCode.OK,
                    $"LuckyNemo MCP server. POST JSON-RPC to {Endpoint}", "text/plain");
                return;
            }
