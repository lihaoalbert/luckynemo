...
public sealed class ExecApprovalsStore
{
    ...
    public ExecApprovalsStore(string dataPath, ILuckyNemoLogger logger)
        : this(
            dataPath,
            logger,
            // Gateway env contract: the LuckyNemo gateway still reads OPENCLAW_*
            // env vars (main repo src/config). Keep the upstream names here.
    ...
    internal ExecApprovalsStore(
        ...
        string? luckyNemoHomeOverride = null,
        string? osHomeOverride = null)
    {
        var stateDir = string.IsNullOrWhiteSpace(stateDirOverride)
            ? dataPath
            : ResolveStateDirPath(stateDirOverride, luckyNemoHomeOverride, osHomeOverride);
    ...
    private static string ResolveStateDirPath(
        string stateDirOverride,
        string? luckyNemoHomeOverride,
        string? osHomeOverride)
    {
        var osHome = NormalizePathValue(osHomeOverride) ?? Environment.CurrentDirectory;
        var luckyNemoHome = NormalizePathValue(luckyNemoHomeOverride);
        var effectiveHome = luckyNemoHome is null
            ? Path.GetFullPath(osHome)
            : Path.GetFullPath(ExpandHomePrefix(luckyNemoHome, osHome));
