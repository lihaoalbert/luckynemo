...
/// <summary>
/// Small helpers for LuckyNemo code paths that want to expose traced spans and metrics.
/// Exporter configuration remains owned by the app process.
/// </summary>
public static class LuckyNemoTelemetry
{
    ...
    /// </summary>
    /// <remarks>
    /// Prefer <see cref="Trace(string, Action, IEnumerable{LuckyNemoTelemetryTag}?, System.Diagnostics.ActivityKind, LuckyNemoActivitySourceName)"/>
    /// or <see cref="TraceAsync(string, Func{CancellationToken, Task}, IEnumerable{LuckyNemoTelemetryTag}?, System.Diagnostics.ActivityKind, CancellationToken, LuckyNemoActivitySourceName)"/>
    ...
    public static Activity? StartActivity(
        ...
        LuckyNemoActivitySourceName source = LuckyNemoActivitySourceName.LuckyNemo)
    {
    ...
    /// <see cref="Add(Counter{long}, long, IEnumerable{LuckyNemoTelemetryTag}?)"/> whenever an event occurs.
    ...
    public static Counter<long> CreateCounter(
        ...
        LuckyNemoMeterName meter = LuckyNemoMeterName.LuckyNemo)
    {
    ...
    /// <see cref="Record(Histogram{double}, double, IEnumerable{LuckyNemoTelemetryTag}?)"/> for each value.
    ...
    private static void ApplyTags(Activity? activity, IEnumerable<LuckyNemoTelemetryTag>? tags)
    {
    ...
    private static TagList ToTagList(IEnumerable<LuckyNemoTelemetryTag>? tags)
    {
    ...
    public static void MarkSuccess(Activity? activity)
    {
        ...
        activity.SetTag(LuckyNemoTelemetryTagKey.Outcome.ToTelemetryName(), OutcomeSuccess);
    }
    ...
    public static void MarkCanceled(Activity? activity) =>
        activity?.SetTag(LuckyNemoTelemetryTagKey.Outcome.ToTelemetryName(), OutcomeCanceled);
    ...
    public static void MarkFailure(Activity? activity, Exception exception)
    {
        ...
        activity.SetTag(LuckyNemoTelemetryTagKey.Outcome.ToTelemetryName(), OutcomeFailure);
        activity.SetTag(LuckyNemoTelemetryTagKey.ErrorType.ToTelemetryName(), exception.GetType().FullName);
    }
}
