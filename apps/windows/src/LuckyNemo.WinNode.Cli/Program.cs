...
namespace LuckyNemo.WinNode.Cli;
...
internal static class CliRunner
{
        ...
        HttpMessageHandler? httpHandler = null)
    {
        ...
        {
            stderr.WriteLine("[winnode] WARN: --mcp-token is visible to other processes on this machine; prefer LUCKYNEMO_MCP_TOKEN or the on-disk token file.");
        }
        ...
        try
        {
            options.Identity = LuckyNemoAppIdentity.ResolveIdentity(envLookup, options.Identity);
        }
    ...
    internal static int EmitResult(string body, TextWriter stdout, TextWriter stderr, bool verbose)
    {
        ...
        using (doc)
        {
            ...
            // (pretty-printed) so the output matches what `luckynemo nodes invoke`
    ...
    {
        var raw = envLookup("LUCKYNEMO_MCP_PORT");
        if (string.IsNullOrEmpty(raw)) return DefaultMcpPort;
        if (!int.TryParse(raw, NumberStyles.Integer, CultureInfo.InvariantCulture, out var parsed))
        {
            if (verbose)
                stderr.WriteLine($"[winnode] LUCKYNEMO_MCP_PORT={raw} is not an integer; using default {DefaultMcpPort}.");
            return DefaultMcpPort;
        }
        if (parsed < 1 || parsed > 65535)
        {
            if (verbose)
                stderr.WriteLine($"[winnode] LUCKYNEMO_MCP_PORT={parsed} is out of range [1,65535]; using default {DefaultMcpPort}.");
            return DefaultMcpPort;
        }
        return parsed;
    }
    ...
    ///   <item><c>LUCKYNEMO_MCP_TOKEN</c> env var (literal). Standard
    ...
    ///   <c>%APPDATA%\LuckyNemoTray\mcp-token.txt</c> by default,
    ///   <c>%APPDATA%\LuckyNemoTray-Dev\mcp-token.txt</c> when
    ///   <c>--identity dev</c> or <c>LUCKYNEMO_APP_IDENTITY=dev</c> is set, or
    ///   <c>$LUCKYNEMO_TRAY_DATA_DIR\mcp-token.txt</c> when the tray was launched
        ...
        TextWriter stderr)
    {
        ...
        var envToken = envLookup("LUCKYNEMO_MCP_TOKEN");
        if (!string.IsNullOrWhiteSpace(envToken))
        {
            return new AuthTokenResult(envToken, "LUCKYNEMO_MCP_TOKEN");
        }
    ...
    internal static string ResolveTokenPath(Func<string, string?> envLookup, string? identity = null)
    {
        ...
        // launched with LUCKYNEMO_TRAY_DATA_DIR, settings (including the token
        ...
        return LuckyNemoAppIdentity.ResolveMcpTokenPath(envLookup, identity);
    }
        ...
        for (var i = 0; i < args.Length; i++)
        {
            ...
            switch (arg)
            {
                ...
                case "--mcp-token":
                    ...
                    break;
                case "--identity":
                    options.Identity = LuckyNemoAppIdentity.NormalizeIdentity(RequireValue(args, ref i, arg));
                    break;
                case "--verbose":
    ...
    {
        stdout.WriteLine("winnode - invoke LuckyNemo node commands on the local Windows tray over MCP");
        stdout.WriteLine();
        stdout.WriteLine("Mirrors the flag surface of `luckynemo nodes invoke`. The --node value is");
        ...
        stdout.WriteLine("  --node <idOrNameOrIp>        Accepted for parity with `luckynemo nodes invoke`; ignored");
        ...
        stdout.WriteLine("  --mcp-port <port>            Override MCP port [1-65535] (default: $LUCKYNEMO_MCP_PORT or 8765)");
        ...
        stdout.WriteLine("                               $LUCKYNEMO_MCP_TOKEN or %APPDATA%\\LuckyNemoTray\\mcp-token.txt");
        stdout.WriteLine("  --identity <release|dev>     Select tray profile for default token lookup");
        stdout.WriteLine("                               (default: $LUCKYNEMO_APP_IDENTITY or release)");
