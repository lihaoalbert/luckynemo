<!--
  ...
  removed, or renamed in src/LuckyNemo.Shared/Mcp/McpToolBridge.cs
...
`winnode.exe` invokes LuckyNemo Windows-node commands on the local tray over a
...
- `--node` — accepted for parity with `luckynemo nodes invoke`; **ignored**
...
- `--mcp-url <url>` / `--mcp-port <port>` — override the endpoint. Falls back to
  `LUCKYNEMO_MCP_PORT` env var, then port 8765. `--mcp-port` must be in
...
- `--mcp-token <token>` — bearer token override (testing / explicit only). The
  ...
  used. **Prefer `LUCKYNEMO_MCP_TOKEN` (env var) or the on-disk
  `%APPDATA%\LuckyNemoTray\mcp-token.txt`** which the release tray writes when
  MCP is enabled. Both `LUCKYNEMO_MCP_TOKEN` and the on-disk file should
...
- `--identity release|dev` — selects which tray profile supplies the default
  on-disk MCP token. Defaults to `LUCKYNEMO_APP_IDENTITY`, then `release`.
  Use `--identity dev` for a side-by-side dev tray; its default token path is
  `%APPDATA%\LuckyNemoTray-Dev\mcp-token.txt`. `LUCKYNEMO_TRAY_DATA_DIR` still
...
JSON (matches `luckynemo nodes invoke`). stderr receives errors. Exit code:
...
```
{"title": "LuckyNemo", "body": "string", "subtitle": "string", "sound": true}
```
...
`%LOCALAPPDATA%\LuckyNemoTray\exec-policy.json`.
```
{
...
### browser.proxy
Proxy an HTTP request to the local LuckyNemo browser control host (Chrome DevTools Protocol server) running on gateway port + 2.
```
{
...
- `chat.send`, `sessions.list`, `usage.list`, `node.list` — these belong to the
  operator-side `LuckyNemo.Cli.exe`, not `winnode.exe`.
