...
namespace LuckyNemo.SetupEngine.UI.Pages;
...
public sealed partial class WizardPage : Page
{
    ...
    private LuckyNemoGatewayClient? _client;
    ...
    private void RenderWizardPreview()
    {
        ...
        {
            TitleText.Text = "LuckyNemo onboard hit a problem";
    ...
    private async Task<LuckyNemoGatewayClient> ConnectClientAsync()
    {
        ...
        var client = new LuckyNemoGatewayClient(gatewayUrl, token, logger: NullLogger.Instance, identityPath: identityPath)
        {
            UseV2Signature = true
        };
    ...
    private static async Task<bool> WaitForConnectAsync(LuckyNemoGatewayClient client, TimeSpan timeout)
    {
    ...
    private async Task CompleteSetupAsync(int generation)
    {
        ...
        {
            ShowFinalizationError($"LuckyNemo onboard finished, but Windows node guidance could not be installed: {contextResult.Message}");
            return;
        }
