...
/// <summary>
/// Tails the LuckyNemo gateway log running inside WSL and emits a callback for
/// every line that the upstream luckynemo wizard plugins wrote via
...
/// blank.
///
/// Spawns <c>wsl.exe -- tail -F /tmp/luckynemo/luckynemo-*.log</c> and parses
...
{
    private const string DefaultDistroName = "LuckyNemoGateway";
    private const string LogGlob = "/tmp/luckynemo/luckynemo-*.log";
    ...
    public WizardConsoleTail(ILuckyNemoLogger? logger = null, string? distroNameOverride = null)
    {
    ...
    /// once per <c>console.log</c> line emitted by the upstream luckynemo runtime.
    ...
    /// <summary>
    /// Extracts the human-readable <c>message</c> field from a single luckynemo
    ...
    internal static string? TryExtractConsoleMessage(string? line)
    {
        ...
        // has these markers and the irrelevant ones (HTTP, luckynemo/auth, etc)
        // do not.
        ...
        try
        {
            ...
            // Only surface lines from the root luckynemo logger; per-subsystem loggers
            // (e.g. luckynemo/auth, gateway/ws) write structured records that aren't
            // intended for end users and would just be noise.
            if (!meta.TryGetProperty("name", out var name) || name.GetString() != "luckynemo")
                return null;
