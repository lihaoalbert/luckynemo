...
.PARAMETER Isolated
    Set LUCKYNEMO_TRAY_DATA_DIR to a stable temp directory unique to this worktree
    and branch so multiple local launches can run side-by-side.

.PARAMETER DataDir
    Set LUCKYNEMO_TRAY_DATA_DIR to an explicit directory for this launch.

.PARAMETER UpdateChannel
    Set LUCKYNEMO_UPDATE_CHANNEL for this launch. Use alpha for prerelease update
...
$projectPath = Join-Path $repoRoot "src\LuckyNemo.Tray.WinUI\LuckyNemo.Tray.WinUI.csproj"
$manifestPath = Join-Path $repoRoot "src\LuckyNemo.Tray.WinUI\Package.appxmanifest"
...
$outputDir = Join-Path $repoRoot "src\LuckyNemo.Tray.WinUI\bin\$Configuration\$targetFramework\$runtimeIdentifier"
$exePath = Join-Path $outputDir "LuckyNemo.Tray.WinUI.exe"
...
$previousDataDir = $env:LUCKYNEMO_TRAY_DATA_DIR
$previousUpdateChannel = $env:LUCKYNEMO_UPDATE_CHANNEL
$exitCode = 0
...
try {
    ...
    if ($DataDir) {
        ...
        $effectiveDataDir = Join-Path $env:TEMP "LuckyNemoTray\$repoName-$safeBranch-$repoHash"
    }

    if ($effectiveDataDir) {
        New-Item -ItemType Directory -Path $effectiveDataDir -Force | Out-Null
        $env:LUCKYNEMO_TRAY_DATA_DIR = $effectiveDataDir
    }

    if ($UpdateChannel) {
        $env:LUCKYNEMO_UPDATE_CHANNEL = $UpdateChannel
    }

    if ($UseWinApp) {
        $winappArgs = @("run", $outputDir, "--manifest", $manifestPath, "--executable", "LuckyNemo.Tray.WinUI.exe")
        if (-not $NoDebugOutput) {
            $winappArgs += "--debug-output"
        }
    }

    Write-Host "Launching LuckyNemo Tray" -ForegroundColor Cyan
    ...
    if ($env:LUCKYNEMO_TRAY_DATA_DIR) {
        Write-Host "  Data dir:      $env:LUCKYNEMO_TRAY_DATA_DIR"
    }
    if ($env:LUCKYNEMO_UPDATE_CHANNEL) {
        Write-Host "  Update channel: $env:LUCKYNEMO_UPDATE_CHANNEL"
    }
    if ($UseWinApp) {
    ...
    if ($UseWinApp) {
        ...
        Write-Host "Started LuckyNemo Tray (PID: $($process.Id))" -ForegroundColor Green
        Write-Host "Hint: Look for the 🐠 LuckyNemo tray icon in the system tray (bottom-right). It may be hidden under the ^ button." -ForegroundColor Cyan
        $exitCode = 0
    }
} finally {
    $env:LUCKYNEMO_TRAY_DATA_DIR = $previousDataDir
    $env:LUCKYNEMO_UPDATE_CHANNEL = $previousUpdateChannel
}
