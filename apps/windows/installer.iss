; LuckyNemo Companion Inno Setup Script (WinUI version)
; Pass /DDevBuild=1 to produce a side-by-side dev installer.
#ifdef DevBuild
  #define MyAppName "LuckyNemo Companion (Dev)"
  #define MyAppAumid "LuckyNemo.Companion.Dev"
  ; New GUID for the LuckyNemo fork (upstream used a M0LTB0T pseudo-GUID).
; Fresh upgrade-path identity so LuckyNemo and OpenClaw can install side by side.
#define MyAppId "{{03C3206F-62B8-4A97-A888-53447F091B7D}"
  #define MyInstallDir "LuckyNemoTray-Dev"
  #define MyMutex "LuckyNemoTray-Dev"
  #define MyAutoStartName "LuckyNemoTray-Dev"
  #define MyStartupTaskName "LuckyNemo Companion (Dev)"
  #define MyDistroName "LuckyNemoGateway-Dev"
  #define MyProtocol "luckynemo-dev"
  #define MyOutputSuffix "-Dev"
#else
  #define MyAppName "LuckyNemo Companion"
  #define MyAppAumid "LuckyNemo.Companion"
  ; New GUID for the LuckyNemo fork (upstream used a M0LTB0T pseudo-GUID).
; Fresh upgrade-path identity so LuckyNemo and OpenClaw can install side by side.
#define MyAppId "{{A793B368-B0E0-4BDB-AA01-716E442A117E}"
  #define MyInstallDir "LuckyNemoTray"
  #define MyMutex "LuckyNemoTray"
  #define MyAutoStartName "LuckyNemoTray"
  #define MyStartupTaskName "LuckyNemo Companion"
  #define MyDistroName "LuckyNemoGateway"
  #define MyProtocol "luckynemo"
  #define MyOutputSuffix ""
#endif
; TODO(rebrand): replace with your publisher/legal entity name; should match your code-signing certificate.
#define MyAppPublisher "LuckyNemo"
#define MyAppURL "https://github.com/lihaoalbert/LuckyNemo"
#define MyAppExeName "LuckyNemo.Tray.WinUI.exe"
...
AppSupportURL=https://github.com/lihaoalbert/LuckyNemo/issues
AppUpdatesURL=https://github.com/lihaoalbert/LuckyNemo/releases
DefaultDirName={localappdata}\{#MyInstallDir}
DefaultGroupName={#MyAppName}
DisableProgramGroupPage=yes
OutputBaseFilename=LuckyNemoCompanion{#MyOutputSuffix}-Setup-{#MyAppArch}
Compression={#MyCompression}
SolidCompression={#MySolidCompression}
WizardStyle=modern
PrivilegesRequired=lowest
SetupIconFile=src\LuckyNemo.Tray.WinUI\Assets\luckynemo.ico
...
#if !FileExists(publish + "\LuckyNemo.Tray.WinUI.exe")
  #error Tray payload missing. Publish LuckyNemo.Tray.WinUI before compiling the installer.
#endif

#if FileExists(publish + "\SetupEngine\LuckyNemo.SetupEngine.UI.exe")
  #error SetupEngine.UI.exe should not be shipped. Setup UI is hosted by LuckyNemo.Tray.WinUI.exe.
#endif
...
[Registry]
Root: HKCU; Subkey: "Software\Classes\{#MyProtocol}"; ValueType: string; ValueName: ""; ValueData: "URL:LuckyNemo Protocol"; Flags: uninsdeletekey
...
begin
  ...
  begin
    LocalGatewayCleanupRequested :=
      MsgBox(
        'Do you also want to remove the LuckyNemo local WSL gateway?' + #13#10#13#10 +
...
begin
  ...
  repeat
    ...
    Retry :=
      MsgBox(
        'LuckyNemo could not remove the local WSL gateway.' + #13#10#13#10 +
        'Exit code: ' + IntToStr(ResultCode) + #13#10#13#10 +
        'Select Retry to try again, or Cancel to continue uninstalling LuckyNemo and leave local gateway state on disk.',
        mbError,
