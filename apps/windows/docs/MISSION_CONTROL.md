...
This plan turns the Windows tray from a "connected/not connected" companion into a Mission Control surface for any LuckyNemo gateway topology. It is based on a deep audit of:

- Current Windows code in this repository on `mission-control-audit`.
- Current upstream Mac app code in `luckynemo/apps/macos/Sources/LuckyNemo`.
- Current upstream gateway, node policy, browser proxy, health, presence, usage, pairing, and discovery code in `luckynemo/src` and `luckynemo/extensions`.

The main product decision is deliberate: **do not make a native Windows gateway the center of gravity.** The Windows app should be a first-class node and command center for any LuckyNemo gateway: Mac over SSH tunnel, WSL, Windows Node.js, LAN, Tailscale, or unknown/remote.
...
3. Keep LuckyNemo open and topology-neutral: the tray should observe, classify, diagnose, and repair; it should not force one gateway hosting model.
...
- Deep links including `luckynemo://commandcenter`.
...
- `GatewayEnvironment`: Node runtime, LuckyNemo CLI location/version, port/bind resolution.
...
- Mac implements `browser.proxy` only for local mode, proxying to `127.0.0.1:{gatewayPort+2}` with Bearer or `x-luckynemo-password` auth, and a 10 MB/file extraction cap.
...
- `src/LuckyNemo.Shared/Models.cs`
- `src/LuckyNemo.Shared/SettingsData.cs` if optional declared kind is persisted
- `src/LuckyNemo.Tray.WinUI/App.xaml.cs`
- `src/LuckyNemo.Tray.WinUI/Services/SshTunnelService.cs`
- `src/LuckyNemo.Tray.WinUI/Windows/HubWindow.xaml`
- `src/LuckyNemo.Tray.WinUI/Windows/HubWindow.xaml.cs`
- `tests/LuckyNemo.Shared.Tests/ModelsTests.cs`
- `tests/LuckyNemo.Tray.Tests/SettingsRoundTripTests.cs` if settings change
