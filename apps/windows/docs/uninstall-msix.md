# Uninstalling LuckyNemo Tray — MSIX Package
...
Therefore, removing the MSIX package via **Settings → Apps → LuckyNemo Tray
→ Uninstall** will silently leave behind:

- **WSL distro** — `LuckyNemoGateway` remains in `wsl --list`.
- **Roaming app data** under `%APPDATA%\LuckyNemoTray\` (device key, settings,
  mcp-token).
- **Local app data** under `%LOCALAPPDATA%\LuckyNemoTray\` (setup state, logs,
  VHD parent directory).

> **Note:** If the tray was installed with MSIX and the data landed in the
> package-virtualized path (`%LOCALAPPDATA%\Packages\LuckyNemo.Tray_<hash>\...`)
...
Remove-Item "$env:LOCALAPPDATA\LuckyNemoTray\wsl\LuckyNemoGateway" `
...
Remove-Item "$env:LOCALAPPDATA\LuckyNemoTray" -Recurse -Force -ErrorAction SilentlyContinue
...
Remove-Item "$env:APPDATA\LuckyNemoTray\setup-state.json" -Force -ErrorAction SilentlyContinue
```
