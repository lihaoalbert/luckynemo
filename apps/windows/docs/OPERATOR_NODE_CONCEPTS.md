...
LuckyNemo Companion connects a Windows PC to an LuckyNemo gateway in two separate
...
| Term | Meaning |
| --- | --- |
| Gateway | The LuckyNemo service that coordinates agents, channels, sessions, devices, and nodes. The Windows app talks to it over WebSocket. |
| Local WSL gateway | A dedicated `LuckyNemoGateway` WSL distro installed by the Windows onboarding flow. It is app-owned and locked down rather than a general-purpose Ubuntu profile. |
...
1. LuckyNemo Companion installs or connects to a gateway.
...
LuckyNemo Companion setup flow.
...
`~/.luckynemo/luckynemo.json`. Add exact command names such as `screen.snapshot`,
