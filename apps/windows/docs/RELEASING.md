# Releasing LuckyNemo Windows Hub
...
3. Create a new tag from `origin/main`. Prefer a new alpha tag over moving a
   ...
   git tag -a $tag -m "LuckyNemo Windows Hub $tag"
   git push origin $tag
   ```
...
4. Watch the tagged workflow.
   ...
   ```powershell
   gh run list --repo luckynemo/luckynemo-windows-node `
...
5. Confirm the workflow used the exact tag SemVer. Tagged builds fail before
   ...
   .\scripts\Get-LuckyNemoVersion.ps1 -Variable SemVer
   # Expected: $version
   ```
...
6. Confirm the GitHub release is a prerelease and not latest for alpha tags.
   ...
   ```powershell
   gh release view $tag --repo luckynemo/luckynemo-windows-node `
...
```powershell
git tag -a vX.Y.Z-alpha.N -m "LuckyNemo Windows Hub vX.Y.Z-alpha.N"
...
- Portable ZIP payloads for Updatum:
  - `LuckyNemoTray-<version>-win-x64.zip`
  - `LuckyNemoTray-<version>-win-arm64.zip`
...
Only LuckyNemo-owned executables should be signed by the LuckyNemo release signing
identity.

LuckyNemo-owned executables:

- `LuckyNemo.Tray.WinUI.exe`

Third-party/runtime executables that must not be LuckyNemo-signed:
...
- Account: `luckynemo`
- Certificate profile: `luckynemo`
- Endpoint: `https://eus.codesigning.azure.net/`
- Public trust certificate subject:
  `CN=LuckyNemo Foundation, O=LuckyNemo Foundation, L=Mill Valley, S=California, C=US`
...
`repo:luckynemo/luckynemo-windows-node:environment:release-signing`.

## How CI signs payload executables

The release workflow does not recursively sign every `.exe`. Instead it creates
temporary signing input directories with hardlinks to only the LuckyNemo-owned
...
3. Sign only the LuckyNemo-owned EXEs in both payloads.
...
- In ZIP payload:
  - `LuckyNemo.Tray.WinUI.exe` is LuckyNemo-signed.
