...
LuckyNemo Tray uses WinUI `.resw` resource files for localization. Windows automatically selects the correct language based on the OS locale — no user configuration needed.
...
2. **Create a new folder** for your locale under `Strings/`:
   ...
   ```
   src/LuckyNemo.Tray.WinUI/Strings/<locale>/Resources.resw
   ```
...
Set the `LUCKYNEMO_LANGUAGE` environment variable before launching the app:

```powershell
$env:LUCKYNEMO_LANGUAGE = "fr-fr"  # or nl-nl, zh-cn, zh-tw
...
```powershell
$base = "src\LuckyNemo.Tray.WinUI\Strings"
