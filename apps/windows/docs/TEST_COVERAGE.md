...
|---|---:|
| `LuckyNemo.Shared.Tests` | 2,734 total: 2,703 passed, 31 skipped |
| `LuckyNemo.Tray.Tests` | 1,584 total: 1,584 passed, 0 skipped |
...
|---|---|---:|
| `LuckyNemo.Connection.Tests` | Gateway registry, credential resolution, connection manager/state machine, setup codes, pairing, diagnostics | 325 |
| `LuckyNemo.Shared.Tests` | Shared models, gateway client, capabilities, MCP, exec approval, A2UI security, URL handling, notification categorization | 1,977 |
| `LuckyNemo.Tray.Tests` | Tray state/UI helpers, settings isolation, onboarding, connection page behavior, localization, local gateway setup/uninstall | 1,240 |
| `LuckyNemo.Tray.UITests` | Native WinUI/A2UI control, rendering, and accessibility scan coverage | 64 |
| `LuckyNemo.WinNode.Cli.Tests` | Windows node CLI argument parsing, command behavior, JSON output, uninstall flow | 89 |
| `LuckyNemo.SetupEngine.Tests` | Setup engine, WSL gateway installation, setup-code, and local setup policy coverage | 284 |
| `LuckyNemoTray.FunctionalUI.Tests` | Functional UI smoke coverage | 10 |
| `LuckyNemo.E2ETests` | Gateway-mediated setup/connect, revocation recovery, and network recovery suites | 21 |
| `LuckyNemo.Tray.IntegrationTests` | Real-process tray/MCP integration tests gated by `LUCKYNEMO_RUN_INTEGRATION=1` | 18 |
...
- **LuckyNemo.Connection.Tests** keeps connection architecture tests separate from tray UI concerns.
- **LuckyNemo.Tray.UITests** covers A2UI/native WinUI rendering behavior that is awkward to validate through pure unit tests.
- **LuckyNemo.WinNode.Cli.Tests** covers the standalone Windows node CLI contract.
- **LuckyNemo.SetupEngine.Tests** covers gateway setup and local WSL installation policy.
- **LuckyNemoTray.FunctionalUI.Tests** covers newer UI surfaces outside the main tray test project.
- **LuckyNemo.E2ETests** uses custom `[E2EFact]` / `[MxcE2EFact]` attributes that inherit xUnit `FactAttribute`; CI exercises them with shard filters.
- **LuckyNemo.Tray.IntegrationTests** uses custom `[IntegrationFact]` attributes and runs only when `LUCKYNEMO_RUN_INTEGRATION=1`.
...
| Accessibility scan | `dotnet test .\tests\LuckyNemo.Tray.UITests\LuckyNemo.Tray.UITests.csproj -r win-x64 --filter Category=Accessibility` | UI changes and CI quality gate; runs real-process Axe.Windows scans; see `docs\ACCESSIBILITY.md` |
| Local E2E | `LUCKYNEMO_RUN_E2E=1` with `LuckyNemo.E2ETests` | Gateway setup/connect, recovery, or pairing changes that need real WSL Gateway coverage |
...
dotnet test .\tests\LuckyNemo.E2ETests\LuckyNemo.E2ETests.csproj -r win-x64
...
dotnet test .\tests\LuckyNemo.Tray.UITests\LuckyNemo.Tray.UITests.csproj -r win-x64 --filter Category=Accessibility

# Single project
dotnet test .\tests\LuckyNemo.Connection.Tests\LuckyNemo.Connection.Tests.csproj
...
in `AGENTS.md` and `.agents/skills/luckynemo-proof-validation/SKILL.md`: launch
...
`LuckyNemo.WinNode.Cli.Tests` in focused validation because `SkillMdDriftTests`
