...
> Source of truth: [`luckynemo/luckynemo` — `packages/gateway-protocol/src/schema/{sessions,commands}.ts`](https://github.com/lihaoalbert/LuckyNemo/tree/main/packages/gateway-protocol/src/schema)

## Why this exists

The Windows companion can silently drift from the upstream LuckyNemo gateway
protocol — when methods or session fields change upstream but the Windows client
(`LuckyNemoGatewayClient`) keeps sending the old shapes. Because the gateway
...
`LuckyNemoGatewayClient.Protocol.cs` and the DTO/payload builders in
...
clone of the upstream `luckynemo/luckynemo` repo nor any network access, so it runs
in the normal `LuckyNemo.Shared.Tests` suite. It **complements** the behavioural
...
| File | Role |
| --- | --- |
| `tests/LuckyNemo.Shared.Tests/Protocol/gateway-protocol-snapshot.json` | The pinned canonical schema mirror. **Edit this when upstream changes.** |
| `tests/LuckyNemo.Shared.Tests/Protocol/GatewayProtocolDriftTests.cs` | The guard. Cross-checks the snapshot against `LuckyNemoGatewayClient*.cs` + `GatewayProtocolModels.cs`. |
...
2. Update `tests/LuckyNemo.Shared.Tests/Protocol/gateway-protocol-snapshot.json`:
...
3. Run the guard:
   ...
   dotnet test ./tests/LuckyNemo.Shared.Tests/LuckyNemo.Shared.Tests.csproj --filter GatewayProtocolDriftTests
   ```
4. If the guard fails, reconcile the typed client (`LuckyNemoGatewayClient.cs`,
   `LuckyNemoGatewayClient.Protocol.cs`, and the DTOs in `GatewayProtocolModels.cs` /
...
- The snapshot records `upstreamVerified` (date + the exact `sessions.ts`/`commands.ts`
  ...
  reproducible. **Last verified 2026-06-23** against `luckynemo/luckynemo` main:
...
- Re-verify by fetching `packages/gateway-protocol/src/schema/{sessions,commands}.ts`
  from `luckynemo/luckynemo` main, diffing against the pinned methods/fields, and
