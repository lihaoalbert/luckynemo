...
The Setup Engine is a **config-driven system** for provisioning an LuckyNemo WSL gateway from scratch. It consists of two setup projects plus the tray host:

1. **`LuckyNemo.SetupEngine`** — Headless pipeline library. Runs 24 steps sequentially with full JSONL logging, transaction journal, and rollback support.
2. **`LuckyNemo.SetupEngine.UI`** — WinUI3 setup window/pages that wrap the same pipeline with a fluent wizard UI.
3. **`LuckyNemo.Tray.WinUI`** — The only shipped WinUI executable. It hosts `SetupWindow` directly and self-restarts after successful setup.
...
```
┌─────────────────────────────────────────────────────────────┐
│  LuckyNemo.SetupEngine (net10.0 library)                     │
│                                                             │
...
│                                                             │
│  refs: LuckyNemo.Connection, LuckyNemo.Shared                 │
└─────────────────────────────────────────────────────────────┘
         ▲ callback: Action<string, StepStatus>
         │
┌─────────────────────────────────────────────────────────────┐
│  LuckyNemo.SetupEngine.UI (net10.0-windows10.0.22621, WinUI3)│
...
```
src/LuckyNemo.SetupEngine/
├── LuckyNemo.SetupEngine.csproj    # net10.0 library
...
src/LuckyNemo.SetupEngine.UI/
├── LuckyNemo.SetupEngine.UI.csproj # WinAppSDK library referenced by tray
...
└── Pages/
    ...
    ├── WizardPage.xaml / .cs         # LuckyNemo onboard transcript
...
3. Environment variables (`LUCKYNEMO_SETUP_DISTRO_NAME`, etc.)
...
| 11 | `InstallGatewayServiceStep` | `luckynemo gateway install --force` |
...
Log path defaults to `%APPDATA%\LuckyNemoTray\Logs\Setup\setup-engine-<yyyyMMdd-HHmmss>.jsonl` for setup and `uninstall-engine-<yyyyMMdd-HHmmss>.jsonl` for uninstall.
...
### Page Flow: Security → Welcome → Capabilities → Progress → LuckyNemo onboard → Complete

**SecurityNoticePage**
- Native warning InfoBar for device-trust and setup transparency

**WelcomePage**
- LuckyNemo icon + "LuckyNemo Setup" title bar
...
- Install review showing WSL distro, LuckyNemo CLI, local gateway service, and possible UAC

**ProgressPage**
- Step rows with spinning ProgressRing → ✓/✗ badges
- Live activity ledger collapsed by default
- On success → gateway-installed milestone with explicit LuckyNemo onboard CTA
...
**CompletePage**
- LuckyNemo mascot with corner status badge
- "All set!" / error heading
- Native InfoBar for node mode
- "Launch LuckyNemo at startup" toggle defaults on and is persisted before restart
- "Finish" asks the tray host to self-restart and open chat

### Window Properties
- 720×820 logical pixels (DPI-scaled)
- Mica backdrop
- Custom title bar with LuckyNemo icon
...
```
LuckyNemo.SetupEngine.Program.Main(args)                    # uses bundled default-config.json
LuckyNemo.SetupEngine.Program.Main(["--config", "custom.json"])
LuckyNemo.SetupEngine.Program.Main(["--headless"])
LuckyNemo.SetupEngine.Program.Main(["--dry-run"])           # validate config, don't execute
LuckyNemo.SetupEngine.Program.Main(["--rollback-on-failure"])
LuckyNemo.SetupEngine.Program.Main(["--no-rollback-on-failure"])
LuckyNemo.SetupEngine.Program.Main(["--log-path", "./trace.log"])
```
...
``` 
LuckyNemo.Tray.WinUI.exe luckynemo://setup                   # opens/focuses hosted setup window
LuckyNemo.Tray.WinUI.exe --post-setup-restart --wait-for-pid <oldPid> --post-setup-launch chat
```

The tray hosts `SetupWindow` from `LuckyNemo.SetupEngine.UI`. After successful setup it starts a fresh tray process and exits, preserving clean post-setup state without shipping a second WinUI app.
...
dotnet build src\LuckyNemo.SetupEngine\LuckyNemo.SetupEngine.csproj

# Build tray-hosted UI
dotnet build src\LuckyNemo.Tray.WinUI\LuckyNemo.Tray.WinUI.csproj -r win-x64

# Run hosted setup
& "src\LuckyNemo.Tray.WinUI\bin\Debug\net10.0-windows10.0.22621.0\win-x64\LuckyNemo.Tray.WinUI.exe" luckynemo://setup

# Run headless uninstall through the tray executable
& "src\LuckyNemo.Tray.WinUI\bin\Debug\net10.0-windows10.0.22621.0\win-x64\LuckyNemo.Tray.WinUI.exe" --uninstall --dry-run
...
|-----------|--------|-----|
| WebSocket protocol | `LuckyNemo.Shared` | Project reference |
| Gateway registry/credentials | `LuckyNemo.Connection` | Project reference |
| Credential resolver | `LuckyNemo.Connection` | Direct use |
| Node connector | `LuckyNemo.Connection` | Direct use |
| Setup code decoder | `LuckyNemo.Connection` | Direct use |
