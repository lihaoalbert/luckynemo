...
LuckyNemo telemetry exists to help users and developers diagnose the companion app, Windows node integration, gateway connectivity, and related setup flows. It must not become background consumer tracking.
...
LuckyNemo uses shared instrumentation names so traces, metrics, and logs can be correlated consistently:

- tray service name: `luckynemo-windows-tray`
- Windows node service name: `luckynemo-windows-node`
- shared activity source name: `luckynemo`
- shared meter name: `luckynemo`
...
- `luckynemo.status`
- `luckynemo.reason`
- `luckynemo.error.category`
- `error.type`

Use the named constants in `LuckyNemoTelemetryTagKey` for shared attributes instead of duplicating string literals. Use local span attributes only when the values are reviewed as safe and useful for diagnostics.
...
- operator connect traces: `luckynemo.connection.operator.connect` and
  `luckynemo.connection.operator.reconnect`
- Windows node connect traces: `luckynemo.connection.node.connect` and
  `luckynemo.connection.node.reconnect`
- coarse operator phase spans:
  `luckynemo.connection.operator.prepare`,
  `luckynemo.connection.operator.transport`, and
  `luckynemo.connection.operator.handshake`
- coarse Windows node phase spans:
  `luckynemo.connection.node.prepare`,
  `luckynemo.connection.node.transport`, and
  `luckynemo.connection.node.handshake`
- metrics: `luckynemo.connection.attempts`,
  `luckynemo.connection.attempt.duration`, and
  `luckynemo.connection.state.transitions`
- structured state logs in the `LuckyNemo.Telemetry.Connection` category
...
`luckynemo.connection.node.reconnect` beginning with the transport phase.
Manager-driven starts, including the fresh connection after approval, remain
`luckynemo.connection.node.connect`.
...
- traces: `luckynemo.chat.turn`, `luckynemo.chat.queue.wait`, `luckynemo.chat.send`,
  `luckynemo.chat.response.wait`, `luckynemo.chat.response.receive`,
  `luckynemo.chat.history.load`, and `luckynemo.chat.history.backfill`
- counters: `luckynemo.chat.turns`, `luckynemo.chat.send.attempts`,
  `luckynemo.chat.history.loads`, `luckynemo.chat.history.backfills`, and
  `luckynemo.chat.remote_turns.dropped`, and
  `luckynemo.chat.terminal_events.dropped`
- duration histograms: `luckynemo.chat.turn.duration`,
  `luckynemo.chat.queue.wait.duration`, `luckynemo.chat.send.duration`,
  `luckynemo.chat.response.wait.duration`,
  `luckynemo.chat.response.receive.duration`,
  `luckynemo.chat.history.load.duration`, and
  `luckynemo.chat.history.backfill.duration`
...
it increments `luckynemo.chat.remote_turns.dropped` with the finite reason
...
content-free warning, and increments `luckynemo.chat.terminal_events.dropped`.
...
Each `luckynemo.chat.send` span represents one `chat.send` RPC attempt. A valid
...
- `luckynemo.chat.response.wait` starts at accepted local admission or observed
  lifecycle start and ends at the first recognized assistant, reasoning, or tool
  output.
- `luckynemo.chat.response.receive` starts at that first inbound event and ends
  with the turn's authoritative terminal transition.

If a turn terminates before visible output, the wait span closes with
`luckynemo.chat.response.first_output=none` and no receive span is emitted.
...
`luckynemo.chat.queue.wait` sibling span under the turn. A segment that reaches
...
- `luckynemo.source`: `local`, `remote`, `initial`, or `forced`, as applicable
- `luckynemo.outcome`: `success`, `failure`, or `canceled`
- `luckynemo.reason`: `assistant_final`, `lifecycle_end`, `lifecycle_error`,
  `send_rejected`, `queued_canceled`, `abort_requested`, `reset`, `superseded`,
  `disconnected`, `disposed`, or `other`
- `luckynemo.chat.admission.status`: `accepted`, `deferred`, `rejected`,
  `canceled`, `exception`, or `other`
- `luckynemo.chat.backfill.reason`: `remote_turn` or `reset_reconciliation`
- `luckynemo.chat.remote_turn.drop.reason`: `missing_run_id`
- `luckynemo.chat.terminal_event.drop.reason`: `missing_run_id` or
  `mismatched_run_id`
- `luckynemo.chat.response.first_output`: `none`, `assistant`, `reasoning`,
  `tool`, or `other`
...
- root trace: `luckynemo.node.tool.invoke`
- dispatch child: `luckynemo.node.tool.execute`
- `system.run` children of the dispatch span:
  `luckynemo.node.tool.system_run.authorize` and
  `luckynemo.node.tool.system_run.run`
- counter: `luckynemo.node.tool.invocations`
- duration histogram: `luckynemo.node.tool.duration`
- dropped failure-log counter: `luckynemo.node.tool.logs.dropped`
- failure/cancellation log category: `LuckyNemo.Telemetry.NodeTool`
...
- `luckynemo.node.tool.name`: a registered command or `unknown`
- `luckynemo.node.tool.transport`: `gateway` or `mcp`
- `luckynemo.outcome`: `success`, `failure`, or `canceled`
- `luckynemo.error.category`: a finite typed category
- `luckynemo.node.tool.system_run.approval.pipeline`: `legacy` for the existing
...
- `luckynemo.node.tool.sandbox.requested`: whether sandboxing was configured
- `luckynemo.node.tool.sandbox.applied`: whether the command was known to run
  inside the sandbox; omitted when an infrastructure failure makes that unknown
- `luckynemo.node.tool.sandbox.provider`: `mxc` when MXC was selected
- `luckynemo.node.tool.sandbox.technology`: `windows_appcontainer` for the
  currently wired MXC backend
- `luckynemo.node.tool.sandbox.denial.reason`: a finite host-side pre-execution
...
- `luckynemo.node.tool.sandbox.fallback.target`: `unsandboxed` when an unavailable
  MXC backend caused compatibility fallback
- `luckynemo.node.tool.sandbox.fallback.reason`: `mxc_unavailable` for that
  fallback
...
newest entry and increment `luckynemo.node.tool.logs.dropped` with
`luckynemo.node.tool.log.drop.reason=queue_full`. Entries are stamped with the
