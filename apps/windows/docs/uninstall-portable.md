# Uninstalling LuckyNemo Tray — Portable ZIP
...
.\LuckyNemo.Tray.WinUI.exe --uninstall --confirm-destructive

# With JSON output for programmatic consumption (tokens redacted in output):
.\LuckyNemo.Tray.WinUI.exe --uninstall --confirm-destructive --json-output .\uninstall-result.json

# Dry-run — records what would happen without any destruction:
.\LuckyNemo.Tray.WinUI.exe --uninstall --dry-run
```
...
- **WSL distro orphaned** — `LuckyNemoGateway` remains in `wsl --list`.  
  Manual cleanup: `wsl --unregister LuckyNemoGateway`

- **App data** remains under:  
  - `%APPDATA%\LuckyNemoTray\` — device key, settings, mcp-token  
  - `%LOCALAPPDATA%\LuckyNemoTray\` — setup state, logs, exec policy, VHD parent dir

- **Autostart entry** may remain in  
  `HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Run\LuckyNemoTray`

Manual WSL + registry cleanup:

```powershell
wsl --unregister LuckyNemoGateway
Remove-ItemProperty -Path "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Run" `
    -Name "LuckyNemoTray" -ErrorAction SilentlyContinue
Remove-Item "$env:LOCALAPPDATA\LuckyNemoTray\wsl\LuckyNemoGateway" -Recurse -Force -ErrorAction SilentlyContinue
```
