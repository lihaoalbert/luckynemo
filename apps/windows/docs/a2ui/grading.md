...
- **Lit reference** at `vendor\a2ui\renderers\lit\src\0.8` in the
  upstream LuckyNemo checkout
- **Native WinUI** in this repo at `src/LuckyNemo.Tray.WinUI/A2UI/`

The Lit code looks like the canonical browser renderer the LuckyNemo
canvas host ships; the WinUI code is the native renderer in this repo.

Citations use repo-local paths. Lit paths are anchored at the LuckyNemo
checkout: `luckynemo\vendor\a2ui\renderers\lit\src\0.8\`. WinUI paths are
anchored at `src/LuckyNemo.Tray.WinUI/A2UI/`.
...
| Project | Focus |
| --- | --- |
| `LuckyNemo.Shared.Tests/A2UICapabilitySecurityTests.cs` | protocol, secret redaction |
| `LuckyNemo.Tray.UITests/A2UIRenderingTests.cs` | per-component XAML rendering, data binding, live updates |
| `LuckyNemo.Tray.UITests/A2UIControlMatrixTests.cs` | property matrix coverage |
| `LuckyNemo.Tray.UITests/A2UIDashboardScaleTest.cs` | 1000+ component stress |
| `LuckyNemo.Tray.UITests/A2UIThemeTests.cs` | theme parsing |
| `LuckyNemo.Tray.UITests/A2UISvgTests.cs` | SVG decode + 8s timeout |
| `LuckyNemo.Tray.IntegrationTests/A2UICanvasIntegrationTests.cs` | end-to-end MCP smoke + PNG capture |
...
LuckyNemo's canvas host needs to **wrap action handlers**, never just
forward them.
