...
- **WinUI**: strict RFC 6901 via `DataModelStore.SetByPointer` /
  `Read` (`src/LuckyNemo.Tray.WinUI/A2UI/DataModel/DataModelStore.cs`).
...
- **WinUI**: `DataModelObservable.Subscribe(path, callback)` returns
  ...
  component is recycled (`src/LuckyNemo.Tray.WinUI/A2UI/Rendering/IComponentRenderer.cs`).
...
- **WinUI**: `ActionDispatcher` (`src/LuckyNemo.Tray.WinUI/A2UI/Actions/IActionSink.cs`):
...
(`src/LuckyNemo.Tray.WinUI/A2UI/Actions/GatewayActionTransport.cs`) emits
