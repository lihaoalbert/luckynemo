...
LuckyNemo WinUI strings should stay in `src\LuckyNemo.Tray.WinUI\Strings\<locale>\Resources.resw`. XAML uses `x:Uid` for static UI text, and runtime strings should go through `LocalizationHelper.GetString(...)` or `LocalizationHelper.Format(...)`.
