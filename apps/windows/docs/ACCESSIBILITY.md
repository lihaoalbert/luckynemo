...
LuckyNemo Companion treats accessibility as a CI quality gate for the WinUI app. The Build and Test workflow runs real-process Axe.Windows scans against the tray UI test project after the normal native UI tests.
...
```powershell
dotnet test tests\LuckyNemo.Tray.UITests --no-build -c Debug -r win-x64 --filter Category=Accessibility
```
...
```powershell
dotnet build tests\LuckyNemo.Tray.UITests -c Debug -r win-x64
dotnet test tests\LuckyNemo.Tray.UITests -c Debug -r win-x64 --no-build --filter Category=Accessibility
```
