...
LuckyNemo has **excellent** macOS support — the native menubar app runs as a full node with camera, canvas, screen capture, notifications, location, system exec, and more. Windows users today rely on **WSL2** for the gateway and get a limited experience: no native UI integration, no camera, no canvas surface, and NAT networking quirks.

This issue proposes a comprehensive Windows platform strategy that evolves `LuckyNemo.Tray.WinUI` from a gateway *client* into a **native Windows node** — giving the agent eyes, hands, and a voice on Windows, and eventually exploring a fully native Windows gateway.
...
|-----------|--------|---------|
| `LuckyNemo.Shared` | ✅ Working | Gateway WebSocket client library (.NET) |
| `LuckyNemo.Tray.WinUI` | ✅ Working | System tray app — status, Quick Send, WebChat (WebView2), toast notifications, channel control |
...
│  ┌───────────────────────────────────────────┐  │
│  │ luckynemo gateway  (ws://127.0.0.1:18789)  │  │
...
│  Windows PC                                      │
│  ┌────────────────────┐  ┌────────────────────┐ │
│  │ WSL2 (Ubuntu)      │  │ LuckyNemo.Tray      │ │
│  │ luckynemo node run  │  │ (WS operator only) │ │
...
The tray app becomes **a first-class LuckyNemo node** that registers with `role: "node"` and advertises capabilities using Windows-native APIs. No WSL2 required for the node — only potentially for the gateway (or not at all if we pursue native Windows gateway).
...
| **Setup complexity** | `luckynemo onboard --install-daemon` → done |
...
| **Setup complexity** | Install WSL2 → install Node.js → install luckynemo → configure networking → hope NAT cooperates |
...
| **Setup complexity** | Medium — WSL2 + luckynemo + configure tray app to point at `ws://localhost:18789` |
...
|--------|---------|
| **Gateway** | WSL2 (Ubuntu) |
| **Nodes** | LuckyNemo.Tray registers as `role: "node"` from Windows |
...
| **Nodes** | LuckyNemo.Tray as full Windows node |
...
| **Setup complexity** | Low — `npm install -g luckynemo && luckynemo onboard` from PowerShell. Same as Mac. |
...
**Key question:** Does the LuckyNemo gateway actually *work* on Windows? It's Node.js, so *in theory* yes. But there may be Unix-specific assumptions (signals, file paths, spawning, etc.) that need auditing. See [Architectural Questions](#architectural-questions).
...
ToastNotificationManager.CreateToastNotifier("LuckyNemo.Tray").Show(toast);
```
...
**Critical:** Exec approvals must be enforced locally, same as macOS/headless nodes. Store in `%APPDATA%\LuckyNemo\exec-approvals.json`.
...
### 2. Can the LuckyNemo gateway run natively on Windows?
...
| `launchd`/`systemd` service install | High | `luckynemo onboard --install-daemon` installs a launchd/systemd service. Windows needs a Windows Service or Task Scheduler equivalent. |
...
2. Store it in `%APPDATA%\LuckyNemo\device.json`
...
- [x] Implement node protocol in `LuckyNemo.Shared` (connect with `role: "node"`, handle `node.invoke`)
...
- [ ] Audit LuckyNemo gateway for Unix-specific code
- [ ] Test `luckynemo gateway` on Windows (Node.js native)
...
- [ ] `luckynemo onboard --install-daemon` for Windows (Task Scheduler or Windows Service)
...
```
LuckyNemo.Shared/
├── LuckyNemoGatewayClient.cs    ← operator client
...
7. **Native Windows gateway audit** — Run `luckynemo gateway` on Windows, identify and fix platform-specific failures.
...
.\build.ps1
dotnet run --project src\LuckyNemo.Tray.WinUI\LuckyNemo.Tray.WinUI.csproj
```

Requires .NET 10.0 SDK, Windows 10/11. For testing node protocol, you'll need a running LuckyNemo gateway (in WSL2 or on another machine).
