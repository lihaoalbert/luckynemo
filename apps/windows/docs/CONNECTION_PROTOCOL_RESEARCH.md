...
This document captures the current understanding of LuckyNemo gateway connection
...
- `src/LuckyNemo.Shared/LuckyNemoGatewayClient.cs`
- `src/LuckyNemo.Shared/WindowsNodeClient.cs`
- `src/LuckyNemo.Shared/DeviceIdentity.cs`
- `src/LuckyNemo.Connection/GatewayConnectionManager.cs`
- `src/LuckyNemo.Connection/CredentialResolver.cs`
- `src/LuckyNemo.Connection/GatewayRegistry.cs`
- `src/LuckyNemo.Connection/NodeConnector.cs`
- `src/LuckyNemo.Connection/OperatorScopeHelper.cs`
- `src/LuckyNemo.SetupEngine/SetupSteps.cs`
- `src/LuckyNemo.Tray.WinUI/App.xaml.cs`
...
- `https://github.com/lihaoalbert/LuckyNemo/blob/main/docs/gateway/protocol.md`
- `https://github.com/lihaoalbert/LuckyNemo/blob/main/docs/gateway/pairing.md`
- `https://github.com/lihaoalbert/LuckyNemo/blob/main/docs/gateway/operator-scopes.md`
- `https://github.com/lihaoalbert/LuckyNemo/blob/main/docs/cli/qr.md`
- `https://github.com/lihaoalbert/LuckyNemo/blob/main/docs/cli/devices.md`
- `https://github.com/lihaoalbert/LuckyNemo/blob/main/src/pairing/setup-code.ts`
- `https://github.com/lihaoalbert/LuckyNemo/blob/main/src/infra/device-bootstrap.ts`
- `https://github.com/lihaoalbert/LuckyNemo/blob/main/src/shared/device-bootstrap-profile.ts`
- `https://github.com/lihaoalbert/LuckyNemo/blob/main/src/gateway/server/ws-connection/auth-context.ts`
- `https://github.com/lihaoalbert/LuckyNemo/blob/main/src/gateway/server/ws-connection/handshake-auth-helpers.ts`
- `https://github.com/lihaoalbert/LuckyNemo/blob/main/src/gateway/server/ws-connection/message-handler.ts`
- `https://github.com/lihaoalbert/LuckyNemo/blob/main/src/gateway/server-methods/nodes.ts`
- `https://github.com/lihaoalbert/LuckyNemo/blob/main/packages/gateway-protocol/src/schema/frames.ts`

The setup engine currently pins gateway LKG `2026.6.11` in
`src/LuckyNemo.SetupEngine/GatewayLkgVersion.cs`. Before implementing behavior
   ...
   ```json
   {
     ...
     "params": {
       ...
       "client": {
         ...
         "platform": "windows",
         "mode": "cli",
         "displayName": "LuckyNemo Windows Tray"
       },
       "role": "operator",
       ...
       "auth": { "token": "..." },
       "locale": "en-US",
       "userAgent": "luckynemo-windows-tray/1.2.3",
       "device": {
...
| MCP bearer token | `%APPDATA%\LuckyNemoTray\mcp-token.txt` | local HTTP Authorization header | Local MCP only | Not part of gateway pairing. |
...
3. `LuckyNemoGatewayClient` connects as `role: operator` and sends
   `auth.token`.
...
`luckynemo qr` emits a base64url setup code containing:
...
- `luckynemo devices approve --latest` previews the selected request and does
...
| Multi-role handoff parsing | `LuckyNemoGatewayClient` | Parses `auth.deviceTokens[]` and emits role-specific token events. | Needs integration validation | Real gateway bootstrap fixture/E2E. |
...
- Synthetic connection/pairing state coverage in
  `tests/LuckyNemo.Connection.Tests/PairingFlowTests.cs` and
  `tests/LuckyNemo.Connection.Tests/NodePairAutoApproveTests.cs`.
- Setup-code decode and credential-precedence coverage in
  `tests/LuckyNemo.Connection.Tests/SetupCodeFlowTests.cs`.
- Full setup smoke coverage in
  `tests/LuckyNemo.E2ETests/Setup/SetupAndConnectTests.cs`, including headless
...
- MCP local integration coverage in
  `tests/LuckyNemo.Tray.IntegrationTests/McpHttpServerIntegrationTests.cs`.
...
- Real tray-driven E2E coverage now includes:
  - `RealGateway_QrSetupCodeFlow_ReconnectsThroughTrayMcp`, which generates a
    real `luckynemo qr --json` setup code in WSL, applies it through the tray MCP
...
```powershell
.\build.ps1
dotnet test .\tests\LuckyNemo.Shared.Tests\LuckyNemo.Shared.Tests.csproj --no-restore
dotnet test .\tests\LuckyNemo.Tray.Tests\LuckyNemo.Tray.Tests.csproj --no-restore
```
...
1. **QR/bootstrap happy path**
   ...
   - Generate setup-code with `luckynemo qr --json`.
...
`LuckyNemo.Connection` that owns:
...
Keep `LuckyNemo.Shared` focused on wire protocol and token extraction. Keep the
