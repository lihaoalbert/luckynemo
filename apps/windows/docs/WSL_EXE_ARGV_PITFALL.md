...
```bash
workspace='/home/luckynemo/.luckynemo/workspace'
...
Do not store values in Bash variables; bake the values into the script literally. This is the workaround used by `src/LuckyNemo.SetupEngine/SetupSteps.cs:936-945` in `ValidateWslLockdownStep`. It is acceptable for short scripts with a small fixed value set and no spaces in values.

```csharp
var workspace = "/home/luckynemo/.luckynemo/workspace";
...
- `src/LuckyNemo.SetupEngine/CommandRunner.cs` — `RunInWslAsync` exposes the opt-in `inputViaStdin` parameter.
- `src/LuckyNemo.SetupEngine/SetupSteps.cs:936-945` — `ValidateWslLockdownStep` uses workaround #2, C# interpolation.
- `src/LuckyNemo.SetupEngine/SetupSteps.cs` `WindowsNodeBootstrapContextStep` — uses workaround #1, stdin.
