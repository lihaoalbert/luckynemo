...
Custom regex or keyword rules, evaluated in order. Configure these in `%APPDATA%\LuckyNemoTray\settings.json`:
...
All notification settings are in `%APPDATA%\LuckyNemoTray\settings.json`:
...
The categorization logic lives in `LuckyNemo.Shared.NotificationCategorizer`, making it available to both the WinUI tray app and any other consumers of the shared library. The gateway client (`LuckyNemoGatewayClient`) calls the categorizer when emitting notifications, and the tray app's `ShouldShowNotification` method applies the per-category and chat-toggle filters before showing a toast.
