<#
.SYNOPSIS
    Build script for LuckyNemo Windows Hub
...
Write-Host @"
  ...
  🐠 LuckyNemo Windows Hub - Build Script
  =======================================
...
$projects = @{
    "Shared" = @{ Path = "src/LuckyNemo.Shared/LuckyNemo.Shared.csproj"; UseRid = $false }
    "Cli" = @{ Path = "src/LuckyNemo.Cli/LuckyNemo.Cli.csproj"; UseRid = $false }
    "WinNodeCli" = @{ Path = "src/LuckyNemo.WinNode.Cli/LuckyNemo.WinNode.Cli.csproj"; UseRid = $false }
    "Tray" = @{ Path = "src/LuckyNemo.Tray.WinUI/LuckyNemo.Tray.WinUI.csproj"; UseRid = $true }
    "WinUI" = @{ Path = "src/LuckyNemo.Tray.WinUI/LuckyNemo.Tray.WinUI.csproj"; UseRid = $true }
    "SetupEngine" = @{ Path = "src/LuckyNemo.SetupEngine/LuckyNemo.SetupEngine.csproj"; UseRid = $false }
}
...
if ($failCount -eq 0) {
    ...
    if (($buildResults.ContainsKey("WinUI") -and $buildResults["WinUI"]) -or ($buildResults.ContainsKey("Tray") -and $buildResults["Tray"])) {
        ...
        if ($winUITargetFramework) {
            ...
            Write-Host "            Direct launch is default. -UseWinApp runs: winapp run `"$winUIOutputDirectory`" --manifest `"$winUIManifestPath`" --executable `"LuckyNemo.Tray.WinUI.exe`" --debug-output" -ForegroundColor DarkGray
        } else {
