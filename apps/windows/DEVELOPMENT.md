...
A comprehensive guide for building, running, and contributing to the LuckyNemo Windows Hub.
...
- **A running LuckyNemo gateway instance** - The gateway provides the backend for chat, sessions, and notifications when validating gateway-mediated flows
  - Default gateway URL: `ws://localhost:18789`
  - You'll need a valid authentication token from your LuckyNemo instance
...
```
luckynemo-windows-hub/
├── src/
│   ├── LuckyNemo.Shared/              # Shared gateway client library
│   │   ├── LuckyNemoGatewayClient.cs  # WebSocket client for gateway protocol
│   │   ├── Models.cs                 # Data models (SessionInfo, ChannelHealth, etc.)
│   │   └── ILuckyNemoLogger.cs        # Logging interface
│   │
│   ├── LuckyNemo.Connection/          # Gateway registry, credentials, connection manager
│   │
│   ├── LuckyNemo.Chat/                # Native chat model and reducer
│   │   ├── ChatModels.cs             # Threads, entries, events, provider contract
│   │   └── ChatTimelineReducer.cs    # Timeline state transitions
│   │
│   ├── LuckyNemo.Cli/                 # WebSocket connect/send/probe validator
│   │
│   ├── LuckyNemo.WinNode.Cli/         # winnode local MCP/Windows-node CLI
│   │
│   ├── LuckyNemo.SetupEngine/         # Local WSL gateway setup and setup-code support
│   │
│   ├── LuckyNemo.SetupEngine.UI/      # WinUI setup wizard pages hosted by the tray app
│   │
│   ├── LuckyNemoTray.FunctionalUI/    # Small in-repo declarative WinUI helper
│   │   └── FunctionalUI.cs           # Components, hooks, elements, host control
│   │
│   └── LuckyNemo.Tray.WinUI/          # WinUI 3 system tray application (primary)
...
│   │
├── tests/
│   ├── LuckyNemo.Shared.Tests/        # Unit tests for shared library/capabilities/MCP
│   ├── LuckyNemo.Connection.Tests/    # Gateway registry and connection manager tests
│   ├── LuckyNemo.Tray.Tests/          # Tests for tray helpers (menu, settings, deep links)
│   ├── LuckyNemo.WinNode.Cli.Tests/   # winnode CLI contract tests
│   ├── LuckyNemo.SetupEngine.Tests/      # Setup engine tests
│   ├── LuckyNemo.Tray.UITests/           # Native WinUI/A2UI UI tests
│   ├── LuckyNemoTray.FunctionalUI.Tests/ # FunctionalUI smoke tests
│   ├── LuckyNemo.Tray.IntegrationTests/  # Real-process tray/MCP integration tests
│   └── LuckyNemo.E2ETests/               # Gateway-mediated setup/connect E2E suites
│
├── (CI lives in the monorepo root: .github/workflows/windows-app-ci.yml)
│
├── luckynemo-windows-node.slnx        # Solution file
...
```
LuckyNemo.Tray.WinUI  ──depends on──▶  LuckyNemo.Shared + LuckyNemo.Connection + LuckyNemo.Chat + LuckyNemo.SetupEngine.UI
LuckyNemo.WinNode.Cli  ──depends on──▶  LuckyNemo.Shared
LuckyNemo.SetupEngine.UI  ──wraps──▶  LuckyNemo.SetupEngine
LuckyNemo.SetupEngine  ──supports──▶  local WSL gateway setup
LuckyNemo.*.Tests  ──test──▶  corresponding shared, connection, tray, setup, and CLI surfaces
```
...
|-----------|----------|---------|
| **Gateway Communication** | `LuckyNemo.Shared/LuckyNemoGatewayClient.cs` | WebSocket client with protocol v3, reconnect/backoff logic |
| **Connection Management** | `LuckyNemo.Connection/` | Gateway registry, credential precedence, pairing, tunnels, and reconnect policy |
| **Notification System** | `LuckyNemo.Tray.WinUI/App.xaml.cs` | Event routing, toast notifications, classification |
| **WebView2 Integration** | `LuckyNemo.Tray.WinUI/Windows/ChatWindow.xaml.cs` | Embedded chat panel with lifecycle management |
| **Tray Icon Management** | `LuckyNemo.Tray.WinUI/Helpers/IconHelper.cs` | GDI handle management, dynamic icon generation |
| **Session Tracking** | `LuckyNemo.Shared/LuckyNemoGatewayClient.cs` | Session state, activity tracking, polling |
| **Settings & Logging** | `LuckyNemo.Tray.WinUI/Services/` | JSON settings persistence, file rotation logging |
...
**Shared Library:**
```bash
dotnet build src/LuckyNemo.Shared
```

**Tray App (WinUI):**
```bash
dotnet build src/LuckyNemo.Tray.WinUI -r win-x64
```
...
- **Tray App**: Can be built for either architecture
  ```bash
  dotnet build src/LuckyNemo.Tray.WinUI -r win-x64
  dotnet build src/LuckyNemo.Tray.WinUI -r win-arm64
  ```
...
```bash
dotnet build src/LuckyNemo.Tray.WinUI -r win-x64 -p:EnableWindowsTargeting=true
```
...
1. Open `luckynemo-windows-node.slnx` in Visual Studio 2022
2. Set `LuckyNemo.Tray.WinUI` as the startup project
3. Press F5 to run with debugging

#### Command Line

```bash
dotnet run --project src/LuckyNemo.Tray.WinUI -r win-x64
```

For verbose output:

```bash
dotnet run --project src/LuckyNemo.Tray.WinUI -c Debug -r win-x64
```
...
```bash
dotnet publish src/LuckyNemo.Tray.WinUI -c Release -r win-x64 --self-contained -o publish
```
...
The first-run Windows gateway onboarding wizard lives in `LuckyNemo.SetupEngine.UI` and is hosted by `LuckyNemo.Tray.WinUI`; see [docs/ONBOARDING_WIZARD.md](docs/ONBOARDING_WIZARD.md) for the page flow. The setup pipeline itself is documented in [docs/SETUP_ENGINE_REDESIGN.md](docs/SETUP_ENGINE_REDESIGN.md), including the Windows node context step that injects agent instructions into the WSL workspace.
...
### Native chat surface (FunctionalUI + LuckyNemo.Chat)

The Hub Chat tab (`src/LuckyNemo.Tray.WinUI/Pages/ChatPage.xaml`) and the
tray ChatWindow popup (`src/LuckyNemo.Tray.WinUI/Windows/ChatWindow.xaml`)
render their conversations with native WinUI 3 controls via the in-repo
`LuckyNemoTray.FunctionalUI` helper and `LuckyNemo.Chat` model/reducer code.
...
```
src/LuckyNemo.Tray.WinUI/Chat/    LuckyNemoChatTimeline · LuckyNemoComposer · LuckyNemoSessionHeader
                                 LuckyNemoChatDataProvider (adapts LuckyNemoGatewayClient → IChatDataProvider)
                                 LuckyNemoChatRoot         (FunctionalUI component composing the chat surface)
                                 FunctionalChatHostExtensions (mounts FunctionalUI into a XAML <Border>)
                                 IChatGatewayBridge       (testability seam over LuckyNemoGatewayClient)
        ▲ depends on
src/LuckyNemo.Chat/               ChatThread · ChatTimelineState · IChatDataProvider · ChatTimelineReducer
        ▲ rendered by
src/LuckyNemoTray.FunctionalUI/   Component · RenderContext · FunctionalHostControl · WinUI elements
```

**Lifecycle:**

- One `LuckyNemoChatDataProvider` instance lives on `App` (`App.ChatProvider`),
...
**Adding new chat behavior:** model new events in `LuckyNemo.Chat`'s
`ChatEvent` discriminated union, handle them in `ChatTimelineReducer`, and
emit them from `LuckyNemoChatDataProvider` in response to gateway signals.

### Gateway WebSocket Connection

The `LuckyNemoGatewayClient` manages the connection to the LuckyNemo gateway:
...
3. User data folder: `%LOCALAPPDATA%\LuckyNemoTray\WebView2`
...
**Log File:**
- Location: `%LOCALAPPDATA%\LuckyNemoTray\luckynemo-tray.log`
- Rotation: When log exceeds 5MB, old log → `luckynemo-tray.log.old`
- Thread-safe: Uses lock for concurrent writes

**Easy-button setup diagnostics:**
- Human summary: `%LOCALAPPDATA%\LuckyNemoTray\Logs\Setup\easy-setup-latest.txt`
- Machine-readable latest trace: `%LOCALAPPDATA%\LuckyNemoTray\Logs\Setup\easy-setup-latest.jsonl`
- Per-run traces: `%LOCALAPPDATA%\LuckyNemoTray\Logs\Setup\setup-*.jsonl`
...
`.agents/skills/luckynemo-proof-validation/SKILL.md`: run the build/tests,
...
**Test Coverage:**
- `LuckyNemo.Shared.Tests` covers models, gateway client behavior, capabilities, URL helpers, notification categorization, shell quoting, MCP, device identity, and WinNode client contracts.
- `LuckyNemo.Tray.Tests` covers settings isolation, deep link parsing, onboarding state, setup code decoding, gateway health/chat helpers, security validation, wizard step parsing, gateway discovery, localization, and tray UI helpers.
...
See [tests/LuckyNemo.Shared.Tests/README.md](tests/LuckyNemo.Shared.Tests/README.md) for detailed test documentation.
...
**Tray App:**
1. Launch the app: `dotnet run --project src/LuckyNemo.Tray.WinUI`
...
**Workflow File:** `.github/workflows/windows-app-ci.yml` (monorepo root; this app lives under `apps/windows/`)
...
- The pinned gateway setup version lives in `src/LuckyNemo.SetupEngine/GatewayLkgVersion.cs` (`GatewayLkgVersion.LkgVersion`).
...
- The `test` job in `.github/workflows/windows-app-ci.yml` compares pinned LKG vs npm `luckynemo@latest` and emits a **warning** on drift (non-blocking).
- `.github/workflows/windows-app-gateway-lkg-update.yml` creates or updates one standing draft PR on branch `automation/windows-gateway-lkg-update` to bump `GatewayLkgVersion.LkgVersion` when upstream latest advances.
...
- Runs unit tests: `dotnet test tests/LuckyNemo.Shared.Tests` and `dotnet test tests/LuckyNemo.Tray.Tests`
...
|----------|----------|---------|
| `luckynemo-tray-win-x64` | x64 Tray app binaries | Testing, distribution |
| `luckynemo-tray-win-arm64` | ARM64 Tray app binaries | Testing, distribution |
...
3. **GitHub Release:**
   ...
   - Includes:
     - Installers: `LuckyNemoCompanion-Setup-x64.exe`, `LuckyNemoCompanion-Setup-arm64.exe`
     - Portable ZIPs: `LuckyNemoTray-{version}-win-x64.zip`, `LuckyNemoTray-{version}-win-arm64.zip`
...
**Check Latest Build:**
```bash
gh run list --repo lihaoalbert/luckynemo-windows --limit 5
```

**View Specific Run:**
```bash
gh run view <run-id> --repo lihaoalbert/luckynemo-windows
```

**Download Artifacts:**
```bash
gh run download <run-id> --repo lihaoalbert/luckynemo-windows
```
...
1. **Fork and Clone:**
   ```bash
   git clone https://github.com/YOUR_USERNAME/luckynemo-windows-hub.git
   cd luckynemo-windows-hub
   ```
...
4. **Test Locally:**
   ...
   dotnet build
   dotnet test
   dotnet run --project src/LuckyNemo.Tray.WinUI
   ```
...
1. **Add Model** (`LuckyNemo.Shared/Models.cs`):
   ```csharp
...
2. **Add Event** (`LuckyNemo.Shared/LuckyNemoGatewayClient.cs`):
   ```csharp
   public event EventHandler<MyNewEventData>? MyNewEvent;
   ```

3. **Parse Event** (`LuckyNemoGatewayClient.cs`, in `ListenForMessagesAsync`):
   ```csharp
...
4. **Handle in Tray App** (`LuckyNemo.Tray.WinUI/App.xaml.cs`):
   ```csharp
...
5. **Add Tests** (`tests/LuckyNemo.Shared.Tests/`):
   ```csharp
   [Fact]
...
1. **Build Error: "Windows SDK not found"**
   ...
   - Or build Shared library only: `dotnet build src/LuckyNemo.Shared`

2. **WebView2 Error 0x8007000B on ARM64**
   - The tray app must be built for ARM64
   - Rebuild: `dotnet build src/LuckyNemo.Tray.WinUI -r win-arm64`

3. **Tray Icon Not Appearing**
   - Check Windows notification area settings
   - Verify app is running (Task Manager)
   - Check logs: `%LOCALAPPDATA%\LuckyNemoTray\luckynemo-tray.log`
...
- **Issues**: [GitHub Issues](https://github.com/lihaoalbert/LuckyNemo/issues)
- **Discussions**: [GitHub Discussions](https://github.com/lihaoalbert/LuckyNemo/discussions)
- **Documentation**: [LuckyNemo Docs](https://docs.luckynemo.ai)

## Developing & Testing the Onboarding Wizard

The onboarding wizard is a 6-screen flow built with LuckyNemo's minimal FunctionalUI helper layer for declarative C# WinUI. The chat page uses a WebView2 overlay for visual consistency with the post-setup chat experience.
...
| Variable | Purpose |
|----------|---------|
| `LUCKYNEMO_FORCE_ONBOARDING=1` | Show onboarding wizard even if a token already exists |
| `LUCKYNEMO_SKIP_UPDATE_CHECK=1` | Skip the update dialog (useful during testing) |
| `LUCKYNEMO_LANGUAGE=fr-fr` | Override UI language (validated: en-us, fr-fr, nl-nl, zh-cn, zh-tw) |
| `OPENCLAW_GATEWAY_PORT=19001` | Override default gateway port for local dev |
| `LUCKYNEMO_VISUAL_TEST=1` | Enable automatic screenshot capture on page transitions |
| `LUCKYNEMO_VISUAL_TEST_DIR=path` | Output directory for visual test screenshots |

### Testing the Wizard Locally

1. Start a local gateway (e.g., in WSL): `cd ~/luckynemo && npx luckynemo gateway`
2. Set env vars:
   ```powershell
   $env:LUCKYNEMO_FORCE_ONBOARDING = "1"
   $env:LUCKYNEMO_SKIP_UPDATE_CHECK = "1"
   ```
...
- **FunctionalUI**: `src/LuckyNemoTray.FunctionalUI/` — Minimal declarative WinUI helper layer used by onboarding
- **Pages**: `src/LuckyNemo.Tray.WinUI/Onboarding/Pages/` — Functional UI components for each wizard screen
- **Services**: `src/LuckyNemo.Tray.WinUI/Onboarding/Services/` — State management, setup code decoder, permission checker, health check, input validation
- **Widgets**: `src/LuckyNemo.Tray.WinUI/Onboarding/Widgets/` — Shared UI components (cards, step indicators, feature rows)
- **Window**: `src/LuckyNemo.Tray.WinUI/Onboarding/OnboardingWindow.cs` — Host window with WebView2 overlay for chat
- **Helpers**: `src/LuckyNemo.Tray.WinUI/Helpers/GatewayChatHelper.cs` — Shared WebView2 chat URL builder

---

*Made with 🐠 love by Scott Hanselman and the LuckyNemo community*
