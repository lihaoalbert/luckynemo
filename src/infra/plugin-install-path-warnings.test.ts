...
describe("plugin install path warnings", () => {
  ...
  it("detects stale custom plugin install paths", async () => {
    ...
    expect(
      formatPluginInstallPathIssue({
        issue: issue!,
        pluginLabel: "Matrix",
        defaultInstallCommand: "openclaw plugins install @luckynemo/matrix",
      ...
      'Reinstall with "openclaw plugins install @luckynemo/matrix".',
  ...
  it("applies custom command formatting in warning messages", () => {
    expect(
      ...
      'Reinstall with "<openclaw plugins install @luckynemo/matrix>" when you want to return to the standard Matrix plugin.',
