...
function npmRecord(
  ...
  const resolvedName = overrides.resolvedName ?? "@luckynemo/whatsapp";
  return {
    source: "npm",
...
describe("detectPluginVersionDrift", () => {
  it("returns empty drifts when all externalized plugins match the gateway", () => {
    const result = detectPluginVersionDrift({
      ...
      installRecords: {
        ...
        discord: npmRecord("2026.5.4", { resolvedName: "@luckynemo/discord" }),
      },
    });
    ...
    const result = detectPluginVersionDrift({
      ...
      installRecords: {
        whatsapp: npmRecord("2026.5.3", {
          ...
          spec: "@luckynemo/whatsapp@2026.5.3",
        }),
        discord: npmRecord("2026.5.4", { resolvedName: "@luckynemo/discord" }),
      },
    });
  ...
  it("treats a build-qualifier suffix on either side as matching (2026.5.4-1 ≈ 2026.5.4)", () => {
    const result = detectPluginVersionDrift({
      ...
      installRecords: {
        ...
        discord: npmRecord("2026.5.4-1", { resolvedName: "@luckynemo/discord" }),
      },
    });
    ...
    const result = detectPluginVersionDrift({
      ...
      installRecords: {
        ...
        archive: {
          ...
          spec: "@luckynemo/whatsapp@archive",
        },
        local: {
          source: "path",
          resolvedName: "@luckynemo/whatsapp",
  ...
  it("falls back to the install record's `version` field when `resolvedVersion` is absent", () => {
    const result = detectPluginVersionDrift({
      ...
      installRecords: {
        whatsapp: {
          source: "npm",
          spec: "@luckynemo/whatsapp@latest",
          resolvedName: "@luckynemo/whatsapp",
          version: "2026.5.3",
        },
      },
  ...
  it("skips plugins with no recorded version (cannot detect drift)", () => {
    const result = detectPluginVersionDrift({
      gatewayVersion: "2026.5.4",
      installRecords: {
        whatsapp: { source: "npm", spec: "@luckynemo/whatsapp@latest" },
      },
    });
  ...
  it("skips plugins that are explicitly disabled in config", () => {
    ...
    const result = detectPluginVersionDrift({
      ...
      installRecords: {
        whatsapp: npmRecord("2026.5.3"),
        discord: npmRecord("2026.5.3", { resolvedName: "@luckynemo/discord" }),
      },
      config,
    });
  ...
  it("returns drifts sorted by pluginId for deterministic output", () => {
    const result = detectPluginVersionDrift({
      ...
      installRecords: {
        whatsapp: npmRecord("2026.5.3"),
        discord: npmRecord("2026.5.3", { resolvedName: "@luckynemo/discord" }),
        matrix: npmRecord("2026.5.3", { resolvedName: "@luckynemo/matrix" }),
      },
    });
...
describe("resolvePluginVersionDriftUpdateCommand", () => {
  it("uses an exact npm package target when the drifted install is pinned", () => {
    ...
    ).toBe("openclaw plugins update @luckynemo/brave-plugin@2026.6.10-beta.1");
  });
  ...
  it("prefers the parsed exact npm spec package over inconsistent drift metadata", () => {
    expect(
      resolvePluginVersionDriftUpdateCommand({
        ...
        gatewayVersion: "2026.6.10-beta.1",
        source: "npm",
        packageName: "@luckynemo/other-plugin",
        spec: "@luckynemo/brave-plugin@2026.6.9",
      }),
    ).toBe("openclaw plugins update @luckynemo/brave-plugin@2026.6.10-beta.1");
  });
