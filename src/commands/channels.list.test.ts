...
describe("channels list", () => {
  ...
  it("default output shows configured official external channels when the plugin is missing", async () => {
    ...
    mocks.resolveMissingOfficialExternalChannelPluginRepairHint.mockReturnValue({
      ...
      installCommand: "openclaw plugins install @luckynemo/discord",
      doctorFixCommand: "openclaw doctor --fix",
      repairHint:
        "Install the official external plugin with: openclaw plugins install @luckynemo/discord, or run: openclaw doctor --fix.",
    });
    mocks.readConfigFileSnapshot.mockResolvedValue({
      ...baseConfigSnapshot,
      config: {
    ...
    expect(output).toContain(
      "run openclaw plugins install @luckynemo/discord or openclaw doctor --fix",
    );
