...
import { normalizeLuckyNemoRouterRootUrl } from "./provider-catalog.js";

const LUCKYNEMO_ROUTER_USAGE_RESPONSE_MAX_BYTES = 1024 * 1024;

type LuckyNemoRouterUsageFetchGuard = typeof fetchWithSsrFGuard;

type LuckyNemoRouterBudget = {
  configured?: unknown;
  ledger?: unknown;
  windowKey?: unknown;
  limitMicros?: unknown;
...
type LuckyNemoRouterUsagePayload = {
  budget?: LuckyNemoRouterBudget;
  usage?: {
    summary?: {
...
function buildSummary(payload: LuckyNemoRouterUsagePayload): string | undefined {
...
async function readLuckyNemoRouterUsagePayload(
  response: Response,
  timeoutMs: number,
): Promise<LuckyNemoRouterUsagePayload> {
  const buffer = await readResponseWithLimit(response, LUCKYNEMO_ROUTER_USAGE_RESPONSE_MAX_BYTES, {
    chunkTimeoutMs: timeoutMs,
    onOverflow: ({ maxBytes }) =>
      new Error(`LuckyNemo Router usage response exceeds ${maxBytes} bytes`),
    onIdleTimeout: ({ chunkTimeoutMs }) =>
      new Error(
        `LuckyNemo Router usage response stalled: no data received for ${chunkTimeoutMs}ms`,
      ),
  });
  return JSON.parse(new TextDecoder().decode(buffer)) as LuckyNemoRouterUsagePayload;
}

export async function fetchLuckyNemoRouterUsage(params: {
  token: string;
  baseUrl?: string;
  timeoutMs: number;
  /** Test-only seam; production keeps the shared SSRF guard owning transport. */
  fetchGuard?: LuckyNemoRouterUsageFetchGuard;
  ...
  const rootUrl = normalizeLuckyNemoRouterRootUrl(params.baseUrl);
  ...
  const { response, release } = await fetchGuard(
    withTrustedEnvProxyGuardedFetchMode({
      ...
      auditContext: "luckynemo-router.usage",
    }),
  );
  try {
    if (!response.ok) {
      throw new Error(`LuckyNemo Router usage request failed (HTTP ${response.status})`);
    }
    const payload = await readLuckyNemoRouterUsagePayload(response, params.timeoutMs);
    ...
    const billing: ProviderUsageSnapshot["billing"] =
      budget?.configured === true && limitMicros !== undefined && spentMicros !== undefined
        ...
        : costMicros !== undefined
          ...
          : undefined;
    return {
      provider: "luckynemo" as ProviderUsageSnapshot["provider"],
      displayName: "LuckyNemo",
      windows,
      ...(billing ? { billing } : {}),
