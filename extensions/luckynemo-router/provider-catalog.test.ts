...
import {
  ...
  prepareLuckyNemoRouterRequestModel,
...
function buildFetchGuard(catalog: unknown = CATALOG): {
  ...
  const fetchGuardMock: MockedFunction<LiveModelCatalogFetchGuard> = vi.fn(async () => ({
    ...
    finalUrl: "https://router.example/v1/catalog",
    release: async () => undefined,
  }));
  return { fetchGuard: fetchGuardMock, fetchGuardMock };
}

describe("LuckyNemo Router provider catalog", () => {
  beforeEach(() => {
    clearLiveCatalogCacheForTests();
  });

  it("maps every supported catalog protocol to its LuckyNemo transport", async () => {
    const { fetchGuard, fetchGuardMock } = buildFetchGuard();
    const provider = await buildLuckyNemoRouterProviderConfig({
      apiKey: "luckynemo-test-key",
    ...
    expect(provider.models.find((model) => model.id === "google/gemini-3.5-flash")).toMatchObject({
      ...
      baseUrl: "https://router.example/v1/native/google-gemini/v1beta",
    });
  ...
  it("rewrites only native protocol model ids at the request boundary", async () => {
    ...
    const normalized = normalizeLuckyNemoRouterResolvedModel({
      ...anthropic,
    ...
    expect(prepareLuckyNemoRouterRequestModel(normalized as ProviderRuntimeModel)).toMatchObject({
      id: "claude-sonnet-4-6",
      params: undefined,
    });

    const openai = provider.models.find((model) => model.id === "openai/gpt-5.5");
    const normalizedOpenAi = normalizeLuckyNemoRouterResolvedModel({
      ...openai,
      baseUrl: provider.baseUrl,
      provider: "luckynemo",
    } as ProviderRuntimeModel);
    expect(prepareLuckyNemoRouterRequestModel(normalizedOpenAi as ProviderRuntimeModel).id).toBe(
      "openai/gpt-5.5",
    );
  });
  ...
  it("caches catalog rows per credential scope", async () => {
    ...
    await buildLuckyNemoRouterProviderConfig(params);
    await buildLuckyNemoRouterProviderConfig(params);

    expect(fetchGuardMock).toHaveBeenCalledOnce();
    const headers = fetchGuardMock.mock.calls[0]?.[0].init?.headers;
    expect(headers).toBeInstanceOf(Headers);
    expect((headers as Headers).get("authorization")).toBe("Bearer luckynemo-test-key");
  });

  it("does not advertise Gemini without a streaming route", async () => {
    const catalog = structuredClone(CATALOG);
    const geminiProvider = expectDefined(catalog.providers[3], "Gemini catalog provider");
    geminiProvider.routes = geminiProvider.routes.filter(
      (route) => !route.path.includes(":streamGenerateContent"),
    );
    expectDefined(geminiProvider.models[0], "Gemini catalog model").capabilities = ["llm.generate"];
    const provider = await buildLuckyNemoRouterProviderConfig({
      apiKey: "luckynemo-test-key",
