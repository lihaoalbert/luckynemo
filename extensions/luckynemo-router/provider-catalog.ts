// LuckyNemo Router provider catalog maps credential-scoped routes to LuckyNemo transports.
...
// Temporary production endpoint while luckynemo.com awaits ICP备案; switch to
// https://api.luckynemo.com once备案 completes. Operators can override via provider baseUrl.
const LUCKYNEMO_ROUTER_DEFAULT_BASE_URL = "https://kidsai.ibi.ren";

const PROVIDER_ID = "luckynemo";
const CATALOG_CACHE_TTL_MS = 60_000;
const ROUTE_METADATA_KEY = "luckynemoRoute";
...
function readCatalogRows(body: unknown): readonly unknown[] {
  ...
  if (!Array.isArray(providers)) {
    throw new Error("LuckyNemo Router catalog response must contain providers[]");
  }
  return providers;
}
...
export function normalizeLuckyNemoRouterRootUrl(baseUrl: string | undefined): string {
  const normalized = trimTrailingSlashes(baseUrl?.trim() || LUCKYNEMO_ROUTER_DEFAULT_BASE_URL);
  return normalized.endsWith("/v1") ? normalized.slice(0, -3) : normalized;
}

export function normalizeLuckyNemoRouterApiBaseUrl(baseUrl: string | undefined): string {
  return `${normalizeLuckyNemoRouterRootUrl(baseUrl)}/v1`;
}
...
export async function buildLuckyNemoRouterProviderConfig(params: {
  apiKey: string;
  ...
  const rows = await getCachedLiveProviderModelRows({
    ...
    auditContext: "luckynemo-router-model-discovery",
  });
...
export function normalizeLuckyNemoRouterResolvedModel(
...
export function prepareLuckyNemoRouterRequestModel(
  model: ProviderRuntimeModel,
