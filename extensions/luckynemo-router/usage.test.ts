...
type LuckyNemoRouterUsageFetchGuard = NonNullable<
  Parameters<typeof fetchLuckyNemoRouterUsage>[0]["fetchGuard"]
>;

function mockFetchGuard(response: Response): MockedFunction<LuckyNemoRouterUsageFetchGuard> {
...
describe("LuckyNemo Router usage", () => {
  it("maps the managed monthly budget and usage totals", async () => {
    ...
    const snapshot = await fetchLuckyNemoRouterUsage({
      token: "proxy-key",
    ...
    expect(fetchGuard).toHaveBeenCalledWith(
      expect.objectContaining({
        url: "https://router.example/v1/usage",
  ...
  it("does not expose an upstream error body", async () => {
    await expect(
      fetchLuckyNemoRouterUsage({
        token: "proxy-key",
        timeoutMs: 5000,
        fetchGuard: mockFetchGuard(new Response("secret details", { status: 403 })),
      }),
    ).rejects.toThrow("LuckyNemo Router usage request failed (HTTP 403)");
  });
  ...
  it("bounds successful usage response bodies", async () => {
    ...
    ).rejects.toThrow("LuckyNemo Router usage response exceeds");
  });
