...
describe("LuckyNemo Router plugin", () => {
  beforeEach(() => {
  ...
  it("attaches bounded attribution without overriding configured metadata", () => {
    ...
    const wrapped = wrapLuckyNemoRouterProviderStream({
      provider: "luckynemo",
    ...
    void wrapped?.(
      {
        ...
        id: "openai/gpt-5.5",
        headers: {
          "x-clawrouter-client": "managed-luckynemo",
  ...
  it("resolves managed secret refs before scoped discovery", async () => {
    ...
    if (!result || !("provider" in result)) {
      throw new Error("expected LuckyNemo Router catalog provider result");
    }
  ...
  it("surfaces catalog authentication failures", async () => {
    ...
    await expect(
      provider?.catalog?.run({
        config: { models: {} },
        env: { LUCKYNEMO_API_KEY: "invalid-proxy-key" },
        resolveProviderAuth: () => ({
  ...
  it("resolves configured catalog models through a stored auth profile", async () => {
    ...
    const context = {
      ...
      authProfileId: "luckynemo-profile",
    ...
    expect(provider?.resolveDynamicModel?.(context as never)).toMatchObject({
      ...
      provider: "luckynemo",
      api: "openai-responses",
      baseUrl: "https://kidsai.ibi.ren/v1",
      params: {
        luckynemoRoute: {
          api: "openai-responses",
          baseUrl: "https://kidsai.ibi.ren/v1",
