...
import { prepareLuckyNemoRouterRequestModel } from "./provider-catalog.js";

const ENV_API_KEY_MARKER = "LUCKYNEMO_API_KEY";
const ATTRIBUTION_VALUE_MAX_LENGTH = 256;
const REQUEST_ID_MAX_LENGTH = 128;
// The X-ClawRouter-* header names are the server protocol contract and must
// stay stable; only the client identifier value is rebranded to "luckynemo".
...
function withLuckyNemoRouterHeaders(
  ...
  setHeaderDefault(next, CLIENT_HEADER, "luckynemo");
...
function createLuckyNemoRouterStreamWrapper(
  ...
  return (model, context, options) => {
    ...
    const preparedModel = prepareLuckyNemoRouterRequestModel(model);
    const hasExplicitRequestId =
      findHeader(options?.headers ?? {}, REQUEST_ID_HEADER) !== undefined;
    return underlying(
      {
        ...preparedModel,
        headers: withLuckyNemoRouterHeaders(preparedModel.headers, {
          agentId: ctx.agentId,
...
export function wrapLuckyNemoRouterProviderStream(
  ctx: ProviderWrapStreamFnContext,
): StreamFn | undefined {
  return createLuckyNemoRouterStreamWrapper(ctx);
}
