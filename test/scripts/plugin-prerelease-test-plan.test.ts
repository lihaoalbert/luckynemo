...
describe("scripts/lib/plugin-prerelease-test-plan.mjs", () => {
  ...
  it("uses kitchen-sink npm and ClawHub scenarios as the registry install canary", () => {
    ...
    expect(script).toContain("npm:@luckynemo/kitchen-sink@latest");
    expect(script).toContain("npm-latest-conformance");
    expect(script).toContain("npm-latest-adversarial");
    expect(script).toContain("npm:@luckynemo/kitchen-sink@beta");
    expect(script).toContain("clawhub:@luckynemo/kitchen-sink@latest");
    expect(script).toContain("clawhub:@luckynemo/kitchen-sink@beta");
    expect(script).toContain("OPENCLAW_KITCHEN_SINK_PLUGIN_MAX_MEMORY_MIB");
    expect(script).toContain(
      "npm-to-clawhub|clawhub:@luckynemo/kitchen-sink@latest|openclaw-kitchen-sink-fixture|clawhub|success|basic||${KITCHEN_SINK_NPM_SPEC}",
    );
  ...
  it("wires the full plugin prerelease plan into its release workflow", () => {
    ...
    expect(inspectorRun.run).toContain(
      'npm exec --yes "@luckynemo/plugin-inspector@${OPENCLAW_PLUGIN_INSPECTOR_VERSION}" -- ci',
    );
