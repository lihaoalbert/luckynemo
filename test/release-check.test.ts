      ...
      collectBundledExtensionManifestErrors([
        {
          ...
          packageJson: {
            openclaw: {
              install: { npmSpec: "@luckynemo/broken", minHostVersion: "2026.3.14" },
            },
          },
        },
      ]),
  ...
  it("does not require root deps for root chunks sourced from the owning installed plugin", () => {
    ...
    try {
      ...
      writeFileSync(
        ...
        `{"name":"@luckynemo/memory-lancedb","dependencies":{"root-owned-test-dep":"^1.0.0"}}\n`,
        "utf8",
      );
      writeFileSync(
...
describe("createPackedPluginSdkTypescriptSmokeProject", () => {
  it("writes a consumer project that imports representative public SDK subpaths", () => {
    ...
    try {
      ...
      expect(packageJson.dependencies?.["@luckynemo/ai"]).toBe("file:/tmp/openclaw-ai.tgz");
