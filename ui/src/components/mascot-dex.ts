// The Mascotdex: a quiet localStorage log of every mascot palette that has
...
const LOBSTERDEX_KEY = "openclaw.control.mascotdex.v1";
const FAMILIARITY_KEY = "openclaw.control.fishpet.familiarity.v1";
...
function readDex(): Map<string, MascotdexEntry> {
  try {
    const raw = getSafeLocalStorage()?.getItem(LOBSTERDEX_KEY);
    const parsed: unknown = raw ? JSON.parse(raw) : {};
    const entries = new Map<string, MascotdexEntry>();
...
function writeDex(entries: Map<string, MascotdexEntry>): void {
...
export function getMascotdex(): ReadonlySet<string> {
  return new Set(readDex().keys());
}

export function getMascotdexEntries(): ReadonlyMap<string, MascotdexEntry> {
  return readDex();
}

export function recordMascotVisit(paletteId: string, details: { name?: string } = {}): void {
  try {
...
type MascotFamiliarityTier = "shy" | "regular" | "friend";

export type MascotFamiliarity = {
  tier: MascotFamiliarityTier;
  wary: boolean;
  visits: number;
  shoos: number;
};
...
export const MASCOT_FAMILIARITY_TUNING = {
...
export function recordMascotArrivalStats(): void {
  const counters = readFamiliarityCounters();
  writeFamiliarityCounters({ ...counters, visits: counters.visits + 1 });
}

export function recordMascotShoo(): void {
  const counters = readFamiliarityCounters();
  writeFamiliarityCounters({ ...counters, shoos: counters.shoos + 1 });
}

export function getMascotFamiliarity(): MascotFamiliarity {
  const { visits, shoos } = readFamiliarityCounters();
  const tier: MascotFamiliarityTier = visits < 3 ? "shy" : visits < 15 ? "regular" : "friend";
...
export function mascotHonorific(visits: number): string | null {
...
export function isFirstVisitAnniversary(firstSeenAt: number | null, now: Date): boolean {
