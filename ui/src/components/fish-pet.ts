// Decorative mascot pet that perches on the sidebar footer and mirrors
...
// Look and personality are seeded per session + page load so every new
// session hatches a slightly different look at the chibi clownfish mascot
// derived from /tmp/luckynemo-logo-build/line-icon-256.png.
import { expectDefined } from "@luckynemo/normalization-core";
import { html, LitElement, nothing, svg, type TemplateResult } from "lit";
import { property, state } from "lit/decorators.js";
import { isMascotDay } from "../../../src/shared/mascot-day.js";
import { getSafeLocalStorage } from "../local-storage.ts";
import { PET_IMAGE_BASE64 } from "./fish-pet-image-data.ts";
import {
...
type FishPetMode = "idle" | "busy" | "offline";

type FishPetPersonalityId = "sleepy" | "zoomy" | "friendly" | "showoff";

type FishPetPaletteId =
  | "crimson"
  | "coral"
  | "teal"
  | "violet"
...
type FishPetAccessory = "none" | "crown" | "sprout" | "patch" | "santa" | "pumpkin" | "party";

type FishPetAntennae = "perky" | "droopy";

type FishPetBuild = "round" | "squat" | "slender";

type FishPetClawSize = "dainty" | "regular" | "mighty";

type FishPetLook = {
  palette: FishPetPalette;
  scale: number;
  accessory: FishPetAccessory;
  antennae: FishPetAntennae;
  side: "left" | "right";
  spotPct: number;
  facing: 1 | -1;
  personality: FishPetPersonalityId;
  blinkDelayS: number;
  build: FishPetBuild;
  clawSize: FishPetClawSize;
  tailFan: boolean;
};
...
type ActProfile = {
  ...
  acts: Array<[FishPetAct, number]>;
};

// Act windows mirror the CSS animation durations in fish-pet.css so jsdom
// tests and browsers clear acts on the same clock without animationend.
const FISH_PET_ACT_DURATION_MS: Record<FishPetAct, number> = {
  wave: 1400,
  snip: 1000,
  hop: 750,
  spin: 950,
...
const PERSONALITIES: Record<FishPetPersonalityId, ActProfile> = {
  sleepy: {
    delayMs: [6000, 12000],
    acts: [
      ["nap", 40],
...
const FISH_PET_MODE_ACTS: Record<Exclude<FishPetMode, "idle">, ActProfile> = {
  busy: {
    delayMs: [2200, 4500],
    acts: [
      ["scuttle", 40],
...
// Rarity ladder loosely mirrors real mascot genetics: blue ~1 in 2 million,
...
// (see fish-pet.css and renderFishSvg).
const PALETTES: Array<[FishPetPalette, number]> = [
...
// Catalog order for collection UIs (Mascotdex): common to grail.
export const FISH_PET_PALETTES: readonly FishPetPalette[] = PALETTES.map(([palette]) => palette);

// A neutral look used to render catalog minis outside the pet lifecycle.
export function canonicalFishLook(palette: FishPetPalette): FishPetLook {
  return {
    palette,
    scale: 2,
    accessory: "none",
...
const ACCESSORIES: Array<[FishPetAccessory, number]> = [
  ["none", 62],
  ["sprout", 14],
  ["patch", 14],
  ["crown", 10],
...
function isMascotAnniversary(now: Date): boolean {
...
function seasonalAccessories(now: Date): Array<[FishPetAccessory, number]> {
...
const PERSONALITY_IDS: Array<[FishPetPersonalityId, number]> = [
  ["sleepy", 25],
  ["zoomy", 25],
  ["friendly", 25],
  ["showoff", 25],
...
const BUILDS: Array<[FishPetBuild, number]> = [
  ["round", 40],
  ["squat", 30],
  ["slender", 30],
];

const CLAW_SIZES: Array<[FishPetClawSize, number]> = [
  ["regular", 55],
  ["dainty", 25],
  ["mighty", 20],
];
...
export const FISH_PET_BUILD_MULS: Record<FishPetBuild, { w: number; h: number }> = {
  round: { w: 1, h: 1 },
  squat: { w: 1.14, h: 0.9 },
  slender: { w: 0.88, h: 1.1 },
};

export const FISH_PET_FIN_MULS: Record<FishPetClawSize, number> = {
  dainty: 0.85,
  regular: 1,
  mighty: 1.18,
};
...
// sprite's native title tooltip, so no i18n surface. Now fish-themed.
const PET_NAMES = [
  "Nemo",
  "Marlin",
  "Dory",
...
const RARE_NAMES: Partial<Record<FishPetPaletteId, string>> = {
  blue: "Dory",
  gold: "Sunny",
  calico: "Patches",
  abyss: "Lantern",
...
function fishPetName(look: FishPetLook, seed: number): string {
  return (
    RARE_NAMES[look.palette.id] ??
    expectDefined(PET_NAMES[(seed >>> 3) % PET_NAMES.length], "fish-pet name catalog entry")
  );
}
...
function isFishMoltLoad(seed: number): boolean {
  return mulberry32((seed ^ 0x301d) >>> 0)() < 0.12;
}

function isFishTwinLoad(seed: number): boolean {
...
function isFishLogoLoad(seed: number): boolean {
  return mulberry32((seed ^ 0x1063) >>> 0)() < 0.12;
}

type FishLogoVisitPhase = "in" | "leaving" | "out";

export type FishLogoVisitDetail = {
  phase: FishLogoVisitPhase;
  look: FishPetLook | null;
  name: string | null;
};
...
export const FISH_LOGO_VISIT_EVENT = "openclaw-mascot-logo-visit";

type FishPasserKind = "stranger" | "crab";
...
// other mascots that never stop; the crab is not a mascot and refuses to
// discuss it. Neither counts for the Mascotdex.
function planFishPasser(seed: number): FishPasserPlan | null {
  const rng = mulberry32((seed ^ 0xcab) >>> 0);
  const roll = rng();
  if (roll >= 0.095) {
    return null;
  }
  const kind: FishPasserKind = roll < 0.015 ? "crab" : "stranger";
...
function strangerLookFor(seed: number, own: FishPetPaletteId): FishPetLook {
  for (let offset = 1; offset <= 24; offset++) {
    const look = createFishPetLook((seed + offset * 7919) >>> 0);
    if (look.palette.id !== own) {
      return look;
    }
  }
  return createFishPetLook((seed + 1) >>> 0);
}
...
const MOVING_DAY_KEY = "openclaw.control.fishpet.gatewayVersion.v1";

function detectFishMovingDay(version: string): boolean {
  try {
...
function isFishNightTime(now: Date = new Date()): boolean {
...
function pickWeighted<T>(rng: () => number, entries: Array<[T, number]>): T {
  ...
  return expectDefined(entries.at(-1), "weighted mascot choice fallback")[0];
}
...
// One salt per page load: revisiting the UI re-rolls every session's mascot,
// while re-renders within a load stay stable for a given session key.
const LOAD_SALT = Math.trunc(Math.random() * 0xffffffff);

export function fishPetSeed(sessionKey: string): number {
  return (fnv1a(sessionKey) ^ LOAD_SALT) >>> 0;
}

export function createFishPetLook(seed: number, now: Date = new Date()): FishPetLook {
  ...
  const antennae: FishPetAntennae = rng() < 0.6 ? "perky" : "droopy";
...
type FishRunOutcome = "ok" | "error" | "aborted";
...
export function resolveFishRunOutcome(
  sessions:
    | ReadonlyArray<{
  ...
  let latest: { at: number; outcome: FishRunOutcome } | null = null;
...
export function resolveFishPetMode(
  connected: boolean,
...
const ACCESSORY_SPRITES: Record<Exclude<FishPetAccessory, "none">, TemplateResult> = {
  crown: svg`
    <path
...
const CALICO_SPOTS = svg`
  <g class="fish-spots" fill="#2a1f16" opacity="0.8">
...
// lives inside the .fish-fin--r group so wave/snip acts swing it.
...
const RETRO_ANTENNAE = svg`
  <g class="fish-antennae" stroke="var(--fish-body)" stroke-width="4" stroke-linecap="round" fill="none">
...
// body path so they read as "behind"). Fill color lives in fish-pet.css.
const TAIL_FAN = svg`
  <g class="fish-tail">
...
// On mascot days (see src/shared/mascot-day.ts, shared with the CLI
// banner cousin) the pet wears a little sailor cap - unless the seed already
// rolled headwear, which keeps its place.
const HEADWEAR: ReadonlySet<FishPetAccessory> = new Set([
  "crown",
  "sprout",
  "santa",
  "pumpkin",
...
const ANTENNAE_SPRITES: Record<FishPetAntennae, TemplateResult> = {
  perky: svg`
    <g class="fish-antennae" stroke="var(--fish-body)" stroke-width="4" stroke-linecap="round" fill="none">
...
// Not a mascot. Wide shell, eye stalks, walks sideways across the ledge,
// and the Mascotdex refuses to acknowledge it.
function renderStraySvg() {
  return svg`
    <svg
      class="fish-pet__svg"
      viewBox="0 0 120 105"
...
// Chibi clownfish mascot body. The single PNG replaces the prior hand-drawn
// mascot silhouette: keeps the brand mascot consistent and lets the palette
// machinery tint the silhouette via CSS filters rather than redraw it per
// variant. Accessories that depended on mascot geometry (tail fan, retro
// claw, antennae) are intentionally dropped; crown/sprout/patch are still
// drawn on top so dress-up behavior persists. Sleep/cap markers stay so the
// legacy test selectors (.fish-eye-peek, .fish-cap) keep matching.
export function renderFishSvg(
  ...
  const paletteClass = `fish-pet--palette-${look.palette.id}`;
  return svg`
    <svg
      class=${`fish-pet__svg ${paletteClass}`}
      viewBox="0 0 120 105"
      preserveAspectRatio="none"
      aria-hidden="true"
    >
      <image
        href=${`data:image/png;base64,${PET_IMAGE_BASE64}`}
        x="0"
        y="0"
        width="120"
        height="105"
      ...
      />
      ${
        options.sleeping && !options.standalone
          ? svg`<g class="fish-eye-peek" aria-hidden="true"></g>`
          : nothing
      }
      <g class="fish-eye-open" style="display:none" aria-hidden="true"></g>
      <g class="fish-eye-closed" style="opacity:1" aria-hidden="true"></g>
      ${
        options.sailorCap && !options.shell && !HEADWEAR.has(look.accessory)
          ? svg`<g class="fish-cap" aria-hidden="true"></g>`
          : nothing
      }
      ${options.bindle && !options.shell ? BINDLE : nothing}
      ${look.accessory === "none" || options.shell ? nothing : ACCESSORY_SPRITES[look.accessory]}
    </svg>
  `;
}

class FishPet extends LitElement {
  override createRenderRoot() {
    return this;
  }

  @property({ attribute: false }) seed = 0;
  @property({ attribute: false }) mode: FishPetMode = "idle";

  @property({ attribute: false }) visitsEnabled = true;
  @property({ attribute: false }) runOutcome: FishRunOutcome = "ok";
  @property({ attribute: false }) soundsEnabled = false;
  @property({ attribute: false }) gatewayVersion: string | null = null;

  @state() private act: FishPetAct | null = null;
  @state() private spotPct = 80;
  @state() private facing: 1 | -1 = 1;
  @state() private entering = false;
  @state() private presence: "out" | "in" | "leaving" = "out";
  @state() private anchor: FishPetAnchor = "ledge";
  @state() private scheduledVisiting = false;
  @state() private logoPerched = false;
  private logoPlanned = false;
  private logoDone = false;
  private lastLogoPhase: FishLogoVisitPhase = "out";
  @state() private dismissed = false;
  @state() private grumpy = false;
  @state() private vigil = false;
  @state() private outcomePresenceOwner: "vigil" | null = null;
  @state() private passer: FishPasserPlan | null = null;
  ...
  private familiarity: MascotFamiliarity = { tier: "regular", wary: false, visits: 0, shoos: 0 };
  private greetedThisLoad = false;

  private look: FishPetLook | null = null;
  ...
  override willUpdate(changed: Map<PropertyKey, unknown>) {
    ...
    if (seedChanged) {
      this.look = createFishPetLook(this.seed);
      ...
      this.moltPlanned = isFishMoltLoad(this.seed);
      this.twinPlanned = isFishTwinLoad(this.seed);
      this.logoPlanned = isFishLogoLoad(this.seed);
      this.logoDone = false;
      this.logoPerched = false;
      this.familiarity = getMascotFamiliarity();
      this.sailorDay = isMascotDay(new Date());
      ...
      const previousMode = changed.get("mode") as FishPetMode | undefined;
    ...
    if (!this.movingDayChecked && this.gatewayVersion) {
      ...
      this.movingDay = detectFishMovingDay(this.gatewayVersion);
    }
    ...
    if (visible && this.presence !== "in") {
      ...
      if (this.presence === "out") {
        ...
        if (this.look) {
          ...
          this.anniversary = isFirstVisitAnniversary(
            getMascotdexEntries().get(this.look.palette.id)?.firstSeenAt ?? null,
            new Date(),
          );
          ...
          recordMascotVisit(this.look.palette.id, {
            name: fishPetName(this.look, this.seed),
          });
          recordMascotArrivalStats();
        }
      }
  ...
  private logoVisitPhase(): FishLogoVisitPhase {
  ...
  private dispatchLogoPhase() {
    ...
    const dressed =
      look && this.anniversary && look.accessory !== "party"
        ...
        : look;
    this.dispatchEvent(
      new CustomEvent<FishLogoVisitDetail>(FISH_LOGO_VISIT_EVENT, {
        detail: {
          phase,
          look: dressed,
          name: dressed ? fishPetName(dressed, this.seed) : null,
        },
        bubbles: true,
        composed: true,
      }),
  ...
  // Press-and-hold pets the mascot (content eyes, a floating heart); a
  ...
  private readonly handleGaze = (event: PointerEvent) => {
    ...
    this.lastGazeAt = now;
    const sprite = this.querySelector(".fish-pet:not(.fish-pet--shell)");
    if (!sprite) {
      return;
    }
  ...
  private scheduleVisits() {
    ...
    const tuning = MASCOT_FAMILIARITY_TUNING[this.familiarity.tier];
    this.armArrival(
    ...
    this.visitTimer = window.setTimeout(() => {
      ...
      this.armDeparture(
        randomBetween(this.visitRng, VISIT_STAY_MS[0], VISIT_STAY_MS[1]) *
          MASCOT_FAMILIARITY_TUNING[this.familiarity.tier].stayMul,
      );
    }, delayMs);
  }
  ...
  private armDeparture(stayMs: number) {
    this.visitTimer = window.setTimeout(() => {
      ...
      this.scheduledVisiting = false;
      const tuning = MASCOT_FAMILIARITY_TUNING[this.familiarity.tier];
      const waryMul = this.familiarity.wary ? MASCOT_FAMILIARITY_TUNING.waryGapMul : 1;
      this.armArrival(
  ...
  private schedulePasser() {
    ...
    this.passerWatchTimer = null;
    this.passer = null;
    const plan = planFishPasser(this.seed);
  ...
  private watchPasser(plan: FishPasserPlan) {
  ...
  private actProfile(): ActProfile | null {
    if (this.mode === "busy" || this.mode === "offline") {
      return FISH_PET_MODE_ACTS[this.mode];
    }
    if (isFishNightTime()) {
  ...
  private performAct(act: FishPetAct, presenceOwner: "vigil" | null = null) {
    this.clearActTimers();
    ...
    this.actEndTimer = window.setTimeout(() => {
      ...
      if (act === "droop") {
        // Bad news gets processed mascot-style: tidy the ledge, then move on.
  ...
  private completeMolt() {
    ...
    if (this.look) {
      ...
      this.look = {
        ...this.look,
        scale: expectDefined(tiers[Math.min(index + 1, tiers.length - 1)], "mascot molt size tier"),
      };
    }
  ...
  private spriteStyle(look: FishPetLook, scale: number, spotPct: number, facing: 1 | -1) {
    // Glint color stays class-driven (see fish-pet.css): an inline
    // --fish-glint would out-cascade the offline grey override.
    return [
  ...
  private renderSprite(look: FishPetLook, twin: boolean) {
    // On the month/day anniversary of this palette's first Mascotdex visit,
    ...
    const classes = [
      ...
      twin ? "fish-pet--twin" : "",
      dressed.accessory === "party" ? "fish-pet--party" : "",
      this.presence === "leaving" ? "fish-pet--away" : "",
      this.entering ? "fish-pet--entering" : "",
      this.grumpy ? "fish-pet--grumpy" : "",
      this.vigil ? "fish-pet--vigil" : "",
      this.act ? `fish-pet--act-${this.act}` : "",
    ]
      .filter(Boolean)
      .join(" ");
    const zone = this.currentZone();
    // The twin tags along on the parent's trailing side and copies every act
    // a beat later (--fish-act-delay feeds each act's animation-delay).
    const spotPct = twin
    ...
    const style = twin
      ? `${this.spriteStyle(look, scale, spotPct, this.facing === 1 ? -1 : 1)};--fish-act-delay:0.18s`
    ...
    const honorific = mascotHonorific(this.familiarity.visits);
    const baseName = fishPetName(look, this.seed);
    ...
    return html`
      ...
      >
        <div class="fish-pet__body">
          ${renderFishSvg(dressed, { grumpy: this.grumpy, bindle, sailorCap: this.sailorDay })}
          <span class="fish-pet__z" style="--i:0">z</span>
          <span class="fish-pet__z" style="--i:1">z</span>
          <span class="fish-pet__z" style="--i:2">Z</span>
          <span class="fish-pet__bubble" style="--i:0"></span>
          <span class="fish-pet__bubble" style="--i:1"></span>
          <span class="fish-pet__bubble" style="--i:2"></span>
          <span class="fish-pet__heart">♥</span>
          <svg class="fish-pet__broom" viewBox="0 0 24 40" aria-hidden="true">
  ...
  private renderShell(look: FishPetLook) {
    ...
    return html`
      <div class="fish-pet fish-pet--shell" style=${style} aria-hidden="true">
        <div class="fish-pet__body">${renderFishSvg(look, { shell: true })}</div>
      </div>
    `;
  }
  ...
  // are other mascots (never your palette); the crab is, allegedly, also a
  // mascot. Neither perches, neither counts for the Mascotdex.
  private renderPasser(passer: FishPasserPlan, own: FishPetLook) {
    const crab = passer.kind === "crab";
    const look = crab ? own : strangerLookFor(this.seed, own.palette.id);
    const classes = [
      "fish-pet",
      "fish-pet--passer",
      crab ? "fish-pet--crab" : `fish-pet--palette-${look.palette.id}`,
      passer.direction === 1 ? "fish-pet--passer-ltr" : "fish-pet--passer-rtl",
    ].join(" ");
    const style = crab
      ? `--fish-scale:2;--fish-w:1;--fish-h:0.82;--fish-face:1`
    ...
    return html`
      <div
        ...
        style=${style}
        aria-hidden="true"
        title=${crab ? "definitely a mascot" : "a stranger"}
      >
        <div class="fish-pet__body">
          ${crab ? renderStraySvg() : renderFishSvg(look, { standalone: true })}
        </div>
      </div>
    `;
  }
...
if (!customElements.get("openclaw-fish-pet")) {
  customElements.define("openclaw-fish-pet", FishPet);
}
